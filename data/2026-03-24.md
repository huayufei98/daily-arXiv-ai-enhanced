...
TL;DR: 该研究开发了一个AI框架，通过同时处理多切片病理图像来预测前列腺癌根治术后生化复发风险，在大型数据集上验证了其优于传统临床指标的预测性能。


<details>
  <summary>Details</summary>
Motivation: 前列腺癌是全球男性常见恶性肿瘤，但根治术后生化复发的精确预测仍然困难，主要挑战在于肿瘤在前列腺腺体内的多灶性分布特征。

Method: 提出新型AI框架，同时处理一系列多切片病理图像以捕捉整个前列腺腺体的全面肿瘤景观；使用23,451张切片（来自789名患者）的大规模数据集；采用补丁和切片子采样策略降低计算成本。

Result: AI框架在1年和2年生化复发预测中表现出强大性能，显著优于临床基准；AI风险评分在多变量Cox比例风险分析中被验证为最强独立预后因素，超越术前PSA和Gleason评分等传统指标；子采样策略显著降低计算成本而不影响预测性能；外部验证确认了AI的泛化能力。

Conclusion: 该研究证明了基于AI的多切片分析作为前列腺癌术后管理可扩展工具的临床可行性和预后价值，为精准医疗提供了新方法。
...
TL;DR: 研究发现视觉语言模型解码器层存在深度冗余，通过领域感知激活相似性分析，提出了数学感知、非数学感知和混合排名标准，用于识别对特定领域贡献最小的层进行剪枝，揭示了剪枝的三个阶段特征。


<details>
  <summary>Details</summary>
Motivation: Transformer-based视觉语言模型存在显著的深度冗余，但移除特定解码器层的影响尚不清楚，特别是在需要感知与多步推理紧密耦合的领域（如数学问题）。

Method: 通过领域感知激活相似性分析，测量每个层对数学与非数学输入的表示转换强度，提出数学感知、非数学感知和混合排名标准，识别在目标领域中输入输出激活变化最小的层进行剪枝。

Result: 在两个最先进的视觉语言模型和广泛的数学及通用多模态基准测试中，发现一致的三个剪枝阶段：低预算时性能对移除哪些层高度敏感；中等预算时方法收敛；高预算时结构连续性占主导。领域感知排名在排名敏感阶段实现最强稳定性。

Conclusion: 研究揭示了深度如何影响视觉语言模型的领域特定行为，并提供了一种实用、可解释的方法来减少模型深度，同时保持必要的数学或通用视觉语言能力。
...
TL;DR: STAC提出了一种时空感知缓存压缩框架，用于流式3D重建中的大因果变换器，通过压缩KV缓存减少10倍内存消耗并加速4倍推理
...
Method: 提出STAC框架，包含三个关键组件：1) 工作时序令牌缓存机制，使用衰减累积注意力分数保留长期信息令牌；2) 长期空间令牌缓存方案，将空间冗余令牌压缩为体素对齐表示；3) 基于块的多帧优化策略，联合处理连续帧以改善时间一致性和GPU效率。

Result: 实验表明STAC在保持最先进重建质量的同时，将内存消耗减少近10倍，推理速度提升4倍，显著提高了流式设置中实时3D重建的可扩展性。

Conclusion: STAC通过利用3D重建中因果变换器注意力的内在时空稀疏性，有效解决了流式3D重建中的内存瓶颈问题，实现了高效且高质量的实时重建。
...
TL;DR: 提出了两种面向边缘部署的高效视觉异常检测方法：PatchCore-Lite和Padim-Lite，通过量化压缩和计算优化，显著降低了内存占用和推理时间，适合工业实时检测需求。


<details>
  <summary>Details</summary>
Motivation: 工业质量控制的视觉异常检测系统需要满足严格的实时性和隐私要求，必须从云端处理转向本地边缘部署。但边缘设备的内存和计算资源有限，需要开发高效的VAD方法。

Method: 基于流行的PatchCore和PaDiM模型，提出两种优化方法：1) PatchCore-Lite：先在量化压缩的内存库上进行粗搜索，然后在解码子集上进行精确搜索；2) Padim-Lite：使用对角协方差将马氏距离转化为高效的元素级计算。

Result: 在MVTec AD和VisA基准测试中验证了方法的有效性。PatchCore-Lite实现了79%的总内存占用减少，PaDiM-Lite实现了77%的内存减少和31%的推理时间降低。

Conclusion: 视觉异常检测可以有效地部署在边缘设备上，实现实时、私密且成本效益高的工业检测，为边缘计算环境中的质量控制系统提供了可行的解决方案。
...
TL;DR: 该论文首次对遥感图像去雾进行了系统化统一调研，整合了方法演进、基准评估和物理一致性分析，提出了从手工物理先验到数据驱动深度恢复再到混合物理智能生成的三阶段演进框架。


<details>
  <summary>Details</summary>
Motivation: 遥感图像常受雾、霾、薄云等退化影响，遮蔽地表反射率并阻碍下游应用，需要系统梳理去雾方法演进、评估基准和物理一致性，为未来发展提供指导。

Method: 将现有方法分为三阶段演进：手工物理先验、数据驱动深度恢复、混合物理智能生成；在5个公共数据集上使用12个指标进行大规模定量实验；进行专门的物理辐射一致性实验。

Result: Transformer和扩散模型将SSIM提升12%~18%，感知误差降低20%~35%；混合物理引导设计获得更高辐射稳定性；具有明确传输或大气光约束的模型减少颜色偏差达27%。

Conclusion: 总结了动态大气建模、多模态融合、轻量部署、数据稀缺和联合退化等开放挑战，为可信、可控、高效的去雾系统未来发展指明了方向。
...
TL;DR: 提出基于教师的知识保留方法，用于高光谱图像增量分类，通过增量类别样本缓解模型对旧类别的遗忘，无需依赖旧类别样本，并引入基于掩码的部分类别知识蒸馏算法提升准确性。


<details>
  <summary>Details</summary>
Motivation: 在高光谱图像增量分类任务中，灾难性遗忘是一个不可避免的挑战。虽然记忆回放方法可以缓解这个问题，但它们严重依赖于旧类别样本。本文旨在开发一种不依赖旧类别样本的增量学习方法。

Method: 提出基于教师的知识保留方法，利用增量类别样本缓解模型对旧类别样本的遗忘。引入基于掩码的部分类别知识蒸馏算法，通过解耦知识蒸馏来过滤可能误导学生模型的错误信息。

Result: 对比实验和消融实验表明，所提出的方法具有鲁棒的性能表现。

Conclusion: 该方法能够有效缓解高光谱图像增量分类中的灾难性遗忘问题，不依赖旧类别样本，通过知识蒸馏技术提升模型准确性。
...
<details>
  <summary>Details</summary>
Motivation: Flow Matching模型虽然在高保真视觉生成方面表现出色，但对数据集偏差高度敏感，导致在生成分布外或少数类样本时出现严重的语义退化问题。作者发现这种性能下降是由条件期望平滑机制引起的，该机制在推理过程中不可避免地导致轨迹锁定。

Method: 作者首先在FM框架内对"偏差流形"进行了严格的数学形式化，然后提出了InjectFlow方法。这是一种无需训练的方法，通过在初始速度场计算中注入正交语义，而不需要改变随机种子。这种设计有效防止了潜在漂移向多数模式，同时保持了高生成质量。

Result: 在GenEval数据集上的大量实验证明了该方法的有效性。InjectFlow成功修复了75%的标准流匹配模型无法正确生成的提示。该方法为构建更公平和鲁棒的视觉基础模型提供了即用型解决方案。

Conclusion: 本文的理论分析和算法为解决Flow Matching模型中的数据集偏差问题提供了有效的解决方案，能够显著提高模型在生成少数类样本时的性能，为构建更公平、鲁棒的视觉基础模型提供了重要工具。
...
TL;DR: DiffMark是一种用于扩散模型的即插即用水印方法，通过在每个去噪步骤注入持久性扰动实现单次多比特检测、每图像密钥灵活性和跨模型可迁移性。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型水印方法存在局限性：基于采样的方法需要昂贵的DDIM反演（N=50步）且仅支持零比特检测；基于微调的方法可实现快速多比特提取，但水印与特定模型检查点耦合，需要为每个架构重新训练。

Method: DiffMark在完全冻结的扩散模型的每个去噪步骤注入持久性学习扰动δ，水印信号在最终去噪潜在空间z0中累积并通过单次前向传播恢复。使用潜在一致性模型（LCM）作为可微分训练桥梁，将梯度步数从50步DDIM减少到4步LCM。

Result: DiffMark实现单次检测时间16.4毫秒，比基于采样的方法快45倍。提供真正的每图像密钥灵活性，无需每模型微调即可迁移到未见过的扩散架构。同时保持对失真、再生和对抗攻击的竞争性水印鲁棒性。

Conclusion: DiffMark是一种创新的扩散模型水印方法，通过持久性扰动注入和LCM训练桥梁，解决了现有方法在检测速度、密钥灵活性和模型可迁移性方面的限制，为扩散模型水印提供了高效实用的解决方案。
...
TL;DR: 论文指出当前地理空间数据融合存在"主从"范式局限，缺乏多源数据的对称利用和跨尺度、跨社区的反馈循环，提出通过案例研究建立更有效的交互模式。


<details>
  <summary>Details</summary>
Motivation: 当前地理空间数据融合主要采用"主从"范式，即一种数据源辅助处理另一种"主要"数据源，缺乏多源数据的对称利用和跨尺度、跨社区的反馈循环，限制了数据融合的潜力。

Method: 通过建立相关交互方案和案例研究，探索多源地理空间数据的对称利用模式，讨论跨尺度、跨社区的数据融合研究方向。

Result: 识别出当前数据融合配置中的不足，特别是多源数据对称利用的缺乏，以及跨尺度、跨社区反馈循环的缺失。

Conclusion: 需要建立更有效的多源地理空间数据交互模式，实现"全局-局部循环"，充分利用不同尺度、不同社区数据源的协同潜力。
...
TL;DR: EARTalking是一种端到端的GPT风格自回归模型，用于交互式音频驱动说话头生成，通过帧级流式生成和创新的注意力机制实现高效可控的视频生成。
...
Method: 提出EARTalking模型，采用帧级、上下文感知的音频驱动流式生成范式。引入Sink Frame Window Attention (SFA)机制支持变长视频生成并保持身份一致性，提出流式Frame Condition In-Context (FCIC)方案高效注入多样控制信号。

Result: 实验表明EARTalking优于现有自回归方法，性能可与基于扩散的方法相媲美。实现了上下文流式自回归控制，为灵活高效生成开辟了可扩展方向。

Conclusion: EARTalking证明了上下文流式自回归控制的可行性，为音频驱动说话头生成提供了一种灵活、高效的解决方案，代码将开源以确保可复现性。
...
TL;DR: GraphiContact：一个联合3D人体网格重建和顶点级人-场景接触预测的框架，通过Transformer编码器传递人体先验，并引入SIMU训练策略提升鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型经常基于语言先验而非视觉证据产生幻觉，这在临床应用中存在风险

Method: 提出视觉基础分数引导解码（VGS-Decoding），通过比较原始图像和失真图像的概率分布计算每个token的视觉依赖性，在解码时重新加权概率

Result: 在MIMIC-Diff-VQA和VQA-RAD数据集上，使用LLaVA-Med、CheXagent和MedGemma模型，实现了最高+9.12%的整体提升和+8.98%的开放式召回率提升

Conclusion: VGS-Decoding是一种无需训练、推理开销小的实用方法，可有效减少医学视觉语言模型的幻觉问题，适合临床部署
...
TL;DR: 该论文提出了一个以工作负载为中心的框架，用于决定哪些AI任务适合在轨道上执行而非地面云，并通过语义缩减原型验证了其可行性，展示了99%以上的数据缩减效果。


<details>
  <summary>Details</summary>
Motivation: 随着发射成本下降和AI工作负载增长，太空计算变得可行。需要确定哪些任务适合在轨道执行，哪些应留在地面云，以优化太空数据中心的使用。

Method: 提出了一个工作负载中心的框架和分阶段采用模型，通过轨道语义缩减原型验证。包括地球观测管道（将原始图像转换为紧凑语义特征）和多视角立体重建原型。

Result: 地球观测管道对西雅图和班加罗尔的Sentinel-2图像实现了99.7-99.99%的有效载荷缩减；多视角立体重建原型将约306MB数据缩减到约1.57MB（99.49%缩减）。

Conclusion: 早期适合太空的工作负载应由语义抽象驱动而非原始计算规模，语义缩减技术能显著降低数据传输需求，支持工作负载优先的太空计算策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频人体姿态估计方法依赖热图或隐式时空特征聚合，限制了关节拓扑表达能力并削弱了跨帧一致性，需要解决运动模糊、遮挡和复杂时空动态等挑战。

Method: 1) 设计视觉-时间速度关节嵌入，融合亚像素关节线索和帧间运动；2) 引入注意力驱动的姿态查询编码器，将关节表示映射到姿态感知节点空间；3) 提出双分支解耦时空注意力图，在专门分支中建模时间传播和空间约束推理；4) 节点空间专家融合模块自适应融合两个分支的互补输出。
...
TL;DR: DCG-Net是一个端到端的可解释框架，通过双交叉注意力模块和参数化概念图，结合多模态对齐与结构化概念推理，提升医学图像分析的性能和可解释性。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在医学图像分析中表现出色但缺乏可解释性。现有概念瓶颈模型通常忽略概念间的上下文依赖关系，无法充分捕捉临床概念的内在关联结构。

Method: 提出DCG-Net框架：1）双交叉注意力模块，用双向注意力替代余弦相似度匹配，实现视觉标记与规范化文本概念-值原型之间的空间局部证据归因；2）参数化概念图，使用正点互信息先验初始化并通过稀疏控制的消息传递进行优化，建模符合临床领域知识的概念间依赖关系。

Result: 在白细胞形态和皮肤病变诊断实验中，DCG-Net实现了最先进的分类性能，同时产生临床可解释的诊断解释。

Conclusion: DCG-Net通过整合多模态对齐与结构化概念推理，不仅提升了医学图像分类性能，还提供了符合临床认知的可解释诊断过程，解决了现有概念瓶颈模型忽略概念间依赖关系的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有分割方法计算量大、跨数据集泛化能力有限；基于SAM的方法通常依赖提示或复杂解码器，不适合细胞密集、外观异质的组织病理学图像
...
TL;DR: 提出了一种尺度编码的神经表面表示方法，通过将像素覆盖面积纳入神经表示，解决了多视角法线不一致导致的表面高频细节模糊问题。


<details>
  <summary>Details</summary>
Motivation: 传统多视角法线积分方法通常每个像素只采样一条光线，未考虑像素覆盖的空间区域随相机内参和相机-物体距离变化的问题。当目标物体在不同距离被拍摄时，对应像素的法线在不同视角下可能不同，导致多视角表面法线不一致，进而使重建表面高频细节模糊。

Method: 1. 提出尺度编码的神经表面表示，将像素覆盖面积纳入神经表示；2. 将每个3D点与空间尺度关联，通过混合网格编码计算其法线；3. 引入网格提取模块，基于训练观测为每个顶点分配最优局部尺度，实现尺度感知的表面重建。

Result: 实验结果表明，该方法能够从不同距离观测的法线中一致地产生高保真表面重建，优于现有的多视角法线积分方法。

Conclusion: 通过将像素覆盖面积编码到神经表示中，并采用尺度感知的表面重建方法，有效解决了多视角法线不一致问题，实现了高质量的表面重建。
...
TL;DR: 本文提出了一种新颖的360°显著性图表示方法，用于增强场景定位和基于视觉的室内导航。该方法通过节点、边、边权重和角度位置显式编码场景的视觉、上下文、语义和几何信息，解决了传统方法在室内环境中面临的照明变化、遮挡和阴影等挑战。


<details>
  <summary>Details</summary>
Motivation: 现有的场景表示方法（如RGB-D、LiDAR扫描、关键点等）通常将信息隐式嵌入，对于场景索引和视觉导航等应用不够高效。传统方法在室内环境中面临照明变化、遮挡和阴影等挑战，且缺乏场景特定信息，限制了导航性能。

Method: 提出360°显著性图表示方法，将场景的相关视觉、上下文、语义和几何信息显式编码为图中的节点、边、边权重和角度位置。该表示对场景视角变化具有鲁棒性。首先利用该表示在拓扑地图中定位查询场景，然后利用图中嵌入的几何信息估计朝向目标目的地的下一个移动方向，实现2D导航。

Result: 实验结果表明，所提出的360°显著性图表示方法在增强场景定位和基于视觉的室内导航方面表现出色，相比现有导航方法有显著改进。

Conclusion: 360°显著性图是一种丰富且高效的场景表示方法，能够显式编码多种场景信息，对视角变化具有鲁棒性，有效解决了室内导航中的挑战，显著提升了场景定位和导航性能。
...
Method: 提出Uni-Classifier (Uni-C)模块，利用视频扩散先验来引导上游生成模型的去噪过程，使其输出更符合下游模型的要求。该模块可即插即用，也可独立用于增强单个生成模型的输出质量。

Result: 在视频和3D生成任务上的大量实验表明，Uni-C在基于工作流和独立应用两种设置下都能持续提升生成质量，展现了其多功能性和强大的泛化能力。

Conclusion: Uni-C是一个简单有效的解决方案，能够解决多模型级联中的分布对齐问题，提升AI工作流的整体生成质量，并具有广泛的适用性。
...
TL;DR: 提出多阶段微调方法用于13类白细胞分类，通过不同分类头（线性、余弦、MLP）针对不同成熟阶段细胞实现专业化分类，构建头多样性集成模型提升性能。


<details>
  <summary>Details</summary>
Motivation: 白细胞分类对白血病诊断至关重要，但自动化方法面临类别不平衡、领域偏移和形态连续体混淆等挑战，特别是相邻成熟阶段细胞具有细微重叠特征。

Method: 基于微调的DINOBloom-base模型，训练多种分类头家族（线性、余弦、MLP），发现不同分类头对不同成熟阶段细胞有专业化表现，据此构建头多样性集成模型，MLP作为主要预测器，在四个预定义混淆对中仅当另外两个分类头一致时才替换预测。

Result: 余弦头在成熟粒细胞边界表现最佳（BNE F1=0.470），线性头在较不成熟粒细胞类表现更好（MMY F1=0.585），MLP头在最不成熟粒细胞表现最优（PMY F1=0.733）。集成模型进一步提升了分类性能，并发现所有模型持续误分类的病例很可能存在标注错误或固有形态模糊性。

Conclusion: 不同分类头家族对白细胞不同成熟阶段具有专业化分类能力，基于此构建的头多样性集成模型能有效处理形态连续体混淆问题，同时模型的一致误分类可用于识别可能的标注错误或固有模糊病例。
...
<details>
  <summary>Details</summary>
Motivation: 现有几何方法依赖繁琐的相机标定，而学习方法因缺乏多视角训练数据对未见相机配置泛化能力差，限制了实际应用
...
Conclusion: 提出了一种无需校准、无需多视角训练数据的框架，能够泛化到任意相机配置，解决了现有方法的局限性
...
<details>
  <summary>Details</summary>
Motivation: 传统全微调方法在大规模预训练模型上成本过高，特别是在多任务学习中成本随任务数量线性增长。现有的参数高效微调方法使用固定秩，无法适应不同任务或架构位置的需求，且缺乏捕捉任务间空间关系的能力。
...
Conclusion: FAAR通过自适应秩分配和频谱感知的空间关系学习，为多任务学习提供了高效且有效的微调方案，平衡了参数效率和性能表现。
...
TL;DR: 提出了个性化流式视频理解（PSVU）新任务，建立了PEARL-Bench基准，并开发了PEARL训练免费策略作为强基线方法
...
Method: 1) 提出并正式定义PSVU任务；2) 建立PEARL-Bench基准，包含132个独特视频和2173个细粒度标注，支持帧级和视频级两种模式；3) 提出PEARL训练免费策略作为强基线

Result: PEARL在8个离线和在线模型上实现最先进性能，应用于3种不同架构时均带来一致的PSVU改进，证明其高效性和鲁棒性

Conclusion: 该工作推进了视觉语言模型的个性化研究，为流式个性化AI助手的发展提供了新方向和基准，代码已开源
...
TL;DR: 本文提出了一种轻量级预处理和splatting流程，用于改善热成像相机的新视角合成性能，解决了热图像动态范围低和光度波动问题。


<details>
  <summary>Details</summary>
Motivation: 热成像相机在黑暗和恶劣条件下提供可靠可见性，但热图像的新视角合成比可见光图像困难得多。主要问题包括：1）热图像动态范围极低，削弱了外观线索并限制了优化梯度；2）热数据存在快速帧间光度波动和缓慢辐射漂移，破坏了对应关系估计并导致高频漂浮伪影。

Method: 引入轻量级预处理和splatting流程，扩展可用动态范围并稳定每帧光度测量。该方法不需要任何数据集特定的调优。

Result: 在仅使用热成像的新视角合成基准测试中实现了最先进的性能。

Conclusion: 通过针对热成像特定挑战设计的预处理和splatting方法，显著改善了热图像的新视角合成质量，特别是在没有RGB引导的情况下。
...
TL;DR: CREG是一种无需训练的可解释性框架，通过将多层对比梯度-激活归因投影到极坐标系中，生成方向性证据分布，用于解释视觉语言模型中的空间方向关系。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在空间推理基准上表现良好，但模型如何编码方向关系仍不清楚。现有的归因方法如GradCAM和注意力展开只能显示模型关注的位置，无法揭示对象之间的方向推断。

Method: 提出CREG框架：1) 使用多层对比梯度-激活归因；2) 将归因投影到以参考对象为中心的极坐标系中；3) 生成罗盘扇区的方向性证据分布。同时提出三个评估指标：方向对齐误差、边准确性和因果遮挡分数。

Result: 在Qwen2-VL-7B模型上，CREG在VSR和COCO-Pairs数据集上均优于标准归因基线。在COCO-Pairs上，预测目标CREG的方向对齐误差为55.5度，边准确性为0.553，比注意力展开方法分别改善了16.1度和0.120。因果遮挡实验进一步支持了方向解释的忠实性。

Conclusion: 对比性多层归因能够比标准显著性解释更忠实地揭示视觉语言模型空间推理中的方向证据。CREG在较大规模模型上效果更好，表明它受益于更大规模模型中出现的更结构化空间表示。
...
TL;DR: 该研究首次系统分析了相机陷阱物种识别随时间变化的挑战，提出了包含546个相机陷阱的流式评估基准，发现现有生物基础模型在特定站点表现不佳，揭示了时间偏移和类别不平衡是主要困难，并提出了改进策略。


<details>
  <summary>Details</summary>
Motivation: 现有计算机视觉研究主要关注跨域泛化，但忽视了生态实践中面临的核心挑战：在固定站点随时间推移保持可靠的物种识别能力。生态系统动态变化导致背景和动物分布发生显著时间偏移，需要专门研究来解决这一实际问题。

Method: 提出了第一个相机陷阱物种识别随时间变化的统一研究框架，创建了包含546个相机陷阱的流式评估基准，采用按时间顺序分段的评估协议，模拟真实部署生命周期，对比分析了零样本模型、站点适应策略以及模型更新与后处理技术的集成效果。

Result: 研究发现：(1)生物基础模型（如BioCLIP 2）在许多站点初始阶段就表现不佳；(2)在真实评估场景下，使用过去数据更新模型并评估未来间隔时，简单适应策略甚至可能低于零样本性能；(3)识别出两个主要困难：严重的类别不平衡和连续间隔间物种分布与背景的显著时间偏移；(4)有效集成模型更新和后处理技术可以显著提高准确性，但与理论上限仍有差距。

Conclusion: 该研究为生态实践者提供了可操作的部署指南，同时为计算机视觉和机器学习领域确立了新的研究方向，强调了站点特定适应和时间动态管理的重要性，并提出了预测零样本模型在新站点成功时机以及确定模型更新必要性的关键开放问题。
...
<details>
  <summary>Details</summary>
Motivation: 偏振信息对材料识别很重要，但传统测量需要多次调制偏振状态，过程耗时且对某些识别任务不必要。虽然可以使用部分偏振测量进行分类，但最佳测量角度配置不明确
...
Method: 1) 识别现有检索评估指标的两个关键限制；2) 提出基于多模态大语言模型作为评估器的替代评估框架；3) 探索CLIP嵌入空间中是否存在与否定相关的方向；4) 通过表示工程进行测试时干预，无需微调即可引导CLIP实现否定感知行为；5) 在非常见图像-文本样本上测试否定理解以评估分布偏移下的泛化能力。

Result: 发现CLIP嵌入空间中确实存在与否定相关的方向，通过表示工程干预可以引导CLIP实现否定感知行为，且无需模型微调。多模态大语言模型评估框架提供了更公平的否定理解评估方法。

Conclusion: CLIP嵌入空间中存在可操作的否定相关方向，通过表示工程干预可在不进行微调的情况下提升模型的否定理解能力。多模态大语言模型为评估否定理解提供了更可靠的框架，该方法在分布偏移下也展现出良好的泛化能力。
...
TL;DR: 该论文提出了一种基于弱到强原则的混合指导方法SGG，结合了CFG和AG的优势，并在训练和推理阶段都验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成过程中存在模拟自由目标与迭代过程之间的不匹配问题，导致梯度误差累积和泛化能力不足。现有指导方法（如CFG和AG）虽然有效，但其适用条件尚不明确，选择合适指导方法存在模糊性。

Method: 首先通过合成比较从弱到强原则角度分析CFG和AG的有效机制，然后提出混合实例SGG。进一步将W2S原则和SGG迁移到训练目标中，提升无指导扩散模型的泛化能力。

Result: 在推理阶段，SGG在SD3和SD3.5上优于现有无需训练的指导变体。在训练阶段，基于Transformer架构的实验验证了该方法的有效迁移，在条件和非条件设置下均获得性能提升。

Conclusion: 提出的SGG方法基于弱到强原则，有效结合了不同指导方法的优势，在扩散模型的训练和推理阶段都能提升性能，为选择和应用指导方法提供了理论依据和实践方案。
...
TL;DR: RayMap3R是一个免训练的流式动态场景重建框架，通过对比RayMap和图像预测来识别动态区域，抑制其在内存更新中的干扰，实现实时3D重建。
...
Method: 提出RayMap3R框架，基于RayMap预测具有静态场景偏置的观察，构建双分支推理方案：通过对比RayMap预测和图像预测来识别动态区域，在内存更新时抑制动态区域的干扰。进一步引入重置度量对齐和状态感知平滑来保持度量一致性和稳定预测轨迹。

Result: 该方法在多个基准测试中，在动态场景重建方面实现了流式方法中最先进的性能表现。

Conclusion: RayMap3R通过利用RayMap的静态场景偏置特性，实现了免训练的流式动态场景重建，有效解决了移动物体对重建质量的影响问题。
...
TL;DR: GaussianPile：结合3D高斯泼溅与成像系统感知聚焦模型，用于切片体数据的高效压缩与可视化，实现16倍压缩和11倍加速


<details>
  <summary>Details</summary>
Motivation: 切片体成像应用广泛，需要既能高压缩又能保留内部结构以供分析的表示方法。现有方法在压缩效率、重建质量和实时渲染方面存在挑战。

Method: 提出三个关键创新：1) 切片感知的堆积策略，定位各向异性3D高斯来建模切片贡献；2) 可微分投影算子，编码成像系统的有限厚度点扩散函数；3) 紧凑编码和联合优化管道，同时重建和压缩高斯集合。

Result: 在显微镜和超声数据集上验证，方法减少存储和重建成本，保持诊断保真度，支持快速2D可视化和3D体素化。仅需3分钟获得高质量结果，比NeRF方法快11倍，实现16倍压缩。

Conclusion: GaussianPile为切片体数据集的部署压缩和探索提供了实用路径，结合了高斯原语的压缩效率、实时渲染能力和高频体细节保留能力。
...
<details>
  <summary>Details</summary>
Motivation: 基于指令的图像编辑是统一多模态模型的关键能力，但构建大规模、多样化、高质量编辑数据集面临挑战：现有数据集要么依赖昂贵的闭源API，要么使用固定的合成编辑流程，导致质量有限且泛化能力差。
...
Conclusion: 开源智能体管道可以接近商业级数据质量，同时保持成本效益和可扩展性。框架和数据集都将开源。
...
TL;DR: 本文提出MCL-FIR框架，用于细粒度时尚图像检索的类增量学习，通过多头设计、对比学习和EMA蒸馏解决动态场景中新属性出现的问题。
...
Method: 提出MCL-FIR框架：1）采用多头设计以适应增量中的演化类别；2）将三元组输入重构为使用InfoNCE的双元组，实现更简单有效的训练；3）使用EMA蒸馏进行高效知识迁移。

Result: 在四个数据集上的实验表明，MCL-FIR在可扩展性之外，在效率和准确性之间取得了良好平衡。在相似训练成本下显著优于CIL基线方法，与静态方法相比，仅使用约30%的训练成本就能达到相当的性能。

Conclusion: MCL-FIR框架为细粒度时尚图像检索的类增量学习提供了有效的解决方案，在动态场景中实现了良好的可扩展性、效率和准确性平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有的扩散和流模型虽然能实现高质量的真实世界图像超分辨率，但多步采样导致推理速度慢且难以部署。一步蒸馏虽然能降低成本，但会降低恢复质量并失去多步优化的灵活性。

Method: 提出Mean Flows for Super-Resolution (MFSR)框架，使用MeanFlow作为学习目标，让学生模型能够近似概率流ODE任意状态间的平均速度，无需显式展开就能捕捉教师模型的动态。同时改进了原始MeanFlow的无分类器引导公式，采用教师CFG蒸馏策略来增强恢复能力和保留细节。

Result: 在合成和真实世界基准测试中，MFSR实现了高效、灵活且高质量的超分辨率，结果与多步教师模型相当甚至更好，同时计算成本大幅降低。

Conclusion: MFSR框架成功解决了扩散和流模型在真实世界图像超分辨率中的效率与质量权衡问题，实现了单步高质量恢复的同时保留了多步优化的灵活性。
...
TL;DR: 该研究探索了从卫星图像合成灾后街景视图的方法，以弥补灾害响应中的地面视角数据缺口，提出了两种生成策略并建立了评估框架，揭示了视觉真实性与结构准确性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 自然灾害发生后，快速获取态势感知至关重要。传统卫星观测能评估损害范围，但缺乏地面视角来识别具体结构破坏；而地面数据（如街景图像）在时效性强的灾害事件中难以获取。因此需要卫星到街景的合成技术来弥合这一数据缺口。

Method: 提出了两种生成策略：1）视觉语言模型（VLM）引导方法；2）损害敏感的专家混合（MoE）方法。使用提出的结构感知评估框架进行基准测试，该框架包含三层协议：像素级质量评估、基于ResNet的语义一致性验证、以及新颖的VLM-as-a-Judge感知对齐评估。

Result: 在300个灾害场景的实验中发现关键的现实主义-保真度权衡：基于扩散的方法（如ControlNet）实现高感知真实性，但经常幻觉结构细节。定量结果显示标准ControlNet获得最高语义准确度（0.71），而VLM增强和MoE模型在纹理合理性方面表现优异，但在语义清晰度方面存在困难。

Conclusion: 这项工作为可信的跨视图合成建立了基准，强调视觉上逼真的生成可能仍无法保留可靠灾害评估所需的关键结构信息，指出了未来研究需要平衡感知真实性和结构准确性的方向。
...
TL;DR: 本文提出CogAlign框架，通过临床认知对齐和反事实强化学习解决MLLM在胃肠内镜诊断中的两个关键问题：临床推理不对齐和视觉特征与诊断结果缺乏因果关联。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在医学图像分析中表现出潜力，但在胃肠内镜应用中面临两个关键限制：1）通用模型推理与标准化临床认知路径不对齐；2）视觉特征与诊断结果之间缺乏因果关联。

Method: 提出CogAlign框架：1）构建层次化临床认知数据集并通过监督微调赋予模型严谨的临床分析能力；2）提出反事实驱动的强化学习策略，通过病灶掩蔽生成反事实正常样本，并以临床认知为中心进行奖励优化，确保诊断严格基于因果病灶特征。

Result: 在多个基准测试中实现了最先进的性能，显著提升了复杂临床场景下的诊断准确性。

Conclusion: CogAlign框架通过临床认知对齐和因果校正，有效解决了MLLM在胃肠内镜诊断中的关键限制，为医学图像分析提供了新的解决方案。
...
TL;DR: 本文系统研究了3D高斯泼溅（3DGS）方法中初始化与稠密化之间的关系，发现当前稠密化方法无法充分利用密集初始化，在稀疏SfM初始化基础上改进有限。


<details>
  <summary>Details</summary>
Motivation: 3DGS通常使用稀疏的SfM点云作为初始化，然后通过稠密化阶段获得密集高斯云。然而，不同初始化类型（如密集激光扫描、多视图立体点云等）与稠密化方案之间的关系尚未得到系统研究。

Method: 提出了一个新的基准测试，系统研究不同类型初始化（密集激光扫描、密集多视图立体点云、密集单目深度估计、稀疏SfM点云）与不同稠密化方案的组合效果。

Result: 研究发现当前稠密化方法无法充分利用密集初始化，通常无法显著超越基于稀疏SfM初始化的结果。

Conclusion: 需要开发更好的稠密化方法来充分利用密集初始化数据，作者将公开其基准测试供社区使用。
...
TL;DR: 提出弱监督多模态分割框架，通过阈值引导的伪标签学习，结合二维声波图像和一维测井数据，实现无标注的井壁结构分割。


<details>
  <summary>Details</summary>
Motivation: 声波井壁图像提供高分辨率结构信息，但大规模解释困难，因为密集专家标注稀缺，且地下信息本质上是多模态的。需要开发结合二维图像纹理和深度对齐一维测井的弱监督方法。

Method: 引入弱监督多模态分割框架，通过阈值引导的伪标签进行学习细化。保持经典阈值和聚类工作流的无标注特性，同时加入去噪、置信感知伪监督和物理结构化融合。比较了多种融合策略：直接拼接、深度感知交叉注意力、门控融合和置信感知调制。

Result: 阈值引导的学习细化相比原始阈值、去噪阈值和潜在聚类基线提供最稳健的改进。多模态性能强烈依赖融合策略：直接拼接增益有限，而深度感知交叉注意力、门控融合和置信感知调制显著提高与弱监督参考的一致性。最强模型CG-DCA（置信门控深度感知交叉注意力）始终优于基于阈值、仅图像和早期多模态基线。
...
TL;DR: 提出VSD-MOT框架，通过视觉语义蒸馏解决低质量视频中的多目标跟踪性能下降问题，使用CLIP图像编码器提取全局视觉语义信息，并通过知识蒸馏和动态权重调节模块提升跟踪效果。
...
Method: 提出视觉语义蒸馏引导的多目标跟踪框架(VSD-MOT)：1) 引入CLIP图像编码器提取全局视觉语义信息补偿信息损失；2) 采用教师-学生学习框架进行知识蒸馏，避免直接集成影响效率；3) 设计双约束语义蒸馏方法(DCSD)使学生模型获得适合多目标跟踪任务的视觉语义提取能力；4) 提出动态语义权重调节模块(DSWR)，根据实时帧质量评估自适应分配融合权重。

Result: 大量实验证明该方法在现实世界低质量视频场景中的有效性和优越性，同时在常规场景中也能保持良好的性能。

Conclusion: 通过视觉语义蒸馏和动态权重调节，VSD-MOT框架能够有效解决低质量视频中的多目标跟踪问题，在保持常规场景性能的同时，显著提升低质量视频场景的跟踪效果。
...
TL;DR: SATTC是一种无标签的测试时校准方法，通过几何专家和结构专家优化跨被试EEG-图像检索，解决被试偏移和中心性问题，提升检索可靠性。
...
Method: SATTC直接在冻结的EEG和图像编码器的相似性矩阵上操作，结合几何专家（被试自适应白化和自适应CSLS）和结构专家（互最近邻、双向top-k排名、类别流行度），通过专家乘积规则融合。

Result: 在THINGS-EEG数据集上，SATTC在严格留一被试协议下显著提升Top-1和Top-5准确率，减少中心性和类别不平衡，产生更可靠的小k候选列表，且增益可迁移到多个EEG编码器。

Conclusion: SATTC作为一种编码器无关、无标签的测试时校准层，能有效解决跨被试神经解码中的被试偏移和中心性问题，提升检索性能。
...
TL;DR: SADG是一个基于Mamba的上下文学习框架，用于点云的多任务域泛化，通过结构感知序列化、分层域感知建模和谱图对齐来保持跨域的结构层次性。


<details>
  <summary>Details</summary>
Motivation: 现有的Transformer和Mamba架构通常针对单任务或单域设置设计，直接应用于多任务域泛化会导致性能下降。Transformer虽然能有效建模全局依赖但计算成本高且缺乏显式结构排序，Mamba虽然提供线性时间递归但依赖于坐标驱动的序列化，对视角变化和缺失区域敏感，导致结构漂移和不稳定的序列建模。

Method: 提出结构感知域泛化(SADG)框架：1) 结构感知序列化(SAS)：使用基于质心的拓扑和测地曲率连续性生成变换不变的序列；2) 分层域感知建模(HDM)：通过整合域内结构和融合域间关系来稳定跨域推理；3) 谱图对齐(SGA)：在测试时通过谱域将目标特征向源原型对齐而不更新模型参数；4) 引入MP3DObject数据集用于多任务域泛化评估。

Result: 综合实验表明，该方法提高了结构保真度，在重建、去噪和配准等多个任务上持续优于最先进的方法。

Conclusion: SADG通过结构感知序列化、分层域感知建模和谱图对齐，有效解决了点云多任务域泛化中的结构漂移问题，在保持结构层次性的同时实现了跨域和跨任务的稳定性能。
...
TL;DR: 提出跨时间步自校准（CTCal）方法，利用小时间步的可靠文本-图像对齐信息来校准大时间步的表示学习，解决扩散模型中文本-图像对齐不精确的问题。
...
Method: 提出跨时间步自校准（CTCal）方法：1）利用小时间步（噪声较少）形成的可靠文本-图像对齐（交叉注意力图）来校准大时间步（噪声较多）的表示学习；2）提出时间步感知自适应加权，实现CTCal与扩散损失的和谐集成。该方法模型无关，可无缝集成到现有文本到图像扩散模型中。

Result: 在T2I-Compbench++和GenEval基准测试上的广泛实验证明了CTCal的有效性和泛化能力。该方法适用于基于扩散（如SD 2.1）和基于流（如SD 3）的方法。

Conclusion: CTCal通过提供明确的监督信号，有效解决了扩散模型中文本-图像对齐不精确的问题，提高了文本到图像合成的质量。
...
<details>
  <summary>Details</summary>
Motivation: 手术中纱布遗留在患者体内（称为"纱布瘤"）会导致严重并发症和医疗纠纷，传统人工计数方法耗时且分散护理资源

Method: 使用YOLOv5深度学习模型，通过实时视频监控识别标记为"In"和"Out"托盘上的纱布，从手术室采集大量图像进行训练并增强数据，开发集成模型同时识别人体和纱布

Result: 模型准确率提高，帧率从8 FPS提升到15 FPS，训练图像从2800张增加到11000张，系统支持手动计数调整功能

Conclusion: AI系统能有效预防纱布遗留问题，提高手术安全性，减少医疗纠纷风险，并已在新加坡中央医院实际应用
...
TL;DR: DiT-BlockSkip：一种内存高效的扩散变换器微调框架，通过动态补丁采样和块跳过机制，显著降低计算复杂度和内存使用，使大规模扩散变换器能够在资源受限设备上部署。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器（DiTs）虽然提升了文本到图像生成质量，但其微调过程需要大量计算资源和内存，限制了在资源受限环境（如智能手机、物联网设备）中的实际部署。

Method: 提出DiT-BlockSkip框架，包含两个核心技术：1）基于时间步的动态补丁采样策略，根据扩散时间步调整补丁大小并重采样到固定低分辨率；2）块跳过机制，选择性微调关键变换器块，并为跳过的块预计算残差特征，同时引入基于交叉注意力掩码的块选择策略。

Result: 评估显示该方法在保持竞争力的个性化生成质量的同时，显著减少了内存使用，为实现大规模扩散变换器在资源受限设备上的部署提供了可行性。

Conclusion: DiT-BlockSkip通过创新的动态补丁采样和块跳过机制，有效解决了扩散变换器微调中的内存和计算瓶颈问题，为实现边缘设备上的高质量个性化图像生成迈出了重要一步。
...
<details>
  <summary>Details</summary>
Motivation: 推理诱导的视觉语言模型（VLMs）通过文本推理推进了图像质量评估（IQA），但其标量评分往往缺乏敏感性，并塌缩到少数几个值（即离散崩溃问题）。

Method: 1. 构建记忆库并使用推理摘要检索语义和感知对齐的邻居；2. 将VLM重构为概率比较器以获得成对偏好概率，并在Thurstone Case V模型下将序数证据与初始分数融合；3. 执行门控反射并巩固记忆以改进未来决策。

Result: 实验表明，ME-IQA在多个IQA基准测试中相比强推理诱导VLM基线、现有非推理IQA方法以及测试时缩放替代方案均取得了一致的性能提升。

Conclusion: ME-IQA框架能够产生更密集、对失真更敏感的预测，有效缓解了离散崩溃问题，显著提升了推理诱导VLMs在图像质量评估中的性能。
...
TL;DR: Co-VLN：一个模型无关的协作视觉语言导航框架，通过让多个智能体在共享环境中交换感知记忆来克服单智能体的部分可观测性限制，无需额外探索成本即可扩展感知范围。


<details>
  <summary>Details</summary>
Motivation: 传统视觉语言导航系统受限于部分可观测性，智能体只能积累自己访问过的位置信息。随着多机器人系统在共享环境中日益普遍，研究人员希望探索智能体之间能否通过共享观测信息来相互受益。

Method: 提出Co-VLN框架，当独立导航的智能体识别到共同经过的位置时，它们交换结构化的感知记忆。该框架模型无关，在R2R基准上验证了两种代表性范式：基于学习的DUET和零样本的MapGPT。

Result: 实验表明，通过视觉共享的模型在两种范式下都取得了显著的性能提升，为协作具身导航的未来研究奠定了坚实基础。

Conclusion: Co-VLN框架系统性地揭示了智能体间观测共享的潜在动态，证明了多智能体协作能够有效扩展感知范围并提升导航性能，为视觉语言导航开辟了新的研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在视觉语言任务上表现优异，但其语言驱动的训练过程对模型内部视觉基础能力的影响尚不清楚。研究者希望诊断分析MLLMs是否存在视觉表征退化问题，并理解这种现象的成因。

Method: 通过详细的诊断分析揭示MLLMs中视觉表征退化现象，发现与初始视觉特征相比，LLM中间层的视觉表征在全局功能和补丁结构上都出现退化。提出预测正则化方法，强制退化的中间特征预测初始视觉特征，从而保持MLLM内部表征的固有视觉属性。

Result: 实验证实MLLMs确实存在普遍的视觉表征退化问题，这种现象是由单一文本生成目标驱动的视觉牺牲造成的。缓解这种视觉退化能有效提升视觉语言性能，表明在MLLMs中培养强大的内部视觉表征对全面多模态理解至关重要。

Conclusion: 一个强大的MLLM需要同时具备强大的跨模态推理能力和核心视觉能力。预测正则化方法通过保持内部视觉表征的完整性，能够有效提升模型的多模态理解性能，强调了在MLLMs中维护视觉表征质量的重要性。
...
TL;DR: EDC框架通过三流编码器和差异条件混合融合机制，在降低计算复杂度的同时提升光学-SAR遥感图像在云遮挡下的语义分割鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 云遮挡严重破坏光学遥感图像的语义完整性，而结合SAR数据提供互补信息。现有方法依赖密集全局注意力进行长程依赖建模，但会传播云噪声，且增强鲁棒性通常需要增大模型容量，导致计算开销增加，形成效率-可靠性权衡困境。

Method: 提出EDC框架：1) 三流编码器配合载波令牌实现紧凑全局上下文建模；2) 差异条件混合融合机制在全局聚合时选择性抑制不可靠区域；3) 辅助云去除分支通过教师引导蒸馏增强遮挡下的语义一致性。

Result: 在M3M-CR和WHU-OPT-SAR数据集上，mIoU分别提升0.56%和0.88%，参数数量减少46.7%，推理速度加快1.98倍，实现精度和效率的双重提升。

Conclusion: EDC框架有效解决了光学-SAR遥感图像语义分割中的效率-可靠性权衡问题，通过紧凑全局建模和差异条件融合机制，在降低计算复杂度的同时提升了云遮挡下的分割鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于结构的重定位方法主要依赖点对应关系，而平面基元作为投影几何中的基本实体，能够同时封装结构和语义信息，为结构化环境中的相机重定位提供了更丰富的区域表示
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像扩散模型在处理需要深层世界知识（自然科学、文化常识等）的隐式提示时，存在生成反事实图像的严重缺陷，其根源在于底层知识结构的错位。

Method: 提出EruDiff框架：1) 扩散知识分布匹配(DK-DM)，将难以处理的隐式提示的知识分布与明确定义的显式锚点对齐；2) 仅负强化学习(NO-RL)策略，用于细粒度校正显式提示渲染中的固有偏差。

Result: 在科学知识基准(Science-T2I)和世界知识基准(WISE)上的严格评估表明，该方法显著提升了FLUX和Qwen-Image等领先扩散模型的性能，证明了其有效性和泛化能力。

Conclusion: EruDiff通过重构扩散模型中的知识结构，有效解决了隐式提示处理中的知识错位问题，提升了模型在需要深层世界知识场景下的图像生成质量。
...
TL;DR: MERIT提出首个统一的多域RAW图像翻译框架，使用单一模型处理任意相机域间的转换，通过传感器感知噪声建模和多尺度大核注意力模块提升性能，在质量和可扩展性上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 不同相机传感器捕获的RAW图像存在显著域偏移（光谱响应、噪声特性、色调行为差异），这阻碍了其在计算机视觉任务中的直接使用。现有方法需要为每对源-目标域训练特定翻译器，无法扩展到涉及多种商业相机的真实场景。

Method: 提出MERIT统一框架：1）传感器感知噪声建模损失，显式对齐生成图像与目标域的信号相关噪声统计；2）条件多尺度大核注意力模块，增强上下文和传感器感知特征建模；3）构建MDRAW数据集，包含五种不同相机传感器的配对和非配对RAW图像。

Result: MERIT在质量和可扩展性上均显著优于先前模型：质量提升5.56 dB，训练迭代次数减少80%。MDRAW数据集为多域RAW图像翻译提供了标准化评估基准。

Conclusion: MERIT是首个统一的多域RAW图像翻译框架，通过传感器感知噪声建模和条件注意力机制，能够高效处理任意相机域间的转换，解决了现有方法无法扩展到多相机场景的问题。
...
TL;DR: GOLDMARK是一个用于计算病理学的标准化基准框架，基于TCGA队列和OncoKB生物标志物标签，提供结构化中间表示和可重复评估，在33个肿瘤-生物标志物任务中平均AUROC为0.689-0.630。


<details>
  <summary>Details</summary>
Motivation: 计算病理学缺乏标准化的中间数据格式、溯源追踪、检查点约定和可重复评估指标，这阻碍了临床级计算生物标志物的部署。现有方法虽然提升了预测性能，但缺乏统一的基准框架。

Method: 开发GOLDMARK基准框架，基于TCGA队列和OncoKB 1-3级生物标志物标签，提供结构化中间表示（包括切片坐标图、特征嵌入、质量控制元数据、患者级分割、训练模型和评估输出）。在TCGA上训练模型，在独立MSKCC队列上进行互惠测试。

Result: 在33个肿瘤-生物标志物任务中，平均AUROC为0.689（TCGA）和0.630（MSKCC）。在8个表现最佳的任务中，平均AUROC分别为0.831和0.801，这些任务对应已知的形态-基因组关联（如LGG IDH1、COAD MSI/BRAF等）。不同编码器间的差异相对于任务特异性变异较小。

Conclusion: GOLDMARK为计算病理学建立了共享实验基础，实现了跨数据集和模型的方法可重复基准测试和直接比较，有助于推动临床级计算生物标志物的发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统手部视频缺乏接触力、运动信号等物理信息，且容易受到遮挡，这限制了手物交互理解在计算机视觉、机器人学和AR/VR中的应用。

Method: 提出Glove2Hand框架：1) 使用新型3D高斯手部模型确保时间渲染一致性；2) 通过基于扩散的手部修复器将渲染的手部无缝集成到场景中，处理复杂的手物交互和非刚性变形。

Result: 创建了HandSense数据集，这是首个包含同步触觉和IMU信号的手套到裸手视频的多模态手物交互数据集。该数据集显著提升了基于视频的接触估计和严重遮挡下的手部跟踪等下游应用性能。

Conclusion: Glove2Hand框架成功解决了传统手部视频缺乏物理信息和易受遮挡的问题，通过将传感手套数据转换为逼真的裸手视频，为手物交互理解提供了更丰富的数据支持。
...
TL;DR: 本文提出了一种轻量级集成方法用于白细胞分类，特别关注粒细胞生成、单核细胞生成和淋巴细胞生成过程，通过数据集扩展缓解类别不平衡，使用三种轻量级预训练模型的集成取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 自动化白细胞分类对于白血病诊断具有重要意义，可以替代耗时耗资源的病理专家检查。然而，由于染色、扫描差异以及患者间异质性，设计用于罕见细胞类型分类的鲁棒算法仍然具有挑战性。

Method: 提出轻量级集成方法，使用三种预训练模型（SwinV2-Tiny、DinoBloom-Small和ConvNeXT-V2-Tiny）进行集成。通过数据集扩展缓解类别不平衡，采用分层3折交叉验证框架训练每个架构的3个实例，对输入图像通过所有9个模型进行前向传播，并通过逻辑平均进行聚合。

Result: 该方法在这个具有挑战性的数据集上取得了优异的性能。作者进一步分析了模型在混淆相似外观的粒细胞生成中的髓细胞和淋巴细胞生成中的淋巴细胞方面的弱点。

Conclusion: 轻量级集成方法在白细胞分类任务中表现出色，特别是在处理罕见细胞类型和类别不平衡问题上具有优势。代码已开源供进一步研究。
...
Method: 1. 集成逐高斯嵌入与粗到细时间嵌入策略，通过早期融合时间嵌入加速渲染；2. 引入深度和误差引导采样策略，在低偏差初始位置添加新3D高斯，减轻变形场优化负担；3. 通过调节不透明度变化缓解暗场景中的局部最优问题。

Result: 综合实验结果表明，该方法在保持最先进视觉质量的同时实现了加速渲染，在动态场景重建中表现出优越性能。

Conclusion: FRoG框架通过创新的时间嵌入策略、采样方法和局部最优缓解技术，有效解决了现有动态3D高斯泼溅方法的局限性，实现了高效鲁棒的高质量动态场景重建。
...
Method: 提出TAFG-MAN框架，结合感知优化的自编码器、紧凑潜在空间中的条件潜在扩散恢复，以及轻量级时间步自适应频率门控设计。TAFG将条件特征分解为低频和高频分量，从当前去噪特征和时间步嵌入预测自适应门控，在交叉注意力前逐步释放高频指导。

Result: 实验表明TAFG-MAN在质量-效率权衡方面优于代表性基线方法。与无TAFG的基础变体相比，进一步提高了细节保留和感知质量，同时保持基本相同的推理成本。

Conclusion: TAFG-MAN通过时间步自适应频率门控机制，在早期去噪步骤依赖稳定的结构指导，在后期逐步引入精细细节，有效改善了低剂量CT图像去噪中噪声抑制与细节保留的平衡。
...
TL;DR: 提出可控视频分割与字幕生成任务，通过用户提示（如边界框）同时生成相关掩码和字幕，开发SG-FSCFormer框架实现用户意图驱动的细粒度视频理解


<details>
  <summary>Details</summary>
Motivation: 现有视频多模态理解方法主要关注全局理解，用户交互有限。需要让用户能够通过特定提示（如边界框）来指导视频内容的理解和生成

Method: 提出SG-FSCFormer框架，包含提示引导的时间图变换器来捕捉用户意图，以及细粒度掩码-语言解码器协同预测高质量字幕-掩码对，使用多实体对比损失实现细粒度对齐

Result: 在两个基准数据集上的实验表明，SG-FSCFormer实现了显著性能，有效捕捉用户意图并生成符合用户要求的精确多模态输出

Conclusion: 提出的可控视频分割与字幕生成任务和SG-FSCFormer框架能够增强用户对视频的理解，通过用户提示实现精确的多模态输出生成
...
<details>
  <summary>Details</summary>
Motivation: 神经元形态编码了电路功能、发育和疾病的关键信息，但现有方法分别分析拓扑或图形结构，缺乏统一框架。

Method: 采用多模态架构：视觉分支通过DINOv2-ViT-S处理三通道持久性图像（未加权、持久性加权和半径加权拓扑密度）；并行使用TreeLSTM编码器从骨架图捕获几何和径向属性；两者投影到共享嵌入空间，通过对称InfoNCE损失训练，并辅以保持拓扑语义的持久性空间变换。

Result: 在六个基准测试（BIL-6, ACT-4, JML-4, N7, M1-Cell, M1-REG）中，GraPHFormer在五个基准上达到最先进性能，显著优于仅拓扑、仅图形和形态计量基线。能够区分皮质区域和物种间的胶质细胞形态，并检测发育和退化过程的特征。

Conclusion: GraPHFormer通过统一拓扑和图形结构分析，为神经元形态分析提供了强大的多模态框架，在神经科学研究中具有实际应用价值。
...
TL;DR: SkinCLIP-VL是一个资源高效的视觉语言模型框架，用于皮肤癌诊断，通过冻结CLIP编码器和轻量级Qwen2.5-VL结合，解决了计算成本高、数据稀缺和模型黑盒问题。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在皮肤病学部署面临三重困境：高计算成本、极端数据稀缺和深度学习黑盒性质。这些挑战阻碍了VLMs在皮肤癌诊断中的实际应用。

Method: 采用"冻结感知、自适应推理"范式，将冻结的CLIP编码器与轻量级、量化的Qwen2.5-VL通过低秩适应（LoRA）集成。提出一致性感知焦点对齐（CFA）损失函数，在长尾分布下严格对齐视觉区域与临床语义。

Result: 在ISIC和Derm7pt基准测试中，SkinCLIP-VL在准确率上超过130亿参数基线模型4.3-6.2%，同时参数减少43%。盲法专家评估和分布外测试证实，其视觉基础推理显著增强了临床信任度。

Conclusion: SkinCLIP-VL框架成功解决了VLMs在皮肤病学中的部署障碍，通过资源高效的设计和可信赖的视觉推理，为皮肤癌诊断提供了实用且可信的解决方案。
...
TL;DR: SpatialFly是一个用于无人机视觉语言导航的几何引导空间表示框架，通过几何引导的2D表示对齐机制，在不进行显式3D重建的情况下解决2D视觉感知与3D轨迹决策空间的结构表示不匹配问题。
...
Method: 提出SpatialFly框架，包含几何先验注入模块和几何感知重参数化模块。前者将全局结构线索注入2D语义标记中，后者通过跨模态注意力将2D语义标记与3D几何标记对齐，并使用门控残差融合保持语义区分度。
...
TL;DR: 该研究提出了一个可扩展的对抗性评估框架，利用大语言模型生成临床合理的对抗性提示变体，评估医学视觉语言模型在超声图像问答任务中的对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 虽然超声在临床实践中广泛应用，但图像获取和解释高度依赖操作者，需要开发鲁棒的AI辅助分析方法。医学视觉语言模型在医学图像分析中表现出色，但其可信度存在显著问题，特别是对抗鲁棒性方面。由于Med-VLMs通过自然语言指令操作，提示表述成为现实的、可被利用的脆弱点，小的变化（如拼写错误、简写、不明确的请求或模糊措辞）可能显著改变模型输出。

Method: 提出一个可扩展的对抗性评估框架，利用大语言模型生成临床合理的对抗性提示变体，通过"人性化"重写和最小编辑来模拟常规临床交流。使用超声多选题问答基准，系统评估最先进的Med-VLMs对这些攻击的脆弱性，分析攻击者LLM能力如何影响攻击成功率，研究攻击成功率与模型置信度之间的关系，并识别跨模型的一致失败模式。

Result: 研究结果突显了现实的鲁棒性差距，这些差距必须在安全临床转化前得到解决。代码将在评审过程后公开发布。

Conclusion: 医学视觉语言模型在对抗性提示攻击下存在显著脆弱性，需要解决这些鲁棒性问题才能实现安全的临床转化。提出的评估框架为系统测试Med-VLMs的对抗鲁棒性提供了有效方法。
...
TL;DR: 本文提出了一种用于驾驶员监控的时序动作定位框架，采用两阶段方法：VideoMAE特征提取+增强自掩码注意力检测器，通过空间金字塔池化快速模块捕捉多尺度时序特征，在准确性和计算效率之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 从车内视频流中识别危险驾驶行为对提升道路安全至关重要，但现有时序动作定位技术难以平衡准确性与计算效率。本文针对驾驶员监控场景（如交通检查站、车队管理系统）开发专门的时序动作定位框架。

Method: 采用两阶段流水线：1) VideoMAE特征提取（使用ViT-Giant或ViT-base变体）；2) 增强自掩码注意力检测器，集成空间金字塔池化快速模块以捕捉多尺度时序特征。

Result: ViT-Giant骨干网在特征提取阶段达到88.09% Top-1测试准确率，但计算成本高（1584.06 GFLOPs/段）；ViT-base变体达到82.55%准确率，计算成本显著降低（101.85 GFLOPs/段）。在定位任务中，SPPF模块持续提升性能，ViT-Giant+SPPF达到92.67% mAP峰值，轻量级ViT-base配置也保持稳健结果。

Conclusion: 该框架在驾驶员监控场景中实现了准确性与计算效率的良好平衡，为交通检查站和车队管理等周期性检查场景提供了实用的解决方案，SPPF模块对性能提升有显著贡献。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D高斯溅射方法在RGBD SLAM中使用的3D高斯表示要么过于灵活（导致收敛慢），要么过于受限（导致渲染质量有限），需要一种更好的平衡方案。

Method: 1. 采用像素对齐的高斯体，但允许每个高斯体沿其光线调整位置以最大化渲染质量；2. 为加速跟踪，将每个像素周围的深度分布建模为高斯分布，并用这些分布快速将每帧对齐到3D场景。

Result: 在广泛使用的基准测试中，该方法在视图渲染、相机跟踪、运行时间和存储复杂度方面都优于最新方法。

Conclusion: 提出的像素对齐高斯体表示方法在保持系统可扩展性的同时，显著提升了3D高斯溅射在RGBD SLAM中的渲染质量和跟踪速度。
...
TL;DR: LRHPerception是一个实时单目感知系统，用于自动驾驶，通过单摄像头视频理解环境，在保持计算效率的同时实现物体跟踪、道路分割和深度估计。


<details>
  <summary>Details</summary>
Motivation: 当前基于摄像头的自动驾驶技术往往优先考虑效果而忽视计算效率，需要一种既能实时处理又能提供丰富环境感知的系统。

Method: 结合端到端学习的计算效率和局部映射方法的丰富表示细节，将物体跟踪预测、道路分割和深度估计集成到统一框架中，将单目图像处理成包含RGB、道路分割和像素级深度估计的五通道张量。

Result: 在单个GPU上实现29 FPS的实时处理，比最快的基于映射的方法快555%，在物体跟踪预测、道路分割和深度估计方面表现出色。

Conclusion: LRHPerception成功平衡了计算效率和感知性能，为自动驾驶提供了一个高效实用的实时单目感知解决方案。
...
TL;DR: NoOVD是一个新的开放词汇目标检测训练框架，通过自蒸馏机制和冻结视觉语言模型的知识来解决训练和测试阶段的差距问题，显著提升了新类别目标的检测性能。


<details>
  <summary>Details</summary>
Motivation: 当前开放词汇目标检测（OVD）存在训练和测试阶段的显著差距。在训练过程中，RPN和RoI头经常将未标记的新类别目标误分类为背景，导致一些提案被RPN过早过滤，而其他提案被RoI头进一步误分类。在测试时，这些提案再次获得低分并在后处理中被移除，导致召回率显著下降，最终削弱了新类别检测性能。

Method: 提出了NoOVD训练框架，创新性地集成了基于冻结视觉语言模型（VLM）知识的自蒸馏机制。具体包括：1）设计K-FPN，利用VLM的预训练知识指导模型发现新类别目标，并促进知识蒸馏（无需额外数据），从而防止将新目标强制对齐到背景；2）引入R-RPN，在推理过程中调整提案的置信度分数，以提高新类别目标的召回率。

Result: 在OV-LVIS、OV-COCO和Objects365数据集上的跨数据集评估表明，该方法在多个指标上始终实现卓越性能。

Conclusion: NoOVD框架通过自蒸馏机制和VLM知识引导，有效解决了开放词汇目标检测中训练-测试差距问题，显著提升了新类别目标的检测性能，在多个基准数据集上表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在跨模态感知和推理方面取得显著进展，但视觉编码器应该微调还是冻结的问题仍未解决。现有方法在不同训练设置下表现不一致，且无法稳定超越冻结基线，这源于视觉编码器的上下文无关特性导致的多模态上下文下的参数更新冲突。

Method: 提出上下文感知视觉微调（CoVFT）框架，包含上下文向量提取（CVE）和上下文混合专家（CoMoE）模块。CVE提取多模态上下文信息，CoMoE分解冲突的优化信号，实现稳定、上下文敏感的视觉参数更新。

Result: 在12个多模态基准测试中，CoVFT实现了最先进的性能，具有优越的稳定性。使用CoVFT微调的7B MLLM超越了其13B对应模型的平均性能，揭示了视觉编码器优化中的巨大潜力。

Conclusion: CoVFT通过显式引入多模态上下文解决了视觉微调中的视觉偏好冲突问题，为MLLM中视觉编码器的优化提供了有效框架，显著提升了模型性能。
...
TL;DR: TextCSP：一种基于文本引导的层次化脑肿瘤分割框架，通过文本调制软级联解码器、子区域感知提示调优和文本语义通道调制器，在TextBraTS数据集上显著提升了分割性能。


<details>
  <summary>Details</summary>
Motivation: 脑肿瘤分割面临挑战，因为三个标准子区域（全肿瘤、肿瘤核心、增强肿瘤）常具有模糊的视觉边界。现有多模态方法通常将放射学报告压缩为单一全局文本嵌入，忽略了不同子区域的独特临床特征。

Method: 提出TextCSP框架，包含三个核心组件：1）文本调制软级联解码器，按照解剖包含层次（WT→TC→ET）进行粗到细的预测；2）子区域感知提示调优，使用可学习的软提示和LoRA适配的BioBERT编码器为每个子区域生成专门的文本表示；3）文本语义通道调制器，将文本表示转换为通道级细化信号，使解码器能够强调与临床描述模式对齐的特征。

Result: 在TextBraTS数据集上的实验表明，相比最先进方法，在所有子区域上均取得一致改进，主要指标Dice系数和HD95分别提升1.7%和6%。

Conclusion: TextCSP通过层次化文本引导框架有效整合放射学文本描述和医学影像，显著提升了脑肿瘤分割性能，证明了考虑子区域特异性临床特征的重要性。
...
### [68] [Taming Sampling Perturbations with Variance Expansion Loss for Latent Diffusion Models](https://arxiv.org/abs/2603.21085)
...
<details>
  <summary>Details</summary>
Motivation: 先前研究主要关注潜在空间的重建精度和语义对齐，但作者发现另一个关键因素——对采样扰动的鲁棒性——也对生成质量起着至关重要的作用。现有β-VAE分词器产生的过度紧凑潜在流形对扩散采样中的随机扰动高度敏感。

Method: 提出了一种简单有效的解决方案：引入方差扩展损失来抵消方差崩溃，利用重建和方差扩展之间的对抗性相互作用实现自适应平衡，在保持重建精度的同时提高对随机采样的鲁棒性。

Result: 大量实验表明，该方法在不同潜在扩散架构中一致地提高了生成质量，证实了潜在空间鲁棒性是稳定和忠实扩散采样的关键缺失要素。

Conclusion: 潜在空间对采样扰动的鲁棒性是决定生成质量的关键因素，提出的方差扩展损失方法能够有效构建鲁棒的潜在空间，显著提升潜在扩散模型的生成稳定性。
...
### [69] [DGRNet: Disagreement-Guided Refinement for Uncertainty-Aware Brain Tumor Segmentation](https://arxiv.org/abs/2603.21086)
*Bahram Mohammadi,Yanqiu Wu,Vu Minh Hieu Phan,Sam White,Minh-Son To,Jian Yang,Michael Sheng,Yang Song,Yuankai Qi*

Main category: cs.CV

TL;DR: DGRNet提出了一种结合多视图不一致性不确定性估计和文本条件细化的脑肿瘤分割方法，解决了单模型不确定性量化不足和放射学报告信息利用不充分的问题。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法存在两个关键限制：1）单模型预测缺乏可靠的不确定性量化，这对临床部署至关重要；2）放射学报告中的丰富信息在模糊区域分割中未得到充分利用。

Method: 提出DGRNet框架，通过四个轻量级视图特定适配器生成多样化预测，实现单次前向传播的高效不确定性量化。构建不一致性图识别高不确定性区域，并基于临床报告进行选择性细化。采用多样性保持训练策略，结合成对相似性惩罚和梯度隔离防止视图坍塌。

Result: 在TextBraTS数据集上，DGRNet在主要指标Dice和HD95上分别比现有最佳方法提高了2.4%和11%，同时提供了有意义的不确定性估计。

Conclusion: DGRNet成功解决了脑肿瘤分割中的不确定性量化和文本信息利用问题，在提高分割精度的同时提供了临床决策所需的不确定性信息。
...
TL;DR: 本文提出了一种临床引导的多任务框架RLAR，用于联合预测甲状腺结节的轮廓分割和TI-RADS风险分类，通过表示层对抗梯度正则化解决多任务学习中的梯度竞争问题。


<details>
  <summary>Details</summary>
Motivation: 甲状腺超声检查中，放射科医生需要同时进行结节轮廓分割和TI-RADS风险分级，但不同医生在这两项任务上存在标注风格和风险判断的差异，这种不一致的监督会降低标准学习管道的性能。

Method: 1) 提出临床引导的多任务框架，联合预测结节掩码和TI-RADS类别；2) 在训练中使用紧凑的TI-RADS对齐放射组学目标来引导分类嵌入；3) 引入RLAR（表示层对抗梯度正则化），通过任务特定对抗方向的角度对齐惩罚来控制梯度竞争。

Result: 在公共TI-RADS数据集上，临床引导的多任务模型结合RLAR相比单任务训练和传统多任务基线，在保持分割质量的同时，持续改善了风险分层性能。

Conclusion: RLAR方法通过显式控制多任务学习中的梯度竞争，实现了更好的临床任务协同，为甲状腺结节评估提供了更一致和准确的自动化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在将RGB预训练模型适配到多模态跟踪时存在不足：1）忽略了高级适配策略；2）未能有效调制单一特定模态、跨模态交互和预测头；3）由于配对多模态数据有限，通常采用参数高效微调模块但效果不佳。

Method: 提出渐进式多模态跟踪适配方法(PATrack)，包含三个核心组件：1）模态依赖适配器：通过分解高低频分量增强模态特定信息；2）模态纠缠适配器：基于跨模态共享信息实现交叉注意力操作，确保模态间特征传递可靠性；3）任务级适配器：专门针对预测头设计，适应融合信息。这三种适配器集成到统一框架中，实现从单模态到跨模态再到任务级的渐进适配。

Result: 在RGB+热成像、RGB+深度和RGB+事件跟踪任务上进行了广泛实验，结果表明该方法在多项基准测试中表现出色，优于当前最先进的方法。

Conclusion: PATrack通过渐进式适配策略有效解决了RGB预训练模型在多模态跟踪中的适配问题，通过模态依赖、模态纠缠和任务级适配器的协同工作，显著提升了多模态跟踪性能。
...
### [72] [Frequency Switching Mechanism for Parameter-E!cient Multi-Task Learning](https://arxiv.org/abs/2603.21111)
...
<details>
  <summary>Details</summary>
Motivation: 当前参数高效微调方法主要局限于单任务适应，而多任务学习旨在让单个模型高效解决多个任务，需要更有效的多任务参数高效学习方法。

Method: 提出Free Sinewich框架，包含Sine-AWB层（结合低秩因子和卷积先验）和轻量级Clock Net。通过正弦变换对内核进行元素级调制，生成任务专用权重，Clock Net产生有界频率以稳定训练中的调制。

Result: 在密集预测基准测试中，Free Sinewich实现了最先进的性能-效率权衡（例如，仅用6.53M可训练参数就比单任务微调提升高达+5.39%），提供了基于频率参数共享的紧凑可扩展范式。

Conclusion: Free Sinewich通过频率切换的权重调制，为多任务学习提供了一个参数高效且性能优越的解决方案，理论上正弦调制增强了低秩适配器的秩，频率分离则解耦了不同任务的权重相关性。
...
### [73] [CVT-Bench: Counterfactual Viewpoint Transformations Reveal Unstable Spatial Representations in Multimodal LLMs](https://arxiv.org/abs/2603.21114)
...
<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在单视角空间推理任务上表现出色，但尚不清楚它们在反事实视角变化下是否能保持稳定的空间状态表示。研究者希望评估MLLMs在假设性相机轨道变换下的关系一致性，而无需重新渲染图像。

Method: 引入了一个受控诊断基准，评估假设性相机轨道变换下的关系一致性。使用100个合成场景和6,000个关系查询，测量视角一致性、360°循环一致性和顺序变换下的关系稳定性。评估了多种输入表示：视觉输入、文本边界框和结构化场景图。

Result: 尽管单视角准确率高，但最先进的MLLMs在反事实视角变化下表现出系统性退化，频繁违反循环一致性，关系稳定性迅速衰减。增加表示结构（如结构化场景图）能提高稳定性。
...
### [74] [LiFR-Seg: Anytime High-Frame-Rate Segmentation via Event-Guided Propagation](https://arxiv.org/abs/2603.21115)
*Xiaoshan Wu,Xiaoyang Lyu,Yifei Yu,Bo Wang,Zhongrui Wang,Xiaojuan Qi*

Main category: cs.CV

TL;DR: 提出Anytime Interframe Semantic Segmentation新任务，使用单帧RGB图像和异步事件流预测任意时刻的语义分割，解决低帧率相机在动态环境中的感知间隙问题。


<details>
  <summary>Details</summary>
Motivation: 标准相机的低帧率特性在动态环境中造成关键感知间隙，限制了密集语义分割的性能。需要一种方法能够利用事件数据来填补帧间空白，实现高帧率感知。

Method: 提出LiFR-Seg框架，通过不确定性感知的扭曲过程传播深度语义特征，使用事件驱动的运动场及其学习到的显式置信度指导，并采用时间记忆注意力模块确保动态场景中的一致性。

Result: 在DSEC数据集和新的高频合成基准SHF-DSEC上验证，低帧率系统达到73.82% mIoU，与完全访问目标帧的高帧率上限性能（差异仅0.09%）统计上无区别。

Conclusion: 这项工作提出了一个高效的新范式，用低帧率硬件实现鲁棒的高帧率感知，为动态环境中的实时语义分割提供了创新解决方案。
...
TL;DR: 本文提出多聚类记忆(MCM)框架，用于解决实际测试时自适应(PTTA)中单聚类记忆结构不匹配多模态测试流的问题，通过聚类组织、合并和平衡检索机制显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有测试时自适应方法普遍使用单一非结构化记忆池存储样本，但实际测试流具有时间相关性和非独立同分布特性，且本质上是多模态的。单一聚类设计与实际测试流的多模态特性存在根本性结构不匹配。

Method: 提出多聚类记忆(MCM)框架：1) 使用轻量级像素级统计描述符将存储样本组织成多个聚类；2) 相邻聚类合并(ACC)机制限制内存使用；3) 均匀聚类检索(UCR)确保适应过程中所有模态的平衡监督。

Result: 在CIFAR-10-C、CIFAR-100-C、ImageNet-C和DomainNet四个数据集上，与三种当代TTA方法集成，MCM在全部12种配置中均取得一致改进，在ImageNet-C上提升达5.00%，在DomainNet上提升达12.13%。标签空间越大、多模态性越强的分布从多聚类组织中获益最多。

Conclusion: 多聚类记忆框架有效解决了实际测试时自适应中记忆组织的关键设计问题，通过捕捉分布模态、控制内存使用和确保平衡监督，显著提升了模型在复杂分布变化下的适应性能，确立了记忆组织作为实际测试时自适应的重要设计维度。
...
TL;DR: MS-CustomNet是一个用于多主体定制的扩散模型框架，允许用户零样本集成多个对象并明确控制它们的分层排列和空间关系，同时保持主体身份。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像生成方法在多主体场景定制方面存在局限，难以提供对主体间组合结构和空间关系的显式用户控制。需要一种能够同时保持主体身份并实现精确空间控制的解决方案。

Method: 提出了MS-CustomNet框架，支持多主体零样本集成，允许用户显式定义分层排列和空间布局。同时创建了基于COCO的MSI数据集来训练复杂的多主体组合。

Result: 在身份保持方面获得DINO-I分数0.61，在位置控制方面获得YOLO-L分数0.94，表明该方法在多主体定制任务中能够生成高保真图像并实现精确的空间控制。

Conclusion: MS-CustomNet为多主体图像生成提供了增强的细粒度控制，能够生成具有精确用户导向多主体组合和空间控制的高质量图像。
...
TL;DR: 本文提出RLVC框架，通过强化学习结合视觉线索改进生成式零样本学习，提升特征合成的任务相关性和生成质量，在三个基准数据集上实现4.7%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有生成式零样本学习方法存在两个主要问题：1）合成的视觉特征通常是任务无关的，导致性能下降；2）仅从语义原型推断分布对于语义相似但视觉差异大的类别不够充分。需要一种能够生成任务相关特征并考虑视觉差异的方法。

Method: 提出RLVC框架：1）使用基于结果的强化学习奖励机制，鼓励生成任务相关的特征；2）引入类别级视觉线索，对齐合成特征与视觉原型并稳定RL训练；3）采用新颖的冷启动策略进行训练过程。

Result: 在三个主流零样本学习基准数据集上的综合实验和分析表明，RLVC实现了最先进的性能，获得了4.7%的性能提升。

Conclusion: RLVC框架通过强化学习自我进化机制和视觉线索的引入，有效解决了生成式零样本学习中特征合成任务无关性和语义-视觉对齐不足的问题，显著提升了零样本学习性能。
...
TL;DR: 提出无需训练、无需位姿预处理的3D室内场景重建系统，通过点云重建、实例提升和扩散模型渲染实现稀疏RGB图像的高质量3D重建与编辑。


<details>
  <summary>Details</summary>
Motivation: 传统辐射场方法需要密集视角和逐场景优化，限制了3D场景重建的效率和应用范围。本文旨在开发无需训练、无需位姿预处理的系统，从稀疏RGB图像实现高质量3D重建、理解和渲染。

Method: 系统包含三个核心创新：1）基于变形异常去除的鲁棒点云重建模块；2）变形引导的2D到3D实例提升机制，将2D分割掩码传播为一致的实例感知3D表示；3）新颖的渲染方法，将点云投影到新视角并用3D感知扩散模型细化渲染结果。

Result: 方法在稀疏输入条件下实现高保真3D重建，利用扩散模型的生成能力补偿缺失几何并增强真实感。支持通过修改点云实现对象级场景编辑（如实例移除），无需重新训练即可合成一致的编辑视图。

Conclusion: 该方法为无需场景特定优化的高效、可编辑3D内容生成开辟了新方向，展示了训练无关系统在3D场景重建、理解和渲染方面的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型在多模态生成方面表现出色，但进行精确、无需训练的图片编辑仍然是一个挑战。与连续扩散模型不同，DLLM固有的离散标记化阻碍了标准噪声反演技术的应用，导致编辑过程中的结构退化。

Method: GIDE框架包含三个主要阶段：1) 接地阶段，2) 反演阶段，采用新颖的离散噪声反演机制在离散标记空间中准确捕获潜在噪声模式，3) 精炼阶段。该设计支持文本、点和框等多种编辑指令，同时严格保护未编辑的背景。

Result: 在包含805个组合编辑场景的GIDE-Bench基准测试中，GIDE显著优于先前的训练免费方法，语义正确性提高了51.83%，感知质量提高了50.39%。在ImgEdit-Bench上的额外评估证实了其广泛适用性，显示出与训练基线相比的一致优势，并实现了与领先模型相媲美的照片级一致性。

Conclusion: GIDE成功解决了DLLM中精确训练免费图像编辑的挑战，通过离散噪声反演机制实现了高保真重建，支持多种编辑操作，并在多个基准测试中表现出卓越性能，为DLLM图像编辑提供了有效的统一框架。
...
TL;DR: 提出了一种无提示、边界感知的实例分割框架，通过预测带符号距离函数（SDFs）来精确分离显微镜视频中密集的细胞实例，解决了传统方法难以区分相邻细胞的问题。


<details>
  <summary>Details</summary>
Motivation: 显微镜视频中细胞实例的准确分割对于研究细胞动力学至关重要，但分离接触或重叠的细胞实例仍然是一个持续挑战。虽然像SAM这样的基础模型拓宽了图像分割的可及性，但在没有大量提示的情况下，它们仍然难以分离密集显微镜场景中的相邻细胞实例。

Method: 提出了一种无提示、边界感知的实例分割框架，预测带符号距离函数（SDFs）而不是二值掩码，从而实现细胞轮廓的平滑和几何一致性建模。通过学习的sigmoid映射将SDFs转换为概率图，产生锐利的边界定位和相邻实例的鲁棒分离。训练由统一的改进豪斯多夫距离（MHD）损失指导，该损失整合了基于区域和基于边界的项。

Result: 在公共和私有高通量显微镜数据集上的评估表明，与最近的基于SAM的方法和基础模型方法相比，该方法在边界准确性和实例级性能方面都有所改进。

Conclusion: 该方法通过预测SDFs和统一的MHD损失，实现了对密集显微镜场景中细胞实例的更准确分割，特别是在分离相邻细胞方面表现出色，为细胞动力学研究提供了更好的工具。
...
Method: 提出Positional Seg-CFT方法，将每个解剖结构细分为区域段，为每个区域独立推导测量值，从而实现空间局部化和解剖一致的反事实生成。该方法在冠状动脉CT血管造影上进行实验验证。

Result: 实验表明Pos-Seg-CFT能够生成逼真的区域特异性修改，为疾病进展建模提供更精细的空间控制。

Conclusion: Positional Seg-CFT通过区域细分和独立测量，实现了空间局部化的反事实图像生成，相比现有方法提供了更精细的解剖结构控制能力。
...
TL;DR: 提出一个基于损失函数视角的长尾分布学习框架，通过分组知识保存和分组锐度感知模块，解决模型在头部类别过拟合、尾部类别快速遗忘的问题，无需外部训练数据或预训练模型。


<details>
  <summary>Details</summary>
Motivation: 长尾数据分布中性能权衡的困境源于"尾部性能退化"现象——模型倾向于严重过拟合头部类别，同时快速遗忘尾部类别。作者从损失函数视角分析，发现不同类别在损失函数空间中有不同的收敛点，且当模型陷入尖锐、非鲁棒的最小值时，这种差异会加剧。

Method: 提出一个受持续学习启发的框架：1) 分组知识保存模块：记忆组特定的收敛参数，促进向共享解决方案的收敛，避免低效的每类参数保存；2) 分组锐度感知模块：通过显式处理损失函数的几何形状，寻求更平坦的最小值。该框架无需外部训练样本或预训练模型。

Result: 在四个基准测试上进行的广泛实验表明，该方法相比最先进的方法取得了显著的性能提升。

Conclusion: 从损失函数视角解决长尾学习问题，通过分组知识保存和锐度感知模块有效缓解尾部性能退化，实现了更好的性能权衡，具有广泛的适用性。
...
<details>
  <summary>Details</summary>
Motivation: 当前视觉领域在细粒度、分类学感知的植物计数方面研究不足。与人群计数不同，植物具有非刚性形态，在不同生长阶段和环境条件下外观变化大，需要专门的基准数据集来推动这一领域发展。

Method: 构建TPC-268数据集，包含实例级点标注、林奈分类标签（界->种）和器官类别，支持分层推理和物种感知评估。采用类无关计数问题设置，提供分类学一致、尺度感知的数据划分，并基准测试了最先进的回归和检测方法。

Result: 数据集包含10,000张图像、678,050个点标注，涵盖268个可计数植物类别、242个植物和真菌物种，覆盖从冠层遥感图像到组织显微图像的多尺度观测。为细粒度类无关计数提供了生物学基础测试平台。
...
TL;DR: QMoP是一种自适应视觉令牌压缩框架，通过三个协作分支和查询引导路由器动态压缩视觉令牌，显著降低多模态大语言模型的计算和内存开销。
...
Method: 提出查询引导混合投影器(QMoP)框架，包含三个协作分支：1)池化分支用于粗粒度全局语义；2)重采样分支提取高级语义表示；3)剪枝分支用于细粒度令牌选择以保留关键视觉细节。引入查询引导路由器(QGR)基于视觉输入和文本查询动态选择和加权各分支输出，采用混合专家式融合机制聚合输出。

Result: QMoP在性能上优于强基线方法，同时在内存、计算和推理时间方面带来显著节省。还开发了VTCBench基准来系统评估视觉令牌压缩引起的信息损失。

Conclusion: 尽管依赖基本压缩模块，QMoP通过自适应协调多个压缩策略，有效解决了多模态大语言模型的视觉令牌压缩问题，在性能和效率之间取得了良好平衡。
...
TL;DR: 基于Vision Transformers和色度图特征增强的深度学习框架，在BRISC2025数据集上实现98.90%的脑肿瘤分类准确率，优于传统CNN模型。


<details>
  <summary>Details</summary>
Motivation: MRI脑肿瘤的准确分类对早期诊断和治疗规划至关重要，需要提高多类别脑肿瘤分类的性能。

Method: 提出基于Vision Transformers的深度学习框架，结合色度图特征表示来增强MRI扫描中的重要结构和强度变化特征。

Result: 在BRISC2025数据集（包含胶质瘤、脑膜瘤、垂体瘤和非肿瘤四类）上达到98.90%的分类准确率和99.97%的AUC，优于ResNet50、ResNet101和EfficientNetB2等基准模型。

Conclusion: Vision Transformers与色度图特征增强相结合的方法在脑肿瘤分类中表现出色，具有临床决策支持的潜力。
...
TL;DR: CornOrb是一个公开的多模态数据集，包含来自阿尔及利亚患者的Orbscan角膜地形图图像和临床标注，包含1,454只眼睛（889只正常眼和565只圆锥角膜病例），提供四种角膜地图和结构化临床数据。


<details>
  <summary>Details</summary>
Motivation: 创建非洲首个大规模基于Orbscan的公开数据集，支持使用多模态数据进行稳健的AI驱动圆锥角膜检测和分析研究。

Method: 回顾性收集阿尔及利亚患者的Orbscan角膜地形图数据，包括轴向曲率、前表面高度、后表面高度和角膜厚度图，以及人口统计学信息和关键临床参数，所有数据经过完全匿名化和预处理为标准化PNG和CSV格式。

Result: 数据集包含1,454只眼睛（来自744名患者），其中889只正常眼和565只圆锥角膜病例，提供四种角膜地图图像和结构化临床数据，已在Zenodo平台公开可用。

Conclusion: CornOrb数据集为人工智能研究提供了可直接使用的标准化多模态资源，填补了非洲地区Orbscan数据集的空白，有望促进圆锥角膜的AI检测和分析研究。
...
Conclusion: 提出的无监督自进化训练框架能够在不使用人工标注或外部奖励模型的情况下，稳定提升多模态推理模型的性能，为解决标注数据稀缺问题提供了有效方案。
...
TL;DR: TIGON提出了一种结合文本和图像双重条件的3D生成方法，通过双分支架构和轻量级跨模态融合，解决了单一模态（图像或文本）在3D生成中的局限性，实现了更灵活和准确的3D内容生成。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成模型大多依赖单一条件模态：图像条件模型虽然能实现高视觉保真度，但在输入视角有限或模糊时存在视角偏差；文本条件模型提供广泛的语义指导但缺乏低层次视觉细节。这限制了用户表达意图的能力，需要探索如何结合两种模态以实现更灵活和忠实的3D生成。

Method: 提出TIGON方法，采用简约的双分支架构，包含独立的图像条件分支和文本条件分支，通过轻量级的跨模态融合机制实现联合推理。该方法将文本-图像条件3D生成形式化为一个需要同时处理视觉示例和文本规范的任务。

Result: 实验表明，文本-图像双重条件方法始终优于单一模态方法，简单的后期融合策略就能超越单模态模型，揭示了跨模态互补性的强大潜力。TIGON展示了结合视觉-语言指导作为未来3D生成研究的可行方向。

Conclusion: 文本和图像条件的结合为3D生成提供了更灵活和忠实的解决方案，跨模态互补性是一个有前景的研究方向。TIGON作为一个简约的双分支基线方法，为未来更复杂的多模态3D生成研究奠定了基础。
...
TL;DR: Mv²ID是一个多视角参考视频生成框架，通过区域掩码训练策略和参考解耦RoPE机制，在保持身份一致性的同时提升面部运动自然度，解决了单视角方法在大角度变化下的身份一致性问题和多视角方法的视角依赖复制粘贴问题。


<details>
  <summary>Details</summary>
Motivation: 单视角参考视频方法在大角度面部变化下难以保持身份一致性，而简单地引入多视角参考图像会加剧"复制粘贴"问题，特别是视角依赖的复制粘贴伪影，这会降低面部运动的自然度。虽然交叉配对数据可以缓解这个问题，但收集成本高昂。

Method: 1. 提出Mv²ID多视角条件框架，采用非配对监督；2. 引入区域掩码训练策略防止捷径学习，通过鼓励模型聚合跨视角的互补身份特征来提取本质身份特征；3. 设计参考解耦RoPE机制，为视频和条件标记分配不同的位置编码以更好地建模它们的异构特性；4. 构建了具有多样化面部角度变化的大规模数据集。

Result: 大量实验表明，该方法显著提高了身份一致性，同时保持了运动自然度，性能超过了使用交叉配对数据训练的现有方法。

Conclusion: Mv²ID框架通过创新的训练策略和位置编码机制，有效平衡了身份一致性和运动自然度，为多视角参考视频生成提供了有效的解决方案。
...
TL;DR: F4Splat提出了一种基于密度分数引导的自适应高斯分布策略，用于前馈式3D高斯泼溅方法，在控制高斯总数量的同时保持重建质量，减少冗余并提升新视角合成性能。


<details>
  <summary>Details</summary>
Motivation: 现有前馈式3D高斯泼溅方法采用刚性的像素到高斯或体素到高斯流程，均匀分配高斯分布，导致跨视角的冗余高斯分布。同时缺乏有效机制来控制高斯总数同时保持重建保真度。

Method: 提出F4Splat方法，采用密度分数引导的分配策略，根据空间复杂度和多视角重叠自适应分布高斯分布。模型预测每个区域的密度分数来估计所需的高斯密度，并允许在不重新训练的情况下显式控制最终的高斯预算。

Result: 实验表明，与先前未校准的前馈方法相比，F4Splat在显著减少高斯数量的情况下，实现了更优的新视角合成性能。自适应分配减少了简单区域的冗余，并最小化了重叠视角间的重复高斯分布。

Conclusion: F4Splat通过密度分数引导的自适应高斯分配策略，解决了前馈式3D高斯泼溅方法中的冗余和数量控制问题，实现了紧凑且高质量的3D表示。
...
<details>
  <summary>Details</summary>
Motivation: 视频面部表情识别需要针对不同主体进行个性化建模，现有测试时自适应方法依赖无监督参数优化，计算开销大，不适用于实际部署。同时，传统基于缓存的方法仅使用动态记忆存储测试样本，容易积累错误和漂移。

Method: 提出TTA-CaP方法，包含三个协调缓存：个性化源缓存（存储源域原型）、正目标缓存（积累可靠的主体特定样本）、负目标缓存（存储低置信度样本作为负样本）。通过基于时间稳定性、置信度和与个性化缓存一致性的三选通机制控制缓存更新和替换，最后通过嵌入融合细化预测。

Result: 在BioVid、StressID和BAH三个具有挑战性的视频FER数据集上，TTA-CaP在主体特定和环境偏移下优于最先进的TTA方法，同时保持低计算和内存开销。

Conclusion: TTA-CaP提供了一种高效、计算成本低的个性化方法，通过协调缓存机制减少噪声伪标签的影响，实现了在实际部署中可行的视频面部表情识别性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有随机人体运动预测方法存在高频抖动和时间不连续的问题，需要一种能够生成平滑、物理合理且多样化的运动预测框架。

Method: KHMP框架包含两个核心创新：1) 在DCT域应用自适应卡尔曼滤波器，将高频DCT系数视为频率索引的噪声信号，根据估计的信噪比动态调整噪声参数；2) 训练时加入物理约束（时间平滑性和关节角度限制），将生物力学原理编码到生成模型中。

Result: 在Human3.6M和HumanEva-I数据集上的实验表明，KHMP达到了最先进的准确度，有效减轻了抖动伪影，生成了平滑且物理合理的运动。
...
TL;DR: 提出Coarse-to-Fine扩散模型与时间步序列重分配方法，通过粗到细去噪和时间步序列优化，在保持性能的同时显著减少计算量


<details>
  <summary>Details</summary>
Motivation: 扩散模型的多步去噪过程计算开销大，阻碍在资源受限边缘设备上的部署。现有方法忽略输入冗余且搜索时间长，需要更高效的加速方案

Method: 提出Coarse-to-Fine Denoising (C2F)方法，利用早期生成图像难以区分的特性减少粗特征生成计算；设计Time Step Sequence Redistribution (TRD)进行高效采样轨迹调整，搜索时间少于10分钟

Result: 在CIFAR10和LSUN-Church数据集上，方法实现了接近无损的性能，同时计算量减少了80%到90%

Conclusion: 提出的C2F和TRD方法有效解决了扩散模型计算开销大的问题，为边缘设备部署提供了高效解决方案
...
TL;DR: 使用视频Transformer技术识别呼吸窘迫，通过运动后恢复期视频创建数据集，采用ViViT编码器结合Lie相对编码和运动引导掩码，在时序排序任务上达到F1分数0.81
...
Method: 收集健康志愿者剧烈运动后恢复期的视频，利用参与者呼吸状态的自然恢复过程创建标记数据集。将视频分割成短片段，早期片段对应更严重的呼吸短促，设计时序排序挑战来评估AI系统检测呼吸窘迫的能力。采用ViViT编码器增强Lie相对编码和运动引导掩码，结合基于嵌入的比较策略。

Result: 增强的ViViT编码器结合Lie相对编码和运动引导掩码，配合嵌入比较策略，在该任务上达到F1分数0.81。这表明现代视频Transformer能够识别呼吸力学的细微变化。

Conclusion: 研究结果表明，现代视频Transformer技术能够识别呼吸力学的细微变化，为AI系统检测呼吸窘迫提供了可行方案，具有临床应用潜力。
...
TL;DR: FluidGaussian：一种将流体-结构相互作用与3D重建相结合的插件方法，通过流体模拟评估表面质量，提升视觉和物理保真度


<details>
  <summary>Details</summary>
Motivation: 当前基于多视角2D图像的3D重建方法主要优化视觉保真度，忽略了物理交互和物体功能特性，导致重建结果在物理交互中不真实、不可信

Method: 提出FluidGaussian方法，将几何重建与流体-结构相互作用紧密耦合，定义基于流体模拟的不确定性度量，并与主动学习结合，优先选择能同时提升视觉和物理保真度的视角

Result: 在NeRF Synthetic (Blender)、Mip-NeRF 360和DrivAerNet++数据集上的实验表明，FluidGaussian方法在视觉PSNR上提升高达+8.6%，流体模拟中的速度散度降低-62.3%

Conclusion: 通过将物理交互（特别是流体-结构相互作用）纳入3D重建过程，可以显著提升重建结果的物理合理性和视觉质量，使重建对象在真实世界交互中表现更可信
...
TL;DR: 本文提出Relax Forcing方法，通过结构化时间记忆机制改进自回归视频扩散模型的长时生成能力，将时间上下文分解为三个功能角色，选择性整合最相关的历史信息，减少误差累积并保持运动演化。


<details>
  <summary>Details</summary>
Motivation: 现有自回归视频扩散模型在扩展到分钟级时间尺度时面临渐进性时间退化问题。研究发现这并非主要由于内存不足，而是时间记忆在推理过程中的利用方式不当。时间记忆不应被视为同质缓冲区，其时间位置安排对运动动态有显著影响。

Method: 提出Relax Forcing方法，将时间上下文分解为三个功能角色：Sink用于全局稳定性，Tail用于短期连续性，动态选择的History用于结构运动指导。该方法选择性整合最相关的历史信息，而不是关注密集生成的历史，从而减少外推过程中的误差累积。

Result: 在VBench-Long上的实验表明，Relax Forcing改善了运动动态和整体时间一致性，同时减少了注意力开销。该方法在保持视觉质量的同时，显著提升了长时视频生成的质量。

Conclusion: 结构化时间记忆对于可扩展的长视频生成至关重要，补充了现有的基于强制的训练策略。通过适当的时间记忆管理，可以显著改善自回归视频扩散模型的长期生成能力。
...
TL;DR: HamVision是一个医疗图像分析框架，使用阻尼谐振子作为结构化归纳偏置，通过相位空间分解获得三种功能不同的表示，分别用于分割和分类任务，在多个医疗影像基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 医疗图像分析需要有效的特征表示，传统方法缺乏对图像内在物理特性的建模。阻尼谐振子作为信号处理的基本构建块，可以提供结构化的归纳偏置，自然地分解出功能不同的特征表示。

Method: 使用阻尼谐振子作为核心组件，通过相位空间分解获得三种表示：位置q（特征内容）、动量p（空间梯度编码边界和纹理信息）、能量H（无参数显著性图）。这些表示从动力学中自然涌现，无需监督。对于分割任务，HamSeg使用能量门控跳跃连接，动量在每个解码器层级注入边界信息；对于分类任务，HamCls将三种表示全局池化并连接成相位空间特征向量。

Result: 在十个医疗影像基准测试中表现出色：分割方面，HamSeg在ISIC 2018（89.38%）、ISIC 2017（88.40%）、TN3K（87.05%）和ACDC（92.40%）上达到最先进的Dice分数，仅使用8.57M参数；分类方面，HamCls在BloodMNIST（98.85%）和PathMNIST（96.65%）上达到最先进准确率，在其他MedMNIST数据集上与MedMamba和MedViT竞争。诊断分析确认动量编码了内部>边界>外部的梯度，能量图与分类的判别区域相关。

Conclusion: HamVision展示了阻尼谐振子作为结构化归纳偏置在医疗图像分析中的有效性，通过相位空间分解自然涌现出功能不同的特征表示，为分割和分类任务提供了统一的框架，在多个基准测试中达到最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 开放词汇全景分割面临两个耦合问题：掩码选择偏差（在封闭词汇上训练的物体性头会抑制训练中未见类别的掩码）和视觉语言模型（如CLIP）的区域理解有限（这些模型是为全局图像分类而非局部分割优化的）。

Method: 提出OVRCOAT框架，包含两个组件：1) CLIP条件物体性调整(COAT)，更新背景/前景概率，保留高质量的外词汇对象掩码；2) 开放词汇掩码到文本细化(OVR)，增强CLIP的区域级对齐，以更低的记忆成本改进已见和未见类别的分类。

Result: 在ADE20K上实现了+5.5% PQ的新SOTA，在Mapillary Vistas和Cityscapes上分别获得+7.1%和+3% PQ的明显提升。

Conclusion: OVRCOAT通过联合改进物体性估计和掩码识别，以简单模块化的方式有效解决了开放词汇全景分割的关键挑战，取得了显著的性能提升。
...
TL;DR: Beta-KD是一种不确定性感知的知识蒸馏框架，通过贝叶斯视角自适应调节学生对教师指导的依赖程度，在视觉语言模型蒸馏中表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏难以确定数据监督和教师指导之间的最优平衡，因为有些样本可能包含噪声，而其他样本可能受到教师不确定性的影响，需要自适应地平衡数据和教师监督。

Method: 提出Beta加权知识蒸馏框架，从统一贝叶斯视角构建教师-学生学习，将教师监督解释为学生激活的Gibbs先验，得到闭式的不确定性感知加权机制，支持任意蒸馏目标及其组合。

Result: 在多模态VQA基准测试上的广泛实验表明，从大型教师VLM蒸馏学生视觉语言模型能持续提升性能，Beta-KD优于现有的知识蒸馏方法。

Conclusion: Beta-KD通过不确定性感知的自适应加权机制，有效解决了知识蒸馏中数据监督和教师指导的平衡问题，为视觉语言模型的蒸馏提供了有效解决方案。
...
TL;DR: 开发了一个能够从手绘图纸中解决理想化梁模型问题的程序，结合计算机视觉和统计学习技术进行结构元素检测与视觉解释，并探讨了计算机视觉在土木工程中的应用挑战与现状。


<details>
  <summary>Details</summary>
Motivation: 将计算机视觉技术应用于土木工程领域，特别是结构分析中，旨在解决从手绘图纸自动识别和解析结构模型的问题，提高工程分析的效率和自动化水平。

Method: 基于计算机视觉和统计学习技术，开发了PhotoBeamSolver程序，通过视觉检测和解释技术识别结构元素，实现从手绘图纸到结构模型的自动转换。

Result: 成功开发了能够处理教科书和学术练习中常见理想化梁模型的程序，分析了计算机视觉在结构分析中集成的主要挑战和限制，并确定了可靠应用所需的技术要求。

Conclusion: 计算机视觉在土木工程领域具有重要应用潜力，特别是在结构分析、基础设施检测和工程决策支持系统方面，但需要克服技术集成挑战才能实现可靠应用。
...
TL;DR: PAS3R：一种姿态自适应的流式3D重建框架，通过动态调整状态更新来解决稳定性与适应性之间的平衡问题，显著提升长视频序列的重建质量。


<details>
  <summary>Details</summary>
Motivation: 在线单目3D重建面临稳定性-适应性困境：重建模型需要快速适应新视角，同时保持已积累的场景结构。现有流式方法使用均匀或基于注意力的更新机制，无法有效处理突然的视角转换，导致长序列中的轨迹漂移和几何不一致。

Method: 1. 运动感知更新机制：利用帧间姿态变化和图像频率线索联合估计帧重要性，动态调整状态更新权重；2. 轨迹一致性训练目标：引入相对姿态约束和加速度正则化；3. 轻量级在线稳定模块：抑制高频轨迹抖动和几何伪影，不增加内存消耗。

Result: 在多个基准测试中，PAS3R显著提高了长视频序列的轨迹精度、深度估计和点云重建质量，同时在短序列上保持竞争力。

Conclusion: PAS3R通过姿态自适应的流式重建框架有效解决了稳定性与适应性的平衡问题，为在线单目3D重建提供了更鲁棒和准确的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 持续遗忘面临的主要挑战是：顺序遗忘更新会扭曲共享表示，在视觉语言对和拒绝行为之间创建虚假关联，导致无法精确识别拒绝目标，产生不适当的拒绝响应。
...
Conclusion: 通过将拒绝行为锚定在细粒度概念描述上，提出的框架能够有效解决持续遗忘中的精确拒绝问题，同时保持模型的通用能力。
...
Method: 提出TrajSeg框架：1）引入双向文本-轨迹对齐，MLLMs接受定位导向（文本到轨迹）和描述导向（轨迹到文本）的指令；2）通过帧级内容集成模块将轨迹级令牌适配为帧特定信息；3）使用统一掩码解码器将所有帧的分割统一到单一结构中，实现端到端训练。

Result: 在指代和推理视频分割数据集上的大量实验表明，TrajSeg在所有指标上都优于现有的视频推理分割方法。

Conclusion: TrajSeg通过双向文本-轨迹对齐和统一架构，有效提升了视频推理分割的性能，为视频动态场景下的对象分割提供了更强大的解决方案。
...
TL;DR: StreamingEval是一个统一的评估框架，用于在真实约束下评估视频大语言模型的流式视频理解能力，通过标准化协议衡量效率、存储和准确性之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有流式视频理解研究通常只关注孤立方面（如有限视觉上下文下的问答准确性或编码效率改进），而忽视了实际部署中的资源约束。需要弥合这一差距，评估视频大语言模型在真实流式应用中的能力。

Method: 引入StreamingEval框架，采用固定容量记忆库来标准化可访问的历史视觉上下文，联合评估视觉编码效率、文本解码延迟和任务性能，以量化整体系统可部署性。

Result: 在多个数据集上的广泛实验显示，当前视频大语言模型与真实流式应用需求之间存在显著差距，为未来研究方向提供了系统基础。

Conclusion: StreamingEval为评估视频大语言模型的流式视频理解能力提供了统一框架，揭示了当前模型与实际应用需求之间的差距，推动了该领域的研究发展。
...
<details>
  <summary>Details</summary>
Motivation: 全切片图像规模巨大，需要分割成小块处理，但获取实例级标注成本高昂。现有基于视觉语言模型的方法存在参数多、推理开销大、丢弃低对齐实例导致信息损失等问题。

Method: 1) 提出参数高效的提示调优方法，通过在文本编码器中缩放和移位特征来减少计算成本；2) 引入软层次文本指导策略，利用WSI的层次结构而不进行硬实例过滤。

Result: 在乳腺癌、肺癌和卵巢癌数据集上分别实现了10.9%、7.8%和13.8%的性能提升，可训练参数减少了18.1%（乳腺癌/肺癌）和5.8%（卵巢癌），同时在弱监督肿瘤定位方面表现优异。

Conclusion: 该方法通过参数高效的提示调优和软层次文本指导，有效解决了现有FSWC方法的局限性，在减少计算成本的同时显著提升了分类性能。
...
TL;DR: VIGIL提出了一种基于部件中心的结构化取证框架，通过"计划-检查"流程进行深度伪造检测，使用阶段门控注入机制确保部件选择不受外部证据干扰，并在OmniFake基准上表现出优越的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前基于多模态大语言模型的深度伪造检测方法将证据生成和操纵定位结合为一个统一步骤，模糊了忠实观察和幻觉解释之间的界限，导致不可靠的结论。需要更可靠的解释性检测方法。

Method: 提出VIGIL框架：1) 基于全局视觉线索计划需要检查的面部部件；2) 使用独立来源的取证证据检查每个部件；3) 阶段门控注入机制确保部件选择基于模型自身感知；4) 渐进式三阶段训练范式，强化学习阶段使用部件感知奖励确保解剖有效性和证据-结论一致性。

Result: 在OmniFake基准测试中，VIGIL在所有泛化级别上均优于专家检测器和并发的MLLM方法。OmniFake是一个5级分层基准，模型仅在三个基础生成器上训练，但能逐步测试到野外社交媒体数据。

Conclusion: VIGIL通过结构化的部件中心取证框架，实现了更可靠和可解释的深度伪造检测，其"计划-检查"流程和阶段门控机制确保了检测过程的忠实性和可靠性，在泛化能力方面表现出色。
...
TL;DR: PROBE协议揭示当前文本到视频扩散模型的概念擦除技术仅实现了输出级抑制而非表征移除，存在可测量的残留能力和时间性重现问题。


<details>
  <summary>Details</summary>
Motivation: 当前文本到视频扩散模型的概念擦除技术评估仅限于检查目标概念是否从生成帧中消失，将输出级抑制视为表征移除的证据，但缺乏对擦除概念重新激活潜力的量化评估。

Method: 提出PROBE诊断协议，在模型参数冻结的情况下，通过去噪重构目标结合新颖的潜在对齐约束优化轻量级伪令牌嵌入，量化擦除概念的重新激活潜力。

Result: 系统实验表明所有测试方法都存在可测量的残留能力，其鲁棒性与干预深度相关；发现了时间性重现这一视频特定的失败模式，即抑制概念在帧间逐渐重新出现。
...
TL;DR: 提出了一种自适应部件-整体层次结构的单图像3D生成方法，通过软组合掩码自动发现潜在结构槽，实现跨类别形状共享和结构适应。


<details>
  <summary>Details</summary>
Motivation: 现有单图像3D生成方法通常采用整体建模或固定部件数量，容易导致过拟合、结构碎片化或缺失，以及面对新物体布局时组合泛化能力有限。需要一种能自适应发现部件结构、实现跨类别形状共享的方法。

Method: 提出部件到整体的3D生成世界模型：1) 自适应槽门控机制动态确定槽激活概率并合并冗余槽；2) 将提取的槽与可学习的类别无关原型库对齐，实现跨类别形状共享；3) 引入轻量级3D去噪器通过统一扩散目标重建几何和外观。

Result: 实验显示在跨类别迁移和部件数量外推方面获得一致提升，消融研究确认原型库对形状先验共享和槽门控对结构适应的互补效益。

Conclusion: 通过自适应部件-整体层次结构学习，在灵活3D潜在空间中实现了更可靠的单图像3D生成，能够自主发现结构槽并实现跨类别形状共享，解决了现有方法的泛化限制。
...
<details>
  <summary>Details</summary>
Motivation: 现有仅依赖视觉信息的无监督持续异常检测方法在复杂场景中难以充分捕捉正常模式流形，限制了异常检测精度的进一步提升。

Method: 1. 提出持续多模态提示记忆库(CMPMB)，从视觉和文本域逐步提炼并保留原型正常模式；2. 设计缺陷语义引导的自适应融合机制(DSG-AFM)，包含自适应归一化模块(ANM)和动态融合策略(DFS)。

Result: 在MVTec AD和VisA数据集上的基准实验表明，该方法在图像级AUROC和像素级AUPR指标上达到了最先进的性能。

Conclusion: 通过融合视觉和文本多模态信息，提出的框架能够更全面地表示正常模式，有效提升了无监督持续异常检测的准确性和鲁棒性。
...
TL;DR: CataractSAM-2是基于Meta的Segment Anything Model 2开发的领域自适应模型，专门用于白内障眼科手术视频的实时语义分割，具有高精度和跨手术泛化能力。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够为机器人辅助和计算机引导手术系统提供精确术中感知的实时语义分割模型，同时解决医疗数据标注耗时的问题，促进眼科手术数据集的可扩展创建。

Method: 基于Segment Anything Model 2进行领域自适应，开发交互式标注框架结合稀疏提示和基于视频的掩码传播，显著减少标注时间并生成高质量地面实况掩码。

Result: 模型在白内障眼科手术视频中实现高精度实时语义分割，在青光眼小梁切除术中展示强大的零样本泛化能力，标注工具显著减少标注时间，促进数据集开发。

Conclusion: CataractSAM-2为扩展眼前段眼科手术数据集和推进医疗机器人实时AI驱动解决方案建立了基础，模型和标注工具开源发布，具有广泛的手术应用潜力。
...
TL;DR: 该论文提出组合隐私风险分类法(CPRT)，将隐私视为组合属性而非二元属性，并开发了可解释的隐私严重性评分系统，构建了包含6.7K图像的数据集，评估了前沿和开源视觉语言模型在组合隐私评估上的表现。
...
Method: 引入组合隐私风险分类法(CPRT)，这是一个基于法规的框架，根据独立可识别性和组合伤害潜力组织视觉属性。CPRT定义了四个分级严重性级别，并配有可解释的评分函数，分配连续的隐私严重性分数。构建了包含6.7K图像的分类法对齐数据集，并推导了地面真实的组合风险分数。

Result: 评估前沿和开源视觉语言模型发现：前沿模型在提供结构化指导时与组合严重性对齐良好，但系统性地低估组合驱动风险；较小模型难以内化分级隐私推理。为弥合这一差距，引入了可部署的8B监督微调模型，在组合隐私评估上接近前沿模型性能。

Conclusion: 隐私本质上是组合性的，需要超越二元分类的评估框架。CPRT提供了结构化方法来评估组合隐私风险，而开发的8B SFT模型展示了在保持高性能的同时实现可部署组合隐私评估的可行性。
...
TL;DR: 本文提出了一种针对3D旋转回归的半监督学习方法，通过难度感知课程学习和结构化数据增强策略，在仅有少量标注2D图像的情况下提升性能。


<details>
  <summary>Details</summary>
Motivation: 从2D图像回归3D物体旋转是自动驾驶、虚拟现实等领域的关键任务。现有方法通常需要大量标注数据或额外信息（如点云、CAD模型），而半监督方法FisherMatch存在伪标签筛选机制僵化的问题，无法有效区分可靠与不可靠的无标注样本。

Method: 提出硬度感知课程学习框架，动态选择基于难度的伪标签样本，从简单到复杂逐步学习。采用多阶段和自适应课程策略替代固定阈值筛选，并提出专门针对旋转估计的结构化数据增强策略，通过增强补丁组装复合图像，在保持几何完整性的同时引入特征多样性。

Result: 在PASCAL3D+和ObjectNet3D数据集上的实验表明，该方法优于现有的监督和半监督基线方法，特别是在低数据情况下表现突出，验证了课程学习框架和结构化增强方法的有效性。

Conclusion: 提出的硬度感知课程学习框架和结构化数据增强策略有效解决了半监督3D旋转回归中的伪标签筛选问题，在有限标注数据下显著提升了性能，为相关应用提供了实用解决方案。
...
TL;DR: SARe是一个用于3D碎片重组的生成框架，包含SARe-Gen用于生成装配方案和SARe-Refine用于推理时优化，通过显式接触建模解决多碎片装配中的可靠性问题。


<details>
  <summary>Details</summary>
Motivation: 现有端到端方法在碎片数量增加时容易因不可靠的接触推理（特别是碎片邻接关系不准确）而产生级联失败，需要更稳健的装配方法。

Method: 提出结构感知重组框架：SARe-Gen联合预测断裂表面标记概率和碎片间接触图来定位接触区域和推断候选邻接关系；采用基于查询点的条件方案，从冻结的几何编码器中提取对齐的局部几何标记；SARe-Refine通过几何一致性检查验证候选接触边，选择可靠子结构并重新采样不确定区域。

Result: 在合成断裂、扫描真实物体的模拟断裂和真实物理断裂扫描三种设置下评估，展示了最先进的性能，在具有挑战性的大规模重组中随着碎片数量增加表现出更优雅的性能下降和更高的成功率。

Conclusion: SARe通过显式接触建模和推理时优化，有效解决了多碎片3D重组中的可靠性问题，在碎片数量增加时仍能保持稳定和一致的装配结果。
...
TL;DR: AdaEdit提出了一种基于流匹配模型的训练自由自适应图像编辑框架，通过渐进注入调度和通道选择性潜在扰动解决特征注入困境，在保持背景的同时提升编辑内容生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于反转的图像编辑方法存在注入困境：注入源特征能保持原始图像背景，但会抑制模型生成编辑内容的能力。现有方法使用固定的注入策略（二进制时间调度、均匀空间混合比、通道无关的潜在扰动），忽略了时间和通道维度上注入需求的异质性。

Method: 1. 渐进注入调度：用连续衰减函数（sigmoid、余弦或线性）替代硬二进制截止，实现从源特征保持到目标特征生成的平滑过渡，消除特征不连续伪影。2. 通道选择性潜在扰动：基于反转和随机潜在之间的分布差距估计每通道重要性，相应应用差异化扰动强度——强烈扰动编辑相关通道，同时保持结构编码通道。
...
Conclusion: AdaEdit通过自适应注入策略有效解决了流匹配模型中图像编辑的注入困境，在保持背景保真度的同时显著提升编辑质量，为训练自由的文本引导图像编辑提供了更优解决方案。
...
TL;DR: 4DGS360是一个无需扩散的框架，用于从单目视频重建360度动态物体，通过3D原生初始化解决现有方法在遮挡区域几何模糊的问题，并在新基准iPhone360上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法过度依赖2D原生先验，导致初始点过拟合到每个训练视图的可见表面，无法重建一致的360度几何形状，特别是在遮挡区域存在几何模糊问题。

Method: 提出3D原生初始化方法，通过AnchorTAP3D 3D跟踪器利用置信度高的2D跟踪点作为锚点，生成增强的3D点轨迹，抑制漂移并提供可靠的初始化，保持遮挡区域的几何形状，然后结合优化获得一致的360度4D重建。

Result: 在iPhone360、iPhone和DAVIS数据集上实现最先进的性能，iPhone360基准测试中训练视图与测试相机视角相差高达135度，能够提供现有数据集无法支持的360度评估。

Conclusion: 4DGS360通过先进的3D原生初始化解决了360度动态物体重建中的几何模糊问题，在多个数据集上验证了其有效性，为单目视频的完整360度重建提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像模型的快速发展使AI生成图像越来越逼真，给准确检测生成内容带来重大挑战。基于训练的方法泛化能力有限，而无训练方法虽然鲁棒性更好，但难以捕捉真实与合成图像之间的细微差异。

Method: 提出一种无需训练的AI生成图像检测方法，通过测量表示对结构化频率扰动的敏感性来检测细微操作。该方法计算轻量，扰动生成仅需对输入图像进行一次傅里叶变换。
...
<details>
  <summary>Details</summary>
Motivation: 多目标跟踪（MOT）在训练和测试数据之间存在外观、运动模式和类别的分布偏移，导致在线推理时性能显著下降。现有的测试时适应（TTA）方法主要关注帧级适应，忽略了跨帧和视频的时间一致性和身份关联。

Method: 提出测试时经验与直觉校准（TCEI）框架：直觉系统利用瞬时记忆回忆最近观察到的对象进行快速预测；经验系统利用先前测试视频积累的经验重新评估和校准这些直觉预测。同时利用在线测试中的置信和不确定对象分别作为历史先验和反思案例。

Result: 在多个基准数据集上的广泛实验表明，TCEI框架始终实现卓越性能，显著增强了模型在分布偏移下的适应性。

Conclusion: TCEI框架通过模拟人类决策过程，结合直觉快速反应和经验校准，有效解决了MOT中的测试时适应问题，提升了模型在分布偏移下的鲁棒性。
...
TL;DR: SparseVoxelDet是首个完全稀疏的事件相机目标检测器，使用3D稀疏卷积处理事件流，仅对占用体素位置进行计算，实现了高效的内存和存储压缩。


<details>
  <summary>Details</summary>
Motivation: 现有事件相机检测器通常将稀疏事件流转换为密集张量，丢弃了神经形态传感的表示效率。需要一种能够原生处理稀疏事件数据的方法，以充分利用事件相机的优势。

Method: 提出SparseVoxelDet，基于3D稀疏卷积构建完全稀疏的目标检测器。包括稀疏主干特征提取、特征金字塔融合和检测头，所有阶段都只在占用的体素位置上操作，不实例化任何密集特征张量。

Result: 在FRED基准测试上达到83.38% mAP@50，每帧仅处理14,900个活跃体素（占网格的0.23%）。相比密集YOLOv11基线（87.68% mAP@50），实现了858倍GPU内存压缩和3,670倍存储减少。将IoU阈值从0.50放宽到0.40时，mAP恢复至89.26%。
...
TL;DR: FedCVU是一个用于跨视角视频理解的联邦学习框架，通过VS-Norm处理视角特定统计、CV-Align改善跨视角表示对齐、SLA减少通信开销，在动作理解和行人重识别任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在多摄像头视频理解中面临三个主要挑战：1) 异构视角和背景导致高度非独立同分布的数据分布和视角特定模式过拟合；2) 局部分布偏差导致表示不对齐，阻碍跨视角语义一致性；3) 大型视频架构带来过高的通信开销。

Method: 提出FedCVU框架，包含三个组件：VS-Norm（保留归一化参数处理视角特定统计）、CV-Align（轻量级对比正则化模块改善跨视角表示对齐）、SLA（选择性层聚合策略减少通信开销而不牺牲准确性）。

Result: 在跨视角协议下的动作理解和行人重识别任务上进行广泛实验，FedCVU在保持强视角性能的同时，持续提升未见视角的准确性，优于最先进的联邦学习基线方法，并展现出对领域异构性和通信约束的鲁棒性。

Conclusion: FedCVU有效解决了跨视角联邦学习中的关键挑战，通过创新的组件设计在保持隐私的同时实现了更好的跨视角视频理解性能，为多摄像头场景下的联邦学习应用提供了实用解决方案。
...
TL;DR: OmniFM是一个面向医学图像分析的模态和任务无关的联邦学习框架，通过频域方法解决多模态异质性问题，统一支持分类、分割、超分辨率、视觉问答等多种任务。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习框架与特定任务架构紧密耦合，对异质成像模态脆弱，限制了在现实世界中的部署。医疗机构在模态分布上差异很大，需要支持多样化的下游任务。

Method: 基于频域洞察：低频频谱分量具有跨模态一致性，编码模态不变解剖结构。包含：(1)全局频谱知识检索注入全局频率先验；(2)嵌入级交叉注意力融合对齐表示；(3)前缀-后缀频谱提示联合调节全局和个性化线索；(4)频谱-近端对齐目标稳定聚合。

Result: 在真实数据集上的实验表明，OmniFM在模态内和跨模态异质性下持续超越最先进的联邦学习基线，在微调和从头训练设置下均取得优越结果。

Conclusion: OmniFM提供了一个模态和任务无关的联邦学习框架，通过频域方法有效解决医学图像分析中的异质性问题，为现实世界部署提供了有前景的解决方案。
...
TL;DR: 该论文提出了一种跨场景去雨自适应框架，通过利用目标域的无雨背景图像，结合超像素生成和分辨率自适应融合策略，合成多样化的伪数据，显著提升了去雨模型在未见分布场景下的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习去雨方法在训练数据分布内表现良好，但在未见分布场景下性能严重下降。这主要源于合成训练数据与真实世界复杂雨物理动态之间的显著域差异。需要一种能够适应不同场景的去雨方法，而无需目标域的配对雨天观测数据。

Method: 提出跨场景去雨自适应框架：1) 超像素生成模块从源域提取稳定结构先验；2) 分辨率自适应融合策略通过纹理相似性对齐源域结构与目标域背景；3) 伪标签重合成机制使用多阶段噪声生成模拟真实雨纹；4) 作为即插即用模块可集成到任意去雨架构中。

Result: 在最新模型上的大量实验表明，该方法在未见分布域中获得了32%到59%的显著PSNR增益，同时显著加速了训练收敛速度。

Conclusion: 该框架通过利用目标域无雨背景图像，结合结构先验提取和自适应融合策略，有效解决了去雨模型在跨场景应用中的域适应问题，为低层计算机视觉任务提供了实用的解决方案。
...
TL;DR: 论文提出VR-SDR框架和HumanOmni-Speaker基准，解决多模态大模型在复杂多人对话中"谁在何时说了什么"的识别难题，通过视觉增量编码器在25fps采样下捕捉细微唇部动作，实现端到端的说话人识别和定位。


<details>
  <summary>Details</summary>
Motivation: 现有全模态大语言模型在处理复杂多人对话动态时存在"能力幻觉"问题，它们利用传统基准中的视觉偏差来绕过真正的跨模态对齐，同时依赖稀疏的低帧率视觉采样，破坏了唇部运动等高频率动态信息。需要打破这种幻觉，实现真正的端到端时空身份绑定。

Method: 提出视觉注册说话人分离与识别（VR-SDR）框架和HumanOmni-Speaker基准，通过严格消除视觉捷径，要求仅使用自然语言查询进行真正的端到端时空身份绑定。核心是视觉增量编码器，以25fps采样原始视频，将帧间运动残差压缩为每帧仅6个token，捕捉细粒度视觉音素和说话人轨迹，避免token爆炸问题。

Result: HumanOmni-Speaker展示了强大的多模态协同能力，原生支持端到端唇读和高精度空间定位，无需侵入性裁剪，在广泛的说话人中心任务上实现了优越性能。

Conclusion: 通过VR-SDR框架和HumanOmni-Speaker基准，成功解决了全模态大模型在复杂多人对话动态识别中的根本缺陷，实现了真正的跨模态对齐和细粒度说话人识别，为多模态对话理解提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统NeRF和3DGS等方法假设光线直线传播，无法处理非平面折射表面（如水面）导致的严重光学畸变，需要新的方法来处理折射效应

Method: 将折射边界与目标物体解耦：使用神经高度场建模折射水面几何，3D高斯场表示水下场景；提出折射感知的高斯光线追踪方法，基于斯涅尔定律计算非线性光线轨迹，通过端到端联合优化实现高质量渲染

Result: 在合成和真实场景的复杂波浪实验中，RefracGS在视觉质量上优于现有折射方法，训练速度提升15倍，实时渲染达到200 FPS

Conclusion: RefracGS通过解耦折射表面和场景表示，结合折射感知的高斯光线追踪，成功解决了非平面折射表面下的新视角合成问题，实现了高质量、高效率的渲染
...
TL;DR: PPGL-Swarm：一个基于多智能体的嗜铬细胞瘤和副神经节瘤诊断系统，通过自动化GAPP评分、基因型风险评估和多模态报告生成，解决现有诊断方法的局限性。


<details>
  <summary>Details</summary>
Motivation: PPGLs是罕见神经内分泌肿瘤，15-25%会发展为转移性疾病，5年生存率低至34%。现有GAPP评分系统存在三大局限：1) 需要手动评估六个独立组件，工作量高；2) 细胞密度和Ki-67等关键组件评估主观；3) 未包含SDHB突变等临床相关转移风险因素。现有智能诊断系统缺乏可追溯的推理过程，且未整合基因型信息等领域知识。

Method: 提出PPGL-Swarm多智能体诊断系统，将诊断分解为微任务分配给专门智能体。系统包含基因智能体和表格智能体，通过知识增强更好解释基因型和实验室发现。在训练中使用强化学习优化工具选择和任务分配，生成包含自动化GAPP评分（量化细胞密度和Ki-67）、基因型风险警报和整合证据的多模态报告。

Result: 系统能够生成全面的诊断报告，包括：1) 自动化GAPP评分，解决主观评估问题；2) 基因型风险警报，整合SDHB突变等关键风险因素；3) 多模态报告，提供整合证据；4) 可审计的推理轨迹，通过微任务分解实现决策过程透明化。

Conclusion: PPGL-Swarm通过多智能体架构解决了现有PPGL诊断系统的关键局限，提供了自动化、客观化、全面化的诊断解决方案，特别在整合基因型信息和提供可追溯推理方面具有优势，有望改善PPGL的临床诊断和管理。
...
TL;DR: MetaCompress提出了一种基于学习的提示无关视觉令牌压缩方法，用于解决多轮视觉问答中的高推理成本问题，相比现有启发式方法实现了更好的效率-精度平衡。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在视觉理解和推理方面表现出色，但过多的视觉令牌导致高推理成本。现有的令牌减少方法主要针对单轮视觉问答，而更实用的多轮视觉问答场景面临额外挑战：后续问题未知且可能涉及任意图像区域，使现有减少策略失效。

Method: 提出MetaCompress方法：1) 将令牌减少形式化为可学习的压缩映射，统一剪枝和合并等现有格式为单一学习目标；2) 引入数据高效训练范式，能够以有限计算成本学习最优压缩映射。

Result: 在多轮视觉问答基准测试和多个LVLM架构上的广泛实验表明，MetaCompress实现了优越的效率-精度平衡，同时在对话轮次间保持强泛化能力。

Conclusion: MetaCompress通过基于学习的提示无关方法克服了启发式设计的局限性，为多轮视觉问答中的视觉令牌压缩提供了有效解决方案，在保持性能的同时显著降低推理成本。
...
TL;DR: 论文研究指向机制在大型视觉语言模型认知任务中的作用，通过零样本计数实验发现"先指向再计数"方法比直接计数有更好的分布外泛化能力，坐标预测能帮助模型学习技能而非过拟合特定任务。


<details>
  <summary>Details</summary>
Motivation: 虽然指向机制已被证明能提高大型视觉语言模型的准确性，但其支持这些增益的具体机制及其在认知任务中的相关性尚不清楚。此外，中间指向点的可靠性研究不足，限制了它们作为视觉解释的用途。

Method: 研究采用两种方法微调最先进的大型视觉语言模型：1) 直接计数：模型仅预测物体总数；2) 先指向再计数：模型先生成目标物体的坐标，然后基于这些坐标进行计数。研究在认知任务——视觉场景的零样本计数中进行实验。

Result: 先指向再计数方法实现了更高的分布外泛化能力，表明坐标帮助模型学习技能而非过拟合狭窄任务。预测的点在超过89%的情况下能准确地在图像中定位（通过F1分数衡量），但性能在不同图像区域存在差异，显示出空间偏差。机制分析表明计数性能的提升源于坐标中编码的空间信息。

Conclusion: 指向机制通过将空间信息编码到坐标中，帮助大型视觉语言模型在认知任务中获得更好的泛化能力。虽然预测的坐标通常准确，但存在空间偏差，这限制了它们作为可靠视觉解释的潜力。
...
TL;DR: DaP-ICoT提出动态精确视觉思维的交错模态思维链推理，解决现有方法中静态视觉定位和破碎视觉表示的问题，显著提升推理效率并减少72.6%的token消耗。


<details>
  <summary>Details</summary>
Motivation: 当前交错模态思维链推理方法存在两个主要局限：1）静态视觉思维定位，在固定步骤静态插入视觉信息，导致推理效率低下且不灵活；2）破碎视觉思维表示，视觉token不连续且语义不连贯。

Method: 提出DaP-ICoT方法，包含两个关键组件：1）动态视觉思维集成，根据推理需求自适应引入视觉输入，减少冗余提高效率；2）精确视觉思维指导，确保视觉语义连贯且上下文对齐的表示。

Result: 在多个基准测试和模型上的实验表明，DaP-ICoT实现了最先进的性能，同时显著减少插入图像数量，使token消耗降低72.6%，实现更高效的交错模态思维链推理。

Conclusion: DaP-ICoT通过动态视觉思维集成和精确视觉思维指导，有效解决了现有交错模态思维链推理方法的局限性，在保持高性能的同时大幅提升了推理效率。
...
TL;DR: 本文提出SHARP方法，通过动态调整RoPE位置编码的缩放策略，结合专门针对遥感图像的RS-FLUX模型，实现无需训练的高分辨率遥感图像生成。


<details>
  <summary>Details</summary>
Motivation: 当前遥感图像生成面临两大障碍：缺乏领域专用的DiT先验模型，以及在遥感应用所需的高分辨率下训练成本过高。现有的静态位置编码缩放方法会损害遥感图像中重要的中高频细节信息。
...
Result: 在六种正方形和矩形分辨率上的实验表明，SHARP在CLIP分数、美学分数和HPSv2指标上始终优于所有无需训练的基线方法，在更激进的推断因子下优势更明显，且计算开销可忽略。

Conclusion: SHARP通过动态调整位置编码缩放策略，结合领域专用先验模型，实现了高质量、多尺度的遥感图像生成，解决了现有方法在遥感图像生成中的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 资源受限的自主机器人依赖稀疏直接和半直接视觉（惯性）里程计（VO）管道，但大多数系统性能严重依赖于手工调整的超参数。这些参数通常在部署时固定，而最优值会随场景特征（纹理密度、光照、运动模糊、传感器噪声）变化，导致在实际环境中性能脆弱

Method: 将前端配置制定为顺序决策问题，学习直接映射视觉输入到特征检测和跟踪参数的策略。使用轻量级纹理感知CNN编码器，在训练期间使用特权批评器。与先前仅依赖内部VO统计的RL方法不同，该方法观察图像内容并在跟踪退化前主动调整参数

Result: 在TartanAirV2和TUM RGB-D数据集上的实验显示，尽管完全在模拟中训练，但实现了3倍更长的特征跟踪和3倍更低的计算成本

Conclusion: 提出的图像条件强化学习框架能够有效嵌入专家知识到VO系统中，通过在线调整前端参数适应不同场景特征，显著提升了特征跟踪质量和计算效率
...
TL;DR: 该论文提出"通用正态嵌入"假设，认为生成模型和视觉编码器共享一个高斯潜在空间，并通过实验证明扩散模型的反向噪声和编码器表示之间存在线性语义对齐。


<details>
  <summary>Details</summary>
Motivation: 生成模型和视觉编码器虽然优化目标不同，但都表现出潜在空间的高斯特性。作者假设它们共享一个共同的潜在源，即通用正态嵌入，并希望验证这一假设。

Method: 引入NoiseZoo数据集，包含每张图像的DDIM反向扩散噪声和对应的编码器表示（CLIP、DINO）。在CelebA数据集上使用线性探针分析两个空间的语义对齐，并通过正交化方法实现可控编辑。

Result: 线性探针在两个空间都能进行强属性预测，表明生成噪声沿线性方向编码有意义的语义。这些方向能够实现忠实可控的图像编辑，正交化方法有效缓解了虚假纠缠。

Conclusion: 实验结果支持UNE假设，揭示了编码和生成之间共享的高斯潜在几何结构，为理解生成模型和编码器的内在联系提供了实证基础。
...
TL;DR: 该研究在工业机器人场景中评估了基于事件的物体检测方法，发现循环架构的YOLOv8s相比非循环基线有9.6%的性能提升，事件域预训练效果显著，但源域不匹配的预训练可能比从头训练更差。


<details>
  <summary>Details</summary>
Motivation: 事件相机在工业机器人中具有高时间分辨率、高动态范围和减少运动模糊等优势，但现有事件检测研究主要关注户外驾驶场景或有限类别设置，缺乏对工业多类别识别场景的系统评估。

Method: 在MTEvent数据集上对循环ReYOLOv8s进行基准测试，使用非循环YOLOv8s变体作为基线分析时间记忆效应，比较了从头训练、GEN1事件域预训练和PEDRo预训练三种初始化策略。

Result: 最佳循环模型（C21）达到0.285 mAP50，相比非循环基线（0.260）相对提升9.6%；GEN1预训练微调获得最佳结果0.329 mAP50；PEDRo初始化降至0.251，表明不匹配的源域预训练可能比从头训练更差；主要失败模式是类别不平衡和人-物交互。

Conclusion: 该研究为工业环境中基于事件的循环检测提供了系统的基准测试和分析，证实了循环架构和事件域预训练的有效性，同时指出了类别不平衡和人-物交互是主要挑战。
...
<details>
  <summary>Details</summary>
Motivation: 传统喜剧研究主要关注语言内容，但现场表演同样依赖于身体表现和观众反馈。为了全面理解喜剧表演的动态特征，需要建立能够同时捕捉语言、手势和观众反应的多模态资源。

Method: 采用多模态数据处理流程：BERTopic进行60秒主题分割，Whisper-AT检测0.8秒笑声，YOLOv8-cls进行镜头分类，YOLOv8s-pose提取1fps的原始关键点。保留17个关节的原始骨骼坐标，计算连续运动学信号（手臂展开、动能、躯干倾斜）。所有流通过分层时间包含对齐，无需重采样。

Result: 研究发现：1）动能与观众笑声率呈负相关（r=-0.75），符合"笑点前的静止"模式；2）个人和身体主题比地缘政治主题引发更多笑声；3）特写镜头比例与笑声呈正相关（r=+0.28），符合反应性蒙太奇理论。

Conclusion: TIC-TALK资源为研究喜剧表演的多模态动态提供了新工具，揭示了身体运动、主题内容和镜头选择如何共同影响观众反应，为理解现场表演的复杂性提供了实证基础。
...
TL;DR: 提出基于预训练医学图像分词器和潜在变换器的主动采样框架，利用量化视觉标记的离散结构来指导k空间采样，通过标记熵度量不确定性，实现更高效的压缩感知MRI重建。


<details>
  <summary>Details</summary>
Motivation: 传统MRI数据采集速度慢，限制了临床吞吐量并增加患者不适。压缩感知MRI通过欠采样k空间数据重建图像来加速采集，但需要最优的采样轨迹和高保真重建模型。现有方法在采样策略上仍有改进空间。

Method: 提出主动采样框架，利用预训练医学图像分词器和潜在变换器将解剖结构表示为量化视觉标记的字典，获得潜在空间的概率分布。基于标记熵推导不确定性度量，指导主动采样过程。引入两种策略：1) 潜在熵选择(LES)，将补丁级标记熵投影到k空间域识别信息丰富的采样线；2) 基于梯度的熵优化(GEO)，通过总潜在熵损失的k空间梯度识别最大不确定性减少区域。

Result: 在fastMRI单线圈膝关节和脑部数据集上，以×8和×16加速进行评估。结果显示，提出的主动策略在感知度量和基于特征的距离方面优于最先进的基线方法。

Conclusion: 提出的主动采样框架通过利用预训练医学图像分词器的离散潜在结构，能够有效指导k空间采样，提高压缩感知MRI的重建质量，为加速MRI采集提供了新思路。
...
TL;DR: 提出了一种基于多任务学习的无级联架构，用于中文普通话视觉语音识别，通过联合集成音素和视素等中间表示来提升性能，同时避免级联架构中的错误累积和延迟问题。


<details>
  <summary>Details</summary>
Motivation: 中文普通话视觉语音识别(VSR)由于声调特性，性能落后于英语等非声调语言。现有系统通常采用级联架构引入拼音等中间表示，但这会导致错误累积和推理延迟增加。

Method: 提出基于多任务学习的无级联架构，联合集成音素和视素等多种中间表示。引入语义引导的局部对比损失来时序对齐特征，在推理时实现按需激活，在推理效率和性能之间取得平衡。

Result: 在公开数据集上的实验表明，该方法实现了优越的识别性能，同时避免了级联架构中的错误累积问题。

Conclusion: 提出的无级联多任务学习架构有效解决了中文VSR中的声调识别挑战，通过联合中间表示和时序对齐特征，在保持推理效率的同时提升了识别准确性。
...
<details>
  <summary>Details</summary>
Motivation: 眼底成像成本低但HTN相关线索微弱，MRI提供更强血管标记但昂贵且与眼底数据不配对，需要解决模态隔离数据集下的知识迁移问题

Method: 构建基于共享生物标记的临床相似性kNN图连接两个队列，训练MRI教师模型，通过图传播其表征，为眼底患者生成脑信息表征目标，眼底学生模型通过联合目标（HTN监督、目标蒸馏、关系蒸馏）训练

Result: 在新收集的无配对MRI-眼底-生物标记数据集上，CGMD相比标准蒸馏和非图插补基线持续改善眼底HTN预测，消融实验证实临床基础图连接的重要性

Conclusion: CGMD框架成功在无配对多模态数据情况下，通过临床相似性图将MRI学到的HTN知识迁移到眼底模型，为跨模态知识蒸馏提供新方法
...
<details>
  <summary>Details</summary>
Motivation: 传统数据增强方法需要手动设计增强策略和初始化增强强度，这在不同图像视觉任务中需要大量工程工作。作者希望开发一种能够自动、动态调整增强强度的算法，减少人工干预。

Method: Ctrl-A 采用控制理论中的控制循环架构，基于相对操作响应曲线动态调整每个增强操作的强度分布。算法在训练过程中在线调整，能够抑制对模型性能有负面影响的增强风格。

Result: 在 CIFAR-10、CIFAR-100 和 SVHN-core 基准数据集上使用 WideResNet-28-10 架构的实验表明，Ctrl-A 与现有最先进的数据增强策略相比具有高度竞争力。

Conclusion: ControlAugment 提供了一种自动化、自适应的数据增强方法，通过控制理论原理动态调整增强强度，减少了手动设计增强策略的需求，在多个基准数据集上表现出色。
...
TL;DR: 本文挑战了红外与可见光图像融合中严格配对训练范式(SPTP)的必要性，系统研究了非配对(UPTP)和任意配对(APTP)训练范式，提出了能够显著丰富跨模态关系的实用框架。
...
Method: 建立了APTP的理论目标，反映了UPTP和SPTP之间的互补性。开发了一个实用框架，即使在训练数据严重受限且未对齐的情况下也能显著丰富跨模态关系。设计了三个轻量级端到端基线模型（CNN、Transformer、GAN）和一组创新损失函数。

Result: 实验表明，APTP和UPTP是可行的，能够在严重受限且内容不一致的红外与可见光数据集上训练模型，达到与SPTP下100倍更大数据集相当的性能。

Conclusion: 这一发现从根本上减轻了数据收集的成本和难度，从数据角度增强了模型鲁棒性，为红外与可见光图像融合研究提供了可行的解决方案。
...
TL;DR: SteelDefectX是一个包含7,778张图像、25个缺陷类别的钢铁表面缺陷视觉-语言数据集，提供从粗到细的文本描述，旨在提升缺陷检测的可解释性和泛化能力。
...
Method: 创建SteelDefectX数据集，包含粗粒度（类别级信息：缺陷类型、视觉属性、工业原因）和细粒度（样本特定属性：形状、大小、深度、位置、对比度）的文本标注。建立包含四个任务的基准测试：视觉分类、视觉-语言分类、少样本/零样本识别、零样本迁移。

Result: 实验表明，从粗到细的文本标注显著提高了模型的可解释性、泛化能力和迁移能力。数据集将公开提供。

Conclusion: SteelDefectX数据集为推进可解释、可泛化的钢铁表面缺陷检测研究提供了宝贵资源，通过丰富的视觉-语言标注改善了现有方法的局限性。
...
TL;DR: 使用视频扩散模型生成MJO序列，通过低维条件控制实现理论框架与高分辨率大气复杂性的桥梁


<details>
  <summary>Details</summary>
Motivation: 生成式深度学习在热带MJO建模中很强大，但其与传统理论框架的关系尚不清楚，需要建立低维理论与高分辨率大气复杂性之间的桥梁

Method: 提出基于大气再分析数据训练的视频扩散模型，通过关键低维指标条件化生成长MJO序列，并可基于理想化条件生成更易处理的MJO

Result: 生成的MJO捕捉了关键特征包括合成图、功率谱和多尺度结构（如对流耦合波），尽管存在一些偏差；通过理想化条件可解构底层过程并识别物理驱动因素
...
Method: 提出专门针对视频扩散模型的蒸馏框架，包含三个核心创新：(1) 自适应回归损失：动态调整空间监督权重，防止因分布偏移过大而产生的伪影；(2) 时间正则化损失：对抗时间崩溃，促进平滑且物理上合理的采样轨迹；(3) 推理时帧插值策略：在保持感知质量的同时减少采样开销。

Result: 在VBench和VBench2基准测试上的大量实验和消融研究表明，该方法实现了稳定的少步视频合成，显著提高了感知保真度和运动真实感。在多个指标上持续优于现有的蒸馏基线方法。

Conclusion: 提出的专门针对视频扩散模型的蒸馏框架有效解决了现有图像蒸馏方法直接应用于视频模型时产生的问题，通过自适应回归损失、时间正则化损失和推理时帧插值策略的组合，实现了高效且高质量的少步视频合成。
...
TL;DR: 提出对抗伪装方法保护人脸识别隐私，通过优化颜色、形状、角度等低维参数生成对抗图案，在物理世界中简单可复现，能有效降低多种人脸识别模型的性能


<details>
  <summary>Details</summary>
Motivation: 人脸识别技术的快速发展带来了有益应用，但也引发了大规模监控威胁个人隐私的担忧，需要保护用户隐私的解决方案

Method: 定义颜色、形状、角度参数化的低维图案空间，优化图案后投影到语义有效的人脸区域进行评估，最大化多种架构的识别错误率，确保跨模型可迁移性

Result: 显著降低了所有测试的先进人脸识别模型在模拟中的性能，在真实世界人体实验中表现出有希望的结果，揭示了模型鲁棒性差异和攻击跨架构可迁移性证据

Conclusion: 对抗伪装是一种高效、简单可复现的隐私保护方法，能有效对抗包括黑盒系统在内的多种人脸识别模型，为保护个人隐私提供了实用解决方案
...
TL;DR: SAGE-GRPO提出了一种稳定的视频生成对齐方法，通过约束探索在预训练模型定义的数据流形附近，解决了现有GRPO方法在视频生成中因ODE-to-SDE转换引入过多噪声而导致的不稳定问题。


<details>
  <summary>Details</summary>
Motivation: 视频生成的GRPO方法相比语言模型和图像生成可靠性较差，主要原因是视频生成解空间复杂，ODE-to-SDE转换会注入过多噪声，降低生成质量并使奖励估计不可靠，从而破坏训练后对齐的稳定性。
...
Conclusion: SAGE-GRPO通过约束探索在预训练模型定义的数据流形附近，有效解决了视频生成GRPO方法的不稳定问题，实现了更可靠的训练后对齐和更高质量的视频生成。
...
TL;DR: 提出首个红外领域的通用物理攻击方法UPPA，使用参数化贝塞尔块建模扰动，通过粒子群优化算法实现全局优化，将数字扰动转化为物理冷贴片，实现高效物理攻击。


<details>
  <summary>Details</summary>
Motivation: 现有红外行人检测器对物理对抗攻击的脆弱性日益明显，现有物理攻击方法主要依赖实例特定的在线优化和刚性模式设计，导致部署成本高且物理鲁棒性不足。

Method: 采用几何约束的参数化贝塞尔块建模扰动，利用粒子群优化算法在全局数据分布上进行统一优化，保持动态变形下的拓扑稳定性；在物理部署阶段将优化的数字扰动转化为物理冷贴片，实现连续平滑的低温分布。

Result: UPPA在无需在线计算开销的情况下实现了出色的物理攻击成功率，同时表现出强大的跨域泛化能力和可靠的黑盒可迁移性。

Conclusion: 该方法解决了现有红外物理攻击方法的局限性，为红外行人检测器的安全评估提供了有效的通用物理攻击工具。
...
TL;DR: 将现代学习型立体匹配算法集成到卫星立体处理流程中，在数字表面模型生成方面相比传统方法有显著改进，但植被等复杂表面仍存在挑战。


<details>
  <summary>Details</summary>
Motivation: 虽然学习型立体匹配算法在标准基准测试中表现出色，但由于视角几何和视差假设的差异，将其集成到实际卫星处理流程中仍面临挑战。本研究旨在将现代学习型立体匹配器集成到卫星立体处理流程中，以改进数字表面模型的生成质量。

Method: 将多种现代学习型立体匹配算法（包括StereoAnywhere、MonSter、Foundation Stereo以及卫星微调的MonSter变体）集成到卫星立体处理流程中，通过调整校正阶段来确保兼容的视差极性和范围。

Result: 实验表明，相比传统的基于成本体积的方法，学习型方法在数字表面模型精度方面有持续改进，但常用的平均绝对误差等指标显示出饱和效应。定性结果显示几何细节和结构清晰度显著提升，但在植被等挑战性表面类型上的性能仍然有限。

Conclusion: 学习型立体匹配算法可以成功集成到卫星处理流程中，带来显著的性能提升，但需要更好的评估策略来反映感知和结构保真度，同时植被等自然环境的挑战仍然是开放性问题。
...
TL;DR: LoRA²：一种自适应秩选择的LoRA微调方法，通过变分方法让每层的秩在微调过程中自适应调整，实现性能与内存消耗的更好平衡


<details>
  <summary>Details</summary>
Motivation: 当前LoRA微调中秩的选择通常基于社区共识，没有考虑个性化主题的复杂性。为每个LoRA组件选择合适秩的成本是组合性的，因此通常采用固定所有组件相同秩的简化方法

Method: 受变分方法启发，让每层的秩在微调过程中自由适应，通过对秩位置施加重要性排序，鼓励在严格需要时创建更高秩

Result: LoRA²在29个主题上实现了DINO、CLIP-I和CLIP-T指标的竞争性平衡，同时比高秩LoRA版本需要更少的内存和更低的秩

Conclusion: 该方法首次解决了LoRA微调中自适应秩选择的挑战，为个性化图像生成提供了更高效的微调策略
...
TL;DR: CLEAR提出了一种无需掩码的视频字幕去除框架，通过上下文感知自适应学习实现真正的端到端推理，在中文基准上优于现有方法，并具有出色的跨语言零样本泛化能力。
...
Method: 提出CLEAR框架，采用两阶段设计：第一阶段通过双编码器的自监督正交约束学习解耦的字幕表示；第二阶段采用基于LoRA的自适应和生成反馈进行动态上下文调整。仅需基础扩散模型0.77%的参数进行训练。

Result: 在中文字幕基准上，CLEAR比依赖掩码的基线方法提升了+6.77dB PSNR和-74.7% VFID。在六种语言（英语、韩语、法语、日语、俄语、德语）上展示了优异的零样本泛化能力。

Conclusion: CLEAR通过上下文感知自适应学习实现了无需掩码的端到端视频字幕去除，在性能和泛化能力上都优于现有方法，为实际部署提供了更实用的解决方案。
...
TL;DR: SHAPE提出了一种用于医学图像分割的无监督域自适应框架，通过层次特征调制和超图合理性评估来解决现有方法在特征对齐和伪标签验证方面的局限性，在心脏和腹部跨模态基准上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有无监督域自适应方法存在两个根本性限制：1）语义无关的特征对齐导致分布保真度差；2）伪标签验证忽视全局解剖约束，无法防止形成全局不合理的结构。这些问题限制了医学分割模型在不同临床环境中的部署效果。

Method: SHAPE框架基于DINOv3构建，包含三个核心模块：1）层次特征调制（HFM）模块，生成高保真度和类别感知的特征；2）超图合理性估计（HPE），利用超图评估标准图无法捕获的全局解剖合理性；3）结构异常修剪（SAP），通过跨视图稳定性消除剩余伪影。

Result: 在心脏和腹部跨模态基准测试中，SHAPE显著优于先前方法，取得了最先进的平均Dice分数：心脏数据上MRI->CT为90.08%，CT->MRI为78.51%；腹部数据上MRI->CT为87.48%，CT->MRI为86.89%。

Conclusion: SHAPE通过将自适应重新定义为全局解剖合理性问题，有效解决了现有无监督域自适应方法的局限性，为医学图像分割模型在不同临床环境中的鲁棒部署提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统辐射传输模型计算成本高且通常只能生成光谱级输出，需要更高效的高光谱图像生成方法用于大规模模拟、算法开发和任务设计

Method: 基于潜在表示的高光谱模拟框架，学习高光谱数据的潜在生成表示，支持单步训练或两步策略（VAE预训练+参数到潜在空间插值）

Result: 在PROSAIL模拟植被数据和Sentinel-3 OLCI影像上，该方法在重建精度、光谱保真度和对真实空间变化的鲁棒性方面优于经典回归模拟器

Conclusion: 模拟的高光谱图像在下游生物物理参数反演中保持性能，证明模拟数据对遥感应用具有实际相关性
...
<details>
  <summary>Details</summary>
Motivation: 当前最先进的NeRF模型在卫星图像重建中存在过拟合导致的几何伪影问题，需要改进几何精度

Method: 提出三种模型无关的正则化方法：重力对齐平面正则化（对齐深度推断的表面法线与重力轴）、粒度正则化（强制粗到细的几何学习方案）、深度监督正则化（稳定早期训练）

Result: 在DFC2019卫星重建基准上，SatGeo-NeRF相比EO-NeRF和EO-GS等最先进基线，平均高度误差分别降低了13.9%和11.7%

Conclusion: SatGeo-NeRF通过几何正则化有效缓解了卫星图像NeRF中的几何伪影问题，显著提升了重建几何精度
...
TL;DR: 提出一种相机无关的3D高斯点云剪枝方法，仅依赖属性推导的邻域描述符，通过Beta证据模型量化每个点的可靠性，实现高效剪枝同时保持重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯点云剪枝方法大多依赖相机参数、渲染图像或视角相关度量，这在相机无关的交换场景中成为障碍。需要一种不依赖相机信息的剪枝方法，以支持点云表示的直接共享。

Method: 提出混合描述符框架，从点云表示中直接捕获结构和外观一致性；将剪枝建模为统计证据估计问题，引入Beta证据模型，通过概率置信分数量化每个点的可靠性。

Result: 在ISO/IEC MPEG CTC标准测试序列上的实验表明，该方法能实现显著剪枝同时保持重建质量，为现有相机依赖剪枝策略提供了实用且可泛化的替代方案。

Conclusion: 提出了一种相机无关、一次性、训练后剪枝方法，仅依赖属性推导的邻域描述符，通过Beta证据模型量化点云可靠性，为3D高斯点云的存储、传输和下游处理提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 医学影像中的定量疾病严重程度评分成本高、耗时长且存在读者间差异，而临床档案包含大量纵向影像数据但缺乏专家标注。现有自监督方法通常忽略时间顺序结构，需要利用这些未标注的时间信息来减少标注需求。

Method: 提出ChronoCon对比学习方法，基于患者纵向扫描的就诊时间顺序（而非标签）构建排序损失。在不可逆疾病单调进展的临床合理假设下，仅利用时间顺序信息学习疾病相关表征，将Rank-N-Contrast思想从标签距离推广到时间排序。

Result: 在类风湿关节炎X光片严重程度评估中，学习到的表征显著提高了标签效率。在低标签设置下，ChronoCon明显优于从ImageNet权重初始化的全监督基线。仅用5名患者的专家评分进行少样本学习，严重程度预测的组内相关系数达到86%。

Conclusion: 时序对比学习能够利用常规可用的影像元数据，在不可逆疾病领域显著减少标注需求，为医学影像分析提供了有效的无监督学习方法。
...
TL;DR: GSA是一种新颖的3D高斯泼溅模型对齐方法，能够通过相似变换（旋转、平移、缩放）对齐两个独立的3DGS模型，即使它们是同一类别中的不同对象（如不同汽车）。


<details>
  <summary>Details</summary>
Motivation: 现有方法只能对齐同一对象的3DGS模型，且通常需要输入真实尺度，而GSA旨在解决类别级3DGS注册问题，实现对不同对象但同类别3D模型的对齐。

Method: 采用两阶段优化框架：1）粗配准使用迭代特征引导的绝对定向求解器，对初始化不敏感；2）精配准采用多视角特征一致性约束，受逆辐射场公式启发。利用视点引导的球形图特征获取鲁棒对应关系。

Result: 在相同对象情况下，GSA显著优于现有方法；在不同对象但同类别情况下，GSA大幅超越现有方法，为类别级3DGS注册提供了首个有效解决方案。

Conclusion: GSA实现了类别级3D高斯泼溅模型对齐，解锁了新的应用场景，其两阶段优化框架在相同对象和不同对象情况下都表现出色。
...
TL;DR: MultiBind：一个用于评估多主体图像生成中跨主体属性错误绑定的新基准，通过真实多人物照片构建，提供槽位有序的主体裁剪、背景参考和实体索引提示，并提出维度混淆评估协议来诊断绑定失败。
...
Method: 构建MultiBind基准：从真实多人物照片构建，提供槽位有序的主体裁剪（带掩码和边界框）、规范化主体参考、修复的背景参考以及从结构化注释派生的密集实体索引提示。提出维度混淆评估协议：将生成的主体与真实槽位匹配，使用面部识别、外观、姿态和表情等专家模型测量槽位间相似性，通过减去对应的真实相似性矩阵来分离自退化与真正的跨主体干扰。

Result: MultiBind能够揭示传统重建指标遗漏的绑定失败模式，如漂移、交换、主导和混合等可解释的失败模式，实验表明现代多参考生成器存在这些绑定问题。

Conclusion: MultiBind基准和维度混淆评估协议为多主体图像生成中的跨主体属性绑定问题提供了有效的诊断工具，能够识别传统指标无法检测的绑定失败模式，有助于改进多参考生成器的性能。
...
TL;DR: FeatDistill是一个用于AI生成图像检测的框架，通过特征蒸馏和多专家集成解决真实世界取证中的三个瓶颈问题，在NTIRE挑战中表现出强大的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 深度伪造技术的快速迭代和广泛传播对信息安全构成严重挑战，需要鲁棒且泛化能力强的AI生成伪造图像检测方法。该研究针对真实世界取证中的三个实际瓶颈：退化干扰、特征表示不足和泛化能力有限。

Method: 1. 构建四骨干Vision Transformer集成（CLIP和SigLIP变体）以捕捉互补的取证线索
2. 扩展训练集并引入全面的退化建模，使检测器暴露于多样化的质量变化和合成伪影
3. 采用两阶段训练范式：先用标准二分类目标优化，然后通过密集特征级自蒸馏进行表示对齐
4. 推理时通过平均四个独立训练专家的概率获得最终预测

Result: 在NTIRE挑战设置中的广泛评估表明，FeatDistill在多样化的"野外"条件下实现了强大的鲁棒性和泛化能力。尽管采用集成设计，框架仍然高效，仅需约10GB峰值GPU内存。

Conclusion: FeatDistill通过特征蒸馏和多专家集成，为真实世界深度伪造图像检测提供了一个有效且实用的解决方案，能够有效缓解过拟合并增强学习特征的语义一致性。
...
TL;DR: GeoFlow提出了一种轻量高效的细粒度跨视角地理定位框架，通过概率映射和迭代优化采样算法，在保持高精度的同时实现实时处理速度（29 FPS），打破了传统方法在精度和速度之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 在GPS拒止环境中，精确快速的定位对于自主导航安全至关重要。现有的细粒度跨视角地理定位方法面临一个困难权衡：高精度模型速度慢，难以满足实时应用需求。

Method: GeoFlow采用轻量级框架，学习直接的概率映射，预测修正位置假设所需的位移（距离和方向）。配合新颖的推理算法——迭代优化采样（IRS），该算法通过迭代优化一组假设，使其从随机起点"流动"到稳健的收敛共识。

Result: 在KITTI和VIGOR数据集上的实验表明，GeoFlow实现了最先进的效率，以29 FPS的实时速度运行，同时保持了有竞争力的定位精度。该方法支持灵活的推理时缩放，无需重新训练即可在性能和计算之间进行权衡。

Conclusion: GeoFlow打破了精度和速度之间的传统权衡，为开发实用的实时地理定位系统开辟了新路径，在保持高精度的同时实现了实时处理能力。
...
TL;DR: Group3D提出了一种多视角开放词汇3D检测框架，通过语义约束直接整合到实例构建过程中，解决了几何一致性方法中的过合并和碎片化问题。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D检测方法通常将几何实例构建与语义标注解耦，仅依赖几何一致性进行实例合并。当几何证据不完整时，这种纯几何方法会导致不可逆的关联错误，包括不同对象的过合并或单个实例的碎片化。

Method: Group3D从多模态大语言模型获取场景自适应词汇表，并将其组织为语义兼容组，编码合理的跨视角类别等价关系。这些组作为合并时的约束条件：3D片段只有在同时满足语义兼容性和几何一致性的情况下才会被关联。该方法支持已知姿态和自由姿态设置，仅依赖RGB观测。

Result: 在ScanNet和ARKitScenes上的实验表明，Group3D在多视角开放词汇3D检测中实现了最先进的性能，同时在零样本场景中表现出强大的泛化能力。

Conclusion: 通过将语义约束直接整合到实例构建过程中，Group3D有效缓解了几何驱动的过合并问题，同时吸收了多视角类别变异性，为开放词汇3D检测提供了一种更鲁棒的解决方案。
...
TL;DR: 提出一种统一的时空令牌压缩方法，通过全局令牌保留池和文本感知合并，在仅保留约2%视觉令牌的情况下保持90.1%的基线性能，显著降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型面临高计算成本问题，现有两阶段时空压缩方法在极低保留率下会导致视觉证据丢失和不平衡分配，影响问答准确性。

Method: 1) 将令牌压缩重新定义为全局令牌保留池内的时空分配任务；2) 提出统一选择机制，整合注意力权重和语义相似性全局选择高贡献、低冗余令牌；3) 未选中令牌通过聚类和重填充保留信息完整性；4) 在LLM内部引入文本感知合并进行基于查询相关性的二次压缩。

Result: 仅保留约2%视觉令牌即可保持90.1%的基线性能，FLOPs降低至约2.6%，在多个基准测试中表现优异，降低端到端推理延迟和内存消耗。

Conclusion: 该方法作为即插即用模块兼容现有Video-LLMs，无需重新训练，在超低令牌保留率下建立了视频理解的最先进水平，显著提升了计算效率。
...
### [158] [BHDD: A Burmese Handwritten Digit Dataset](https://arxiv.org/abs/2603.21966)
...
<details>
  <summary>Details</summary>
Motivation: 缅甸文字的手写数字识别研究缺乏公开可用的数据集，现有数据集主要针对拉丁数字，而缅甸数字的圆形特征带来独特的识别挑战，需要专门的数据集来支持相关研究。

Method: 收集了超过150名不同年龄和背景的参与者手写样本，构建了包含60,000个训练样本和27,561个测试样本的数据集，分析了类别分布、像素统计和形态变化，并建立了多层感知器、两层CNN以及改进CNN（含批归一化和数据增强）作为基线模型。

Result: 数据集包含87,561张图像，训练集各类别均匀分布，测试集保持收集时的自然分布。基线模型在测试集上分别达到99.40%、99.75%和99.83%的准确率，识别缅甸数字的圆形特征存在特定混淆对。

Conclusion: BHDD是首个公开的缅甸手写数字数据集，填补了该领域的数据空白，为缅甸文字识别研究提供了基础资源，数据集采用CC BY-SA 4.0许可在GitHub上开源。
...
### [159] [GeoFusion-CAD: Structure-Aware Diffusion with Geometric State Space for Parametric 3D Design](https://arxiv.org/abs/2603.21978)
*Xiaolei Zhou,Chuangjie Fang,Jie Wu,Jingyi Yang,Boyi Lin,Jianwei Zheng*

Main category: cs.CV

TL;DR: GeoFusion-CAD：基于扩散模型的参数化CAD序列生成框架，通过层次树编码和C-Mamba块解决长序列生成问题，在DeepCAD-240基准上超越Transformer方法


<details>
  <summary>Details</summary>
Motivation: 现有参数化CAD生成方法（特别是基于Transformer的）在处理长命令序列时面临困难，二次注意力成本和有限上下文窗口限制了可扩展性，尤其是在复杂几何和拓扑依赖下

Method: 提出GeoFusion-CAD端到端扩散框架：1）将CAD程序编码为层次树，在状态空间扩散过程中联合捕获几何和拓扑信息；2）使用轻量级C-Mamba块通过选择性状态转移建模长程结构依赖；3）引入DeepCAD-240扩展基准，序列长度40-240，保持ABC数据集的草图-拉伸语义

Result: 在短序列和长序列范围均取得优越性能，保持高几何保真度和拓扑一致性，而基于Transformer的模型在长序列上性能下降；在长序列参数化CAD生成上达到新的SOTA

Conclusion: GeoFusion-CAD为下一代CAD建模系统建立了可扩展的基础，通过扩散框架和状态空间建模解决了长序列生成的挑战，代码和数据集已开源
...
### [160] [Speed by Simplicity: A Single-Stream Architecture for Fast Audio-Video Generative Foundation Model](https://arxiv.org/abs/2603.21986)
...
TL;DR: daVinci-MagiHuman是一个开源的人体中心音频-视频生成基础模型，使用单流Transformer统一处理文本、视频和音频，在人体相关场景中表现优异，支持多语言语音生成，推理速度快。


<details>
  <summary>Details</summary>
Motivation: 开发一个能够联合生成同步视频和音频的开源基础模型，专注于人体中心场景，简化现有多流或交叉注意力架构的复杂性，提供高质量、高效的人体表现生成能力。

Method: 采用单流Transformer架构，将文本、视频和音频统一编码到同一token序列中，仅使用自注意力机制处理。结合模型蒸馏、潜在空间超分辨率和Turbo VAE解码器实现高效推理。

Result: 在自动评估中，daVinci-MagiHuman在视觉质量和文本对齐方面领先开源模型，语音可懂度的词错误率最低（14.60%）。在人工成对评估中，对Ovi 1.1和LTX 2.3的胜率分别为80.0%和60.9%。

Conclusion: daVinci-MagiHuman是一个高效、高质量的开源音频-视频生成模型，在人体中心场景中表现出色，支持多语言生成，并开源了完整的模型栈和推理代码库。
...
### [161] [LRC-WeatherNet: LiDAR, RADAR, and Camera Fusion Network for Real-time Weather-type Classification in Autonomous Driving](https://arxiv.org/abs/2603.21987)
*Nour Alhuda Albashir,Lars Pernickel,Danial Hamoud,Idriss Gouigah,Eren Erdal Aksoy*

Main category: cs.CV

TL;DR: LRC-WeatherNet是一个新颖的多传感器融合框架，结合LiDAR、RADAR和摄像头数据，用于自动驾驶中的实时天气条件分类，在恶劣天气下显著优于单模态基线方法。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆在雨、雾、雪等恶劣天气条件下面临感知和导航挑战，这些天气会降低LiDAR、RADAR和RGB摄像头传感器的性能。虽然每种传感器都有独特优势（如RADAR在低能见度下的鲁棒性，LiDAR在清晰条件下的精确性），但在环境遮挡下也各有局限性。

Method: 提出LRC-WeatherNet多传感器融合框架，集成LiDAR、RADAR和摄像头数据。采用早期融合（使用统一的鸟瞰图表示）和中期门控融合（融合模态特定的特征图），根据变化天气下各传感器的可靠性进行自适应调整。

Result: 在涵盖9种天气类型的MSU-4S数据集上评估，LRC-WeatherNet实现了优越的分类性能和计算效率，在恶劣条件下显著优于单模态基线方法。

Conclusion: 这是首个结合所有三种模态用于自动驾驶中鲁棒实时天气分类的工作。作者发布了训练模型和源代码，为恶劣天气下的自动驾驶感知提供了有效的多传感器融合解决方案。
...
TL;DR: SegMaFormer：轻量级混合架构，结合Mamba和Transformer模块，在3D医学图像分割中实现高效长程依赖建模，参数减少75倍，性能保持竞争力。


<details>
  <summary>Details</summary>
Motivation: Transformer和Mamba架构虽然提升了3D医学图像分割的全局上下文建模能力，但现有Transformer模型计算复杂度和参数量过大，对体积数据和有限标注医学图像数据集不友好。
...
Result: 在三个公开基准（Synapse、BraTS、ACDC）上达到与显著更大模型相当的Dice系数，参数减少高达75倍，FLOPs大幅降低，性能保持竞争力。

Conclusion: SegMaFormer为3D医学图像分割提供了一个高效且高性能的解决方案，通过巧妙结合Mamba和Transformer的优势，在保持竞争力的同时显著降低了计算和参数需求。
...
TL;DR: 提出一种用于机器人安装OCT探头的六自由度手眼标定标记方法，实现大范围曲面组织扫描的精确拼接，避免传统平移扫描和图像配准的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统OCT扫描拼接方法存在局限性：手持扫描需要图像重叠配准；机器人扫描通常限制为平移运动以避免复杂的手眼标定，但这无法有效扫描曲面组织。需要一种能实现六自由度手眼标定的方法来解决曲面组织扫描问题。

Method: 提出一种标记方法，用于机器人安装OCT探头的完整六自由度手眼标定。该方法不依赖图像配准，通过标定标记确定机器人坐标系与OCT探头坐标系之间的精确变换关系。

Result: 标定结果具有高度可重复性。在两个体模曲面上的实验表明，该方法能够实现大范围曲面组织的一致扫描。相比传统三维平移机器人扫描方法有明显改进，且不会沿扫描路径累积误差。

Conclusion: 提出的六自由度手眼标定方法能够有效解决曲面组织扫描问题，实现精确的机器人OCT扫描拼接，避免了传统方法的局限性，为临床大范围曲面组织成像提供了可靠解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管基于扩散的真实世界图像修复取得了显著进展，但如何高效利用超大规模预训练文本到图像模型并充分挖掘其潜力仍是重大挑战

Method: 1. 基于FLUX.1-dev流匹配模型构建图像修复框架；2. 采用统一多模态融合技术，将多模态条件编码为统一序列指导高质量图像合成；3. 引入训练免费的测试时缩放范式，通过奖励模型反馈动态引导去噪方向

Result: 在多个标准基准测试中实现了最先进的性能表现，验证了流匹配模型在低层视觉任务中的强大能力

Conclusion: 该工作不仅验证了流匹配模型在低层视觉任务中的强大能力，更重要的是提出了一种适用于大型预训练模型的新型高效推理时缩放范式
...
<details>
  <summary>Details</summary>
Motivation: 现有的推理时防御方法通常进行类别无关的token级干预，无法捕捉分布在完整token序列中的恶意语义，且容易受到对抗性提示的攻击

Method: 提出DTVI双阶段防御框架：1）在完整提示嵌入上进行类别感知的序列级干预，更好地捕捉分布式恶意语义；2）在视觉生成阶段进一步衰减剩余的不安全影响

Result: 在现实世界不安全提示、对抗性提示和多个有害类别上，DTVI实现了有效且鲁棒的防御，在性类别基准上平均防御成功率达94.43%，在七个不安全类别上达88.56%，同时保持良性提示的生成质量
...
TL;DR: FontCrafter：基于元素驱动的艺术字体生成框架，通过视觉上下文生成策略和注意力重定向机制，实现高保真度的结构和纹理重建，支持灵活的风格混合控制。


<details>
  <summary>Details</summary>
Motivation: 现有艺术字体生成方法存在风格多样性有限和粗粒度控制的问题。本文探索元素驱动的字体生成，将元素作为字体基本视觉单元，旨在实现更丰富多样的艺术字体创作。

Method: 1) 构建ElementFont大规模数据集；2) 提出上下文生成策略，将元素图像作为视觉上下文，使用修复模型在像素级将元素风格转移到字形区域；3) 设计轻量级上下文感知掩码适配器(CMA)注入形状信息；4) 采用无需训练的注意力重定向机制实现区域感知风格控制；5) 应用边缘重绘使边界更自然。

Result: FontCrafter在零样本生成方面表现出色，特别是在保持结构和纹理保真度方面，同时支持风格混合等灵活控制。实验证明该方法在艺术字体生成任务中具有优越性能。

Conclusion: FontCrafter框架通过元素驱动的方法解决了艺术字体生成中的风格多样性和控制精度问题，实现了高保真的结构和纹理重建，为艺术字体创作提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大规模预训练视觉编码器主要在2D图像数据上训练，缺乏对真实世界中物体与背景之间3D空间关系的理解，这限制了它们在许多下游应用中的有效性。

Method: 将2D图像中的密集3D空间信息转换为语言表达，通过大型语言模型(LLM)将这种空间知识注入视觉编码器。采用多轮思维链推理过程，逐步整合密集空间知识并建立层次化的空间理解。

Result: 将SpatialBoost应用于DINOv3等最先进的视觉编码器，在需要3D感知和通用视觉能力的广泛基准测试中显示出性能提升。例如，在ADE20K上，SpatialBoost将DINOv3的性能从55.9 mIoU提升到59.7 mIoU，比预训练的DINOv3提高了3.8%。

Conclusion: SpatialBoost通过语言描述注入3D空间知识，有效增强了现有预训练视觉编码器的空间感知能力，在多个视觉任务上实现了显著的性能提升。
...
TL;DR: 提出BayesMM框架，通过多模态贝叶斯分布学习解决3D视觉语言模型在域偏移下的测试时适应问题，实现持续自适应而无需重新训练。


<details>
  <summary>Details</summary>
Motivation: 多模态3D视觉语言模型在域偏移下性能显著下降，现有测试时适应方法存在历史信息有限和预测融合不稳定的问题。

Method: 将文本先验和流式视觉特征建模为高斯分布：文本参数来自语义提示，视觉参数在线更新；通过贝叶斯模型平均融合两种模态，根据后验证据自动调整贡献度。

Result: 在多个点云基准测试中，BayesMM在分布偏移下保持鲁棒性，平均提升超过4%。

Conclusion: BayesMM通过多模态贝叶斯分布学习框架，有效解决了测试时适应中的信息丢失和融合不稳定问题，实现了持续自适应能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型虽然能遵循文本提示，但动态视觉效果（如物体破碎、爆炸等随时间演变的视觉现象）的定制化研究不足。传统方法主要关注主体或相机的低级运动控制，而动态视觉效果涉及更高级的语义，更适合通过文本提示控制。然而，人工编写准确描述这些复杂时间演变效果的提示既困难又耗时。

Method: P-Flow采用无需训练的框架，利用视觉语言模型的语义和时间推理能力，进行测试时提示优化。通过比较参考视频的视觉效果与生成输出的差异，迭代优化提示，使提示逐步演化以更好地在新场景中诱导所需的动态效果。

Result: 实验表明，P-Flow在文本到视频和图像到视频生成任务中，都能实现高保真度和多样化的视觉效果定制，并且优于其他模型。

Conclusion: P-Flow通过测试时提示优化，成功解决了动态视觉效果定制化的挑战，无需修改底层视频生成模型，为复杂时间演变效果的生成提供了有效解决方案。
...
TL;DR: NullSteer：一种零空间投影激活防御框架，通过线性变换在模型激活中构建拒绝方向，在良性子空间保持零扰动，同时在潜在有害方向上动态诱导拒绝，在增强安全性的同时不损害模型通用能力。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在开放世界场景中部署时容易受到视觉越狱攻击诱导生成有害内容，现有激活导向方法虽然能诱导拒绝行为，但可能导致过度拒绝而降低良性输入的性能，且缺乏理论可解释性导致鲁棒性和有效性有限。

Method: 提出NullSteer框架，通过零空间投影的激活防御方法，在模型激活中构建拒绝方向的线性变换，在良性子空间保持零扰动，同时在潜在有害方向上动态诱导拒绝，理论上实现安全性增强而不损害模型通用能力。

Result: 实验表明NullSteer在各种越狱攻击下显著减少有害输出（在MiniGPT-4上平均ASR降低超过15%），同时在通用基准测试上保持与原始模型相当的性能。

Conclusion: NullSteer框架通过零空间投影的激活防御方法，在增强视觉语言模型安全性的同时保持了模型的通用能力，实现了安全性和实用性的更好平衡。
...
TL;DR: FreeArtGS是一种从自由移动的单目RGB-D视频重建关节物体的新方法，通过结合自由移动部件分割、关节估计和端到端优化，无需轴对齐或复杂设置，具有高可扩展性。
...
Conclusion: FreeArtGS提出了一种新的自由移动场景设置，通过简单设置实现高可扩展性的关节物体重建，结合了部件分割、关节估计和端到端优化，为增强现实和机器人应用提供了实用的解决方案。
...
TL;DR: DA-VAE是一种通过扩展预训练VAE的潜在维度来减少token数量的方法，保持原有结构的同时提高压缩率，实现高分辨率图像生成的高效训练和推理。
...
Method: 提出Detail-Aligned VAE，采用显式潜在布局：前C个通道来自预训练VAE的基础分辨率，额外D个通道编码更高分辨率细节。通过细节对齐机制保持扩展潜在空间的结构，采用热启动微调策略。
...
Conclusion: DA-VAE通过轻量级适配预训练扩散骨干，显著提高压缩率，实现高分辨率图像生成的高效训练和推理，为大规模扩散模型的高效部署提供了实用解决方案。
...
TL;DR: OpenEarth-Agent：首个面向开放环境的遥感工具创建智能体框架，通过自适应工作流规划和工具创建，无需预定义工具即可处理多样化遥感数据和任务。


<details>
  <summary>Details</summary>
Motivation: 当前遥感智能体局限于封闭环境，依赖预定义工具，难以适应开放环境中多源数据和异构任务的多样性，限制了其泛化能力。

Method: 提出OpenEarth-Agent框架，采用自适应工作流规划和工具创建机制，结合多阶段工具和跨领域知识库的开放集成，实现整个遥感流程的鲁棒执行。

Result: 在OpenEarth-Bench基准测试中，仅使用6个基础预训练模型的工具创建智能体，性能媲美依赖104个专用工具的工具调用智能体；在完整工具集下表现更优，且创建的工具对数据异常更具鲁棒性。

Conclusion: OpenEarth-Agent成功实现了开放环境下跨多个领域的全流程遥感处理，展示了工具创建智能体在开放环境遥感应用中的优越性和潜力。
...
TL;DR: Bearing-UAV是一种纯视觉驱动的跨视角导航方法，联合预测无人机绝对位置和航向，在GNSS拒止环境中实现准确、轻量且鲁棒的导航。


<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位方法主要关注无人机视图与地图瓦片的匹配，存在精度与存储开销的权衡问题，且忽略了无人机航向在导航中的重要性。此外，跨视角场景中的显著差异和重叠变化考虑不足，限制了其在真实场景中的泛化能力。

Method: 提出Bearing-UAV方法，利用全局和局部结构特征，显式编码相对空间关系，使其对跨视角变化、错位和特征稀疏条件具有鲁棒性。同时构建了Bearing-UAV-90k多城市基准数据集进行评估。

Result: 广泛实验表明，Bearing-UAV在不同地形下比之前的匹配/检索范式具有更低的地理定位误差，显示出令人鼓舞的结果。

Conclusion: Bearing-UAV是一种有效的纯视觉跨视角导航方法，能够实现准确、轻量且鲁棒的无人机导航，特别是在GNSS拒止环境中。代码和数据集将公开提供。
...
TL;DR: ACPO是一种针对多模态大模型对齐的新方法，通过非对称约束偏好优化解决DPO中的似然位移问题，防止视觉锚点崩溃，减少幻觉现象。


<details>
  <summary>Details</summary>
Motivation: 直接偏好优化（DPO）在大视觉语言模型对齐中存在似然位移问题，导致选择和拒绝响应的概率都崩溃。在多模态设置中，这种优化缺陷尤为有害：选择似然的侵蚀（称为视觉锚点崩溃）导致模型放弃视觉证据而依赖语言先验，引发显著幻觉。
...
Conclusion: ACPO通过打破梯度对称性，解决了DPO在多模态任务中的关键缺陷，有效防止视觉证据被语言先验压制，显著减少幻觉现象，提升模型对齐效果。
...
TL;DR: VFLM是一个基于视觉反馈的自改进布局生成框架，通过强化学习和OCR准确度奖励模型实现迭代优化，显著提升布局的可读性和美观性。


<details>
  <summary>Details</summary>
Motivation: 现有基于代码生成的布局方法无法看到渲染后的视觉效果，难以保证可读性和美观性，因此需要引入视觉反馈机制来改进布局生成质量。

Method: 提出视觉反馈布局模型(VFLM)，采用自改进框架，通过强化学习结合基于OCR准确度的视觉奖励模型，实现自适应反思生成和迭代优化。

Result: 在多个基准测试中，VFLM一致优于先进的多模态大语言模型、现有布局模型和纯代码基线，验证了视觉反馈对设计导向MLLM的重要性。

Conclusion: 视觉反馈是布局生成的关键因素，VFLM框架通过迭代反思生成和视觉奖励机制，显著提升了布局生成的质量和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有手-物交互生成研究存在三个分离的领域：仅姿态合成缺乏像素输出；单图像生成缺乏动态；视频生成需要完整姿态序列和真实第一帧。需要统一的框架整合姿态、外观和运动。

Method: 提出PAM（姿态-外观-运动引擎），通过结合深度、分割和关键点等多条件输入，实现可控的手-物交互视频生成。

Result: 在DexYCB上获得FVD 29.13（优于InterDyn的38.83），MPJPE 19.37mm（优于CosHand的30.05mm）；在OAKINK2上FVD从68.76提升到46.31；合成数据增强训练可使仅使用50%真实数据的模型达到100%真实数据的性能。

Conclusion: PAM统一了手-物交互生成的姿态、外观和运动，在多个基准测试中表现出色，并能有效支持下游任务如手部姿态估计的数据增强。
...
TL;DR: MAMMOTH是一个参数高效的多头专家混合模块，旨在通过为每个补丁的表型定制低秩变换来改进任何MIL模型的性能，解决了MIL中从通用特征到任务特定特征转换层的性能瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 在计算病理学中，多实例学习（MIL）是处理千兆像素全切片图像的主要框架。虽然已有大量工作致力于优化补丁特征提取和聚合，但尚未有人关注第二个关键层——将通用特征转换为任务特定特征的线性层。作者假设这一层构成了被忽视的性能瓶颈，通过为每个补丁的表型定制低秩变换可以获得更强的表示能力。

Method: 提出了MAMMOTH（参数高效的多头专家混合模块），该模块设计用于以最小的参数数量改动来改进任何MIL模型的性能。它通过为每个补丁的表型定制低秩变换，实现从通用特征到任务特定特征的有效转换。

Result: 在8种MIL方法和19个不同分类任务上的实验表明，这种任务特定的变换对性能的影响比聚合方法的选择更大。配备MAMMOTH后，即使是简单的最大或平均池化方法也能获得比标准线性层更高的平均性能。在152个测试配置中，MAMMOTH在130个配置中提高了性能，平均性能提升+3.8%。

Conclusion: MAMMOTH通过解决MIL框架中被忽视的特征转换瓶颈，显著提高了计算病理学中全切片图像分类的性能，且具有参数高效的特点，可以与现有的任何MIL方法协同工作。
...
TL;DR: Omni-WorldBench是一个专门评估4D世界模型交互响应能力的基准测试，包含系统化的提示套件和基于智能体的评估框架，揭示了当前世界模型在交互响应方面的关键局限性。


<details>
  <summary>Details</summary>
Motivation: 现有评估基准要么过于关注生成模型的视觉保真度和文本-视频对齐，要么依赖静态3D重建指标而忽略时间动态。世界建模的未来在于4D生成，其核心能力是交互响应——反映交互动作如何驱动时空状态转换，但目前缺乏系统评估这一关键维度的基准。

Method: 提出Omni-WorldBench基准，包含两个关键组件：1) Omni-WorldSuite——涵盖不同交互级别和场景类型的系统化提示套件；2) Omni-Metrics——基于智能体的评估框架，通过测量交互动作对最终结果和中间状态演化轨迹的因果影响来量化世界建模能力。
...
TL;DR: SpatialReward是一个用于评估生成图像中空间布局的可验证奖励模型，通过多阶段流程分析空间关系，结合到RL训练中能提升空间一致性和生成质量。
...
Method: 采用多阶段流程：1) Prompt Decomposer从自由格式提示中提取实体、属性和空间元数据；2) 专家检测器提供对象位置和属性的准确视觉定位；3) 视觉语言模型通过链式推理评估复杂空间关系。
...
TL;DR: 本文对四种先进的深度学习模型在真实航空LiDAR数据上的3D语义分割性能进行了基准测试，发现所有模型总体准确率超过93%，KPConv在平均IoU上表现最佳，Point Transformer V3在车辆类别上表现突出。


<details>
  <summary>Details</summary>
Motivation: 现有3D语义分割模型主要针对室内或地面数据集，在真实航空采集条件下的性能研究不足。虽然少数研究涉及类似场景，但在数据集设计、采集条件和模型选择上存在差异，需要填补这一研究空白。

Method: 在西班牙纳瓦拉地区通过实际飞行条件获取的大规模航空LiDAR数据集上进行实验基准测试，该数据集涵盖城市、乡村和工业等异质景观。比较了四种代表性深度学习模型：KPConv、RandLA-Net、Superpoint Transformer和Point Transformer V3，评估了航空调查中常见的五个语义类别（地面、植被、建筑物、车辆等）。
...
Conclusion: 该研究填补了航空LiDAR数据上3D语义分割模型的性能评估空白，展示了不同模型在真实航空采集条件下的优势和局限性，为实际应用中的模型选择提供了参考依据，并突出了航空数据中类别不平衡和几何变异性的固有挑战。
...
TL;DR: 该研究探索使用Point Transformer v2对多光谱LiDAR点云进行语义分割，实现河流环境土地覆盖分类，在芬兰Oulanka河上获得0.950的mIoU，强度与反射率特征对分类精度至关重要。


<details>
  <summary>Details</summary>
Motivation: 河流环境中的精确土地覆盖制图对于有效的河流管理、生态理解和地貌变化监测至关重要。传统方法可能无法充分利用多光谱LiDAR数据的几何和光谱信息，需要先进的深度学习架构来处理点云数据。

Method: 使用Point Transformer v2（PTv2）架构对多光谱LiDAR点云进行语义分割，利用3通道LiDAR点云的几何和光谱信息。在芬兰北部Oulanka河的点云数据上进行训练和评估，同时研究多数据集训练策略，通过添加稀疏标注的额外河流数据集来提高模型在新河流环境中的泛化能力。

Result: 完整特征配置获得了0.950的平均交并比（mIoU），显著优于仅使用几何特征的基线。消融研究表明强度和反射率特征是准确土地覆盖制图的关键。多数据集训练实验显示出改善的泛化性能，表明即使在高质量标注数据有限的情况下也能开发更鲁棒的模型。

Conclusion: 该研究证明了基于Transformer的架构在多光谱点云河流环境应用中的潜力，为监测沉积物运输和其他河流管理应用提供了新的能力。多数据集训练策略有助于提高模型在不同河流环境中的泛化能力。
...
TL;DR: EgoGroups数据集：首个第一人称视角的社会群体检测数据集，涵盖65个国家不同人群密度和天气条件，用于评估VLM/LLM模型在群体检测任务上的表现


<details>
  <summary>Details</summary>
Motivation: 现有社会群体检测基准存在场景多样性不足、依赖第三人称摄像头（如监控视频）的局限性，无法真实评估不同文化背景和自然环境下群体形成与演化的动态过程

Method: 构建EgoGroups数据集，包含65个国家第一人称视角视频，涵盖低、中、高人群密度场景和四种天气/时间条件，提供密集的人员和社交群体标注以及丰富的地理和场景元数据

Result: 评估发现：1）在零样本设置下，VLM和LLM模型能够超越监督基线模型；2）人群密度和文化区域显著影响模型性能

Conclusion: EgoGroups填补了社会群体检测领域的数据空白，为评估模型在不同文化背景和真实世界环境中的表现提供了重要基准，揭示了VLM/LLM在群体检测任务上的潜力
...
2. 双流蒸馏：联合优化DMD流和RFS-GAN流，后者利用真实和虚假评分模型的判别特征提供互补对抗监督
3. 偏好引导精炼：进一步将学生模型与感知质量偏好对齐

Result: 大量实验表明，DUO-VSR在视觉质量和效率方面优于先前的一步VSR方法，实现了高质量且高效的视频超分辨率。

Conclusion: DUO-VSR通过创新的三阶段双流蒸馏框架，成功解决了扩散模型在视频超分辨率中的采样效率问题，为高质量实时VSR应用提供了可行方案。
...
TL;DR: GLD提出了一种用于新颖视角合成的几何潜在扩散框架，利用几何基础模型的几何一致特征空间作为多视角扩散的潜在空间，相比传统VAE方法在图像质量和3D一致性方面表现更好，训练速度提升4.4倍。


<details>
  <summary>Details</summary>
Motivation: 当前新颖视角合成方法通常在视角无关的VAE潜在空间中操作，缺乏几何一致性。虽然生成潜在空间在单图像生成方面取得进展，但适合NVS的最优潜在空间尚未充分探索，需要跨视角的几何一致生成能力。

Method: 提出几何潜在扩散框架，重新利用几何基础模型的几何一致特征空间作为多视角扩散的潜在空间。这些特征不仅支持高保真RGB重建，还编码了强大的跨视角几何对应关系，为NVS提供了合适的潜在空间。

Result: GLD在2D图像质量和3D一致性指标上优于VAE和RAE方法，训练速度比VAE潜在空间快4.4倍以上。尽管没有使用大规模文本到图像预训练，仅从零开始训练扩散模型，但仍能与最先进方法竞争。

Conclusion: 几何基础模型的几何一致特征空间是新颖视角合成的理想潜在空间，GLD框架展示了其在多视角生成任务中的优越性，为几何一致的跨视角生成提供了有效解决方案。
...
TL;DR: 多模态视觉语言模型通过两种机制处理空间关系：语言模型中间层处理内容无关的空间关系，但主要空间信息来自视觉编码器，其全局表示直接影响模型预测。


<details>
  <summary>Details</summary>
Motivation: 研究视觉语言模型如何计算对象属性与空间关系，澄清模型内部处理空间关联的机制。

Method: 分析VLMs中两种并发机制：语言模型中间层处理视觉token上的空间关系，以及视觉编码器编码对象布局的表示。

Result: 发现主要空间信息来自视觉编码器，其表示全局分布在所有图像token中；增强这些视觉衍生的空间表示能提升自然图像上的空间推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型和视觉语言模型在细粒度视觉编辑中缺乏空间理解和布局一致性，需要更结构化的推理方法

Method: 基于场景图的结构化推理框架，输入场景图和自然语言指令，通过图推理生成满足文本条件且保持空间一致性的更新场景图

Result: 在文本引导布局编辑基准测试中，相比CoT-SFT和GRPO基线，IoU平均提升15%，中心距离误差减少25%；相比零样本LLMs，最佳模型mIoU提升高达20%

Conclusion: 结构化推理框架通过显式引导推理过程，显著改善了空间关系的可解释性和控制能力，在空间布局编辑任务中表现出色
...
TL;DR: DualCoT-VLA提出了一种并行推理机制，通过视觉和语言双链思维解决VLA模型在复杂任务中的规划与空间感知问题，显著提升性能并降低推理延迟。


<details>
  <summary>Details</summary>
Motivation: 标准VLA模型在处理需要逻辑规划的多步骤任务和需要细粒度空间感知的精确操作时存在困难。现有的CoT方法存在两个关键限制：1) 无法同时捕捉低级视觉细节和高级逻辑规划；2) 逐步自回归解码导致高推理延迟和误差累积。

Method: 提出DualCoT-VLA方法，包含两个核心创新：1) 集成视觉CoT用于低级空间理解和语言CoT用于高级任务规划的全面多模态推理；2) 引入并行CoT机制，使用两组可学习查询令牌，将自回归推理转变为单步前向推理。

Result: 在LIBERO和RoboCasa GR1基准测试以及真实世界平台上，DualCoT-VLA实现了最先进的性能表现。

Conclusion: DualCoT-VLA通过并行视觉-语言思维链机制，有效解决了VLA模型在复杂任务中的推理和延迟问题，为机器人视觉语言动作模型提供了更高效、更准确的解决方案。
...
### [189] [ThinkJEPA: Empowering Latent World Models with Large Vision-Language Reasoning Model](https://arxiv.org/abs/2603.22281)
*Haichao Zhang,Yijiang Li,Shwai He,Tushar Nagarajan,Mingfei Chen,Jianglin Lu,Ang Li,Yun Fu*

Main category: cs.CV

TL;DR: 提出VLM引导的JEPA式潜在世界建模框架，结合密集帧动态建模与长时程语义引导，通过双时间路径提升轨迹预测性能


<details>
  <summary>Details</summary>
Motivation: 现有潜在世界模型（如V-JEPA2）受限于短观察窗口，偏向局部低层次外推，难以捕捉长时程语义；而视觉语言模型（VLMs）虽提供强语义基础，但受稀疏采样、语言输出瓶颈和数据不匹配限制，不适合作为独立的密集预测器

Method: 提出双时间路径框架：密集JEPA分支用于细粒度运动和交互线索，均匀采样的VLM思考分支用于知识丰富的语义引导；引入分层金字塔表示提取模块，聚合多层VLM表示到与潜在预测兼容的引导特征

Result: 在手动操作轨迹预测实验中，该方法优于纯VLM基线和JEPA预测器基线，产生更鲁棒的长时程展开行为

Conclusion: VLM引导的JEPA框架成功结合了密集动态建模与长时程语义推理的优势，在轨迹预测任务中展现出优越性能
...
TL;DR: UniMotion是首个统一处理人体运动、自然语言和RGB图像的理解与生成框架，通过连续模态表示和跨模态对齐技术，在单一架构中实现了三模态的任意转换。
...
Method: 1. 将运动作为与RGB平等的连续模态处理；2. 提出跨模态对齐运动VAE（CMA-VAE）和对称双路径嵌入器，在共享LLM骨干中构建平行的连续路径；3. 提出双后验KL对齐（DPA），将视觉融合编码器的丰富后验蒸馏到纯运动编码器中；4. 提出潜在重建对齐（LRA），使用密集运动潜在作为明确条件进行自监督预训练，解决冷启动问题。

Result: UniMotion在涵盖三个模态之间任意转换的七个任务上实现了最先进的性能，特别是在跨模态组合任务上表现出显著优势。

Conclusion: UniMotion通过将运动作为连续模态处理，结合创新的跨模态对齐和自监督预训练策略，成功构建了首个能够统一理解和生成人体运动、自然语言和RGB图像的框架，为多模态人工智能提供了新的解决方案。
...
### [191] [End-to-End Training for Unified Tokenization and Latent Denoising](https://arxiv.org/abs/2603.22283)
*Shivam Duggal,Xingjian Bai,Zongze Wu,Richard Zhang,Eli Shechtman,Antonio Torralba,Phillip Isola,William T. Freeman*

Main category: cs.CV

TL;DR: UNITE提出了一种统一的自动编码器架构，通过权重共享将图像标记化和潜在扩散生成整合到单一模型中，实现了单阶段训练，在ImageNet 256×256上达到接近SOTA的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的潜在扩散模型(LDMs)需要复杂的多阶段训练：先训练标记器，然后在冻结的潜在空间中训练扩散模型。这种分离训练限制了模型性能，作者希望开发一种统一的单阶段训练方法。

Method: 提出UNITE架构，包含一个生成编码器，通过权重共享同时作为图像标记器和潜在生成器。核心洞察是将标记化（从完整图像推断潜在）和生成（从噪声和文本/类别条件推断潜在）视为同一潜在推断问题的不同条件形式。采用单阶段训练程序，通过同一生成编码器的两次前向传播联合优化两个任务。

Result: 在图像和分子模态上，UNITE无需对抗损失或预训练编码器（如DINO）就达到了接近SOTA的性能：在ImageNet 256×256上，Base模型FID为2.12，Large模型FID为1.73。通过表示对齐和压缩分析验证了生成编码器的有效性。

Conclusion: 研究表明，从零开始单阶段联合训练标记化和生成是可行的，共享参数能够共同塑造潜在空间，促进"共同潜在语言"的形成，简化了现有LDMs的复杂训练流程。
...
### [192] [VideoDetective: Clue Hunting via both Extrinsic Query and Intrinsic Relevance for Long Video Understanding](https://arxiv.org/abs/2603.22285)
*Ruoliu Yang,Chu Wu,Caifeng Shan,Ran He,Chaoyou Fu*

Main category: cs.CV

TL;DR: VideoDetective框架通过整合查询-片段相关性和片段间亲和力，解决长视频理解中稀疏查询相关片段定位问题，显著提升MLLMs在长视频问答中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅基于查询定位视频线索，忽略了视频内在结构和不同片段的相关性差异，导致长视频理解受限。

Method: 将视频分割成片段，构建视觉-时间亲和力图，通过假设-验证-精炼循环估计片段相关性并传播到未观察片段，生成全局相关性分布指导关键片段定位。

Result: 在主流MLLMs和代表性基准测试中取得显著提升，VideoMME-long准确率最高提升7.5%。

Conclusion: VideoDetective通过整合查询相关性和片段间亲和力，有效解决了长视频理解中的稀疏观察问题，显著提升了MLLMs的长视频问答能力。
...
### [193] [WorldCache: Content-Aware Caching for Accelerated Video World Models](https://arxiv.org/abs/2603.22286)
*Umair Nawaz,Ahmed Heakl,Ufaq Khan,Abdelrahman Shaker,Salman Khan,Fahad Shahbaz Khan*

Main category: cs.CV

TL;DR: WorldCache提出了一种感知约束的动态缓存框架，通过运动自适应阈值、显著性加权漂移估计、混合与变形优化以及相位感知阈值调度，在保持99.4%基线质量的同时实现2.3倍推理加速。
...
Result: 在Cosmos-Predict2.5-2B模型上评估，在PAI-Bench上实现了2.3倍推理加速，同时保持了99.4%的基线质量，显著优于先前的免训练缓存方法。

Conclusion: WorldCache通过感知约束的动态缓存框架，实现了自适应、运动一致的特征重用，无需重新训练即可显著加速扩散变换器的推理速度，同时保持高质量输出。
