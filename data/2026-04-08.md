...
Motivation: 大型视觉语言模型存在推理成本过高的问题，主要原因是语言解码器需要处理大量视觉token。现有剪枝方法通常导致显著的性能下降，因为不可逆的token移除会导致隐藏状态分布偏移，偏离预训练的全token机制。

Method: 提出Representation Consistency Pruner（RCP）框架，包含两个核心组件：1）跨注意力剪枝器，利用LLM的固有注意力作为基线预测累积掩码，确保跨层的一致性和单调token减少；2）延迟修复适配器（DRA），缓存被剪枝token的精华，并专门对答案生成token应用FiLM调制。使用修复损失来匹配剪枝表示与全token教师模型的一阶和二阶统计量。

Result: 在LVLM基准测试中，RCP能够移除高达88.9%的视觉token，减少高达85.7%的FLOPs，平均准确率仅轻微下降，并且在多个广泛使用的基准测试中优于先前无需微调原始模型的方法。

Conclusion: RCP通过累积视觉token剪枝和延迟修复机制，有效解决了LVLM推理成本过高的问题，在保持性能的同时实现了显著的计算效率提升，为大型视觉语言模型的实际部署提供了可行的解决方案。
...
TL;DR: Video-MME-v2是一个全面的视频理解基准测试，通过三级渐进层次结构和基于组的非线性评估策略，严格评估模型的鲁棒性和可靠性，揭示当前模型与人类专家之间的显著差距。
...
Method: 1. 设计三级渐进层次结构：从多点视觉信息聚合，到时间动态建模，再到复杂多模态推理；2. 提出基于组的非线性评估策略，强调相关查询的一致性和多步推理的连贯性；3. 通过严格的人工标注流程构建数据集，涉及12名标注员和50名独立评审员，投入3300人工小时和最多5轮质量保证。

Result: 实验显示当前最佳模型Gemini-3-Pro与人类专家之间存在显著差距，发现视觉信息聚合和时间建模的错误会传播并限制高级推理能力，同时发现基于思考的推理高度依赖文本线索。

Conclusion: Video-MME-v2通过暴露当前视频理解模型的局限性，为下一代视频多模态大语言模型的发展建立了严格的新测试平台。
...
TL;DR: ID-Sim是一个前馈度量标准，旨在反映人类对身份的选择性敏感度，用于评估身份识别、检索和生成任务
...
Method: 构建ID-Sim度量标准，使用高质量的真实世界图像数据集，并通过生成式合成数据进行增强，提供受控的细粒度身份和上下文变化。创建统一的评估基准来评估与人类标注的一致性。

Result: 提出了ID-Sim度量标准和新的统一评估基准，用于评估身份识别、检索和生成任务中与人类标注的一致性。

Conclusion: ID-Sim旨在促进视觉模型在身份相关任务上的进展，通过提供反映人类选择性敏感度的评估指标来改进身份识别、检索和生成任务。
...
Method: 提出SVAgent框架，包含：1）故事情节智能体基于细化建议智能体分析历史失败后建议的帧逐步构建叙事表示；2）跨模态决策智能体在演化故事情节指导下从视觉和文本模态独立预测答案；3）元智能体评估跨模态预测以对齐预测并增强推理鲁棒性和答案一致性。

Result: 实验结果表明，SVAgent通过模拟人类般的故事情节推理在视频理解中实现了优越的性能和可解释性。

Conclusion: SVAgent框架通过故事情节引导的跨模态多智能体协作，有效提升了视频问答的推理能力和性能，更接近人类理解视频的方式。
...
<details>
  <summary>Details</summary>
Motivation: 许多乳腺X光数据集缺乏完整的配对视图，限制了依赖跨视图一致性的算法开发，需要解决这一数据缺口

Method: 使用三通道DDPM模型，两个通道分别存储CC和MLO视图，第三个通道编码它们的绝对差异以指导模型学习投影间的解剖关系

Result: 差异编码有助于保持跨视图的全局乳腺结构，生成的合成CC-MLO对与真实采集相似，展示了同时双视图合成的可行性

Conclusion: 基于差异引导的DDPM能够同时合成双视图乳腺X光图像，具有数据集增强和未来跨视图感知AI应用的潜力
...
TL;DR: 研究发现当前视频理解基准测试中40-60%的问题仅通过文本线索就能回答，揭示了数据质量问题。提出VidGround方法，仅使用真正需要视觉基础的问题进行后训练，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态建模进展迅速，但视频理解性能仍落后于文本推理。研究发现常用视频理解基准中40-60%的问题仅凭文本线索就能回答，后训练数据集也存在类似问题，这限制了视觉语言模型视频理解能力的提升。

Method: 提出VidGround方法：仅使用真正需要视觉基础的问题进行后训练，排除语言偏见。该方法与基于强化学习的后训练算法结合使用，通过数据筛选提升训练质量。

Result: 使用VidGround方法后，性能相对使用完整数据集提升了6.2个百分点，同时仅使用了原始后训练数据的69.1%。简单的数据筛选方法优于多种复杂的后训练技术，表明数据质量是提升视频理解能力的主要瓶颈。

Conclusion: 数据质量是提升视觉语言模型视频理解能力的关键瓶颈。需要精心筛选真正需要视觉基础的后训练数据和评估基准，才能推动更强大的视觉语言模型发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有脑VAE主要关注单模态数据（如T1加权MRI），忽视了其他模态（如T2加权MRI）的互补诊断价值。需要开发能够处理多模态脑MRI的鲁棒VAE模型。

Method: 1. 使用因子化多轴注意力学习跨模态共享潜在表示；2. 双流3D编码器将模态不变解剖结构与模态依赖外观特征分离；3. 解剖编码通过共享码本离散化，解码时通过FiLM与模态特定外观特征结合；4. 采用2D/3D联合训练策略适应3D MRI数据的切片采集特性。

Result: 在两个多模态脑MRI数据集上的广泛实验表明，NeuroQuant相比现有VAE实现了更优的重建保真度，为下游生成建模和跨模态脑图像分析提供了可扩展的基础。

Conclusion: NeuroQuant是一种模态感知、基于解剖结构的3D VQ-VAE，能够有效重建多模态脑MRI，在保持解剖结构一致性的同时捕捉模态特定特征，为医学图像分析提供了强大的基础模型。
...
TL;DR: MIRAGE框架解决了多实例图像编辑中的过编辑和空间错位问题，通过训练免费的多分支并行去噪策略实现精确的局部化编辑。


<details>
  <summary>Details</summary>
Motivation: 现有指令引导的图像编辑模型（如FLUX.2和Qwen-Image-Edit）在处理包含多个相似实例且每个实例需要单独编辑的复杂场景时，存在严重的过编辑和空间错位问题。
...
Conclusion: MIRAGE是一个训练免费的框架，能够有效解决多实例和多指令场景下的精细一致性编辑问题，并提供了专门的评估基准。
...
TL;DR: LSRM通过扩展transformer上下文窗口（增加20倍物体token和2倍图像token）和稀疏注意力机制，实现了高质量的前馈式3D重建和逆渲染，性能接近密集视图优化方法。


<details>
  <summary>Details</summary>
Motivation: 当前基于物体中心的前馈式3D重建方法虽然稳健且质量较高，但在恢复精细纹理和外观方面仍落后于密集视图优化方法。研究扩展transformer上下文窗口是否能缩小这一差距。

Method: 1) 采用原生稀疏注意力架构设计；2) 高效粗到细管道，通过预测稀疏高分辨率残差聚焦计算在信息丰富区域；3) 3D感知空间路由机制，使用显式几何距离而非标准注意力分数建立准确的2D-3D对应关系；4) 自定义块感知序列并行策略，使用All-gather-KV协议平衡GPU间的动态稀疏工作负载。

Result: LSRM处理比先前SOTA方法多20倍的物体token和2倍以上的图像token。在新视角合成基准测试中，PSNR提高2.5 dB，LPIPS降低40%。在逆渲染任务中，纹理和几何细节持续改进，LPIPS达到或超过SOTA密集视图优化方法。

Conclusion: 扩展transformer上下文窗口能显著提升前馈式3D重建的保真度，使其接近密集视图优化的质量。稀疏注意力架构和创新的计算聚焦策略是实现高效扩展的关键。
...
TL;DR: 提出了一种无需训练的正交适配器合并方法，利用几何性质实现概念和风格特征的融合


<details>
  <summary>Details</summary>
Motivation: 在模型训练快速发展的背景下，参数高效微调技术备受关注，但如何将针对不同任务训练的多个适配器合并成一个能同时处理多个任务的适配器仍然是一个开放问题，特别是生成模型中主题和风格适配器的合并问题尚未解决。

Method: 基于正交微调（OFT）框架，利用结构化正交参数化及其几何特性，推导出无需训练的适配器合并公式。具体包括：1）推导最近提出的Group-and-Shuffle正交矩阵形成的流形结构；2）获得两点间测地线近似的高效公式；3）提出频谱恢复变换以恢复合并适配器的频谱特性，实现更高质量的融合。

Result: 在主题驱动生成任务上的实验表明，该技术能够成功合并两个Group-and-Shuffle正交矩阵，将不同适配器的概念和风格特征融合在一起。这是首个无需训练的多重正交适配器合并方法。

Conclusion: 通过利用正交微调的几何特性，开发了一种无需训练的适配器合并技术，能够有效融合不同适配器的概念和风格特征，为参数高效微调提供了新的解决方案。
...
TL;DR: 该研究提出了一种检测引导的小型视觉语言模型框架，通过结合目标检测和多模态推理，在保持高效率的同时提升建筑工地危险识别的准确性。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然具有强大的上下文推理能力，但计算需求高，难以实现近实时建筑危险检测。小型视觉语言模型虽然效率高，但在分析复杂建筑场景时存在准确性下降和幻觉问题。需要解决这一权衡问题。

Method: 提出检测引导的sVLM框架：首先使用YOLOv11n检测器定位场景中的工人和建筑机械，然后将检测到的实体嵌入结构化提示中，引导sVLMs进行空间基础的危险评估。在零样本设置下评估了6种sVLM模型。

Result: 该方法在所有模型中一致提高了危险检测性能。最佳模型Gemma-3 4B的F1分数达到50.6%（基线为34.5%）。解释质量也显著提升，BERTScore F1从0.61提高到0.82。框架仅增加2.5毫秒的推理开销。
...
TL;DR: 提出用于异质网格分析的层次化Transformer框架，支持任意阶单纯复形的空间自适应树划分，可同时处理体积和表面网格，通过特征投影模块整合顶点级形态测量描述符，在阿尔茨海默病分类和局灶性皮质发育不良检测任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 大规模非结构化体积和表面网格的表示学习面临挑战，现有方法要么忽略临床信息特征，要么仅支持单一网格拓扑，限制了在不同成像流程中的使用。

Method: 基于任意阶单纯复形构建空间自适应树划分的层次化Transformer框架，包含特征投影模块将可变长度的顶点临床描述符映射到空间层次中，通过掩码重建坐标和形态测量通道进行自监督预训练。

Result: 在ADNI数据集上实现阿尔茨海默病分类和淀粉样蛋白负荷预测，在MELD数据集上实现局灶性皮质发育不良检测，在所有基准测试中均达到最先进结果。

Conclusion: 提出的框架能够处理异质网格分析，支持多种网格模态和特征集，为神经影像学中的多尺度表示学习提供了通用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在天文学等应用中，测量特定子集（如星团）的两点相关函数需要大量标注工作，传统方法耗时且效率低下，需要更高效的标注策略。

Method: 采用人机协作框架，利用预训练分类器指导自适应采样，选择信息量最大的点进行人工标注，每次标注后生成跨多个距离区间的无偏配对计数估计。

Result: 相比简单蒙特卡洛方法，该方法显著降低方差，大幅减少标注工作量，实现了可扩展且统计可靠的两点相关测量。

Conclusion: 提出的无偏估计器、采样策略和置信区间构建方法共同实现了天文数据集中两点相关函数的高效、统计可靠的测量。
...
<details>
  <summary>Details</summary>
Motivation: 抗议相关的社交媒体数据对理解集体行动有价值，但存在高风险，包括监控、镇压和个人隐私问题。现有AI系统能识别个体、推断敏感属性、跨平台交叉引用信息，对抗议者和旁观者构成风险。现有抗议分析方法缺乏整合隐私风险评估、下游分析和公平性考虑的整体流程

Method: 提出负责任计算框架，使用条件图像合成技术将敏感的抗议图像替换为带标签的合成再现图像。该方法能在不直接暴露可识别个体的前提下分析集体模式，平衡下游分析效用与隐私风险降低，并评估生成数据的群体公平性

Result: 该方法能生成真实且多样化的合成图像，在保持下游分析效用的同时降低隐私风险。进一步评估了生成数据的群体公平性，检查合成表示是否对特定亚群体产生不成比例的影响

Conclusion: 该方法采用务实的伤害缓解策略，不提供绝对的隐私保证，而是承认残余风险，使社会敏感分析成为可能。框架在分析集体抗议动态的同时，通过合成图像减少对个体隐私的风险
...
TL;DR: 该论文提出了一种名为COVER的主动视角选择方法，通过最小化Fisher信息增益的近似值来选择信息丰富的相机姿态，从而提升3D重建质量。


<details>
  <summary>Details</summary>
Motivation: 用于学习3D重建的数据质量对于实现高效准确的场景建模至关重要。主动视角选择问题需要一种原则性分析方法来获得简单可解释的相机姿态选择标准。

Method: 核心洞见是通过最小化Fisher信息增益的可处理近似值来选择信息丰富的视角，这简化为偏好覆盖过去相机观察不足的几何结构的视角。该方法避免了昂贵的透射率估计，对噪声和训练动态具有鲁棒性。

Result: 在真实数据集上的固定和实体化数据采集场景中，与最先进的主动视角选择方法相比，该方法在多个数据集和辐射场基线上一致提高了重建质量。

Conclusion: 提出的COVER方法提供了一种轻量级的基于覆盖的视角选择指标，能够有效选择信息丰富的相机姿态，显著提升3D场景重建的质量和效率。
...
TL;DR: Region-R1：一种用于多模态检索增强生成的查询端区域裁剪框架，通过动态裁剪判别性区域来减少视觉干扰，提高重排序性能


<details>
  <summary>Details</summary>
Motivation: 标准重排序器通常将完整查询图像作为全局嵌入处理，容易受到视觉干扰物（如背景杂乱）的影响，导致相似性评分偏差。需要一种能够聚焦问题相关区域的方法来提高重排序准确性。

Method: 提出Region-R1框架，将区域选择制定为决策问题，使用区域感知的组相对策略优化（r-GRPO）学习策略，动态裁剪判别性区域，可以选择保留完整图像或仅关注问题相关区域。

Result: 在两个具有挑战性的基准测试E-VQA和InfoSeek上，Region-R1实现了持续的性能提升，通过将条件Recall@1提高高达20%，达到了最先进的性能水平。

Conclusion: 查询端适应是一种简单但有效的方法，可以增强多模态检索增强生成的重排序能力，Region-R1展示了这种方法的重要前景。
...
TL;DR: 论文介绍了UFPR-VeSV数据集，这是一个包含24,945张车辆图像的真实世界监控数据集，具有13种颜色、26个品牌、136个型号和14种类型的细粒度标注，用于车辆分类和车牌识别任务。


<details>
  <summary>Details</summary>
Motivation: 现有车辆信息提取研究通常假设理想条件、探索有限属性，且忽视了细粒度车辆分类（FGVC）与自动车牌识别（ALPR）的集成。需要真实世界条件下的综合数据集来弥补这些不足。

Method: 从巴西巴拉那州军事警察监控系统收集24,945张图像，涵盖16,297辆独特车辆，标注13种颜色、26个品牌、136个型号和14种类型。使用车牌信息验证FGVC标注，并提供文本和角点标注。使用5个深度学习模型进行基准测试，并探索FGVC与ALPR的联合应用。

Result: 数据集包含真实世界挑战条件（部分遮挡、夜间红外成像、不同光照）。基准测试揭示了处理多色车辆、红外图像以及区分共享平台车辆模型的具体挑战。FGVC与ALPR集成显示出实际应用潜力。

Conclusion: UFPR-VeSV数据集填补了真实世界车辆监控数据集的空白，支持细粒度车辆分类和车牌识别研究。集成FGVC和ALPR具有实际应用价值，数据集已公开提供。
...
TL;DR: 论文提出了一种评估视觉嵌入模型中身份信息泄露的基准测试方法，并设计了一种线性投影器ISP来移除身份子空间，在保护隐私的同时保持视觉搜索的实用性。


<details>
  <summary>Details</summary>
Motivation: 当前基于CLIP、DINOv2/v3、SSCD等冻结视觉嵌入的检索和完整性系统在处理人脸数据时存在身份信息泄露风险，但缺乏可量化的评估方法和实用的缓解措施。

Method: 1) 建立视觉嵌入基准测试，包括低误接受率下的开放集验证、基于扩散的模板反转检查、以及使用等面积扰动的人脸上下文归因分析；2) 提出一次性线性投影器ISP，通过估计并移除身份子空间来保护隐私，同时保留实用功能所需的互补空间。

Result: 在CelebA-20和VGGFace2数据集上测试显示：这些编码器在开放集线性探测中表现稳健，CLIP的身份泄露相对高于DINOv2/v3和SSCD；编码器对模板反转具有鲁棒性且上下文占主导；ISP能将身份识别准确率降至接近随机水平，同时保持高非生物特征实用性，且在不同数据集间具有良好的迁移性。

Conclusion: 该研究首次建立了非人脸识别编码器的攻击者校准面部隐私审计框架，证明线性子空间移除方法能够在保持视觉搜索和检索实用性的同时提供强大的隐私保护保证。
...
TL;DR: SmokeGS-R是一个用于烟雾场景3D重建的实用管道，通过解耦几何恢复和外观校正，在NTIRE 2026挑战赛中取得了优于基线的性能。


<details>
  <summary>Details</summary>
Motivation: 真实世界烟雾会衰减场景辐射、增加大气光并破坏多视角外观一致性，这使得鲁棒的3D重建特别困难。需要开发能够处理这些挑战的有效方法。

Method: 采用几何恢复与外观校正解耦策略：1) 使用改进的暗通道先验和引导滤波生成物理引导的伪干净监督；2) 训练锐利的仅干净3D高斯溅射源模型；3) 通过几何平均参考聚合、LAB空间Reinhard传输和轻量高斯平滑，使用捐赠集合协调渲染结果。

Result: 在官方测试排行榜上获得PSNR=15.217和SSIM=0.666。在RealX3D发布后重新评估相同结果，在七个挑战场景上获得PSNR=15.209、SSIM=0.644和LPIPS=0.551，比最强官方基线平均高出+3.68 dB PSNR。

Conclusion: 几何优先的重建策略结合稳定的后渲染外观协调是真实世界多视角烟雾恢复的有效方法。该方法在烟雾场景3D重建中表现出色。
...
TL;DR: 提出了一种在3D高斯泼溅场景中进行目标室内资产对象级检测和分割的方法，通过3D对象码本结合掩码语义和空间信息，实现多视角掩码关联和室内资产检测。


<details>
  <summary>Details</summary>
Motivation: 针对从360°无人机图像重建的3D高斯泼溅场景，需要实现对象级的室内资产检测和分割，但现有方法在多视角掩码关联和3D对象实例化方面存在挑战。

Method: 引入3D对象码本，联合利用掩码语义和对应高斯基元的空间信息来指导多视角掩码关联；集成2D对象检测和分割模型，通过语义和空间约束的合并过程，将多视角掩码聚合成一致的3D对象实例。

Result: 在两个大型室内场景上的实验表明：方法实现了可靠的多视角掩码一致性，F1分数比最先进基线提高了65%；实现了准确的对象级3D室内资产检测，mAP比基线方法提升了11%。

Conclusion: 该方法能够有效解决3D高斯泼溅场景中的对象级检测和分割问题，通过语义和空间约束的码本机制显著提升了多视角掩码关联的准确性和3D对象检测性能。
...
TL;DR: 本文提出VLA-InfoEntropy方法，通过图像熵和注意力熵量化视觉和语义信息，结合时间步信息实现动态注意力转移，减少冗余计算同时保持关键内容，提升VLA模型的推理效率。
...
Result: VLA-InfoEntropy方法在广泛实验中减少了推理参数，加速了推理速度，并在性能上超越了现有方法。

Conclusion: 该方法通过整合空间、语义和时间线索，在减少冗余的同时保留关键内容，有效解决了VLA模型的推理效率问题。
...
TL;DR: 本文提出了一种无监督多智能体与单智能体感知框架（UMS），通过多智能体协作无需人工标注同时解决多智能体和单智能体感知问题，在两个公开数据集上取得了优于现有方法的3D检测性能。


<details>
  <summary>Details</summary>
Motivation: 当前LiDAR感知方法在机器人和自动驾驶车辆的环境理解中表现出色，但缺乏能够同时以无监督方式解决多智能体和单智能体感知的现有方法。通过多智能体间的传感器数据共享，本文发现了两个关键洞察：数据共享后点云密度的提升有利于无监督物体分类，多智能体的协作视角可作为单视角3D物体检测的无监督指导。

Method: 提出UMS无监督感知框架，包含：1）基于学习的提案净化过滤器，用于在多智能体点云密度协作后更好地分类候选提案；2）渐进式提案稳定模块，通过易到难的课程学习生成可靠的伪标签；3）跨视角一致性学习，利用多智能体协作视角指导单智能体视角的检测。

Result: 在V2V4Real和OPV2V两个公开数据集上的实验结果表明，UMS方法在无监督设置下的多智能体和单智能体感知任务中，3D检测性能显著优于现有最先进方法。

Conclusion: UMS框架成功利用多智能体协作无需人工标注，同时解决了多智能体和单智能体感知问题，通过点云密度协作、提案净化和跨视角一致性学习等创新方法，在无监督3D物体检测中取得了突破性进展。
...
Method: 构建联合语义-法线预测头和深度稳定性预测头，提出语义-深度感知关键点机制和统一三线索融合模块，自适应融合多属性特征
...
TL;DR: 该论文提出SPIRE方法，将红外小目标检测重新定义为质心回归任务，使用单点监督和概率响应编码，在保持高分辨率特征的同时降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有基于像素级监督的编码器-解码器分割方法忽略了小目标仅占少数像素且边界模糊的事实，导致难以区分目标区域和背景噪声。作者认为红外小目标检测的首要原则应该是目标定位而非分割所有目标区域。
...
Result: 在SIRST-UAVB和SIRST4等多个红外小目标检测基准测试中，SPIRE实现了具有竞争力的目标级检测性能，保持较低的虚警率(Fa)，同时显著降低了计算成本。

Conclusion: 将红外小目标检测重新定义为质心回归任务，并采用单点监督和概率响应编码的方法是有效的，能够在保持检测性能的同时降低计算复杂度，为红外小目标检测提供了新的思路。
...
TL;DR: 3DTurboQuant是一种无需训练、无需学习码本的3D模型压缩方法，通过随机旋转和预计算量化实现高效压缩，在3DGS和DUSt3R上分别实现3.5倍和7.9倍压缩，质量损失极小。


<details>
  <summary>Details</summary>
Motivation: 现有3D模型压缩方法（如3DGS、NeRF、基于transformer的3D重建器）都需要通过逐场景微调来学习数据相关的码本，这种方法既耗时又复杂。本文旨在证明这种数据依赖的码本学习是不必要的。

Method: 1. 利用随机旋转将参数向量转换为已知Beta分布的坐标，使预计算的Lloyd-Max量化接近最优
2. 开发维度相关准则预测可量化参数及其比特宽度
3. 建立范数分离边界连接量化MSE与渲染PSNR
4. 扩展旋转量化到2维哈希网格特征的入口分组策略
5. 设计可组合的剪枝-量化流水线，具有闭式压缩比

Result: 在NeRF Synthetic数据集上：3DGS压缩3.5倍，PSNR损失仅0.02dB；DUSt3R KV缓存压缩7.9倍，点图保真度39.7dB。压缩仅需数秒，无需训练、码本学习或校准数据。

Conclusion: 3DTurboQuant证明了数据无关的预计算量化方法在3D模型压缩中的有效性，显著优于传统的数据依赖码本学习方法，实现了高效、快速、高质量的压缩。
...
TL;DR: UAVReason：首个面向无人机俯视场景的统一多模态基准，包含27.3万VQA对，涵盖空间推理、时序分析和跨模态生成任务，通过多任务学习建立统一基线模型，显著提升无人机场景性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在无人机高空场景中表现不佳，主要由于俯视视角带来的领域偏移问题，包括微小密集物体、重复纹理和模糊的俯视方向，这些因素严重影响了语义理解和空间推理能力。

Method: 构建UAVReason基准数据集，包含27.3万视觉问答对（23.6万单帧详细描述、68.2万双帧时序序列、188.8万跨模态生成样本），涵盖22种推理类型，并通过多任务学习建立统一基线模型。

Result: 实验验证了统一多任务学习方法在多种指标上的有效性，包括VQA的EM/F1分数、分割的mIoU和生成的CLIP分数，表明该方法显著提升了无人机场景的性能表现。

Conclusion: 通用领域视觉语言模型在无人机场景存在局限性，而统一的多任务学习方法能有效提升无人机场景的多模态理解能力，UAVReason基准将为无人机多模态研究提供重要支持。
...
TL;DR: 提出对象锚定组合图像检索（OACIR）新任务，强调实例级一致性而非语义匹配，构建首个大规模真实图像基准OACIRR，并提出AdaFocal框架通过上下文感知注意力调制器增强实例区域关注。


<details>
  <summary>Details</summary>
Motivation: 传统组合图像检索（CIR）过于关注语义匹配，难以在跨场景中可靠检索用户指定的具体实例。实际应用中，保持具体实例的保真度比宽泛的语义匹配更为重要。

Method: 提出对象锚定组合图像检索（OACIR）任务，要求严格的实例级一致性。构建OACIRR基准数据集（包含16万+四元组和四个挑战性候选库）。提出AdaFocal框架，采用上下文感知注意力调制器自适应增强指定实例区域的注意力，动态平衡锚定实例和组合上下文之间的关注。

Result: AdaFocal在OACIR任务上显著优于现有组合检索模型，特别是在保持实例级保真度方面表现突出，为该挑战性任务建立了强大的基线。
...
TL;DR: LSGS-Loc是一种针对大规模无人机场景的视觉定位方法，通过尺度感知的姿态初始化和拉普拉斯可靠性掩码机制，解决了现有3DGS方法在大规模场景中姿态初始化困难和渲染伪影敏感的问题。


<details>
  <summary>Details</summary>
Motivation: 大规模无人机场景的视觉定位面临几何复杂性和环境变化的挑战。现有基于3D高斯泼溅（3DGS）的视觉定位方法在大规模场景中难以实现鲁棒的姿态初始化，并且对渲染伪影（如模糊和漂浮物）过于敏感。

Method: 提出LSGS-Loc视觉定位流水线：1）尺度感知姿态初始化策略，结合场景无关的相对姿态估计和显式的3DGS尺度约束；2）拉普拉斯可靠性掩码机制，在姿态优化阶段引导光度优化关注高质量区域，减少重建伪影的影响。

Result: 在大规模无人机基准测试上的广泛实验表明，该方法在无序图像查询中实现了最先进的准确性和鲁棒性，显著优于现有的3DGS方法。

Conclusion: LSGS-Loc通过创新的尺度感知初始化和伪影抑制机制，有效解决了大规模3DGS场景中视觉定位的关键挑战，为自主系统在复杂环境中的定位提供了可靠解决方案。
...
TL;DR: 本文提出了一种针对恶劣天气下3D目标检测的自适应多模态感知框架，通过天气条件分支路由动态调整激光雷达和4D雷达的模态偏好，实现鲁棒感知。


<details>
  <summary>Details</summary>
Motivation: 现有激光雷达-4D雷达融合方法主要依赖固定或弱自适应管道，无法根据环境条件变化动态调整模态偏好，导致在恶劣天气下鲁棒性不足。

Method: 将多模态感知重新表述为天气条件分支路由问题，维护三个并行3D特征流：纯激光雷达分支、纯4D雷达分支和条件门控融合分支。通过从视觉和语义提示中提取的条件令牌，轻量级路由器动态预测样本特定权重来软聚合这些表示。引入天气监督学习策略，包含辅助分类和多样性正则化以防止分支崩溃。

Result: 在K-Radar基准测试中实现了最先进的性能，提供了明确且高度可解释的模态偏好洞察，透明揭示了自适应路由如何在不同恶劣天气场景下鲁棒地转移激光雷达和4D雷达的依赖关系。

Conclusion: 提出的天气条件分支路由框架能够动态调整多模态感知中的模态偏好，在恶劣天气下实现鲁棒的3D目标检测，同时提供可解释的模态选择机制。
...
TL;DR: CRISP是一个基于"正区域秩稳定性"原理的无参数、模型无关的医学图像分割框架，通过特征扰动和迭代训练解决分布偏移问题，显著提升多中心、人口统计和模态偏移下的分割鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 医学影像中的分布偏移是医学AI临床转化的主要瓶颈，会导致在未见环境中性能严重下降并加剧健康不平等。现有域适应方法通过模拟偏移或伪监督来穷举预定义可能性，但在开放、不可预测的真实世界中，分布偏移是无限的，这些方法难以应对。

Method: 基于"正区域秩稳定性"原理（正体素预测概率的相对秩在分布偏移下保持稳定），提出CRISP框架：1）通过潜在特征扰动模拟分布偏移下的模型行为；2）识别保持高概率的区域（注定阳性）和保持低概率的区域（可安全分类为阴性）；3）构建高精度（HP）和高召回（HR）先验，在扰动下递归细化；4）设计迭代训练框架，使HP和HR逐步"挤压"到最终分割。

Result: 在多中心心脏MRI和CT肺血管分割上的广泛评估显示，CRISP具有卓越的鲁棒性，显著优于最先进方法：在多中心、人口统计和模态偏移下，HD95分别降低了0.14像素（7.0%改进）、1.90像素（13.1%改进）和8.39像素（38.9%改进）。

Conclusion: CRISP是第一个基于秩而非概率进行分割的框架，无需目标域信息，通过利用分布偏移下正区域秩稳定性的原理，有效解决了医学影像中的分布偏移问题，为临床转化提供了更鲁棒的解决方案。
...
TL;DR: SGPer是一个语义-几何先验协同框架，通过结合DINOv2的语义感知和SAM的几何定位能力，在有限数据下实现小麦病害的精确分割。


<details>
  <summary>Details</summary>
Motivation: 小麦病害分割面临生长阶段间显著的外观变化挑战，收集代表性训练数据集既费力又不切实际，需要解决有限数据下的分割问题。

Method: 提出SGPer框架，利用DINOv2提供类别感知语义先验，转换为空间提示指导SAM进行精确边界定位。设计病害敏感适配器，将DINOv2特征转化为密集的类别特定点提示，并通过交叉参考SAM的掩码置信度和DINOv2的语义一致性动态过滤提示。

Result: 在多个小麦病害和器官分割基准测试中，特别是在数据受限场景下，SGPer始终达到最先进的性能表现。

Conclusion: SGPer通过语义和几何先验的协同作用，实现了对时间外观变化严格不变性的精确稳健分割，为有限数据下的农业病害分割提供了有效解决方案。
...
TL;DR: VideoStir是一个结构化、意图感知的长视频检索增强生成框架，通过时空图结构和多跳检索解决现有方法忽略视频时空结构和依赖显式语义匹配的问题。
...
Method: VideoStir首先将视频在片段级别组织为时空图，然后执行多跳检索以聚合跨距离但上下文相关的事件证据。此外，引入基于MLLM的意图相关性评分器，根据帧与查询推理意图的对齐程度进行检索。为此还构建了IR-600K大规模数据集来学习帧-查询意图对齐。

Result: 实验表明，VideoStir在不依赖辅助信息的情况下与最先进的基线方法具有竞争力，突显了将长视频RAG从扁平化语义匹配转向结构化、意图感知推理的前景。

Conclusion: VideoStir通过结构化时空图和多跳检索，结合意图感知的检索机制，成功解决了长视频检索增强生成中的关键限制，为长视频理解提供了新的有效框架。
...
TL;DR: 提出跨阶段注意力传播（CSAP）解码器框架，通过在最深层特征计算注意力并传播到浅层，避免冗余计算，显著降低计算成本的同时保持多尺度上下文推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有轻量级语义分割方法中，多尺度解码器在各特征尺度独立计算注意力，由于跨尺度注意力分布高度相关，引入了大量冗余计算。
...
Conclusion: CSAP框架在保持多尺度上下文推理能力的同时，显著降低了解码器的计算成本，实现了计算效率与分割性能的良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 传统少样本语义分割方法依赖大量的情景训练来学习可迁移表示，计算成本高且对分布变化敏感。本文旨在探索现代视觉基础模型（特别是SAM3）作为训练免费解决方案的潜力。

Method: 利用SAM3的可提示概念分割能力，采用简单的空间拼接策略将支持图像和查询图像放置在共享画布上，让完全冻结的SAM3无需任何微调或架构修改即可执行分割。

Result: 在PASCAL-5^i和COCO-20^i数据集上的实验表明，这种最小化设计已经达到了最先进的性能，超越了许多精心设计的工程方法。同时发现负提示在少样本设置中可能适得其反。

Conclusion: 强大的跨图像推理能力可以通过简单的空间公式实现，同时揭示了当前基础模型在处理冲突提示信号方面的局限性。该方法为少样本分割提供了一种高效、无需训练的新范式。
...
Method: 1) 将输入转换到规范正交空间缓解透视畸变；2) HUG-MVD模块联合建模个体和组上下文生成完整多视角法线和图像；3) HUG-GR模块利用物理交互先验优化几何；4) 多视角图像融合生成高保真纹理。

Result: 大量实验表明HUG3D显著优于单人体和现有多人体方法，从单张图像生成物理合理、高保真的交互人体3D重建。

Conclusion: HUG3D通过组级上下文建模和物理交互先验，成功解决了多人场景3D重建中的重叠、遮挡和交互失真问题，实现了高质量的多人交互场景重建。
...
<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶模型将所有智能体同等对待，难以从复杂背景中解耦真实碰撞威胁。自动驾驶的核心不在于感知与规划的简单集成，而在于统一表示空间中的动态多智能体博弈。

Method: 提出风险优先博弈规划概念，开发GameAD框架，包含：风险感知拓扑锚定、战略负载适配器、极小极大风险感知稀疏注意力、风险一致均衡稳定化等技术，实现风险优先交互的博弈论决策。同时提出规划风险暴露度量来量化长期轨迹的累积风险强度。

Result: 在nuScenes和Bench2Drive数据集上的大量实验表明，该方法显著优于现有最先进方法，特别是在轨迹安全性方面表现突出。

Conclusion: GameAD通过将自动驾驶建模为风险感知博弈问题，实现了更安全的决策规划，为端到端自动驾驶提供了新的研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大型视觉语言模型通过海量互联网数据训练取得了显著能力，但行为模态数据（如眼动）仍然稀缺、标注成本高且涉及隐私问题。模拟生成合成数据为解决这一不对称性提供了有前景的替代方案。

Method: 提出一个合成眼动视频生成管道：从参考视频中提取真实人类虹膜轨迹，通过无头浏览器自动化在3D眼动模拟器中重放这些轨迹，生成自动标注的合成数据。

Result: 发布了final_dataset_v1数据集：包含144个会话（72个阅读、72个对话），总计12小时25fps的合成眼动视频。评估显示生成的轨迹保留了源数据的时序动态特性（所有指标的KS D < 0.14），但3D模拟器在阅读尺度运动上表现出有界敏感性。

Conclusion: 该基础设施支持下游行为分类器开发，为行为建模与视觉语言系统的交叉领域提供了数据集和评估工具，同时发现模拟器设计需要考虑头部运动的耦合效应。
...
TL;DR: 提出一种结合视觉语言模型引导的流匹配分割和随机矩阵理论检测的犬类气胸自动诊断新范式，通过公开数据集解决数据稀缺问题，实现高精度可解释诊断。


<details>
  <summary>Details</summary>
Motivation: 犬类气胸自动诊断面临数据稀缺和模型可信度挑战，需要开发既准确又可解释的诊断系统。

Method: 1. 引入公开像素级标注数据集；2. 提出信号定位与频谱检测协同范式：使用VLM引导迭代流匹配进行高精度分割，然后应用随机矩阵理论分析分割特征，将健康组织建模为可预测随机噪声，通过检测统计显著异常特征值识别病理信号。

Result: 流匹配提供的高保真定位能净化信号，最大化RMT检测器灵敏度，生成分割与第一性原理统计分析协同产生高精度可解释诊断系统。

Conclusion: 该研究提出了一种结合生成式分割和统计分析的创新诊断范式，解决了犬类气胸诊断的数据稀缺和可信度问题，提供了公开数据集和开源代码促进相关研究。
...
<details>
  <summary>Details</summary>
Motivation: 探地雷达地下缺陷检测面临"微弱信号"挑战：低信噪比的微弱衍射双曲线、高波场相似性和几何退化。现有轻量级检测器优先考虑效率而非灵敏度，无法保留低频结构或解耦异质杂波。

Method: 提出WSA-Net框架，集成四种机制：1) 使用部分卷积进行信号保留；2) 通过异质分组注意力进行杂波抑制；3) 几何重建以锐化双曲线弧；4) 上下文锚定以解决语义模糊性。超越简单的参数减少，专注于物理特征重建。

Result: 在RTST数据集上，WSA-Net达到0.6958 mAP@0.5和164 FPS，仅需2.412M参数。证明信号中心感知在轻量级架构中能有效减少基础设施检测中的漏检。

Conclusion: WSA-Net通过物理特征重建增强微弱信号，在轻量级架构中实现了效率与灵敏度的平衡，显著提高了探地雷达地下缺陷检测的准确性。
...
TL;DR: ContrAR：首个针对AR环境中虚拟内容矛盾攻击的基准测试，包含312个真实AR视频，评估了11个视觉语言模型在检测对抗性虚拟内容操纵方面的鲁棒性。
...
Method: 系统化建模矛盾虚拟内容攻击，创建ContrAR基准测试，包含312个经过10名人类参与者验证的真实世界AR视频，并对11个商业和开源视觉语言模型进行基准测试。

Result: 实验结果显示，当前视觉语言模型对矛盾虚拟内容表现出合理的理解能力，但在检测和推理AR环境中的对抗性内容操纵方面仍有改进空间。同时，在检测准确性和延迟之间的平衡仍然具有挑战性。

Conclusion: ContrAR基准测试为评估视觉语言模型在AR环境中的鲁棒性提供了重要工具，揭示了当前模型在应对虚拟内容矛盾攻击方面的局限性，为未来研究指明了改进方向。
...
TL;DR: GCNV-Net：一种新颖的3D医学图像分割框架，通过三向动态非空体素变换器、几何交叉注意力模块和非空体素化技术，在保持高精度的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有3D医学图像分割方法难以同时实现高精度和计算效率，特别是在处理多样解剖结构和成像模态时存在局限性。需要一种既能准确分割又能高效计算的方法来满足临床诊断和治疗规划的需求。

Method: 提出GCNV-Net框架，包含三个核心组件：1）三向动态非空体素变换器（3DNVT），沿横断面、矢状面和冠状面三个正交解剖平面动态分区相关体素；2）几何交叉注意力模块（GCA），在多尺度特征融合中显式融入几何位置信息；3）非空体素化，仅处理信息区域，大幅减少冗余计算。

Result: 在BraTS2021、ACDC、MSD Prostate、MSD Pancreas和AMOS2022等多个基准测试中均达到最先进性能：Dice提升0.65%、IoU提升0.63%、NSD提升1%、HD95相对提升14.5%。相比传统体素化，FLOPs减少56.13%，推理延迟降低68.49%。
...
TL;DR: CR-Diff通过剪枝扩散模型参数来提升跨分辨率视觉一致性，改善在训练未见分辨率上的图像生成质量


<details>
  <summary>Details</summary>
Motivation: 现有UNet扩散模型通常在固定分辨率训练，当生成图像的分辨率超出训练范围时，质量会显著下降。这源于分辨率依赖的参数行为，即在默认分辨率下表现良好的权重在其他空间尺度下可能产生负面影响，削弱语义对齐并导致结构不稳定。

Method: CR-Diff采用两阶段方法：首先进行块级剪枝，选择性消除对跨分辨率生成有害的权重；然后进行剪枝输出放大，进一步纯化剪枝后的预测结果。该方法还支持基于提示的特定细化，可按需增强生成质量。

Result: 大量实验表明，CR-Diff能够提升各种扩散模型主干在不同未见分辨率上的感知保真度和语义连贯性，同时基本保持模型在默认分辨率上的性能。该方法具有广泛的适用性。

Conclusion: 通过分析分辨率依赖的参数行为并针对性剪枝，CR-Diff有效解决了扩散模型在跨分辨率生成时的质量退化问题，为提升扩散模型的跨分辨率一致性提供了新思路。
...
Method: 提出STSF-Net框架：1) 联合建模模态特定特征以捕获真实语义变化信号；2) 嵌入时空共同特征以抑制成像机制差异引起的伪变化；3) 引入基于预训练基础模型语义先验的自适应特征融合策略；4) 创建Delta-SN6数据集作为首个公开的多类别多模态变化检测基准。

Result: 在Delta-SN6、BRIGHT和Wuhan-Het数据集上，STSF-Net的mIoU分别比现有最佳方法提高了3.21%、1.08%和1.32%。

Conclusion: STSF-Net通过有效建模模态特定特征和时空共同特征，结合语义引导的自适应融合策略，显著提升了多模态变化检测的性能，为解决光学和SAR图像变化检测中的挑战提供了有效方案。
...
TL;DR: 提出ProMMA框架解决多模态情感分析中的缺失模态问题，通过缺失模态评估器、模态不变提示解缠、动态提示加权和多级提示动态连接等模块，在多个基准测试中达到SOTA性能。
...
Method: 提出基于提示的缺失模态适应框架(ProMMA)，包含四个核心模块：1) 缺失模态评估器：使用预训练模型和伪标签动态评估缺失模态重要性，避免低质量数据填补；2) 模态不变提示解缠模块：将共享提示分解为模态特定私有提示，捕捉内在局部相关性；3) 动态提示加权模块：基于互信息计算权重，自适应抑制缺失模态干扰；4) 多级提示动态连接模块：通过残差连接整合共享提示与自注意力输出，增强全局一致性。

Result: 在CMU MOSI、CMU MOSEI和CH-SIMS三个公开基准测试上进行广泛实验，证明该框架在多种缺失模态设置下实现了最先进的性能和稳定的结果。

Conclusion: ProMMA框架有效解决了多模态情感分析中的缺失模态问题，通过系统性的模块设计实现了对缺失模态的鲁棒适应，在多个数据集上展现了优越性能。
...
TL;DR: SpecMamba是一个参数高效、频率感知的元学习框架，用于少样本高光谱目标检测，通过解耦稳定语义表示和敏捷光谱适应，在多个数据集上优于现有方法。
...
Method: 提出SpecMamba框架，包含三个核心组件：1）离散余弦变换Mamba适配器（DCTMA），将光谱特征投影到频域并利用Mamba的线性复杂度状态空间递归；2）先验引导三编码器（PGTE），利用实验室光谱先验指导可学习适配器优化；3）自监督伪标签映射（SSPLM）策略，用于测试时适应的不确定性感知采样和双路径一致性约束。

Result: 在多个公开数据集上的广泛实验表明，SpecMamba在检测精度和跨域泛化能力方面持续优于最先进的方法。

Conclusion: SpecMamba通过参数高效的频域适应方法，有效解决了少样本高光谱目标检测中的过拟合和泛化问题，为高光谱分析提供了新的解决方案。
...
TL;DR: PTA提出"净化-对齐"框架，通过元学习权重机制净化知识源，再用扩散知识蒸馏对齐模态，增强单模态编码器的跨模态知识，提升缺失模态场景下的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 多模态人类感知面临缺失模态的关键挑战，主要障碍是异构数据间的表示差距和低质量模态的污染效应。这些障碍存在因果关联，污染引入的噪声从根本上阻碍了表示差异的减少。

Method: PTA采用"净化-对齐"框架：1) 使用元学习驱动的权重机制动态降低噪声、低贡献模态的影响来净化知识源；2) 引入基于扩散的知识蒸馏范式，用净化后的共识构建信息丰富的干净教师模型，细化每个学生模态的特征。

Result: 在MM-Fi和XRF55数据集上，在显著表示差距和污染效应条件下，PTA实现了最先进的性能，显著提高了单模态模型在多种缺失模态场景下的鲁棒性。

Conclusion: PTA通过"净化-对齐"策略解决了表示差距和污染效应之间的因果依赖关系，创建了具有跨模态知识的强大单模态编码器，为多模态人类感知中的缺失模态问题提供了有效解决方案。
...
TL;DR: BPC-Net：一种用于无标注皮肤病变分割的边界概率校准框架，通过高斯概率平滑解决边界概率置信度不足问题，在无标注条件下达到接近有监督方法的性能。


<details>
  <summary>Details</summary>
Motivation: 无标注皮肤病变分割面临三个耦合挑战：噪声伪标签监督、有限目标域数据下的不稳定迁移、以及边界概率置信度不足。现有方法主要关注伪标签去噪，而边界概率压缩对最终掩码质量的影响（直接影响轮廓完整性）未得到充分关注，无法通过全局阈值调整充分纠正。
...
Method: ID-Selection采用重要性评估与多样性感知迭代选择相结合的策略：首先为每个token分配重要性分数，然后逐个选择高分token，同时逐步抑制相似token的分数，从而在统一选择过程中保留信息性token并减少冗余。

Result: 在5个LVLM主干和16个主要基准测试上的实验表明，ID-Selection在性能和效率方面始终表现优异，特别是在极端剪枝比例下。例如，在LLaVA-1.5-7B上，ID-Selection剪除了97.2%的视觉token（仅保留16个），推理FLOPs减少超过97%，同时保持91.8%的原始性能，且无需额外训练。
...
TL;DR: 本文通过系统实证研究解决了风格迁移数据增强中的三个关键设计矛盾，提出了StyleMixDG方法，在GTAV到真实世界数据集基准上实现了稳定的性能提升。


<details>
  <summary>Details</summary>
Motivation: 计算机视觉深度学习模型在真实世界部署时泛化能力差，特别是使用合成数据训练时存在Sim2Real差距。尽管风格迁移作为领域泛化的数据增强策略越来越受欢迎，但文献中存在三个关键设计矛盾：风格池的多样性、纹理复杂性的作用以及风格源的选择。

Method: 进行了系统的实证研究，隔离并评估了每个因素对场景理解的影响。基于研究发现，提出了StyleMixDG（领域泛化的风格混合），这是一种轻量级、模型无关的数据增强方法，无需架构修改或额外损失函数。

Result: 研究发现：(1) 扩展风格池比重复使用少数风格进行增强带来更大收益；(2) 当风格池足够大时，纹理复杂性没有显著影响；(3) 多样化的艺术风格优于领域对齐的替代方案。在GTAV → {BDD100k, Cityscapes, Mapillary Vistas}基准测试中，StyleMixDG相比强基线实现了稳定的改进。

Conclusion: 通过系统实证研究解决了风格迁移增强中的设计矛盾，提出的经验性设计原则能够转化为实际性能提升。StyleMixDG作为轻量级、模型无关的增强方法，在领域泛化任务中表现出色，代码将在GitHub上发布。
...
TL;DR: 提出STGR框架，通过语义-拓扑图推理解决医学图像分割中临床指令语义模糊和解剖结构重叠问题，结合LLaMA-3-V和MedSAM模型，使用SAFT策略实现高效微调，在肺筛查任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态和基础模型在处理临床报告语义模糊和解剖结构重叠时表现不佳，且在有限医学数据上微调大型架构容易导致严重过拟合，需要更有效的语言引导医学图像分割方法。

Method: 提出STGR框架：1) TVID模块从文本提取精确诊断指导；2) 将掩码选择建模为动态图推理问题，候选病变作为节点，边捕获空间和语义亲和性；3) SAFT策略仅更新不到1%参数，确保部署可行性。

Result: 在LIDC-IDRI和LNDb数据集上5折交叉验证达到SOTA：LIDC-IDRI上DSC为81.5%，优于LISA等LLM工具5%以上；SAFT策略提供出色交叉折稳定性（DSC方差仅0.6%）。

Conclusion: STGR框架通过语义-拓扑图推理有效解决临床指令模糊和解剖重叠问题，SAFT策略确保高效微调和部署可行性，为鲁棒、上下文感知的临床部署铺平道路。
...
TL;DR: FunRec是一种从第一人称RGB-D交互视频直接重建室内场景功能化3D数字孪生的方法，能够自动发现可动部件、估计运动学参数并生成仿真兼容的网格。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖受控设置、多状态捕捉或CAD先验，无法直接从真实世界的人类交互序列中恢复可交互的3D场景。

Method: 从第一人称RGB-D交互视频直接操作，自动发现可动部件，估计运动学参数，跟踪3D运动，在规范空间中重建静态和动态几何，生成仿真兼容的网格。

Result: 在新真实和模拟基准测试中大幅超越先前工作：部件分割mIoU提升高达+50，关节和姿态误差降低5-10倍，重建精度显著提高。

Conclusion: FunRec能够从真实世界交互视频重建功能化3D数字孪生，支持URDF/USD导出用于仿真、手引导的可用性映射和机器人-场景交互应用。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型的发展，图像描述从简短句子演变为包含数百词的全面叙述，这使得精确检测和定位幻觉变得极其困难。现有基准测试缺乏评估这种能力的细粒度和领域多样性。

Method: 构建DetailVerifyBench基准测试，包含1000张高质量图像，涵盖5个不同领域，提供平均超过200词的长描述，并进行密集的标记级多类型幻觉标注。

Result: DetailVerifyBench成为迄今为止长图像描述领域中用于精确幻觉定位最具挑战性的基准测试，提供了细粒度的评估能力。

Conclusion: DetailVerifyBench填补了现有基准测试在细粒度和领域多样性方面的空白，为评估多模态大语言模型在长图像描述中精确检测和定位幻觉的能力提供了重要工具。
...
TL;DR: LLaRS是首个统一的多模态多任务遥感低层视觉基础模型，通过最优传输对齐异构波段，采用混合专家架构处理空间、光谱和全局特征，在百万级多任务数据集上训练，在11个恢复和增强任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 遥感图像面临云层、雾霾、噪声、分辨率限制和传感器异质性等多种退化问题。现有方法通常为每种退化类型训练单独模型，缺乏统一的处理框架。需要开发一个能够处理多模态、多任务的统一基础模型。

Method: 1. 使用Sinkhorn-Knopp最优传输将异构波段对齐到语义匹配的槽中；2. 通过三个互补的混合专家层路由特征：卷积专家处理空间模式、通道混合专家保持光谱保真度、带低秩适配器的注意力专家处理全局上下文；3. 通过步级动态权重调整稳定联合训练；4. 构建LLaRS1M百万级多任务数据集，包含11个恢复和增强任务。

Result: 实验表明LLaRS在11个恢复和增强任务上持续优于7个竞争模型。参数高效微调实验展示了在未见数据上的强大迁移能力和适应效率。

Conclusion: LLaRS是首个统一的多模态多任务遥感低层视觉基础模型，通过创新的对齐机制和混合专家架构，实现了对多种遥感图像退化问题的统一处理，具有优异的性能和迁移能力。
...
TL;DR: 该研究探索使用单目广播视频进行足球运动员疲劳监测，通过先进的比赛状态重建方法提取球员轨迹，并提出新的运动学处理算法来获取速度和加速度估计，构建加速度-速度曲线作为疲劳指标。


<details>
  <summary>Details</summary>
Motivation: 当前足球疲劳监测主要依赖主观报告、实验室生物标志物或侵入式传感器（如心率监测器、GPS），缺乏基于低成本视频的客观分析方法。本研究旨在探索单目广播视频是否能够提供足够质量的时空信号来支持疲劳分析。

Method: 基于先进的比赛状态重建方法提取球员在球场坐标中的轨迹，提出新颖的运动学处理算法从重建轨迹中获取时间一致的速度和加速度估计，构建加速度-速度曲线并分析其作为疲劳相关性能指标的行为。

Result: 在SoccerNet-GSR基准测试上评估完整流程，考虑30秒片段和完整的45分钟半场。结果表明单目比赛状态重建能够恢复与加速度-速度曲线分析兼容的运动学模式，同时揭示了对广播视频固有的轨迹噪声、校准误差和时间不连续性的敏感性。

Conclusion: 单目广播视频可作为低成本疲劳分析的基础，同时为未来研究划定了方法学挑战，包括处理轨迹噪声、校准误差和时间不连续性等问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于神经表示的4D重建方法在上下文推理方面存在局限，特别是在处理复杂语义（如交互、时序动作、空间关系）时，难以将噪声、视角依赖的预测转化为全局一致的4D解释

Method: 基于4D高斯泼溅进行高保真动态重建，引入多视角语义共识机制，通过聚合多视角和时序的2D语义预测来锚定自然语言查询，过滤不一致输出，强制几何一致性，并通过神经场优化将2D语义提升为结构化4D锚定

Result: 在复杂语言查询上达到新的最先进水平，有效处理属性、动作、空间关系和多对象交互，并提出了新的基准Panoptic-L4D用于动态场景语言查询评估

Conclusion: PanopticQuery框架通过统一查询时推理，成功解决了动态4D场景中语言查询的语义理解挑战，为4D场景理解提供了新的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 胃肠道疾病全球负担日益加重，内窥镜检查是早期诊断的主要工具，但常规内窥镜图像解读存在漏诊和效率限制。现有AI辅助诊断模型因医疗数据有限、领域偏移和异构标注等问题，缺乏泛化性、适应性、鲁棒性和可扩展性。

Method: 开发RATNet基础模型，基于类比推理机制。通过循环预训练策略从五个胃肠道内窥镜数据集的异构专家标注中获取和转移知识。架构包括编码器、相关性知识获取与转移(RAT)模块、投影器和多任务头，支持微调、线性探测和零样本迁移。

Result: RATNet在六个场景中优于现有基础模型(GastroNet和GastroVision)：常见胃肠道疾病诊断、罕见疾病少样本学习、新医疗机构的零样本迁移、长尾疾病分布的鲁棒性、新疾病的适应性以及通过联邦学习的隐私保护部署。

Conclusion: RATNet通过类比推理机制将图像衍生的后验知识与学习到的先验知识库匹配，并转移相对知识来指导诊断，提高了泛化能力和抗偏置能力。该模型开放且成本效益高，支持自动整合异构标注而无需手动标签统一，降低了数据获取成本，是智能胃肠道诊断的实用基础，特别适用于资源有限的环境。
...
TL;DR: 该研究探索医学视觉任务之间的内在关系，通过任务对比学习框架将30个不同任务嵌入共享表示空间，分析任务间的相似性和差异。


<details>
  <summary>Details</summary>
Motivation: 医学计算机视觉领域大多关注特定任务的性能提升，但任务之间的内在关系（如它们如何关联、重叠或差异）尚未得到充分探索。本研究旨在探究医学视觉任务之间的内在关系，揭示任务间的根本属性和相互联系。

Method: 提出任务对比学习（TaCo）框架，将30个医学视觉任务（包括语义任务、图像生成任务和图像变换任务）从39个不同医学成像模态的数据集中嵌入到共享表示空间，分析任务在嵌入空间中的关系。

Result: 通过TaCo框架成功将异构任务映射到联合空间，能够识别哪些任务有独特表示、哪些任务混合在一起，以及任务迭代变化如何在嵌入空间中反映，揭示了医学视觉任务的内在结构。

Conclusion: 该研究为理解医学视觉任务的内在结构奠定了基础，提供了对任务相似性和嵌入空间中相互连接属性的深入理解，有助于推动医学计算机视觉领域的任务关系研究。
...
<details>
  <summary>Details</summary>
Motivation: 烟雾环境下的多视角图像重建面临三大挑战：强散射效应、视角依赖的外观变化、跨视角一致性严重退化，现有方法难以有效处理这些烟雾引起的退化问题。
...
Conclusion: 提出的框架通过整合视觉先验与高效3D场景建模，有效解决了烟雾退化多视角图像的3D重建问题，实现了烟雾场景的高质量重建和恢复导向的新视角合成。
...
<details>
  <summary>Details</summary>
Motivation: 解决跨模态图像配准中的挑战，包括模态差异、大尺度变化和几何变形，需要一种能够学习模态无关特征表示并保持结构一致性的鲁棒方法。

Method: 提出一致性循环特征流Transformer（CRFT），包含粗阶段（通过多尺度特征相关性建立全局对应）和细阶段（通过分层特征融合和自适应空间推理细化局部细节），采用迭代差异引导注意力机制和空间几何变换循环优化流场。

Result: 在多个跨模态数据集上的广泛实验表明，CRFT在准确性和鲁棒性方面一致优于最先进的配准方法，能够处理大仿射和尺度变化，同时保持跨模态的结构一致性。
...
TL;DR: GUIDE是一个渐进式几何先验注入框架，通过多级采样和对齐融合几何信息到MLLM早期层，提升模型的空间感知能力
...
Method: 提出GUIDE框架：1）在几何编码器中进行多级采样，从局部边缘到全局拓扑全面捕获多粒度特征；2）将这些多级几何先验逐步对齐融合到MLLM的早期层；3）引入上下文感知门控机制，使模型能基于当前语义获取所需空间线索，最大化空间先验利用率并抑制冗余几何噪声

Result: 在多个复杂空间推理和感知任务上，GUIDE显著优于现有基线方法

Conclusion: GUIDE为将3D几何先验集成到大模型中建立了一个新颖的范式，通过渐进式几何信息注入指导模型学习2D到3D的过渡过程
...
TL;DR: GaussianGrow提出了一种从3D点云生长3D高斯的新方法，通过文本引导的多视角扩散模型合成外观监督，解决了现有方法因几何估计不可靠导致的生成质量问题。


<details>
  <summary>Details</summary>
Motivation: 当前3D高斯泼溅在渲染效率和质量上表现出色，但缺乏适当几何先验时生成3D高斯仍具挑战。现有方法通过预测点云作为几何参考来推断高斯基元，但不可靠的几何估计可能导致生成质量差。

Method: 提出GaussianGrow方法：1）设计文本引导的高斯生长方案，利用多视角扩散模型从输入点云合成一致外观进行监督；2）通过约束在重叠区域非预设相机姿态生成的新视角来减轻融合伪影；3）迭代检测相机姿态观察最大未生长区域，用预训练2D扩散模型修复渲染视图，直到生成完整高斯。
...
TL;DR: FoleyDesigner是一个基于多智能体架构的拟音设计框架，通过视频分析、时空可控的拟音生成和专业混音功能，实现了电影拟音制作的自动化，解决了传统手动制作耗时耗力的问题。


<details>
  <summary>Details</summary>
Motivation: 电影拟音制作对于增强沉浸式听觉体验至关重要，但传统的手动创建时空对齐音频非常耗时耗力。现有方法缺乏高质量的空间音频数据集和专业工作流程集成。

Method: 采用多智能体架构进行精确时空分析，利用从视频帧提取时空线索训练的潜在扩散模型实现时空对齐，结合LLM驱动的混合机制模拟电影后期制作实践。引入了首个专业立体声音频数据集FilmStereo，包含空间元数据、精确时间戳和语义标注。

Result: 实验表明该方法在时空对齐方面优于现有基线，能够无缝兼容专业电影制作标准，包括符合ITU-R BS.775标准的5.1声道杜比全景声系统。

Conclusion: FoleyDesigner提供了一个创新的拟音设计框架，通过自动化专业工作流程，显著提高了拟音制作的效率和质量，同时保持了与专业电影制作管道的无缝集成。
...
TL;DR: 该论文研究了基于反向信道编码（RCC）的扩散式图像压缩方法对比特翻转错误的鲁棒性，发现其比传统和学习的编解码器更稳健，并提出了一种改进的Turbo-DDCM变体，在保持率-失真-感知权衡的同时显著提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现代图像压缩方法通常针对率-失真-感知权衡进行优化，但很少研究其对比特级损坏的鲁棒性。作者旨在探索不同压缩方法在比特翻转错误下的稳健性差异。

Method: 研究基于反向信道编码（RCC）范式的扩散式压缩器，特别是Turbo-DDCM方法。作者进一步提出了一种更鲁棒的Turbo-DDCM变体，在保持压缩性能的同时增强对比特翻转的抵抗能力。

Result: 实验表明，基于RCC的扩散式压缩器比传统和学习的编解码器对比特翻转错误具有显著更强的鲁棒性。改进的Turbo-DDCM变体在保持率-失真-感知权衡的同时，大幅提升了鲁棒性。

Conclusion: 基于RCC的压缩方法能够产生更具弹性的压缩表示，在高噪声环境中可能减少对纠错码的依赖，为鲁棒压缩系统设计提供了新方向。
...
TL;DR: 该论文介绍了"Subtle visual Challenge"竞赛，旨在促进对细微视觉信号的研究，包含跨域多模态欺骗检测和远程光电容积描记术(rPPG)估计两个任务。
...
Method: 通过组织"Subtle visual Challenge"竞赛来促进该领域研究，竞赛包含两个具体任务：1) 跨域多模态欺骗检测；2) 远程光电容积描记术(rPPG)估计。提供基线模型并建立评估平台。

Result: 共有22支团队提交了最终结果，相应的基线模型已在MMDD2026平台上发布，为后续研究提供了基准和参考。

Conclusion: 该挑战赛旨在鼓励开发更鲁棒和可泛化的细微视觉理解模型，推动计算机视觉和多模态学习领域的研究进展。
...
TL;DR: BADAS-2.0是第二代碰撞预测系统，通过长尾基准构建、知识蒸馏到边缘设备和可解释性增强三个方面推进技术前沿，实现了在罕见安全关键场景下的性能提升、实时边缘部署和预测可解释性。


<details>
  <summary>Details</summary>
Motivation: 基于BADAS-1.0的成功，研究团队旨在进一步改进碰撞预测系统，特别是在处理罕见但安全关键的场景、实现实时边缘部署以及增强系统可解释性方面。这些改进对于实际ADAS系统的部署和应用至关重要。

Method: 1. 构建10组针对罕见安全关键场景的长尾基准，使用BADAS-1.0作为主动标注工具从数百万未标注驾驶数据中筛选高风险样本进行标注；2. 在225万未标注驾驶视频上进行领域特定自监督预训练，然后蒸馏到紧凑模型（86M和22M参数）；3. 生成实时物体中心注意力热图定位预测证据，并通过视觉语言模型生成结构化推理。

Result: 1. 数据集从4万扩展到17.85万标注视频（约200万片段），在所有子组上获得一致提升，在最长尾的困难案例上改进最大；2. 紧凑模型实现7-12倍加速，精度接近原始模型，支持实时边缘部署；3. 系统能够提供实时注意力热图和结构化文本推理，增强可解释性。

Conclusion: BADAS-2.0在碰撞预测的准确性、效率和可解释性方面取得了显著进展，特别是在处理罕见安全关键场景和实现实时边缘部署方面，为实际ADAS系统的应用提供了重要技术支持。研究代码和评估基准已公开。
...
TL;DR: 提出PDMP策略，通过性能主导模态优先化解决多模态学习中的优化不足问题，认为不平衡学习优于平衡学习


<details>
  <summary>Details</summary>
Motivation: 多模态学习常面临优化不足问题，现有方法通过梯度调制实现模态平衡学习，但本文认为性能主导模态驱动的不平衡学习能带来更好的多模态性能

Method: 提出性能主导模态优先化(PDMP)策略：1)通过独立训练的单模态模型性能排名挖掘性能主导模态；2)引入非对称系数调制各模态梯度，使性能主导模态主导优化过程

Result: 在多个数据集上的广泛实验验证了PDMP的优越性

Conclusion: PDMP策略有效解决了多模态学习优化不足问题，且不依赖多模态模型结构和融合方法，具有实际应用潜力
...
<details>
  <summary>Details</summary>
Motivation: 复杂计算机视觉模型的可解释性对于建立信任和问责制至关重要，特别是在安全关键领域。现有方法面临三方面权衡：传播方法高效但有偏差且架构特定；扰动方法因果基础扎实但计算昂贵且对视觉Transformer只能生成粗糙的块级解释；基于学习的方法快速但通常优化替代目标或从启发式教师蒸馏。

Method: 提出一种直接优化删除和插入指标的学习方案。由于这些指标依赖于不可微的排序和排名，将其框架化为排列学习问题，使用Gumbel-Sinkhorn替代硬排序实现可微松弛。通过目标模型的归因引导扰动实现端到端训练。推理时，通过单次前向传播生成密集像素级归因，并可选择进行少量梯度细化步骤。

Result: 实验表明该方法在定量指标上持续改进，生成更清晰、边界对齐的解释，特别是在基于Transformer的视觉模型上表现优异。

Conclusion: 该方法通过直接优化评估指标，实现了高效、准确且细粒度的视觉模型解释，特别适合现代Transformer架构，为模型可解释性提供了新的有效解决方案。
...
TL;DR: Etch攻击框架通过三层分解（语义伪装、视觉空间锚定、字体编码）实现文本到图像模型的刻录式越狱攻击，能生成包含有害文本的视觉良性图像，平均攻击成功率65.57%，最高达91%


<details>
  <summary>Details</summary>
Motivation: 现代文本到图像模型已能渲染段落长度的可读文本，这带来了新的滥用风险。研究者发现并形式化了"刻录式越狱"攻击，即攻击者迫使T2I系统生成包含有害文本负载（如欺诈文件）但视觉上良性的图像。传统越狱技术主要针对视觉内容操纵，难以在保持字符级保真度的同时绕过多阶段安全过滤器。

Method: 提出Etch黑盒攻击框架，将对抗性提示分解为三个功能正交的层次：语义伪装层（隐藏恶意意图）、视觉空间锚定层（控制文本位置）、字体编码层（确保字符保真度）。通过零阶循环迭代优化，使用视觉语言模型批判生成的图像，将失败定位到特定层次并制定针对性修订。

Result: 在7个模型和2个基准测试上的广泛评估显示，Etch平均攻击成功率达到65.57%，最高可达91.00%，显著优于现有基线方法。这揭示了当前T2I安全对齐的关键盲点。

Conclusion: 刻录式越狱攻击暴露了当前文本到图像模型安全防护的严重漏洞，现有安全机制主要关注视觉内容而忽视了文本渲染能力被滥用的风险。研究结果强调了开发字体感知的多模态防御机制的紧迫性。
...
TL;DR: 该研究提出了一种基于3D-2D配准的自动牙齿形态比对方法，用于法医身份识别，通过计算机视觉技术将死后3D扫描与生前照片进行比对，显著提高了识别效率。


<details>
  <summary>Details</summary>
Motivation: 传统牙齿形态比对方法耗时且依赖生前医疗记录，而在移民死亡等场景中往往缺乏这些记录。社交媒体上可见牙齿的照片为身份识别提供了新可能，但现有方法存在透视失真建模不足和缺乏客观量化标准的问题。

Method: 采用3D（死后扫描）-2D（生前照片）方法，利用计算机视觉和优化技术，通过3D模型重建生前图像进行形态比对。开发了两种自动方法：1）使用配对标志点；2）使用牙齿区域分割来估计相机参数。

Result: 在142个样本的20,164次交叉比对中，两种方法分别获得平均排名值1.6和1.5，明显优于自动牙科图表比对方法的筛选能力，提供了客观、定量的形态对应评分。

Conclusion: 该方法为法医牙齿识别提供了自动化、客观的解决方案，通过可视化叠加图像使结果易于解释和分析，在缺乏生前医疗记录的场景中具有重要应用价值。
...
TL;DR: 论文提出了PVIR基准测试，专注于视频实例移除中的物理一致性评估，包含95个高质量视频，分为简单和困难子集，评估了四种方法在语义、视觉和空间三个维度的表现。


<details>
  <summary>Details</summary>
Motivation: 当前视频实例移除方法主要关注视觉合理性，但忽视了物体移除引发的物理因果关系（如残留阴影、反射等），缺乏评估物理一致性的基准测试。

Method: 提出PVIR基准测试，包含95个高质量视频，标注了实例精确的掩码和移除提示，分为简单和困难子集（后者专门针对复杂物理交互），采用解耦的人类评估协议，从指令遵循、渲染质量和编辑排他性三个维度进行评估。

Result: PISCO-Removal和UniVideo达到最先进性能，DiffuEraser经常引入模糊伪影，CoCoCo在指令遵循方面表现较差。在困难子集上所有方法性能都显著下降，突显了恢复复杂物理副效应的挑战。

Conclusion: PVIR基准测试填补了视频实例移除中物理一致性评估的空白，揭示了现有方法在处理复杂物理交互方面的局限性，为未来研究提供了重要的评估框架。
...
TL;DR: 该论文提出了AICA-Bench基准测试，用于评估视觉语言模型在情感图像内容分析中的综合能力，并提出了GAT提示框架来改进模型的强度校准和描述深度。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在感知方面表现出色，但在将感知、推理和生成整合到统一框架中的整体情感图像内容分析方面研究不足，需要建立综合基准来评估和改进模型在这方面的能力。

Method: 1. 提出AICA-Bench基准测试，包含三个核心任务：情感理解、情感推理和情感引导内容生成；2. 评估了23个视觉语言模型，发现强度校准弱和开放描述浅的问题；3. 提出GAT提示框架，结合视觉支架和层次推理，无需训练即可改进模型性能。

Result: 实验表明，GAT框架显著减少了强度误差，提高了描述深度，为情感多模态理解和生成研究提供了强有力的基线。

Conclusion: 该研究填补了视觉语言模型在整体情感图像内容分析方面的空白，提出的AICA-Bench基准和GAT提示框架为未来情感多模态理解和生成研究提供了重要工具和方向。
...
TL;DR: 本文提出通过选择性地聚合与目标概念最相关的注意力头来提升文本到图像生成模型的视觉可解释性，相比现有方法DAAM获得了更高的mIoU分数。


<details>
  <summary>Details</summary>
Motivation: 现有研究虽然利用交叉注意力图来提升应用性能和解释模型行为，但不同注意力头的独特特性尚未得到充分探索。研究者希望探索选择性聚合最相关注意力头的方法来改善视觉可解释性。

Method: 通过选择性地聚合与目标概念最相关的交叉注意力头，而不是使用所有注意力头。这种方法能够更准确地捕捉概念特定特征，并有助于诊断提示词误解。

Result: 相比基于扩散的分割方法DAAM，该方法获得了更高的平均IoU分数。研究发现最相关的注意力头比最不相关的注意力头更准确地捕捉概念特定特征，选择性聚合有助于诊断提示词误解。

Conclusion: 注意力头选择为提高文本到图像生成的可解释性和可控性提供了一个有前景的方向，能够改善模型行为的理解和控制。
...
<details>
  <summary>Details</summary>
Motivation: 多轨迹场景重建对于自动驾驶仿真和数字孪生构建至关重要，但不同时间采集的序列存在显著的外观不一致性（如光照和环境条件变化），尽管底层几何结构相同，这种外观纠缠问题需要解决。

Method: 提出ADM-GS框架，对静态背景进行显式外观分解：1）分解为轨迹不变的材料属性（内在材料特性）和轨迹相关的光照变化；2）设计神经光场，采用频率分离的混合编码策略；3）结合表面法线和显式反射向量，分别捕捉低频漫反射光照和高频镜面反射。
...
TL;DR: SRCP提出了一种新颖的零样本无监督强化学习框架，通过解耦表示学习和后继训练，引入显著性引导的动态任务来改进视觉环境中的后继表示方法，显著提升了零样本泛化性能。


<details>
  <summary>Details</summary>
Motivation: 后继表示(SR)方法在结构化、低维环境中表现出色，但在高维视觉环境中难以扩展。研究发现SR在视觉无监督强化学习中存在两个关键限制：1) SR目标导致关注动态无关区域的次优表示，影响后继度量和任务泛化；2) 这些有缺陷的表示阻碍SR策略建模多模态技能条件动作分布和确保技能可控性。

Method: 提出SRCP框架：1) 通过引入显著性引导的动态任务来解耦表示学习和后继训练，捕获动态相关表示；2) 集成快速采样一致性策略，采用URL特定的无分类器引导和定制训练目标，改进技能条件策略建模和可控性。

Result: 在ExORL基准的4个数据集共16个任务上进行广泛实验，SRCP在视觉无监督强化学习中实现了最先进的零样本泛化性能，并且与多种SR方法兼容。

Conclusion: SRCP通过解耦表示学习和引入显著性引导机制，有效解决了SR方法在视觉环境中的局限性，显著提升了零样本泛化能力，为构建通用智能体提供了有前景的方向。
...
TL;DR: SonoSelect是一种超声主动视图探索方法，通过自适应引导探头移动，在减少扫描冗余的同时提高器官覆盖率和诊断准确性。


<details>
  <summary>Details</summary>
Motivation: 传统超声检查需要多个扫描视图来减少诊断歧义、缓解声学遮挡并提高解剖覆盖，但并非所有探头视图都具有同等信息量。大量视图采集会引入冗余，增加扫描和处理成本。

Method: 将超声主动视图探索定义为序列决策问题，将每个新的2D超声视图融合到观察解剖结构的3D空间记忆中，指导下一个探头位置。提出超声特异性目标函数，优先选择器官覆盖更大、重建不确定性更低、扫描冗余更少的探头移动。

Result: 在超声模拟器实验中，SonoSelect仅使用N个视图中的2个就实现了有前景的多视图器官分类准确率。对于更困难的肾囊肿检测任务，达到了54.56%的肾脏覆盖率和35.13%的囊肿覆盖率，且轨迹短且始终集中在目标囊肿上。

Conclusion: SonoSelect通过自适应探头移动策略，在减少扫描视图数量的同时提高了超声检查的效率和准确性，为智能超声导航提供了有效解决方案。
...
TL;DR: 提出了一种基于视觉语言扩散基础模型的低数据需求CT金属伪影去除方法，仅需16-128个配对训练样本，通过多参考条件策略实现数据高效的去伪影重建。


<details>
  <summary>Details</summary>
Motivation: 高衰减植入物产生的金属伪影严重降低CT图像质量，遮挡关键解剖结构，而传统深度学习方法需要大量配对训练数据。需要探索数据需求更低的解决方案。

Method: 将伪影去除重构为上下文推理任务，通过参数高效的LoRA技术适配通用视觉语言扩散基础模型。提出多参考条件策略，在输入含伪影图像的同时提供来自无关受试者的干净解剖示例，使模型能利用类别特定上下文推断未损坏的解剖结构。

Result: 在AAPM CT-MAR基准测试中，该方法在感知和放射学特征指标上达到最先进性能。仅需16-128个配对训练样本，数据需求降低两个数量级。域适配对减少幻觉至关重要，否则基础模型会将条纹伪影误认为自然物体。

Conclusion: 经过适当适配的基础模型为可解释、数据高效的医学图像重建提供了可扩展的替代方案，证明了基础模型在医学图像处理中的潜力。
...
TL;DR: 提出MoME框架和HTL策略，通过模态专家混合和整体标记学习实现自适应多模态融合，在驾驶员动作识别任务上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有多模态学习方法主要依赖固定融合模块或预定义的跨模态交互，难以适应变化的模态可靠性和捕捉细粒度动作线索，需要更灵活的自适应多模态融合框架。

Method: 提出Mixture-of-Modality-Experts (MoME)框架和Holistic Token Learning (HTL)策略。MoME实现模态特定专家之间的自适应协作，HTL通过类别标记和时空标记改进专家内部细化和专家间知识转移。

Result: 在公开基准测试中，MoME框架和HTL策略联合超越了代表性的单模态和多模态基线方法。消融实验、验证和可视化结果进一步证实HTL策略改进了细微的多模态理解并提供了更好的可解释性。

Conclusion: 该方法形成了一个以知识为中心的多模态学习框架，提高了专家专业化程度，同时减少了多模态融合中的歧义，为自适应多模态分析提供了有效解决方案。
...
TL;DR: HumANDiff是一个新的人类视频生成框架，通过关节化运动一致噪声采样、联合外观-运动学习和几何运动一致性学习，显著提升了人类动作的物理真实性和动态控制能力。


<details>
  <summary>Details</summary>
Motivation: 尽管人类视频生成领域取得了巨大进展，但现有的生成视频扩散模型在捕捉人类动作的动力学和物理特性方面仍然存在困难，无法忠实地再现人体运动的真实动态。

Method: 1) 关节化运动一致噪声采样：在统计人体模板的密集表面流形上采样3D关节化噪声，替代非结构化随机高斯噪声，继承身体拓扑先验；2) 联合外观-运动学习：增强视频扩散模型的标准训练目标，从关节化噪声中联合预测像素外观和相应的物理运动；3) 几何运动一致性学习：通过新颖的几何运动一致性损失在关节化噪声空间中强制执行跨帧的物理运动一致性。

Result: 该方法实现了最先进的性能，能够渲染运动一致、高保真度的人类视频，支持多样化的服装风格，并能在单一框架内实现图像到视频的生成，无需额外的运动模块。

Conclusion: HumANDiff通过关节化噪声采样和物理运动一致性学习，显著提升了人类视频生成的质量和可控性，为可控人类视频生成提供了可扩展的解决方案，且不依赖于特定的扩散模型设计。
...
TL;DR: CLIP模型存在中心偏差问题，过度关注图像中心区域而忽略边缘重要物体，这源于视觉嵌入聚合过程中的信息丢失，可通过训练无关策略缓解


<details>
  <summary>Details</summary>
Motivation: 研究发现对比视觉语言模型如CLIP缺乏对视觉内容的细粒度理解，特别是存在中心偏差问题，即模型过度关注图像中心区域而忽略边缘的重要物体。这种局限性是根本性的，因为无法识别相关物体使得依赖这些物体的复杂任务难以执行。

Method: 从表示和注意力两个角度进行分析，使用可解释性方法（嵌入分解和注意力图分析）来探究问题根源。发现相关概念特别是与离中心物体相关的概念在最终表示中消失，这是由于视觉嵌入聚合过程中的信息丢失，特别是对池化机制的依赖。

Result: 分析表明CLIP模型存在明显的中心偏差问题，离中心物体的相关信息在最终表示中丢失。这种偏差可以通过训练无关策略（如视觉提示和注意力重分配）来缓解，通过将模型注意力重新引导到离中心区域。

Conclusion: CLIP模型存在固有的中心偏差问题，这源于视觉嵌入聚合过程中的信息丢失。虽然可以通过训练无关策略部分缓解，但需要更根本的架构改进来解决这一局限性，以提高模型对细粒度视觉内容的理解能力。
...
TL;DR: OmniCamera是一个统一框架，通过解耦场景动态内容和相机运动，实现灵活的视频生成控制，解决了现有模型对这两个因素的纠缠问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型往往将场景动态内容和相机运动这两个关键因素纠缠在一起，限制了独立的控制能力。视频本质上交织着这两个轴，需要一种能够明确解耦并分别控制它们的方法。

Method: 提出了OmniCamera统一框架，包含两个关键创新：1) 构建OmniCAM混合数据集，结合真实世界视频和合成数据，提供多样化的配对示例；2) 提出双层次课程协同训练策略，通过条件层次逐步引入控制模态，以及数据层次先在合成数据上训练精确控制，再适应真实数据实现照片级真实感。

Result: OmniCamera实现了最先进的性能，能够灵活控制复杂的相机运动，同时保持卓越的视觉质量，为视频生成提供了前所未有的创作控制能力。

Conclusion: 通过解耦场景内容和相机运动，并采用创新的数据集构建和训练策略，OmniCamera框架成功解决了模态冲突和数据稀缺的挑战，实现了灵活可控的高质量视频生成。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在医学影像中表现出色，但基于反向传播的"黑盒"模型阻碍了临床应用，需要满足临床环境对透明度的严格要求。
...
Conclusion: A-ROM框架能够在保持竞争力的性能同时，满足临床环境对模型透明度的严格要求，为医学影像分析提供了可解释的解决方案。
...
TL;DR: 本文提出了一种基于探测的定位技术，发现自注意力层是解决模糊概念的关键，并基于此开发了ICM方法，通过针对性干预实现更优的去偏效果。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在处理不完全描述性提示时需要进行隐式决策，但现有定位技术主要关注显式条件干预，无法有效处理这种隐式决策过程。本文旨在研究这种隐式决策是否在模型架构中具有计算局部性。

Method: 引入基于探测的定位技术来识别对概念具有最高属性可分性的层。研究发现模糊概念的解析主要由自注意力层控制，基于此提出了ICM（隐式选择修改）方法，该方法对一小部分层进行针对性干预。

Result: 实验表明，在特定的自注意力层进行干预相比现有最先进方法具有更优的去偏性能，同时最小化了不精确方法常见的伪影问题。

Conclusion: 文本到图像扩散模型中的隐式决策具有计算局部性，主要位于自注意力层。ICM方法通过精确干预这些层，能够有效引导模型生成，为模型可解释性和控制提供了新途径。
...
TL;DR: EDGE-Shield是一种在去噪过程中进行内容过滤的可扩展方法，通过嵌入匹配和x-pred变换技术，显著降低处理延迟，有效阻止侵权和深度伪造内容生成。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成模型存在版权侵权和深度伪造风险，需要基于参考的训练无关内容过滤器来提供实时保护。现有基于参考的方法在处理大量参考时缺乏可扩展性，且需要等待图像生成完成。

Method: 提出EDGE-Shield：1）使用嵌入匹配进行高效参考比较；2）引入x-pred变换，将模型的噪声中间潜在表示转换为后期伪估计的干净潜在表示，提高早期去噪阶段对侵权内容的分类准确性。

Result: 在Z-Image-Turbo和Qwen-Image两个生成模型上进行实验，EDGE-Shield相比传统基于参考的方法显著降低延迟：Z-Image-Turbo处理时间减少约79%，Qwen-Image减少约50%，同时保持不同模型架构下的过滤准确性。

Conclusion: EDGE-Shield提供了一种可扩展的内容过滤解决方案，在去噪过程中实时检测侵权内容，显著降低处理延迟，为文本到图像生成模型的安全应用提供了有效保护机制。
...
TL;DR: 该论文提出了一个解决图形程序合成中TikZ代码生成问题的闭环框架，包括高质量数据集SciTikZ-230K、多维度基准测试SciTikZ-Bench，以及双自一致性强化学习优化方法，最终模型SciTikZer-8B在性能上超越了Gemini-2.5-Pro等大型模型。


<details>
  <summary>Details</summary>
Motivation: 图形程序合成对于解释和编辑视觉数据至关重要，但现有方法面临两个主要问题：1) 数据质量差距 - 现有图像-TikZ语料库缺乏严格的执行性和可靠的视觉对齐；2) 评估差距 - 缺乏同时评估结构和视觉保真度的基准测试。TikZ作为科学示意图的事实标准，其严格的空间精度要求对多模态大语言模型构成了重大挑战。

Method: 提出了一个闭环框架：1) SciTikZ-230K - 通过执行中心数据引擎构建的大规模高质量数据集，涵盖11个不同科学学科；2) SciTikZ-Bench - 从基本几何构造到复杂层次示意图的多维度基准测试；3) 双自一致性强化学习优化范式，利用往返验证惩罚退化代码并提升整体自一致性。

Result: 训练出的模型SciTikZer-8B实现了最先进的性能，在图形程序合成任务上持续超越了Gemini-2.5-Pro等专有大型模型和Qwen3-VL-235B-A22B-Instruct等超大规模模型。

Conclusion: 该研究通过构建高质量数据集、全面基准测试和创新的优化方法，有效解决了图形程序合成中的关键挑战，显著提升了从静态视觉数据生成可编辑TikZ代码的能力，为科学示意图的逆向工程提供了强大的工具。
...
Method: 使用Σ-Voxfield网格作为离散表示，每个占用的体素存储固定数量的着色表面样本；训练语义条件扩散模型在局部体素邻域操作，使用3D位置编码捕获空间结构；通过渐进空间外推在重叠区域扩展到大场景；最后通过延迟渲染模块渲染生成的网格

Result: 能够生成多样化的大规模城市户外场景，渲染成具有各种传感器配置和相机轨迹的光真实感图像，同时保持适中的计算成本
...
<details>
  <summary>Details</summary>
Motivation: 解决RGB预训练视觉语言模型与热红外图像之间的表征差距，扩展视觉语言模型在生态监测中的应用，实现从物体级识别到栖息地环境解释的升级。

Method: 开发无人机采集的热红外数据集，通过多模态投影器对齐微调视觉语言模型，将RGB视觉表征信息迁移到热辐射输入。测试了InternVL3-8B-Instruct、Qwen2.5-VL-7B-Instruct和Qwen3-VL-8B-Instruct三种模型在闭集和开集提示条件下的性能。

Result: Qwen3-VL-8B-Instruct在开集提示下表现最佳：鹿、犀牛、大象的F1分数分别为0.935、0.915、0.968；计数准确率（误差在1以内）分别为0.779、0.982、1.000。结合热红外和RGB图像还能生成栖息地环境信息，包括土地覆盖特征、关键景观特征和可见的人类干扰。

Conclusion: 轻量级投影器适配为将RGB预训练视觉语言模型迁移到热红外无人机图像提供了有效实用的途径，扩展了其在生态监测中的应用范围，从物体级识别升级到栖息地环境解释。
...
<details>
  <summary>Details</summary>
Motivation: 自注意力机制在处理长序列时计算复杂度高，需要一种更高效的token混合机制来替代自注意力，同时保持模型的表达能力。

Method: 通过学习的多项式函数将输入token聚合为紧凑表示，然后每个token从该表示中检索上下文信息，形成线性复杂度的token混合机制。

Result: 在文本生成、手写文本识别、图像生成、3D建模和地球观测五个不同领域中，PoM与基于注意力的模型性能相当，但在处理长序列时计算成本大幅降低。

Conclusion: PoM是一种有效的自注意力替代方案，具有线性复杂度，能够保持Transformer的通用序列到序列近似能力，同时显著提高长序列处理效率。
...
<details>
  <summary>Details</summary>
Motivation: 当前MLLMs在嵌入任务中未充分利用生成推理能力，直接将思维链推理引入嵌入学习存在两个问题：1）实例级推理与成对对比监督的结构错配导致模型学习推理的表面格式；2）推理并非对所有嵌入任务都有益，强制所有输入进行推理会增加计算延迟并可能掩盖简单案例的语义信号

Method: 提出MMEmb-R1框架：1）将推理建模为潜在变量；2）引入成对感知推理选择，使用反事实干预识别有益于查询-目标对齐的推理路径；3）采用强化学习选择性调用推理，仅在必要时进行推理

Result: 在MMEB-V2基准测试中达到71.2分（仅4B参数），创下新的SOTA，同时显著减少推理开销和推理延迟

Conclusion: 通过自适应推理机制，MMEmb-R1成功解决了MLLMs在嵌入任务中推理能力未充分利用的问题，在保持高性能的同时有效降低了计算成本
...
TL;DR: 提出字符错误向量（CEV）作为OCR评估新指标，解决传统字符错误率（CER）在页面解析错误时无法定义的问题，可分解为解析、OCR和交互错误分量。


<details>
  <summary>Details</summary>
Motivation: 传统字符错误率（CER）假设文本已完美解析，但在实际页面解析错误时CER无法定义，限制了其作为评估指标的使用，特别是在使用不同标注模式的数据时难以评估页面级OCR质量。

Method: 引入字符错误向量（CEV），一种基于字符袋的OCR评估器。CEV可分解为解析错误、OCR错误和交互错误分量。实现方法包括SpACER（空间感知字符错误率）和使用Jensen-Shannon距离的字符分布方法。

Result: 验证表明CEV是解析指标与CER等局部指标之间的有效桥梁。在包含复杂布局退化图像的档案报纸数据集上，发现传统流水线方法优于最先进的端到端模型。CEV通过阈值处理可预测主要错误源，F1分数达0.91。

Conclusion: CEV解决了CER在页面解析错误时的局限性，提供了可分解的评估框架，帮助从业者聚焦文档理解流程中对文本提取质量影响最大的部分。提供了Python库支持文档理解研究。
...
TL;DR: DiffHDR是一个基于视频扩散模型的LDR到HDR转换框架，将转换任务视为生成式辐射度修复问题，在潜在空间中利用时空生成先验合成过曝和欠曝区域的HDR辐射度。


<details>
  <summary>Details</summary>
Motivation: 大多数数字视频以8位低动态范围格式存储，导致原始高动态范围场景辐射度因饱和和量化而丢失，限制了HDR显示器的准确亮度映射和后期制作中的重新曝光能力。现有LDR到HDR转换技术难以恢复过曝和欠曝区域的真实细节。

Method: DiffHDR将LDR到HDR转换构建为视频扩散模型潜在空间中的生成式辐射度修复任务。在Log-Gamma色彩空间中操作，利用预训练视频扩散模型的时空生成先验合成过曝和欠曝区域的合理HDR辐射度，同时恢复量化像素的连续场景辐射度。框架支持通过文本提示或参考图像进行可控转换。为解决配对HDR视频数据稀缺问题，开发了从静态HDRI地图合成高质量HDR视频训练数据的流程。

Result: 大量实验表明，DiffHDR在辐射度保真度和时间稳定性方面显著优于现有最先进方法，能够生成具有较大重新曝光余量的真实HDR视频。

Conclusion: DiffHDR通过将LDR到HDR转换构建为生成式辐射度修复任务，利用视频扩散模型的生成能力，有效解决了过曝和欠曝区域细节恢复问题，为高质量HDR视频转换提供了创新解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在图像描述中会产生物体幻觉，现有基于注意力权重的检测方法不可靠，存在Simpson悖论问题，需要更有效的检测和缓解策略。

Method: 提出HaloProbe贝叶斯框架，通过平衡训练分离内部证据，结合学习到的外部特征先验来估计token级别的幻觉概率，作为外部评分信号进行非侵入式缓解。

Result: HaloProbe引导的解码比最先进的干预方法更有效地减少幻觉，同时保持模型实用性。

Conclusion: HaloProbe通过贝叶斯框架有效解决了注意力权重检测中的混淆问题，提供了一种非侵入式的幻觉缓解方法，在保持模型性能的同时显著减少幻觉。
