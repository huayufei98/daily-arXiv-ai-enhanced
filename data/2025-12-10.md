...
Method: 采用Planet Labs的四波段卫星影像，结合深度学习模型，通过五个苏丹案例研究验证方法有效性，并与基线方法进行比较。

Result: 自动化方法相比基线能更准确地捕捉活跃火灾和烧焦区域；使用八波段影像或时间序列影像仅带来边际增益。

Conclusion: 深度学习结合四波段卫星影像可有效监测武装冲突中的火灾损害，实现近实时监测，为冲突分析提供及时数据支持。
...
Method: 1. 使用关键帧作为条件信号注入目标身份，实现灵活可控的编辑；2. 结合关键帧条件和视频参考引导，进行时间缝合以确保稳定的身份保留和高保真重建；3. 构建Face2Face配对人脸交换数据集解决参考引导训练数据稀缺问题，并通过反转数据对确保可靠的地面真实监督。

Result: 大量实验表明，该方法实现了最先进的结果，能够无缝地将目标身份与源视频的表情、光照和运动相结合，同时显著减少了制作工作流程中的人工工作量。

Conclusion: LivingSwap作为首个视频参考引导的人脸交换模型，通过关键帧条件注入和视频参考引导，成功解决了长视频序列中的人脸交换保真度和时间一致性问题，为影视制作提供了高效解决方案。
...
TL;DR: FRIEDA是一个评估大型视觉语言模型在复杂开放式地图推理能力的新基准，测试拓扑、度量和方向三类空间关系，结果显示当前最先进模型表现远低于人类水平。


<details>
  <summary>Details</summary>
Motivation: 地图推理是解释地理关系的关键认知能力，对灾害响应和城市规划等任务至关重要，但目前缺乏系统评估。现有地图视觉问答研究常将地图视为图表的特例，但地图VQA需要理解分层符号系统和跨地图的空间关系。

Method: 从各领域和地理区域的文档报告中收集真实地图图像，基于GIS文献分类，针对拓扑（边界、相等、相交、包含）、度量（距离）和方向（方位）三类空间关系设计问题。所有问题都需要多步推理，许多需要跨地图定位和推理。

Result: 评估了11个最先进的LVLM，在两种设置下：直接提供相关地图，以及需要模型先识别相关地图再推理。最强的Gemini-2.5-Pro和GPT-5-Think分别仅达到38.20%和37.20%准确率，远低于人类84.87%的表现。

Conclusion: FRIEDA揭示了LVLM在多步地图推理方面存在显著差距，可作为推动空间智能进步的严谨基准，表明当前模型在复杂地图推理能力上仍有很大提升空间。
...
TL;DR: 该论文提出了一种基于频域掩码的训练策略，用于提升深度伪造检测器的泛化能力，使其能够识别多种生成模型（包括未见过的模型）生成的假图像，同时保持计算效率。


<details>
  <summary>Details</summary>
Motivation: 通用深度伪造检测需要在广泛的生成模型（包括未见过的模型）中识别AI生成的图像，这要求检测器具有良好的泛化能力。同时，在绿色AI时代，需要最小化计算开销以实现大规模深度伪造筛查。

Method: 提出频率域掩码作为深度伪造检测器的训练策略。与依赖空间特征或大规模预训练模型的传统方法不同，该方法引入随机掩码和几何变换，特别关注频率掩码因其优越的泛化特性。

Result: 频率掩码不仅提高了对多种生成器的检测准确性，而且在显著模型剪枝下仍能保持性能。该方法在GAN和扩散模型生成的图像数据集上实现了最先进的泛化性能，并在结构化剪枝下表现出一致的鲁棒性。

Conclusion: 频率掩码作为一种实用策略，为实现可持续和可泛化的深度伪造检测提供了有前景的方向。该方法在保持高性能的同时减少了计算资源需求，适合大规模应用。
...
TL;DR: M2A：一种简单的持续测试时自适应方法，通过随机掩码生成多个视图，结合一致性损失和熵最小化损失来适应分布偏移，无需复杂的不确定性校准或注意力机制。


<details>
  <summary>Details</summary>
Motivation: 测试时的分布偏移会降低图像分类器性能。现有的持续测试时自适应方法通常依赖复杂的不确定性校准或注意力分数，并引入额外复杂性。作者探索是否可以通过简单的随机掩码方案在强干扰下实现有效适应。

Method: 提出M2A方法：1）生成短序列的随机掩码视图（空间或频率域）；2）使用两种损失函数：掩码一致性损失（对齐不同视图的预测）和熵最小化损失（鼓励置信输出）；3）研究两种掩码家族：空间掩码（补丁vs像素）和频率掩码（全频段vs低频vs高频）。

Result: 在CIFAR10C/CIFAR100C/ImageNetC（严重程度5）上，M2A（空间）达到8.3%/19.8%/39.2%的平均错误率，优于或匹配现有CTTA基线，而M2A（频率）表现较差。消融实验表明简单随机掩码有效且鲁棒。

Conclusion: 简单的随机掩码方案结合一致性和熵目标足以驱动有效的测试时自适应，无需依赖不确定性或注意力信号。空间掩码优于频率掩码，随机掩码策略具有鲁棒性。
...
TL;DR: CVP是一个受人类中央-周边视觉启发的多模态空间推理框架，通过目标亲和力令牌和全局网格分别模拟中央和周边视觉，显著提升了3D场景理解能力。
...
Method: 提出中央-周边视觉启发框架，包含两个互补组件：1）目标亲和力令牌（模拟中央视觉），引导模型关注查询相关对象；2）全局网格（模拟周边视觉），捕捉全局场景上下文和空间布局。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的非均匀雾霾条件给图像去雾带来巨大挑战，现有Transformer方法虽然能捕捉全局上下文，但二次计算复杂度阻碍了实时部署

Method: 提出基于多状态感知范式的Fourier-RWKV框架：1) 空间形式感知：通过可变形四向令牌移位动态调整感受野；2) 频域感知：在傅里叶混合块中将RWKV的WKV注意力机制扩展到傅里叶域；3) 语义关系感知：通过语义桥接模块实现动态语义核融合

Result: 在多个基准测试上的广泛实验表明，Fourier-RWKV在多样化雾霾场景下实现了最先进的性能，同时显著降低了计算开销

Conclusion: Fourier-RWKV在恢复质量和实际效率之间建立了有利的权衡，为实时高质量去雾提供了有效解决方案
...
TL;DR: GeoLoom是一个用于几何图形生成的文本到图表生成框架，通过形式化语言和坐标求解实现高精度几何图生成


<details>
  <summary>Details</summary>
Motivation: 高质量几何图生成既具有挑战性又有机遇：它要求严格的空间精度，同时提供明确的约束来指导生成。受几何问题求解中形式化语言和符号求解器进展的启发，需要开发一个能够保证正确性和可解释性的几何图生成框架。
...
Conclusion: GeoLoom通过结合形式化语言和优化求解，为几何图生成提供了一个既保证精度又具有可解释性的框架，在结构保真度方面表现出色，为几何图生成领域提供了新的解决方案。
...
TL;DR: 该论文提出了一个视觉知识库框架VisKnow，通过构建多模态对象知识图谱来增强对象理解能力，并以动物知识库AnimalKB作为案例研究。


<details>
  <summary>Details</summary>
Motivation: 当前对象理解任务面临多模态数据不足、组织不系统的问题。虽然已有视觉标注（如部件、属性）和文本知识支持高级任务，但这些数据通常是任务导向的，缺乏系统性组织，难以实现预期的对象类别理解。

Method: 提出了VisKnow框架，通过结合专家设计和大规模模型应用，从文本和图像源中提取多模态、对象级知识，构建结构化知识图谱。具体构建了AnimalKB知识库，涵盖406个动物类别，包含22K文本知识三元组、420K图像及对应的区域标注。

Result: 实验表明AnimalKB能够增强对象级视觉任务（如零样本识别和细粒度VQA），并为知识图谱补全和部件分割提供具有挑战性的基准。证明了自动构建视觉知识库在推进视觉理解及其实际应用方面的潜力。

Conclusion: 视觉知识库通过结构化多模态对象知识，能够显著提升对象理解能力。VisKnow框架和AnimalKB案例展示了自动构建视觉知识库的可行性及其在计算机视觉任务中的实用价值。
...
TL;DR: 本文探索了在3D目标检测领域应用提示调优方法的有效性，研究了基于Waymo数据集训练的基础模型能否通过提示调优适应其他场景，并提出了场景导向的提示池方法。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型展现出的强大泛化能力，通过微调和提示调优等迁移学习技术可以以最小参数调整适应各种下游任务。本文旨在探索常见的提示调优方法在3D目标检测领域的有效性，研究基于大规模Waymo数据集训练的模型能否作为基础模型并适应3D目标检测领域的其他场景。

Method: 本文依次研究了提示标记和提示生成器的影响，并进一步提出了场景导向的提示池（SOP²）。该方法探索了提示池在3D目标检测中的应用，旨在激发未来研究者深入探索提示在3D领域的潜力。

Result: 研究证明了提示池在3D目标检测中的有效性，展示了基于大规模数据集训练的基础模型通过提示调优技术能够适应其他3D目标检测场景。

Conclusion: 提示调优方法在3D目标检测领域具有应用潜力，场景导向的提示池（SOP²）为3D领域的提示研究提供了新思路，有望激发更多研究者探索提示技术在3D计算机视觉任务中的应用。
...
TL;DR: MM-CoT是一个诊断性基准测试，专门用于评估多模态模型中思维链推理的视觉基础和逻辑连贯性，要求模型选择同时满足视觉一致性和逻辑连贯性的事件链，而不是生成自由形式的解释。
...
Method: 设计MM-CoT基准测试，要求模型从多个选项中选择唯一满足两个正交约束的事件链：1) 视觉一致性（所有步骤都基于可观察证据），2) 逻辑连贯性（因果和常识有效性）。通过设计违反其中一个约束的对抗性干扰项来暴露不同的推理失败。

Result: 评估领先的视觉语言模型发现，即使最先进的系统也在MM-CoT上表现不佳，揭示了生成流畅性与真实推理保真度之间的明显差异。MM-CoT与现有基准测试相关性低，确认它测量的是视觉基础和逻辑推理的独特组合。
...
TL;DR: 提出了一种基于法线引导的稀疏深度采样策略，通过PCA法线估计计算像素级深度可靠性，生成更符合真实传感器特性的稀疏深度数据，提升深度补全精度。
...
Conclusion: 通过考虑传感器几何依赖的可靠性，提出的法线引导稀疏深度采样方法能够生成更符合真实场景的稀疏深度数据，从而提升深度补全模型的性能和实用性。
...
TL;DR: FastBEV++是一个相机BEV感知框架，通过算法优化和设计可部署性，实现了高性能与实时性的平衡，在nuScenes基准上达到0.359 NDS和134 FPS


<details>
  <summary>Details</summary>
Motivation: 当前相机BEV感知面临性能与部署可行性的矛盾：先进性能依赖计算密集的视角变换和平台特定内核，难以在车载系统部署

Method: 采用"算法快速"和"设计可部署"两大原则：1) 通过确定性预排序策略将视角变换分解为标准索引-收集-重塑流水线，使用基础算子实现；2) 集成端到端深度感知融合机制，结合时间聚合和数据增强

Result: 在nuScenes基准上达到0.359 NDS的新SOTA性能，在Tesla T4等车载硬件上超过134 FPS的实时性能，无需定制CUDA内核即可部署

Conclusion: FastBEV++通过统一高性能与部署效率，为自动驾驶系统提供了成熟可扩展的设计方案，解决了BEV感知的部署瓶颈
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型在处理数百个视觉补丁标记时面临二次计算成本问题，现有方法存在权衡：连续压缩会稀释高级语义（如物体身份），而离散量化会丢失细粒度细节（如纹理）。

Method: 采用混合框架，通过双通道分离语义和外观：连续通道通过ViT补丁保留细粒度细节，离散通道使用MGVQ量化生成符号锚点（投影为四个标记）。两者融合成580个标记的混合序列，通过解耦注意力掩码和瓶颈压缩为单个voco标记。
...
TL;DR: 提出了一种名为Residual-SwinCA-Net的新型深度混合分割框架，结合残差CNN模块和Swin Transformer块，用于乳腺超声图像病灶分割，在BUSI数据集上取得了优异性能。


<details>
  <summary>Details</summary>
Motivation: 为了解决乳腺超声图像分割中局部特征提取、全局依赖学习、组织连续性增强、噪声抑制和形态完整性保持等挑战，需要开发一种能够同时处理局部相关性和全局依赖性的鲁棒分割框架。

Method: 1. 提出Residual-SwinCA-Net混合框架：结合残差CNN模块提取局部相关特征；2. 定制Swin Transformer块通过内部残差路径学习全局依赖；3. 应用拉普拉斯-高斯区域算子增强组织连续性和抑制噪声；4. 引入边界导向算子保持恶性病灶轮廓形态完整性；5. 采用阶段式收缩策略捕获尺度不变性；6. 设计MSCAS模块选择性强调显著特征；7. 使用像素注意力模块自适应加权病灶像素。

Result: 在公开BUSI数据集上，Residual-SwinCA-Net实现了99.29%的平均准确率、98.74%的IoU和0.9041的Dice系数，优于现有的CNN和ViT技术。

Conclusion: 提出的Residual-SwinCA-Net框架通过有效结合局部和全局特征学习，显著提升了乳腺超声病灶分割性能，有助于改善临床诊断决策的及时性和准确性。
...
Conclusion: FTKD方法有效解决了在线模型学习未来帧知识的挑战，显著提升了3D目标检测性能
...
TL;DR: 本文提出了一种查询感知的枢纽原型学习方法，通过构建查询-支持点二分图识别枢纽点，生成与查询相关的原型，并利用纯度加权对比损失优化原型分布，有效解决了少样本3D点云分割中的原型偏差问题。
...
Method: 提出查询感知枢纽原型学习方法：1) 枢纽原型生成模块：构建查询-支持点二分图，识别频繁连接的枢纽点，生成与查询相关的原型；2) 原型分布优化模块：采用纯度加权对比损失，将坏枢纽和边界模糊原型拉近到对应类中心，优化原型表示。

Result: 在S3DIS和ScanNet数据集上的实验表明，QHP方法相比现有最先进方法取得了显著的性能提升，有效缩小了原型与查询集之间的语义差距。

Conclusion: 通过显式建模支持集与查询集之间的语义相关性，QHP方法能够生成更准确的原型表示，缓解原型偏差问题，在少样本3D点云语义分割任务中表现出优越性能。
...
TL;DR: RLCNet：一种用于LiDAR、RADAR和相机传感器在线标定的端到端可训练深度学习框架，通过加权移动平均和异常值剔除实现实时动态调整，在真实数据集上验证了其优越性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶车辆中多模态传感器（LiDAR、RADAR、相机）的外参标定对可靠感知至关重要，但由于机械振动和动态环境中的传感器漂移等因素，在线标定仍然具有挑战性。

Method: 提出RLCNet，一种新颖的端到端可训练深度学习框架，用于多模态传感器的同时在线标定。引入包含加权移动平均和异常值剔除的在线标定框架，实现动态参数调整，减少预测噪声并提高对漂移的鲁棒性。

Result: 在真实世界数据集上验证，RLCNet在实际部署中表现出鲁棒性能。消融研究突出了架构选择的重要性，与现有方法比较显示该方法在准确性和鲁棒性方面具有优越性。

Conclusion: RLCNet为自动驾驶车辆的多模态传感器在线标定提供了一种实用且有效的解决方案，能够适应动态环境变化并保持标定精度。
...
<details>
  <summary>Details</summary>
Motivation: 从第三人称视角视频转换为第一人称视角视频能够为沉浸式理解开辟新可能，但由于相机姿态变化极大且视角重叠极少，这一任务极具挑战性。需要保持可见内容的同时以几何一致的方式合成未观察区域。

Method: EgoX框架通过轻量级LoRA适配利用大规模视频扩散模型的预训练时空知识，采用宽度和通道级拼接的统一条件策略结合第三人称和第一人称先验，并引入几何引导的自注意力机制选择性地关注空间相关区域。
...
TL;DR: PAVAS提出了一种物理感知的视频到音频生成方法，通过物理参数估计和适配器将物理推理融入音频合成，生成更符合物理规律的声音。


<details>
  <summary>Details</summary>
Motivation: 当前视频到音频生成模型主要基于外观驱动，捕捉视觉-声学相关性，但忽略了塑造真实世界声音的物理因素。需要将物理推理融入音频生成过程，以产生更符合物理规律的声音。

Method: 1. 使用物理参数估计器（PPE）估计物体级物理参数：通过视觉语言模型推断运动物体质量，通过基于分割的动态3D重建模块恢复运动轨迹计算速度；2. 通过物理驱动音频适配器（Phy-Adapter）将这些物理线索融入基于潜在扩散的视频到音频生成模型。

Result: PAVAS在物理合理性和感知一致性方面优于现有视频到音频生成模型。创建了VGG-Impact基准测试和音频-物理相关系数（APCC）评估指标，实验证明该方法在定量和定性评估中都表现出色。

Conclusion: 将物理推理融入视频到音频生成可以产生更符合物理规律的声音，PAVAS方法通过物理参数估计和适配器机制实现了这一目标，为音频合成提供了新的物理感知维度。
...
TL;DR: OpenSubject是一个包含250万样本、435万图像的大规模视频衍生数据集，用于主体驱动的图像生成和编辑，通过四阶段流程构建，显著提升了复杂场景下的生成效果。


<details>
  <summary>Details</summary>
Motivation: 当前主体驱动图像生成模型存在两个主要问题：1）容易偏离参考身份特征；2）在包含多个主体的复杂场景中表现不佳。为了解决这些挑战，需要构建一个大规模、高质量的数据集来提升模型性能。

Method: 采用四阶段流水线构建数据集：1）视频筛选：通过分辨率和美学过滤获取高质量视频片段；2）跨帧主体挖掘与配对：利用视觉语言模型进行类别共识、局部定位和多样性感知配对；3）身份保持的参考图像合成：使用分割图引导的外绘和框引导的内绘技术，配合几何感知增强和不规则边界侵蚀；4）验证与标注：用VLM验证合成样本，失败样本重新合成，并构建短长两种描述。

Result: 构建了包含250万样本、435万图像的大规模数据集OpenSubject，并建立了相应的基准测试。实验表明，使用OpenSubject训练能够显著提升生成和编辑性能，特别是在复杂场景中表现突出。

Conclusion: OpenSubject数据集通过系统化的构建流程解决了主体驱动生成中的身份保持和复杂场景处理问题，为相关研究提供了高质量的数据资源和评估基准，推动了该领域的发展。
...
TL;DR: Terrain Diffusion：一种基于扩散模型的程序化地形生成新方法，结合了AI的高保真度和传统程序化噪声的无缝无限扩展特性


<details>
  <summary>Details</summary>
Motivation: 传统程序化噪声（如Perlin噪声）虽然快速且无限，但在真实感和大尺度一致性方面存在根本限制。需要一种AI时代的新方法来结合扩散模型的高保真度和程序化噪声的关键特性

Method: 1. InfiniteDiffusion算法实现无限生成；2. 分层堆叠的扩散模型耦合行星级上下文与局部细节；3. 紧凑拉普拉斯编码稳定地球尺度动态范围输出；4. 开源无限张量框架支持无界张量操作；5. 少步一致性蒸馏实现高效生成

Result: 建立了扩散模型作为程序化世界生成的实用基础，能够无限制地合成整个行星，具有连贯性、可控性和无限制特性
...
Conclusion: GeoDM通过在多几何乘积空间中操作，成功捕获了数据的固有几何结构，实现了更有效的数据集蒸馏，为处理复杂数据几何提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 牙齿标志点检测在临床正畸学中至关重要，但牙齿几何结构复杂且个体差异大，需要先进技术来实现精确可靠的3D牙齿标志点检测。

Method: 通过组织3DTeethLand挑战赛，与国际医学图像计算和计算机辅助干预会议合作，征集专注于口腔内3D扫描的牙齿标志点检测算法，并提供首个公开数据集。

Result: 创建了首个公开可用的3D牙齿标志点检测数据集，为评估该任务的最先进方法提供了宝贵资源，并鼓励社区提供具有重要临床意义的方法学贡献。

Conclusion: 3DTeethLand挑战赛通过提供公开数据集和促进算法开发，为解决牙齿标志点检测的临床挑战做出了重要贡献，推动了该领域的研究进展。
...
Conclusion: GeoDiffMM通过结合扩散模型和光流几何线索，实现了结构一致的运动放大，有效解决了小位移情况下光子噪声与真实微运动的分离问题，为视频运动放大提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 传统彩色图像分类方法将特征表示为实域向量、矩阵或三阶张量，未能有效保持RGB通道间的内在耦合关系。受四元数数据建模在图像恢复和去噪任务中成功的启发，作者希望开发一种能更好利用彩色图像结构信息的分类方法。

Method: 提出低秩支持四元数矩阵机（LSQMM）：1）将RGB通道视为纯四元数，利用四元数代数保持通道间的耦合关系；2）引入四元数核范数正则化项，作为传统矩阵核范数在四元数域的扩展，以促进强相关颜色通道产生的低秩结构；3）设计基于交替方向乘子法（ADMM）的迭代算法来有效求解四元数优化模型。

Result: 在多个彩色图像分类数据集上的实验结果表明，与使用支持向量机、支持矩阵机和支持张量机等最先进方法相比，提出的分类方法在分类准确率、鲁棒性和计算效率方面都表现出优势。

Conclusion: LSQMM通过四元数表示有效保持了彩色图像通道间的内在耦合关系，四元数核范数正则化促进了低秩结构，ADMM算法有效求解了优化问题，整体方法在彩色图像分类任务中表现优异。
...
TL;DR: 本文系统分析了Glaze和Nightshade等图像保护机制，发现它们通过结构化、低熵的扰动在特征层面操作，而非语义层面的干扰，解释了为何保护信号视觉上微妙但可检测。


<details>
  <summary>Details</summary>
Motivation: 虽然Glaze和Nightshade等图像保护机制在经验上有效，但其内部结构、可检测性和表征行为仍不明确。本研究旨在通过可解释AI方法系统分析这些保护机制的工作原理。

Method: 采用统一框架，结合白盒特征空间检查（潜在空间聚类、特征通道激活分析、基于遮挡的空间敏感性映射）和黑盒信号级探测（频域表征分析），从表征、空间和频谱三个维度系统分析保护机制。
...
Conclusion: 当代图像保护通过结构化特征级变形而非语义错位运行，解释了保护信号视觉上微妙但一致可检测的原因。这项工作推进了对抗性图像保护的可解释性，并为生成AI系统的未来防御和检测策略设计提供了信息。
...
TL;DR: PointDico提出了一种新颖的3D表示学习方法，通过结合扩散模型和对比学习的优点，解决了现有方法在无序、密度不均的点云数据上的局限性，在多个基准测试中达到了新的最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有自监督表示学习方法在3D数据上面临挑战，因为3D点云数据具有无序性和密度不均的特点。对比模型容易过拟合，而3D掩码自编码器难以处理无序点云。这促使研究者探索结合扩散模型和对比学习优点的解决方案。
...
Conclusion: PointDico成功结合了扩散模型和对比学习的优点，有效解决了3D点云表示学习的挑战，为无序点云数据的自监督学习提供了新的解决方案。
...
Method: 提出Bi^2MAC方法，设计轻量级模块生成软硬两种掩码，软掩码初步调制输入特征，硬掩码引导不同类型区域进入不同处理分支：冗余特征进入紧凑分支进行低成本全局处理，异质特征进入聚焦分支投入更多计算资源进行细粒度建模

Result: 在多个基准数据集上的实验表明，Bi^2MAC实现了最先进的性能，同时显著降低了训练时间和参数数量，在自适应卷积模型中计算成本最低

Conclusion: Bi^2MAC方法通过智能分配计算资源，有效利用不同类型区域的信息，在全色锐化任务中实现了性能与效率的良好平衡
...
TL;DR: 本文提出HybridSplat方法，通过反射烘焙高斯追踪和混合溅射框架，在保持反射渲染质量的同时，实现7倍渲染加速和4倍高斯基元减少
...
Method: 提出混合溅射机制，包括反射烘焙高斯追踪、基于瓦片的高斯溅射渲染反射、反射高斯基元与基础高斯基元统一混合框架、流水线级加速和反射敏感高斯剪枝

Result: 在Ref-NeRF和NeRF-Casting复杂反射场景上实现约7倍渲染加速，高斯基元数量比类似基于光线追踪的高斯溅射基线减少4倍

Conclusion: HybridSplat在保持反射渲染质量的同时，显著提升渲染速度和减少内存占用，成为复杂反射场景的新SOTA方法
...
<details>
  <summary>Details</summary>
Motivation: 当BOLD图像损坏或不可用时，从T1w图像生成BOLD图像可以恢复缺失的BOLD信息并支持下游任务，但现有方法能力有限

Method: 提出DINO-BOLDNet框架：集成冻结的自监督DINOv3编码器和轻量级可训练解码器；DINOv3提取切片内结构表示，切片注意力模块融合相邻切片上下文信息；多尺度生成解码器恢复精细功能对比度；基于DINO的感知损失确保预测与真实BOLD在transformer特征空间的结构和纹理一致性
...
TL;DR: TrackingWorld：一种从单目视频中实现世界中心坐标系下密集3D跟踪的新方法，解决了现有方法在分离相机运动与前景动态运动、跟踪新出现动态物体方面的不足


<details>
  <summary>Details</summary>
Motivation: 现有单目3D跟踪方法存在两个主要局限：1）难以将相机运动与前景动态运动有效分离；2）无法密集跟踪视频中新出现的动态物体。需要一种能在世界中心坐标系下跟踪几乎所有像素的解决方案
...
TL;DR: 提出SCU-CGAN模型，结合U-Net、CBAM和额外判别器，从非火灾图像生成逼真火灾图像，用于解决火灾检测数据集不足问题，显著提升火灾检测模型性能。
...
Result: SCU-CGAN在图像质量评估中优于现有模型，KID分数比CycleGAN提升41.5%。使用增强数据集显著提高火灾检测模型准确率，YOLOv5 nano模型的mAP@0.5:0.95指标提升56.5%。
...
TL;DR: MLLMs存在视觉token与文本token的范数差异问题，导致不对称更新动态，影响跨模态特征融合。通过添加LayerNorm层对齐范数，显著提升多模态和纯文本任务性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）普遍采用Pre-Norm架构，导致视觉token的高范数与文本token的低范数之间存在严重差异。这种范数不平衡不是静态问题，而是引发"不对称更新动态"，使得高范数视觉token具有"表示惯性"，语义转换速度远慢于文本token，从而损害跨模态特征融合的有效性。

Method: 首先通过理论分析证明范数不平衡导致的不对称更新动态问题。然后提出简单有效的解决方案：在视觉投影器后插入一个精心初始化的LayerNorm层，强制实现视觉token和文本token的范数对齐。该方法在LLaVA-1.5架构上进行实验验证。

Result: 实验验证表明，范数差异和由此产生的不对称更新速率是主流MLLMs中的普遍现象。提出的LayerNorm干预方法在LLaVA-1.5架构上取得了显著性能提升，不仅在一系列多模态基准测试中表现更好，而且在纯文本评估（如MMLU）上也表现出色，表明解决架构不平衡能产生更全面的模型能力。

Conclusion: MLLMs中视觉与文本token的范数差异是一个关键但被忽视的问题，它导致不对称更新动态，阻碍有效的跨模态融合。通过简单的LayerNorm层实现范数对齐，可以显著提升模型性能，包括纯文本任务，这表明解决架构不平衡对于构建更全面的多模态模型至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 在挑战性光照条件下，捕获的图像通常存在各种退化问题，导致基于视觉的应用性能下降。现有方法要么显著放大固有噪声，要么只在特定光照条件下有效，需要一种能在复杂光照下同时增强图像质量并抑制噪声的解决方案。
...
Result: 在真实世界数据集上的实验结果表明，该方法在对比度增强和噪声抑制方面都优于最先进的方法。
...
TL;DR: 该研究提出利用面部美化评估算法来检测面部修饰，在未知攻击算法的情况下实现了1.1%的D-EER检测性能


<details>
  <summary>Details</summary>
Motivation: 面部美化在社交媒体和广告中广泛应用，但当这些修饰后的图像作为生物特征样本使用时，会对人脸识别系统构成挑战。因此需要有效检测面部修饰，防止其对生物识别系统的影响。

Method: 研究分析面部美化评估算法在修饰图像上的变化，评估基于人工智能的不同特征提取方法，并探索是否可以利用面部美观度评估来提升检测率。

Result: 在攻击修饰算法未知的场景下，单张图像检测达到了1.1%的D-EER（检测等错误率），表明该方法在面部修饰检测方面具有良好性能。

Conclusion: 面部美化评估算法可以有效用于面部修饰检测，在未知攻击算法的情况下仍能实现高精度的检测性能，为生物识别系统的安全性提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图像的HMR方法在视频中应用时存在时间不一致性和遮挡下性能下降的问题，需要利用视频中的人体连续性来解决这些问题

Method: 首先使用可提示视频分割模型生成身份一致的masklets，然后通过遮挡感知模块恢复缺失区域，最后用精炼的masklets指导SAM 3D Body生成一致的全身体网格轨迹

Result: 实验结果表明，SAM-Body4D在具有挑战性的野外视频中实现了更好的时间稳定性和鲁棒性，且无需任何重新训练

Conclusion: SAM-Body4D通过利用视频中的人体连续性，解决了现有HMR方法在视频应用中的时间不一致和遮挡问题，实现了无需训练的时间一致且鲁棒的3D人体网格恢复
...
TL;DR: OneClip-RAG：一种基于单视频片段检索增强的高效长视频理解范式，通过查询引导的视频分块算法和渐进式训练，显著提升多模态大语言模型处理长视频的能力和效率。
...
Method: 提出OneClip-RAG范式，包含：1）查询引导的视频分块算法，将视频分块与跨模态检索统一在一个处理步骤中；2）创建SynLongVideo数据集用于指令跟随训练；3）渐进式训练策略提升模型性能。

Result: 实验表明OneClip-RAG显著提升多个MLLMs在长视频基准测试上的性能，如将InternLV2 8B和Qwen2-VL 7B提升到GPT-4o在MLVU上的水平，且效率极高（LLaVA-Video能在2.2分钟内处理1小时视频）。
...
TL;DR: 提出了一种基于深度图的6D位姿估计方法，通过多视角深度图融合和稀疏TSDF表示，在密集工业拣选场景中实现高精度位姿恢复。


<details>
  <summary>Details</summary>
Motivation: 解决工业拣选环境中由于遮挡、反射和无纹理部件导致的6D位姿估计难题，现有方法在密集杂乱场景中表现不佳。

Method: 1) 多视角深度图融合为精细点云或稀疏TSDF；2) 分阶段热图机制产生场景自适应注意力先验；3) 密度感知稀疏Transformer块处理遮挡和非均匀分布；4) 基于体素的投票策略预测任意数量目标物体的6D位姿。

Result: 在IPD和MV-YCB多视角数据集上验证，在密集工业拣选和家庭拣选场景中表现出竞争力。

Conclusion: 提出了一种完全稀疏的高分辨率体积表示框架，能够有效处理密集杂乱场景中的6D位姿估计问题，为近距离机器人应用提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 手术分割对于场景理解至关重要，但受到标注稀缺和不同手术间语义不一致的限制。现有方法通常只是对自然基础模型（如SAM）进行有限监督的微调，仅作为领域适配器而非真正的手术基础模型，难以泛化到多样化的手术目标。

Method: 提出分层概念演化预训练范式：1）通过具有父子查询嵌入的分层掩码解码器建立腹腔镜概念层次结构，将不同实体统一到具有跨粒度语义一致性的可扩展知识结构中；2）提出置信度驱动的演化标注，基于分层一致性迭代生成和过滤伪标签，逐步将无标注图像中的可靠样本纳入训练，构建了包含114K图像-掩码对的大规模基准LapBench-114K。

Result: 大量实验表明，LapFM显著优于最先进的方法，为通用腹腔镜分割的粒度自适应泛化建立了新标准。

Conclusion: LapFM是一个专门设计用于从大规模无标注手术图像中演化鲁棒分割能力的基础模型，通过分层概念演化预训练解决了现有方法的局限性，在腹腔镜分割任务上取得了突破性进展。
...
Method: 提出一个统一的学习框架，将整个色彩校正流程集成在单一模型中，联合利用高分辨率RGB传感器和辅助低分辨率多光谱传感器的数据，通过重构两种不同的最先进图像到图像架构来展示框架的灵活性和通用性。

Result: 实验表明，该方法相比仅使用RGB和多光谱驱动的基线方法，色彩准确性和稳定性均有提升，误差减少高达50%。

Conclusion: 该端到端学习框架能够充分利用多光谱信息，显著提升色彩校正性能，具有灵活性和通用性，为移动设备和消费级设备的多光谱成像应用提供了有效的解决方案。
...
TL;DR: 该论文提出了一种结合子模子集选择和梯度不确定性估计的框架，以解决现有基于子集选择的深度视觉模型解释方法在分布外（OOD）场景下性能下降的问题。
...
Method: 提出一个框架，将子模子集选择与基于梯度的逐层不确定性估计相结合。通过自适应权重扰动估计不确定性，并利用这些估计指导子模优化，确保多样化和信息丰富的子集选择，无需额外训练或辅助模型。

Result: 经验评估表明，该框架不仅缓解了现有方法在OOD场景下的弱点，还在ID设置中带来了改进。相比现有方法，能产生更鲁棒、稳定且信息丰富的解释。
...
TL;DR: 提出了一种轻量级SAM+CSRT跟踪方法，结合SAM精确初始化、CSRT跟踪器和球衣颜色外观模型，用于足球视频中的球员跟踪，在遮挡恢复和身份一致性方面表现良好。
...
Method: 提出团队感知跟踪系统：1) 使用SAM进行精确初始化；2) CSRT跟踪器进行连续跟踪；3) 基于HSV直方图的球衣颜色外观模型进行重识别以改善遮挡恢复。

Result: 在足球视频序列上达到7.6-7.7 FPS，内存使用稳定(~1880 MB)；轻度遮挡下100%跟踪成功率，拥挤禁区场景(5名以上球员)90%成功率；外观重识别恢复50%重度遮挡；但长期离场遮挡重获取成功率仅8.66%。

Conclusion: SAM+CSRT组合在连续可见性场景中表现稳定，但需要更强的重识别机制处理长期离场情况；为资源受限环境下的足球跟踪系统部署提供了实用指导。
...
TL;DR: ContextDrag：基于拖拽的图像编辑新范式，通过上下文建模和特征注入技术，在保持语义和纹理一致性的同时实现精确的拖拽编辑
...
Conclusion: ContextDrag通过有效利用参考图像的上下文信息，实现了在保持语义和纹理细节一致性的同时进行精确的拖拽编辑，无需微调或反转操作
...
<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅渲染器存在碎片化、笨重、受限于传统管线的问题，导致部署困难，且对动态内容和生成模型支持有限。

Method: 基于WebGPU构建高效渲染器，采用每帧ONNX推理实现动态神经处理；引入标准化的高斯生成器合约，支持插件式算法；提供three.js插件库和简洁的TypeScript API。
...
TL;DR: PCI框架通过分析概念插入成功率来研究扩散模型中概念形成的时间动态，揭示不同概念在不同时间步的锁定机制，为文本驱动图像编辑提供有效干预时机。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型评估主要关注最终输出，但生成过程是动态轨迹，理解概念何时形成并锁定对于模型可控性、可靠性和可预测性至关重要。

Method: 提出PCI（Prompt-Conditioned Intervention）框架，无需训练且模型无关，通过概念插入成功率（CIS）分析概念在扩散时间中的动态形成过程。

Result: 应用于多个先进文本到图像扩散模型和广泛概念分类，发现不同扩散模型具有多样化的时间行为，同一概念类型内特定概念在轨迹的不同阶段形成。

Conclusion: PCI揭示了扩散模型中概念形成的时间动态，为文本驱动图像编辑提供了有效的干预时机，无需访问模型内部或训练，在语义准确性和内容保留之间取得更好平衡。
...
TL;DR: 首个针对多相机阵列的实时3D高斯溅射重建框架，通过层次化相机初始化、轻量级多相机束调整、无冗余高斯采样和频率感知优化调度，在2分钟内重建数百米3D场景。


<details>
  <summary>Details</summary>
Motivation: 现有基于单目RGB流的实时3D高斯溅射重建方法受限于有限视野，无法实现完整的3D覆盖。多相机阵列能从根本上解决这一限制，但缺乏相应的实时重建框架。

Method: 1) 层次化相机初始化方案：无需标定实现粗略相机间对齐；2) 轻量级多相机束调整：稳定轨迹同时保持实时性能；3) 无冗余高斯采样策略：减少高斯基元数量；4) 频率感知优化调度器：减少所需优化迭代次数。

Result: 仅使用原始多相机视频流，在2分钟内重建数百米3D场景，实现了前所未有的速度、鲁棒性和重建保真度，支持无漂移轨迹估计和高效在线重建。

Conclusion: 该方法首次实现了多相机阵列的实时3D高斯溅射重建，通过创新的相机初始化、束调整、采样和优化策略，在保持实时性能的同时显著提升了重建覆盖范围和质量。
...
TL;DR: 提出ReasonBreak对抗框架，专门针对多模态大推理模型的地理位置隐私威胁，通过概念感知扰动破坏其分层推理链，相比现有方法显著提升隐私保护效果


<details>
  <summary>Details</summary>
Motivation: 多模态大推理模型通过分层思维链推理能够从个人图像中推断精确地理位置，现有隐私保护技术主要针对感知模型，对MLRMs的多步推理过程无效，需要专门针对推理过程的保护方法

Method: 提出ReasonBreak对抗框架，基于关键洞察：有效的地理推理破坏需要与概念层次对齐的扰动而非均匀噪声。该方法战略性地针对推理链中的关键概念依赖，生成使特定推理步骤无效的扰动，并级联影响后续推理阶段。为此贡献了包含6,341张超高分辨率图像和分层概念标注的GeoPrivacy-6K数据集
...
Conclusion: 这项工作为对抗基于推理的隐私威胁建立了新范式，证明了概念感知扰动在破坏MLRMs分层推理方面的有效性，为多模态推理模型的隐私保护提供了创新解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有条件扩散模型存在对齐错误和幻觉问题，需要在生成后使用自动对齐检测工具（如Best-of-N），但这需要等待完整生成过程，计算成本高昂。本文假设文本/图像不对齐可以在去噪过程中早期检测，实现实时对齐评估。

Method: 提出NoisyCLIP方法，在噪声潜在空间中测量语义对齐。这是首次探索和基准测试在反向扩散过程中使用双编码器进行提示到潜在空间的对齐检测。

Result: NoisyCLIP在Best-of-N设置中将计算成本降低50%，同时达到98%的CLIP对齐性能。该方法能够在生成过程中进行实时对齐评估，在不牺牲语义保真度的前提下降低成本。

Conclusion: NoisyCLIP证明了在去噪过程中早期检测文本-图像对齐的可行性，为实时对齐评估提供了有效解决方案，显著降低了计算成本，同时保持了高质量的对齐性能。
...
TL;DR: OCCDiff：一种在占用函数空间应用潜在扩散的方法，用于从不同分辨率的LiDAR点云中灵活重建高质量的建筑3D轮廓
...
Method: 提出OCCDiff方法，在占用函数空间应用潜在扩散。结合潜在扩散过程与函数自编码器架构，生成可在任意位置评估的连续占用函数。引入点编码器为扩散学习提供条件特征，约束最终占用预测，并为潜在编码器插入多模态特征。采用多任务训练策略增强模型性能。

Result: 实验结果表明，该方法生成物理一致的样本，对目标分布具有高保真度，并对噪声数据表现出鲁棒性。

Conclusion: OCCDiff通过潜在扩散和函数空间表示，有效解决了LiDAR点云建筑重建中的点密度变化和噪声干扰问题，能够生成高质量、连续的3D建筑轮廓。
...
<details>
  <summary>Details</summary>
Motivation: 现有GUI grounding模型存在显著的坐标预测不稳定性，微小视觉扰动（如裁剪几个像素）就会导致预测结果在正确与错误之间剧烈波动，严重影响模型性能，特别是对于高分辨率和小UI元素的样本

Method: 提出Multi-View Prediction (MVP)训练免费框架，包含两个组件：1) 注意力引导的视图提议，基于指令到图像的注意力分数生成多样化视图；2) 多坐标聚类，通过选择最密集空间簇的质心来集成预测结果
...
TL;DR: 本文提出了一种统一的多模态油画生成与编辑框架，通过空间对齐、语义增强和自监督风格转换技术，实现了交互式油画创作，解决了现有方法在数据分布限制和风格一致性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 油画作为融合人类抽象思维与艺术表达的高级媒介，其复杂的笔触动态和风格化特性给数字生成与编辑带来了巨大挑战。现有技术受限于训练数据分布，主要关注真实照片的修改，难以实现高质量的油画风格生成与编辑。

Method: 1. 在训练阶段采用空间对齐和语义增强条件策略，将掩码和草图映射为空间约束，从参考图像和文本编码上下文嵌入作为特征约束，实现对象级语义对齐。
2. 提出基于笔触渲染的自监督风格转换流程，模拟油画修复的修复动态，将真实图像转换为保留笔触纹理的风格化油画，构建大规模配对训练数据集。
3. 在推理阶段使用AdaIN算子集成特征，确保风格一致性。

Result: 广泛的实验表明，该交互式系统能够在保持油画艺术品质的同时实现细粒度编辑，在风格化油画生成与编辑方面达到了前所未有的想象力实现水平。

Conclusion: 本文提出的统一多模态框架成功解决了油画数字生成与编辑的关键挑战，通过创新的训练策略、数据增强方法和特征集成技术，实现了高质量的交互式油画创作，为艺术创作提供了新的可能性。
...
TL;DR: Photo3D是一个通过GPT-4o-Image生成图像数据来提升3D生成真实感的框架，通过结构对齐的多视角合成和细节增强技术，解决了现有3D生成器在真实外观表现上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有3D原生生成器在几何结构方面取得了进展，但在真实外观表现上仍然不足。主要障碍在于缺乏多样化和高质量的具有丰富纹理细节的真实世界3D资产，因为捕获此类数据存在固有困难，包括场景尺度多样、物体非刚性运动以及3D扫描仪精度有限。

Method: 1. 设计结构对齐的多视角合成流水线，构建细节增强的多视角数据集与3D几何配对；2. 提出真实细节增强方案，利用感知特征适应和语义结构匹配来强制实现具有真实细节的外观一致性，同时保持与3D原生几何的结构一致性；3. 为不同的3D原生生成器提供专门的训练策略，支持几何-纹理耦合和解耦的生成范式优化。

Result: 实验表明，Photo3D能够很好地泛化到不同的3D原生生成范式，并在真实感3D生成方面达到了最先进的性能。

Conclusion: Photo3D框架通过利用GPT-4o-Image生成的图像数据，结合结构对齐的多视角合成和细节增强技术，有效解决了3D生成中真实感不足的问题，为不同3D原生生成器提供了通用的真实感增强方案。
...
TL;DR: Fast-ARDiff提出了一种统一的AR-扩散混合框架，通过联合优化两个组件来加速生成：使用熵感知推测策略减少AR拒绝率，并通过动态调度器和联合蒸馏实现极简步数的扩散解码。


<details>
  <summary>Details</summary>
Motivation: 现有的AR-扩散混合范式结合了AR的结构化建模和扩散模型的光照真实合成能力，但由于顺序AR生成和迭代去噪过程，存在高延迟问题。需要解决这一瓶颈，实现更高效的混合生成。

Method: 1. 熵感知推测策略：鼓励草稿模型产生与目标模型熵特征对齐的高熵表示，缓解熵不匹配和草稿过度自信导致的高拒绝率
2. 动态调度器：将扩散解码集成到端到端框架中，优先优化AR部分以指导后续扩散步骤
3. 联合蒸馏框架：结合轨迹匹配和分布匹配，确保稳定训练和极简步数下的高质量合成
4. 推理时使用AR模块的浅层特征熵预过滤低熵草稿，避免冗余计算

Result: 在ImageNet 256×256上，TransDiff实现了4.3倍无损加速；在文本条件生成任务上，NextStep-1实现了3倍加速。达到了最先进的加速效果。

Conclusion: Fast-ARDiff通过联合优化AR和扩散组件，有效解决了AR-扩散混合范式的高延迟问题，实现了显著的加速效果，同时保持了生成质量。
...
TL;DR: 提出了一种无需迭代的固定点估计器用于扩散反演，通过误差近似方法避免了传统固定点迭代的高计算成本和超参数选择问题，在重建任务中表现优于DDIM反演和其他基于固定点迭代的方法。
...
Method: 首先从理想反演步骤推导出固定点的显式表达式，但该表达式包含未知的数据预测误差。然后引入误差近似方法，使用前一个反演步骤的可计算误差来近似当前步骤的未知误差，从而得到一个可计算的近似固定点表达式，这是一个具有低方差的无偏估计器。

Result: 在NOCAPS和MS-COCO两个文本-图像数据集上评估重建性能。与DDIM反演和其他基于固定点迭代的方法相比，该方法在重建任务中实现了更一致和优越的性能，无需额外迭代或训练。

Conclusion: 提出的无需迭代的固定点估计器有效解决了传统固定点迭代方法的高计算成本和超参数选择问题，在扩散反演任务中表现出色，为扩散模型的反演问题提供了一种高效且性能优越的解决方案。
...
TL;DR: 提出一个大规模自动化框架，用于发现和解释人类大脑皮层的视觉表征，通过无监督分解方法发现可解释模式，并用自然语言描述其视觉含义。


<details>
  <summary>Details</summary>
Motivation: 理解人类大脑如何表示视觉概念以及在哪些脑区编码这些表征是一个长期挑战。现有研究通常规模小、依赖人工检查、关注特定区域和属性，缺乏系统性验证。

Method: 采用两阶段方法：1) 通过无监督数据驱动分解方法从fMRI活动中发现候选可解释模式；2) 通过识别最能激发这些模式的自然图像集，并生成描述其共享视觉含义的自然语言解释。引入自动化流程测试多个候选解释，分配定量可靠性分数，为每个体素模式选择最一致的描述。
...
TL;DR: 本文提出了一种模块化的神经图像信号处理框架，能够处理原始输入并生成高质量的显示参考图像，具有高度模块化设计，支持用户交互式照片编辑。
...
Method: 采用模块化神经ISP框架，将整个渲染过程分解为多个可控制的中间阶段，支持不同容量变体（0.5M-3.9M参数），并开发了用户交互式照片编辑工具。

Result: 在多个测试集上获得具有竞争力的定性和定量结果，实现了高渲染精度，同时提高了可扩展性、可调试性、对未见相机的泛化能力以及匹配不同用户偏好风格的灵活性。

Conclusion: 模块化神经ISP框架不仅实现了高质量图像渲染，还提供了对渲染过程的完全控制，支持无限次可编辑的重新渲染，为照片编辑应用提供了强大工具。
...
TL;DR: 提出一种自适应调整伪标签的方法，通过考虑图像内置信度分布和动态平衡受域偏移影响最大的类别，在持续测试时适应中提升语义分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有持续测试时适应方法通常依赖固定或批量级阈值，无法考虑不同类别和实例之间的难度差异，这在需要密集多类预测的语义分割中尤为问题。

Method: 提出自适应调整伪标签的方法，根据每张图像内的置信度分布动态调整伪标签，并动态平衡学习向受域偏移影响最大的类别，实现细粒度的类别和实例感知适应。

Result: 在八个CTTA和TTA场景（包括合成到真实和长期偏移）的广泛实验中，该方法始终优于最先进技术，为演化条件下的语义分割设定了新标准。

Conclusion: 通过自适应调整伪标签和动态平衡类别学习，该方法在持续测试时适应中产生更可靠的监督，减轻错误累积，显著提升语义分割在演化域中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 肿瘤微环境作为预后生物标志物来源具有巨大潜力，但需要能够捕捉不同细胞类型间复杂相互作用的分析方法。现有方法在捕获细胞邻域中局部和全局关系方面存在局限。

Method: 提出HiGINE分层图方法：1) 从多重免疫荧光图像中编码细胞邻域的局部和全局相互关系；2) 整合细胞类型和形态学信息；3) 通过多模态融合将癌症分期与mIF衍生特征聚合；4) 预测患者生存期（短期vs长期）。

Result: 在两个公共数据集上验证显示：1) 改善了风险分层能力；2) 模型具有鲁棒性；3) 展现出良好的泛化性能；4) 多模态融合进一步提升了预测性能。

Conclusion: HiGINE能够有效利用肿瘤微环境的复杂细胞相互作用信息，结合临床数据提升肺癌患者生存预测和风险分层，为精准医疗提供有力工具。
...
TL;DR: 提出全自动手术视频对齐方法，通过检测照明系统移动、重新对齐帧并选择遮挡最少的摄像头，生成固定视角的手术区域视频


<details>
  <summary>Details</summary>
Motivation: 开放手术视频对教育和研究很重要，但外科医生经常遮挡摄像头视野。现有方法需要安装多个摄像头并手动对齐图像，过程繁琐且需要后期处理

Method: 识别照明系统移动的帧，重新对齐这些帧，选择遮挡最少的摄像头，生成固定视角的连续视频。还实现了多种合成选项供用户选择

Result: 用户研究表明，该方法生成的视频在确认手术区域和观看舒适度方面优于传统方法。视频质量也有提升，并评估了不同合成选项的外科医生偏好

Conclusion: 提出的全自动对齐方法能有效解决手术视频遮挡问题，生成高质量、易于观看的手术视频，为医学教育和研究提供更好的工具
...
TL;DR: 该研究使用深度学习技术（YOLOv8s和MobileNetV3L）在光学和全息显微镜图像中自动检测和分类花粉粒，特别针对兽医细胞学应用。通过数据集扩展和自动标注技术，显著提升了全息图像的检测和分类性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索深度学习在花粉粒检测和分类中的应用，特别是在光学和全息显微镜图像中，重点关注兽医细胞学应用场景，旨在提高检测效率和准确性。

Method: 采用YOLOv8s进行目标检测，MobileNetV3L进行分类任务。针对全息图像性能较低的问题，使用自动标注和边界框区域扩大的数据集扩展技术来改善模型性能。

Result: 在光学图像上，检测达到91.3% mAP50，分类达到97%总体准确率。全息图像初始性能较低（检测2.49% mAP50，分类42%准确率），通过技术改进后提升至检测13.3% mAP50，分类54%准确率。

Conclusion: 研究表明，深度学习技术可以与成本效益高的无透镜数字全息显微镜设备结合使用，至少在图像分类任务中是可行的，为花粉粒的自动检测和分类提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究发现CLIP模型中模板-样本相似性(TSS)会引入偏差，使模型依赖模板接近度而非真实的样本-类别对齐，这降低了分类的准确性和鲁棒性。需要解决这种模板诱导的偏差问题。

Method: 提出两阶段框架：1)预训练阶段使用空提示（不含类别信息的文本输入）来揭示和减少CLIP编码器中的模板诱导偏差；2)少样本微调阶段使用偏差校准损失来强制图像与其类别之间的正确对齐。

Result: 在多个基准测试上的实验表明，该模板校正方法显著减少了由TSS引起的性能波动，获得了更高的分类准确性和更强的鲁棒性。

Conclusion: 通过使用空提示和偏差校准损失，可以有效解耦CLIP中的模板偏差，使模型更关注相关视觉线索，从而提高少样本学习的性能。
...
Method: 提出OpenMonoGS-SLAM框架，统一3D高斯溅射与开放集语义理解。利用视觉基础模型（MASt3R用于视觉几何，SAM和CLIP用于开放词汇语义），仅通过自监督学习目标实现单目相机跟踪和建图。特别设计了内存机制来管理高维语义特征，构建高斯语义特征图。

Result: 实验结果表明，该方法在封闭集和开放集分割任务中达到或超过现有基线性能，且不依赖深度图或语义标注等辅助传感器。

Conclusion: OpenMonoGS-SLAM是首个将3D高斯溅射与开放集语义理解相结合的单目SLAM框架，能够在开放世界环境中实现准确的相机跟踪、建图和丰富的语义理解，为空间AI应用提供了强大的基础。
...
<details>
  <summary>Details</summary>
Motivation: 在缺乏机械稳定器的情况下，相机拍摄时会产生不可避免的旋转运动，导致长曝光场景下的透视模糊。从光学角度看，这种模糊是深度位置相关的：不同空间位置的物体即使在相同成像设置下也会产生不同的模糊程度

Method: 通过分析视频流的模糊模式来估计度量深度和密集轨迹。使用现成的视觉编码器和点跟踪器提取视频信息，通过窗口化嵌入和多窗口聚合估计深度图，利用视觉语言模型对光学算法的稀疏轨迹进行密集化处理

Result: 在多个深度数据集上的评估表明，该方法在大深度范围内表现出色，具有良好的泛化能力。在手持拍摄设置中，光学算法相对于真实轨迹具有优越的精度，密集重建保持强准确性

Conclusion: 该方法通过分析相机运动引起的透视模糊模式，成功实现了深度估计和轨迹重建，为无稳定器拍摄场景提供了有效的解决方案
...
TL;DR: 提出一个仅使用单目RGB图像和自然语言指令的无人机视觉语言导航统一框架，通过多任务学习联合优化空间感知、轨迹推理和动作预测，显著提升轻量化无人机部署的实用性。


<details>
  <summary>Details</summary>
Motivation: 现有无人机视觉语言导航方法通常依赖全景图像、深度输入或里程计来支持空间推理和动作规划，这些要求增加了系统成本和集成复杂度，阻碍了轻量化无人机的实际部署。

Method: 1) 将导航任务建模为下一个令牌预测问题；2) 通过提示引导的多任务学习联合优化空间感知、轨迹推理和动作预测；3) 提出关键帧选择策略减少视觉冗余；4) 引入动作合并和标签重新加权机制缓解长尾监督不平衡问题。

Result: 在Aerial VLN基准测试中，该方法在仅使用单目RGB图像的挑战性设置下，在已见和未见环境中均取得强劲结果，显著优于现有RGB-only基线方法，并缩小了与最先进全景RGB-D方法的性能差距。

Conclusion: 该研究提出了一个仅需单目RGB观察的统一无人机视觉语言导航框架，通过创新的多任务学习设计和训练策略，有效解决了轻量化无人机实际部署中的成本与复杂度问题，为实际应用提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前图像生成模型内部过程是"黑盒"，限制了人类观察和干预能力，影响模型可靠性、安全性和可控性，且其非人类化工作流程难以解释

Method: 提出Chain-of-Image Generation框架：1)使用LLM将复杂提示分解为简单逐步指令；2)图像生成模型按计划逐步生成和编辑图像；3)每个步骤聚焦单个语义实体实现直接监控

Result: 1)开发了CoIG可读性和因果相关性两个新指标评估中间步骤；2)缓解实体崩溃问题；3)显著增强定量可监控性；4)在组合鲁棒性方面与基线模型竞争；5)框架与模型无关可集成任何图像生成模型

Conclusion: CoIG框架通过将图像生成重构为顺序语义过程，实现了类似CoT在LLM中的优势，显著提高了生成过程的可监控性和可解释性，同时保持生成质量
...
<details>
  <summary>Details</summary>
Motivation: 车载摄像头需要实时边缘推理，但轻量模型难以捕捉细粒度行为特征，ROI方法增加计算成本，需要在效率和准确性之间取得平衡。

Method: 结合显著性驱动的Top-K ROI池化和融合分类进行局部特征提取，动态ROI路由仅对高难度样本应用ROI推理，使用伪域标注和对抗学习学习域不变特征。

Result: 在State Farm分心驾驶员检测数据集上，C-DIRA比现有轻量模型显著减少FLOPs和延迟，同时保持高精度，在模糊和低光等视觉退化条件下表现鲁棒。

Conclusion: C-DIRA框架有效实现了紧凑性、效率和泛化能力，为轻量级驾驶员行为识别提供了有效解决方案。
...
TL;DR: 提出MNAME框架，通过内存银行机制提供大量负样本，实现去噪生成模型的自包含对比学习，无需外部编码器，在ImageNet-256上达到SOTA的FID 2.40
...
Method: 提出MNAME框架，采用内存银行机制维护跨训练迭代的动态负样本队列，将负样本数量与小批量大小解耦，使用低维投影头最小化内存和带宽开销，无需外部编码器。

Result: 在ImageNet-256上，MNAME在40万步内实现了最先进的FID 2.40，显著优于可比方法，同时实现了更快的收敛速度和更高的生成质量。

Conclusion: MNAME框架通过自包含的对比学习方法，消除了对外部预训练模型的依赖，不增加推理时的参数或计算成本，实现了更高效的生成模型训练和更好的生成质量。
...
TL;DR: 本文探索了将SAM 3应用于遥感开放词汇语义分割任务，无需训练即可实现精确的土地覆盖分类


<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的训练免费开放词汇语义分割方法在精确定位方面存在挑战，需要复杂流程整合多个模块，特别是在遥感场景中存在大量密集小目标时。最近提出的SAM 3统一了分割和识别功能，为遥感开放词汇语义分割提供了新可能。

Method: 1. 实现掩码融合策略，结合SAM 3的语义分割头和Transformer解码器（实例头）输出，充分利用两个头的优势；2. 利用存在头（presence head）的存在分数过滤场景中不存在的类别，减少由于地理空间场景中词汇量大和补丁级处理导致的误报。

Result: 在广泛的遥感数据集上进行评估，实验表明这种简单适配方法取得了有前景的性能，证明了SAM 3在遥感开放词汇语义分割任务中的潜力。

Conclusion: SAM 3在无需训练的情况下，通过简单的掩码融合和存在分数过滤策略，就能在遥感开放词汇语义分割任务中表现出良好性能，展示了其在遥感应用中的潜力。
...
Result: 实验在CNN和Transformer模型上表明：(1)分类重新加权在捕捉个体层面差异方面存在局限性；(2)基于分布的重新加权表现更优，特别是使用Fidelity相似性(FS)、Wasserstein距离(WD)、Hellinger度量(HM)和Harmonic Mean相似性(HS)时。
...
TL;DR: 提出了一种结合3D渲染和约束扩散模型的工作流程，用于生成高质量、逼真的双孢蘑菇合成图像，并发布了两个合成数据集，在零样本设置下实现了最先进的蘑菇分割性能。


<details>
  <summary>Details</summary>
Motivation: 工业蘑菇种植依赖计算机视觉进行监测和自动收获，但开发准确的检测和分割模型需要大量精确标注的数据集，成本高昂。合成数据虽可扩展，但通常缺乏足够的真实感以泛化到真实场景。

Method: 提出了一种新颖的工作流程，将Blender中的3D渲染与约束扩散模型相结合，自动生成高质量、逼真的双孢蘑菇合成图像。该方法保持对3D场景配置和标注的完全控制，同时无需专业计算机图形学知识即可实现照片级真实感。
...
TL;DR: 本文提出了一种基于偏度指导的剪枝方法，用于压缩多模态Swin Transformer模型，在联邦学习环境中实现高效的模型压缩和隐私保护。


<details>
  <summary>Details</summary>
Motivation: 高性能计算机视觉模型在医学影像中表现出色，但模型计算密集、体积庞大，不适合边缘设备部署。同时，严格的隐私限制阻碍了集中式数据管理，需要采用联邦学习。

Method: 提出偏度指导的剪枝方法，基于多头自注意力层和多层感知机层输出分布的统计偏度，有选择性地对这些层进行剪枝。该方法在水平联邦学习环境中验证。

Result: 实验表明，紧凑型Swin Transformer模型尺寸减少了约36%，同时保持了原有的准确率。在联邦学习环境中能够维持性能并显著降低模型复杂度。
...
TL;DR: Wan-Move是一个简单可扩展的视频生成框架，通过将原始条件特征直接转换为运动感知特征，实现对视频合成的精确运动控制，无需修改基础模型架构。
...
Method: 使用密集点轨迹表示物体运动，实现细粒度场景控制；将轨迹投影到潜在空间，沿每条轨迹传播第一帧特征，生成对齐的时空特征图作为更新的潜在条件；该特征图可直接集成到现成的图像到视频模型中作为运动指导，无需架构修改。

Result: 通过规模化训练，Wan-Move能够生成5秒、480p视频，其运动可控性在用户研究中与Kling 1.5 Pro的商业Motion Brush相媲美；在MoveBench基准测试和公开数据集上的实验一致显示其优越的运动质量。

Conclusion: Wan-Move通过将条件特征直接运动化的简单方法，实现了精确、高质量的视频运动控制，无需辅助运动编码器，使基础模型微调易于扩展，为视频生成提供了实用的运动控制解决方案。
...
TL;DR: 提出自精炼扩散框架，利用可解释AI技术检测生成图像中的伪影和不真实区域，通过缺陷激活图指导扩散过程，显著提升图像生成质量。
...
Method: 提出自精炼扩散框架，采用基于可解释AI的缺陷高亮器生成缺陷激活图（FAMs）。这些FAMs在正向过程中放大缺陷区域的噪声，在反向过程中聚焦于这些区域，从而改善重建质量。
...
<details>
  <summary>Details</summary>
Motivation: 文档阴影去除对提高数字化文档清晰度至关重要。阴影常常会模糊或扭曲文本边缘和线条等高频细节结构，因此需要在去除阴影的同时保护这些精细结构。

Method: 提出MatteViT框架，采用两种保护策略：1) 轻量级高频放大模块(HFAM)，分解并自适应放大高频分量；2) 连续亮度阴影蒙版，使用自定义蒙版数据集和阴影蒙版生成器提供精确的空间指导。

Result: 在RDD和Kligler等公开基准测试中，MatteViT达到了最先进的性能。该方法在下游任务（如光学字符识别）中能更好地保护文本级细节，提高了识别性能。

Conclusion: MatteViT为现实世界文档阴影去除提供了一个鲁棒且实用的解决方案，通过空间和频域信息的结合，在去除阴影的同时有效保护了高频细节结构。
...
Method: 提出了无需训练的双曲适配器（T-DHA），在双曲空间（使用庞加莱球模型）中建模视觉语言语义概念的层次化树结构关系，结合负学习技术，用更少的特征维度实现更准确和鲁棒的分类。

Result: 在多个数据集上的实验结果表明，T-DHA方法在少样本图像识别和领域泛化任务中显著优于现有的最先进方法。

Conclusion: 双曲空间建模视觉语言层次关系是一种有效的适应方法，能够在不进行训练的情况下提升模型性能，为视觉语言模型的领域适应提供了新思路。
...
TL;DR: InfiniteVL是一种线性复杂度的视觉语言模型架构，结合滑动窗口注意力和Gated DeltaNet，在资源受限下实现竞争性多模态性能，显著超越先前线性复杂度VLMs，匹配Transformer VLMs性能，同时保持恒定延迟和内存占用。
...
Method: 提出InfiniteVL架构，结合滑动窗口注意力（SWA）和Gated DeltaNet实现线性复杂度。采用三阶段训练策略：蒸馏预训练、指令调优和长序列SFT。使用不到领先VLMs所需训练数据的2%。

Result: 显著超越先前线性复杂度VLMs，匹配领先Transformer VLMs性能，保持有效长期记忆。相比FlashAttention-2加速的类似大小Transformer VLMs，实现超过3.6倍推理加速，保持恒定延迟和内存占用。在流式视频理解中维持稳定24 FPS实时预填充速度。

Conclusion: InfiniteVL成功解决了窗口注意力和线性注意力的局限性，在资源受限下实现竞争性多模态性能，同时保持线性复杂度和恒定内存占用，为实际应用提供了高效解决方案。
...
TL;DR: 生成式方法通过约束解码器并反转它，能够实现组合泛化，而非生成式方法通常需要大规模预训练或额外监督才能改善泛化能力。


<details>
  <summary>Details</summary>
Motivation: 研究生成式和非生成式方法是否都能实现人类感知的标志性特征——组合泛化，以探讨生成方法是否是实现人类水平视觉感知的必要条件。

Method: 在组合数据生成过程下，形式化生成式（基于解码器）和非生成式（基于编码器）方法实现组合泛化所需的归纳偏置，理论分析这些偏置的可实施性，并通过在真实感图像数据集上训练多种方法进行实证验证。

Result: 理论表明，对编码器实施必要的归纳偏置通常不可行，而生成式方法可以通过约束解码器并反转它来实施这些偏置。实证结果显示，非生成式方法常常无法实现组合泛化，需要大规模预训练或额外监督；相比之下，生成式方法通过利用合适的解码器归纳偏置以及搜索和重放，显著改善了组合泛化能力。

Conclusion: 生成式方法能够通过约束解码器并反转它来实施实现组合泛化所需的归纳偏置，从而在没有额外数据的情况下显著改善组合泛化能力，这支持了生成方法在实现人类水平视觉感知中的重要性。
...
TL;DR: Tri-Bench基准测试评估视觉语言模型在几何推理任务中的表现，发现模型在平面三角形问题上准确率约69%，无法识别特殊三角形类型，且相机倾斜会降低性能，但物体干扰无显著影响。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型（VLMs）具有强大能力，但在实际场景变化下经常失败。需要可验证的几何推理来构建可信且可控的智能体AI，因此创建Tri-Bench基准来隔离相对几何推理并测试部署关键因素。

Method: 提出Tri-Bench基准，包含平面三角形问题，测试两种部署关键因素：相机姿态（平面vs倾斜）和场景上下文（10个日常物体的干扰）。使用单一固定提示评估四个最近的VLMs，提示中明确描述周围方形边框，通过单应性实现正确答案。评估六个简单任务，包括二元和连续目标。

Result: 总体准确率相对于3D地面真值约为69%（最佳约75%，最差约64%）。在图像平面2D投影上的准确率更高，约为72%。所有四个VLM在识别少数形状类别（等边、等腰、直角三角形）时一致失败，准确率降至约0%。相机倾斜使总体VLM准确率下降约4.1%。物体干扰对VLM准确率无显著影响。

Conclusion: 视觉语言模型未能正确利用提示中提供的显式参考框架提示，默认使用2D图像平面线索。这表明当前VLMs在几何推理方面存在局限性，特别是在利用显式几何约束方面，需要改进以实现可靠的可验证几何推理。
...
TL;DR: 提出了一种增强视觉语言模型在卫星图像中视觉定位能力的新方法，通过结构化定位机制和专用控制令牌实现语言与空间信息的联合推理
...
Method: 在预训练VLM上进行微调，同时通过专用控制令牌接口连接专门的定位模块，实现结构化定位机制，促进语言和空间信息的联合推理

Result: 在多个遥感基准测试中持续改进最先进水平，视觉定位任务相对之前方法提升了24.8%，证明了将结构化空间推理集成到VLM中的优势

Conclusion: 通过结构化定位机制增强VLM的视觉定位能力，为更可靠的现实世界卫星数据分析铺平道路，展示了语言与空间信息联合推理的重要性
...
TL;DR: 提出了一种GPU优化的旋转不变卷积框架，通过消除传统im2col步骤并利用对称旋转滤波器间的结构化数据共享，在保持精度的同时显著降低了计算成本和内存流量。
...
Result: 在广泛基准测试中，相比CUDNN，提出的卷积方法训练速度快20-55%，能耗降低15-45%，同时保持与最先进旋转不变方法相当的精度。在8方向设置下，256×256输入上实现45%加速和41%能耗节省，1024×1024输入上实现32%加速和23%能耗降低。集成到U-Net分割模型中，相比非旋转感知基线精度提升达6%。
...
TL;DR: VALOR：无需标注的视觉推理训练框架，通过AI验证器提升推理和视觉定位能力，在多种空间推理任务中超越开源和专有模型


<details>
  <summary>Details</summary>
Motivation: 现有视觉推理方法存在局限：语言思维链方法需要大规模监督数据，程序合成方法虽无需训练但存在逻辑缺陷和定位错误。需要一种无需标注的训练框架来同时提升推理和视觉定位能力。

Method: 提出基于AI验证器的无标注训练框架：1）LLM验证器通过强化学习优化LLM推理；2）VLM验证器通过自动硬负样本挖掘增强视觉定位。结合现代AI系统优势：语言模型分解空间查询，视觉专家模型通过VLM批评器改进。
...
TL;DR: UniLayDiff是一个统一的扩散Transformer模型，首次使用单一可训练模型解决多种内容感知布局生成任务，将布局约束作为独立模态处理，并通过LoRA微调整合关系约束。
...
Method: 提出UniLayDiff：一个统一的扩散Transformer模型，将布局约束作为独立模态，采用多模态扩散Transformer框架捕捉背景图像、布局元素和多样约束之间的复杂交互。通过LoRA微调在预训练后整合关系约束，实现统一的条件生成。

Result: 大量实验表明，UniLayDiff在从无条件到各种条件生成任务中均达到最先进性能，并且据作者所知，是首个统一完整范围内容感知布局生成任务的模型。

Conclusion: UniLayDiff首次实现了使用单一端到端可训练模型解决各种内容感知布局生成任务，不仅实现了统一的条件生成，还提升了整体布局质量，为图形设计自动化提供了更高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于单张图像的3D生成器虽然能恢复合理的几何结构，但由于其面向对象的训练方式，难以泛化到复杂、大规模的场景中，无法保证准确的结构和纹理。需要一种能够处理复杂场景、保持几何稳定性和视图一致性的方法。

Method: EvoScene采用三阶段迭代框架：1) 空间先验初始化；2) 视觉引导的3D场景网格生成；3) 空间引导的新视角生成。通过在2D和3D域之间交替优化，逐步改进场景的结构和外观。

Result: 在多样化场景上的实验表明，EvoScene相比现有基线方法，在几何稳定性、视图一致性纹理和未观察区域补全方面表现更优，能够生成可直接用于实际应用的3D网格。

Conclusion: EvoScene通过结合现有模型的互补优势，无需额外训练即可从单张图像重建高质量的3D场景，为实际应用提供了可直接使用的3D网格，在复杂场景重建方面取得了显著进展。
...
TL;DR: LiDAS是一种闭环主动照明系统，通过动态优化照明场来提升夜间感知性能，使白天训练的模型能在夜间零样本泛化，同时降低能耗。
...
Conclusion: LiDAS通过将现成头灯转变为主动视觉执行器，为鲁棒的夜间感知提供了经济高效的解决方案，并可与领域泛化方法互补增强鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 从视频中理解和重建动态场景的复杂几何与运动是计算机视觉中的重大挑战，现有方法通常计算密集或需要多个任务特定的解码器，限制了效率和可扩展性。

Method: 采用统一的Transformer架构，设计新颖的查询机制，避免密集的逐帧解码和多个任务特定解码器的复杂性，允许模型独立灵活地探测时空任意点的3D位置。

Result: D4RT在广泛的4D重建任务中超越了先前方法，建立了新的最先进水平，同时实现了轻量化和高度可扩展性，训练和推理效率显著。

Conclusion: D4RT通过统一的Transformer架构和创新的查询机制，为动态场景的4D重建提供了一种简单、高效且强大的解决方案，在多个任务上取得了最先进的结果。
...
Conclusion: 特征对齐是将视觉基础模型转换为高质量3D重建引擎的关键步骤，通过自改进流水线利用模型自身输出作为伪真值，显著提升几何一致性和下游任务性能
...
<details>
  <summary>Details</summary>
Motivation: 当前扩散变换器在视频生成方面取得了进展，但能够从过去观察和动作预测长时域未来的通用世界模型仍然研究不足，特别是在通用场景和多样化动作形式方面存在空白。

Method: 提出自回归去噪架构，使用时序因果注意力聚合过去观察并支持流式输出；采用噪声增强历史记忆避免对过去帧过度依赖；引入动作感知适配器直接注入动作信号；开发动作专家混合机制动态路由异构动作模态。

Result: 在多个数据集上的实验表明，Astra在保真度、长程预测和动作对齐方面优于现有最先进的世界模型，支持探索、操作和相机控制等多种真实世界任务。

Conclusion: Astra成功实现了交互式、一致且通用的长期视频预测，支持各种形式的交互，为通用世界模型的发展提供了有效解决方案。
