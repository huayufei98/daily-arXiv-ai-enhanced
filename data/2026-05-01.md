...
TL;DR: RecGen是一个生成式框架，能够从稀疏的RGB-D观测中概率性地联合估计多目标场景中物体和部件的形状、姿态，即使在严重遮挡和部分可见条件下也能表现优异，且相比SAM3D使用更少训练数据实现更优性能。


<details>
  <summary>Details</summary>
Motivation: 从稀疏观测中准确重建复杂多目标场景是计算机视觉的核心挑战，也是实现机器人可扩展可靠仿真的关键步骤。

Method: 利用组合式合成场景生成和强3D形状先验，构建生成式框架，从单张或多张RGB-D图像中概率性地联合估计物体和部件的形状及其在遮挡和部分可见下的姿态。

Result: 在复杂重度遮挡数据集上达到最先进水平；相比SAM3D减少约80%训练网格，但在几何形状质量上提升30.1%，纹理重建提升9.1%，姿态估计提升33.9%。

Conclusion: RecGen能够鲁棒处理严重遮挡、对称物体、物体部件以及复杂几何与纹理，在多种物体类型和真实环境中具有良好泛化能力。
...
TL;DR: 本文提出InterPartAbility，一种可解释的文本到图像行人重识别方法，通过显式部件匹配和短语-区域对齐实现可解释性，在保持竞争性检索性能的同时，在可解释性指标上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有TI-ReID方法虽检索性能强但决策不可解释；已有的可解释方法仅依赖slot-attention突出区域，无法将视觉区域与语义概念可靠绑定，解释局限于有限词汇的定性可视化。

Method: 提出开放词汇、轻量监督的补丁-短语交互模块(PPIM)；利用基于概念的部件短语引导模型关注对应图像区域；约束CLIP ViT自注意力产生空间集中的补丁激活，实现接地解释图；引入基于扰动评估的定量可解释性协议（含反事实区域掩码）。

Result: 在CUHK-PEDES和ICFG-PEDES基准上，在可解释性指标上达到SOTA，同时保持有竞争力的检索精度。

Conclusion: InterPartAbility实现了显式部件级匹配和短语-区域接地，为TI-ReID提供了定量可解释性评估框架，兼顾可解释性与检索性能。
...
TL;DR: 本文研究了知识蒸馏技术，将大模型能力迁移到轻量架构中，实现植物物种和疾病识别的高效部署。在70个模型实验中，蒸馏模型在保持较低计算成本的同时，性能可媲美显著更大的模型。


<details>
  <summary>Details</summary>
Motivation: 现有植物物种和疾病识别模型（如视觉Transformer或多模态基础模型）计算开销大，难以部署到移动或边缘设备等资源受限环境，阻碍了自动化生物多样性监测和精准农业系统的可扩展性。

Method: 采用知识蒸馏方法，将大规模预训练模型的知识迁移到更小、更高效的架构中。在Pl@ntNet300K-v2和Deep-Plant-Disease两个基准上，评估4种架构（两种ConvNeXt和两种ViT），覆盖从头训练、预训练初始化、有无蒸馏等多种训练方案，共训练70个模型。

Result: 知识蒸馏能持续提升不同任务和架构的性能。蒸馏后的模型在保持显著更低计算成本的同时，能够匹配更大模型的性能水平。

Conclusion: 知识蒸馏技术能够支持在真实环境应用中实现高效、可扩展的植物识别系统部署。
...
TL;DR: 提出一种混合量子-经典U-Net架构（HQ-UNet），在U-Net瓶颈处集成紧凑参数化量子电路，实现遥感图像语义分割，在LandCover.ai数据集上优于经典U-Net基线。


<details>
  <summary>Details</summary>
Motivation: 经典深度学习架构（如U-Net）需要大量参数来建模复杂空间关系；量子机器学习在高维图像上直接应用受限于近期量子硬件条件，需要探索参数高效的混合方案。

Method: 在经典U-Net的瓶颈层嵌入一个紧凑的参数化量子电路，采用非池化量子卷积模块对压缩后的编码器特征进行增强，保持量子部分浅层且参数高效。

Result: 在LandCover.ai数据集上，HQ-UNet平均IoU达0.8050，总体精度达94.76%，优于经典U-Net基线。

Conclusion: 紧凑型量子瓶颈能够在近期量子硬件约束下增强遥感图像分割的特征表示能力，混合量子-经典设计是参数高效密集预测的一个有前景的方向。
...
TL;DR: 本文提出VTBench框架，通过将时间序列转化为图表（折线图、面积图、条形图、散点图）并与原始序列进行多模态融合，实现可解释且有效的时间序列分类。实验表明图表单独使用在小数据集上有竞争力，多图表融合可提升准确率，多模态融合在视觉特征提供非冗余信息时表现更好。


<details>
  <summary>Details</summary>
Motivation: 现有时间序列分类方法大多仅依赖原始数值，忽视了图表化可视化表示的价值；已有的纹理编码方法（如GAF、RP）预处理复杂且可解释性差；图表可视化虽更具可解释性，但缺乏系统性的评估。

Method: 设计VTBench框架，将时间序列生成为轻量级、可解释的图表（折线图、面积图、条形图、散点图）；构建模块化架构，支持单图表视觉-数值融合、多图表视觉融合、以及全多模态融合三种策略；在31个UCR数据集上进行系统实验。

Result: (1) 纯图表模型在特定场景（尤其是小数据集）具有竞争力；(2) 多图表类型组合可提升分类准确率；(3) 多模态模型在视觉特征提供非冗余信息时提升或保持性能，但冗余时可能反而降低准确率。
...
TL;DR: 本文针对真实课堂场景中学生目标密集、小目标多、遮挡频繁和类别分布不平衡等问题，基于YOLOv8s提出改进的ALC-YOLOv8s模型，通过引入SPPF-LSKA、CFC-CRB、SFC-G2和ATFLoss等模块，在mAP50和mAP50-95上分别提升1.8%和2.1%，有效满足了复杂课堂场景下学生行为自动识别的需求。


<details>
  <summary>Details</summary>
Motivation: 解决真实课堂场景中学生目标密集、小目标众多、频繁遮挡以及类别分布不平衡等问题，提升学生课堂行为识别的准确性和鲁棒性。

Method: 基于YOLOv8s提出ALC-YOLOv8s模型，引入SPPF-LSKA增强上下文特征提取，采用CFC-CRB和SFC-G2优化多尺度特征融合，并引入ATFLoss提升对少数类和困难样本的学习能力。

Result: 与基线模型相比，改进模型在mAP50上提升1.8%，在mAP50-95上提升2.1%。

Conclusion: 提出的ALC-YOLOv8s模型能够很好地满足复杂课堂场景下学生行为自动识别的需求，优于多种主流检测方法。
...
TL;DR: YOSE是一个高效微调框架，通过批量可变长度索引（BVI）和扩散过程模拟器（DiffSim）模块，仅选择必要令牌处理，实现视频对象移除任务中高达2.5倍的速度提升，同时保持与基线相当的视觉质量。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散Transformer（DiT）的视频对象移除方法虽然效果好（如MiniMax Remover），但推理延迟高（仅约10FPS），原因在于即使只有小部分掩码区域需要处理，模型仍对整个时空令牌空间进行密集计算。因此需要一种能根据掩码信息自适应选择令牌的高效方法。

Method: YOSE框架包含两个核心组件：1) 批量可变长度索引（BVI）——一种可微分的动态索引算子，能根据掩码信息自适应选择必要令牌，支持跨样本的可变长度令牌处理；2) 扩散过程模拟器（DiffSim）模块——为未掩码令牌提供扩散过程近似机制，模拟未掩码区域在DiT自注意力中的影响，从而维持掩码令牌的语义一致性。

Result: YOSE实现了掩码感知加速，推理时间与掩码区域大小近似线性关系，而传统全令牌扩散方法的计算量不随掩码大小变化。实验表明，在70%的案例中实现高达2.5倍加速，且视觉质量与基线相当。

Conclusion: YOSE通过选择性令牌处理有效解决了DiT视频对象移除推理延迟高的问题，证明了掩码感知加速策略的可行性和有效性，代码已开源。
...
TL;DR: 本文提出了一种无需训练的自适应迭代定义优化框架，通过逐步改进类别定义（而非更新模型参数）来提升零样本网页内容分类性能，在10个URL类别、每类1000样本的人类标注基准上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有基于嵌入的零样本分类方法对类别定义质量高度敏感，定义模糊或不当会导致嵌入空间语义重叠，造成系统性错误分类。

Method: 利用大语言模型作为反馈驱动的定义优化器，提出三种定义优化策略：（1）示例引导型、（2）混淆感知型、（3）历史感知型；通过错误分类实例的结构化信号逐步改进类别描述。

Result: 在13种前沿嵌入基础模型上，迭代定义优化持续提升了分类性能，表明定义质量是嵌入系统中关键但被忽视的因素。

Conclusion: 类别定义质量是影响嵌入式零样本分类的核心因素，本文提出的无训练迭代优化框架能有效提升分类准确性，为未来研究提供了新方向。
...
TL;DR: JI-ADF是一个三模态深度学习框架，整合皮肤镜图像、临床照片和结构化患者元数据，通过联合表示学习、模态特定辅助监督和自适应决策融合机制，在MILK10k基准上实现了更优且均衡的皮肤病变分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有计算机辅助皮肤病变分类系统大多仅依赖皮肤镜图像，未能充分利用临床实践中可获取的多模态证据（如临床照片和患者元数据），导致诊断性能受限。

Method: 提出JI-ADF三模态框架，包含：1）联合多模态表示学习；2）模态特定辅助监督；3）自适应决策融合机制（动态校准各模态贡献）；4）多模态融合注意力（MMFA）模块，增强跨模态推理同时保留模态特定证据。

Result: 在MILK10k基准上，JI-ADF在各类病变中表现强且均衡，提升了灵敏度和Dice分数，同时保持了高特异性和良好的校准性能。

Conclusion: JI-ADF为真实临床环境下的多模态皮肤病变分类提供了可靠且实用的基础，消融研究、校准评估和Grad-CAM可视化进一步验证了其鲁棒性和临床意义。
...
TL;DR: 本文提出了一种基于多分支架构的高精度步态识别框架，通过深度融合步态动力学与身体形态特征，在CASIA-B基准上实现了正常步行下94.52%的Rank-1准确率，并在穿外套条件下达到骨架方法最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有步态识别方法难以全面捕捉和利用人体运动中的丰富生物特征，特别是在视角变化、衣着变化和携带物品等协变量干扰下表现不佳。

Method: 采用HRNet进行骨骼关键点估计，构建身体比例、步态速度和骨骼运动三个互补特征分支，使用ResNet-50提取深层特征，并通过基于通道注意力的多分支特征融合模块动态分配各分支贡献权重。

Result: 在CASIA-B跨视图多条件基准上，正常行走条件下Rank-1准确率达94.52%，穿外套条件下取得骨架方法中最佳识别性能。

Conclusion: 该多分支融合框架有效提升了复杂条件下步态识别的精度，验证了深度融合步态动力学与身体形态特征的有效性。
...
TL;DR: CasLayout提出了一种级联扩散框架，将3D室内场景生成分解为四个条件子阶段（家具数量/类别预测、尺寸/特征优化、空间关系建模、朝向包围盒生成），通过稀疏关系图和建筑元素约束，在低数据需求下实现了高保真度、多样性和可控性的场景布局生成。


<details>
  <summary>Details</summary>
Motivation: 现有3D室内场景生成方法面临数据稀缺、难以同时满足全局建筑约束与局部语义一致性的问题，且密集关系图会引入冗余生成错误。

Method: 提出级联扩散框架CasLayout，将场景生成分解为四个条件化子阶段；引入建筑元素（墙、门、窗）作为显式条件约束；设计稀疏关系图（sparse relation graph）并通过双向VAE编码到紧凑潜空间，替代传统全连接关系图。

Result: 在保真度和多样性指标上达到最先进水平，同时支持零样本任务（如图像到场景生成）并具备更好的可控性。

Conclusion: 级联分解策略有效降低了数据需求，稀疏关系图与建筑约束提升了布局的物理有效性和功能组织合理性，LLM/VLM的集成进一步拓展了零样本应用能力。
...
TL;DR: 提出DSCC框架，通过双阶段谱约束聚类分类器实现高光谱图像的端到端区域级分类，在保持边界精度的同时实现了优越的精度-效率平衡。


<details>
  <summary>Details</summary>
Motivation: 现有超像素方法存在内在矛盾：聚类将相似像素聚合为区域，但后续分类器在像素级别操作，破坏了区域一致性，无法保证区域级、边界对齐的分类效果。

Method: 提出双阶段谱约束聚类分类器（DSCC）：1）通过图像级多准则特征距离和局部感知分配正则化生成保留边界的谱超令牌；2）采用密度-隔离中心选择得到代表性中心；3）引入软标签方案编码类别比例以处理混合地物覆盖。

Result: 在WHU-OHS数据集上达到0.728 CF1，速度197.75 FPS，优于现有方法。

Conclusion: DSCC通过显式解耦聚类与分类的双阶段范式，有效实现了区域级、边界对齐的高光谱图像分类，验证了该范式的有效性和通用性。
...
TL;DR: CriticVLA是一个两阶段框架，先由VLA模型生成粗略轨迹，再通过基于VLA的评判器进行多模态评估和单步优化来细化轨迹，在Bench2Drive基准上达到73.33%的成功率，比现有方法提升约30%。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型在自动驾驶中仅用于直接映射输入到控制信号，未充分利用其评判能力来优化驾驶决策，限制了在复杂闭环场景中的性能。

Method: 提出CriticVLA两阶段框架：第一阶段生成粗略轨迹；第二阶段通过VLA-based评判器进行多模态评估和单步优化来细化轨迹。同时构建了包含1290万条标注轨迹的大规模合成数据集来增强评判器的推理和细化能力。

Result: 在Bench2Drive基准的闭环实验中，总成功率达73.33%，在复杂场景中相比最先进基线提升约30%。

Conclusion: 将VLA从"行动"扩展到"判断"的角色，通过评判与优化的两阶段框架显著提升了自动驾驶在复杂场景中的表现。
...
TL;DR: VeraRetouch是一个轻量级、全可微分的多任务照片修图框架，采用0.5B视觉语言模型作为核心智能，结合全可微分的修图渲染器，并引入百万级专业修图数据集AetherRetouch-1M+和强化学习后训练策略DAPO-AE，在多个基准上达到SOTA性能且支持移动端部署。


<details>
  <summary>Details</summary>
Motivation: 现有照片修图方法依赖不可微的外部软件，存在优化障碍、参数冗余高、泛化能力有限的问题。

Method: 1) 采用0.5B VLM作为中央智能制定修图计划；2) 开发全可微分的Retouch Renderer替代外部工具，通过解耦控制潜变量实现端到端像素级训练；3) 构建百万级数据集AetherRetouch-1M+；4) 提出DAPO-AE强化学习后训练策略增强自主审美认知。

Result: 在多个基准上达到SOTA性能，同时模型体积显著更小，支持移动端部署。

Conclusion: VeraRetouch通过全微分设计和轻量化架构有效解决了现有方法的优化障碍和参数冗余问题，为移动端高质量修图提供了可行方案。
...
TL;DR: 本文提出COHERENCE基准，用于评估多模态大语言模型在交错图文上下文中恢复细粒度图文对应关系的能力，涵盖4个领域共6,161个高质量问题，并进行了六类错误分析。
...
Method: 构建COHERENCE基准，包含来自4个代表性领域的交错图文内容及6,161个高质量问题，并设计了六类错误分析框架。

Result: 提出了COHERENCE基准及六类错误分析体系，可细粒度归因MLLMs在交错图文理解中的失败原因。

Conclusion: COHERENCE填补了交错图文细粒度理解评估的空白，有助于诊断和提升MLLMs在真实场景中的能力。
...
TL;DR: 本文系统研究了视觉语言模型（VLM）在自动驾驶中面对物理对抗攻击的跨架构可迁移性问题，发现攻击补丁在不同VLM架构间具有高达73-91%的迁移率，可在64.7-79.4%的关键决策窗口内持续操纵模型输出。


<details>
  <summary>Details</summary>
Motivation: 现有研究对VLM在自动驾驶中面对物理对抗攻击的鲁棒性，特别是攻击在不同VLM架构间的可迁移性了解不足，而攻击者不知车辆所用具体模型时存在实际风险。

Method: 选择了三种代表性VLM架构（Dolphins、OmniDrive、LeapVAD），在人行横道和高速公路两种场景下，通过在路边基础设施放置物理可实现补丁，构建迁移矩阵进行跨架构攻击有效性评估。

Result: 跨架构攻击迁移率高达73-91%（人行横道平均0.815，高速公路平均0.833），可在64.7-79.4%的关键决策窗口内实现持续的帧级操纵。

Conclusion: VLM驱动的自动驾驶系统在面对物理对抗攻击时存在严重的跨架构脆弱性，即使攻击者不知道目标模型具体架构，也能有效实施攻击。
...
TL;DR: 本文提出了一种针对包含干扰物的无约束真实世界场景的3D稀疏视角合成框架，通过参考引导的扩散模型视图细化和稀疏感知高斯复制策略，在稀疏无约束图像集合上实现了高质量3D渲染，性能显著优于现有方法（PSNR提升17.2%，SSIM提升10.8%，LPIPS降低4.0%）。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么处理无瞬态元素的约束稀疏视角合成，要么依赖密集无约束图像集合来增强真实场景的3D表示，缺乏同时应对稀疏性、无约束性和干扰物（瞬态元素）的解决方案。

Method: 1）参考引导的视图细化：利用瞬态掩码和参考图像，结合扩散模型增强3D表示并减少渲染伪影；2）伪视图生成：通过生成伪视图填补高斯场中的稀疏区域；3）稀疏感知高斯复制策略：在稀疏区域放大高斯分布以提升表示能力。

Result: 在公开数据集上，相比现有方法PSNR提升17.2%，SSIM提升10.8%，LPIPS降低4.0%，实现了高保真的3D渲染效果。

Conclusion: 该方法为无约束真实世界场景的3D渲染提供了可行路径，减少了数据采集的劳动成本，推动了稀疏视图合成在复杂真实环境中的应用。
...
TL;DR: Softmax-GS通过在重叠高斯区域引入softmax竞争机制，解决了3D Gaussian Splatting中的视图不一致和边缘模糊问题，实现了更优的重建质量和参数效率。


<details>
  <summary>Details</summary>
Motivation: 3D GS的高效率依赖于高斯在3D空间中不重叠的关键假设，这导致明显的伪影和视图不一致；同时高斯固有的扩散边界阻碍了锐利物体边缘的准确重建。

Method: 在重叠区域通过可学习参数控制竞争强度，在重叠高斯之间施加基于softmax的竞争机制，实现了从平滑颜色混合到清晰边界的连续谱系，并保持了阶不变性和透射率不变性。

Result: 在简单几何体上的消融实验验证了各组件有效性，在真实世界基准上达到了最先进的性能，提升了重建质量和参数效率。

Conclusion: Softmax-GS统一解决了视图不一致和边缘模糊两个问题，为3D GS提供了一种有效的改进方案。
...
TL;DR: 提出了CatSignal，一个贝叶斯启发的概率框架，用于在非语言智能体（如家猫）中从空间上下文、姿态动态和声学线索推断意图，通过上下文门控的专家乘积公式有效抑制上下文驱动的捷径预测，在多模态猫数据集上达到77.72%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现实环境中许多智能体（如宠物、婴儿）无法通过语言可靠传达意图，需要从不完整的行为观测中推断意图，但观测信号存在噪声和歧义，而上下文虽能提供先验信息却容易导致脆弱的捷径预测。

Method: 提出CatSignal框架，采用贝叶斯启发的概率建模：将空间上下文建模为先验约束，将行为观测（姿态动态和声学线索）作为证据，通过上下文门控的专家乘积（Product-of-Experts）公式计算后验意图分布。

Result: 在留一视频交叉验证下，CatSignal达到77.72%的总体准确率，优于特征拼接（71.83%）和强延迟融合基线；在歧义案例中显著减少了上下文驱动的捷径失败。

Conclusion: 该框架在非语言智能体意图推断中有效抑制了上下文捷径坍塌问题，提供了最强的总体准确率；未来可推广到其他非语言智能体场景。
...
TL;DR: 本文提出LA-Pose方法，利用逆动力学自监督预训练学习潜在动作表示，将其重新用于相机姿态估计，仅需少量标注数据即可达到甚至超越现有全监督方法的性能。在Waymo和PandaSet基准上，姿态精度比最新的前馈方法提高10%以上。


<details>
  <summary>Details</summary>
Motivation: 现有相机姿态估计方法依赖大量3D标注数据进行全监督训练，成本高昂且可扩展性差。本文探索利用自监督预训练（逆动力学）作为可扩展替代方案，减少对标注数据的依赖。

Method: 采用逆动力学和正动力学模型从大规模驾驶视频中学习潜在动作表示（类似Genie），然后将这些潜在动作特征重新用作相机姿态估计器的输入，仅需少量高质量3D标注数据进行微调，实现前馈高效推理。

Result: 在Waymo和PandaSet基准上，LA-Pose相比最新前馈方法姿态精度提升超过10%，使用标注数据量少数个数量级，达到与SOTA方法竞争甚至更优的性能。

Conclusion: 首次证明了逆动力学自监督学习在姿态估计任务中的强大潜力，为减少对3D标注依赖提供了有效途径。
...
TL;DR: EdgeFM是一个轻量级、智能体驱动的VLM/LLM推理框架，专为跨平台工业边缘部署设计。它通过去除非必要功能、封装智能体调优的内核优化为可复用技能库，并直接调用这些技能而非依赖闭源工具链，在主流平台（x86、NVIDIA Orin、地平线Journey）上实现了显著优于传统厂商工具链的推理性能（最高达TensorRT-Edge-LLM的1.49倍加速），为多样化的工业边缘场景提供了开源、生产级的解决方案。


<details>
  <summary>Details</summary>
Motivation: 现有VLM/LLM框架在工业边缘部署中面临两大问题：一是通用框架臃肿、延迟高、资源受限；二是厂商特定闭源工具链导致硬件锁定和跨平台适应性差。为此，作者希望利用现代AI智能体自动搜索和调优低层内核的能力，构建一个轻量化、可跨平台移植的开源推理框架。

Method: EdgeFM采用智能体驱动的设计思路：(1) 去除非必要功能以降低单请求延迟；(2) 将智能体调优后的内核优化封装为可复用的模块化技能库；(3) 直接调用这些技能，替代等待闭源实现，从而缩小与专有工具链的性能差距；(4) 原生支持x86、NVIDIA Orin以及地平线Journey等主流平台。

Result: 在大多数场景下，EdgeFM的性能明显优于传统厂商专用工具链。在NVIDIA Orin平台上，相比TensorRT-Edge-LLM实现了最高1.49倍的加速比。实验结果表明，EdgeFM在端到端推理性能上表现优越，为多样的工业边缘场景提供了可行的开源生产级解决方案。

Conclusion: EdgeFM作为首个端到端部署在地平线Journey平台上的VLA框架，有效提升了跨平台可移植性，打破了专有工具链长期主导的性能优势，证明了智能体驱动的方法在边缘工业VLM/LLM部署中的可行性与优越性。
...
TL;DR: Uni-HOI是一个统一框架，利用大语言模型和两个VQ-VAE将文本、人体运动和物体运动三种模态转化为统一token序列，通过两阶段训练策略实现多种HOI任务（文本生成HOI、物体运动生成人体运动、人体运动预测物体运动）的统一建模。


<details>
  <summary>Details</summary>
Motivation: 现有HOI方法通常针对特定任务设计独立架构，缺乏能处理多种条件输入的统一框架，无法充分利用文本、人体运动和物体运动之间的联合分布信息。

Method: 使用大语言模型(LLM)和两个运动专用VQ-VAE，将异构运动数据转换为与LLM输入兼容的token序列，实现三种模态的统一建模；采用两阶段训练策略：第一阶段在大规模HOI数据集上进行多任务学习，第二阶段针对特定任务进行微调。

Result: 在文本驱动HOI生成、物体运动驱动人体运动生成（可选带文本条件）以及人体运动驱动物体运动预测等多个HOI任务上均取得了优异性能。

Conclusion: Uni-HOI证明了统一框架能够有效实现多种HOI相关任务，在保持任务特定性能的同时实现了多模态条件输入的灵活处理。
...
TL;DR: 本文提出了首个大规模红外越野时序自由空间检测数据集IRON（含24,314张标注图像），并设计了基于记忆注意力机制的无流时序框架IRONet，在夜间越野场景下实现了SOTA性能（IoU 82.93%，F1 90.66%），且能泛化到RGB模态。
...
Method: 构建了IRON数据集（24,314张密集标注红外图像+同步RGB图像）；提出IRONet，一种无流时序检测框架，通过记忆注意力机制聚合历史上下文信息，并设计了专门的掩码解码器。

Result: 在IRON数据集上，IoU达82.93%（+1.19%），F1达90.66%（+0.71%），实现实时推理；在ORFD和Rellis数据集上对RGB模态表现出良好泛化能力。

Conclusion: 该工作为全天候越野自动驾驶和红外时序感知研究奠定了基础，代码和数据集已开源。
...
TL;DR: Edit-R1 提出了一种基于链式思维验证器的推理奖励模型（RRM），用于解决图像编辑中缺乏细粒度奖励信号的问题，并通过 GCPO 和 GRPO 等方法显著提升了编辑效果。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑奖励模型仅给出整体评分，忽略不同指令要求，导致奖励存在偏差。作者认为关键在于从简单评分器转向推理验证器。

Method: 1) 构建 CoT 推理奖励模型（RRM），将指令拆解为不同原则并逐条评估；2) 使用 SFT 冷启动生成 CoT 奖励轨迹；3) 提出 GCPO 算法利用人类偏好数据强化点级 RRM；4) 使用 GRPO 训练编辑模型。

Result: Edit-RRM 超越了 Seed-1.5-VL 和 Seed-1.6-VL 等强 VLM 作为编辑奖励模型，且从 3B 到 7B 规模性能一致提升。Edit-R1 显著提升了 FLUX.1-kontext 等编辑模型的效果。

Conclusion: 推理验证器范式在图像编辑奖励建模中有效，Edit-R1 框架展示了可解释、细粒度奖励信号对图像编辑任务的重要性。
...
TL;DR: 本文提出无幻觉反演框架，首次发现视觉编码器中的强叠加现象，证明分类通过破坏性干涉而非空间漏斗假说运作，并据此推导出协方差-体积通道选择算法，揭示了OOD失败的本质。


<details>
  <summary>Details</summary>
Motivation: CNN可解释性的基础假设——空间漏斗假说（编码器抑制背景，分类器仅选择清理后的特征）——因现有可视化工具存在空间幻觉而从未被验证。

Method: 基于幅度-相位解耦和局部伴随校正器构建无幻觉反演框架，数学上保证重构的空间梯度支撑严格来自真实活跃通道；通过该框架进行几何探针分析。

Result: 发现通道级反演全息不可区分，但代数和聚焦于前景；证明分类通过破坏性干涉（分类器权重抵消共享背景方向，构建类别判别残差）运行，证伪空间漏斗假说；推导协方差-体积通道选择算法，具有(1-1/e)近似保证，并揭示OOD失败是协方差体积的坍缩。

Conclusion: 破坏性干涉模型是CNN分类的真实机制，其几何量（可允许干涉子空间体积）决定了通道需求；该框架可无缝扩展到注意力头部。
...
TL;DR: 本文研究了自监督学习（SSL）在植物图像识别中的应用，发现标准SSL数据增强（如高斯模糊、灰度化、曝光）会破坏植物细粒度识别所需的细微判别特征，转而提出仿射变换和色调分离等更适合植物领域的增强方法。在iNaturalist 2021 Plantae子集上训练SimDINOv2比在ImageNet-1K上训练效果更好，在少样本场景下甚至优于强监督基线模型。


<details>
  <summary>Details</summary>
Motivation: 当前自动植物识别依赖监督学习，受限于专家标注数据的稀缺性。自监督学习虽具可扩展性，但现有方法多针对粗粒度视觉任务设计，难以直接迁移到植物物种识别这类细粒度领域。

Method: 1）分析标准SSL数据增强（高斯模糊、灰度化、曝光）对植物图像的负面影响；2）提出更适合植物领域的仿射变换和色调分离等增强策略；3）在iNaturalist 2021 Plantae子集上训练SimDINOv2模型，并与ImageNet-1K训练进行对比；4）在ViT-Base和ViT-Large两种架构上验证一致性。

Result: 域适配的增强策略和数据集选择显著提升SSL在植物识别上的表现；在少样本设定下，所提模型达到竞争性能，有时甚至超越Pl@ntCLEF和BioCLIP等强监督基线。

Conclusion: 域适配的数据增强策略和数据集选择对自监督学习至关重要，为构建可扩展的生物多样性监测模型提供了实用指南。
...
TL;DR: 本文提出残差高斯溅射（RGS），将小波多分辨率分析与3D高斯溅射结合，通过频谱解耦的高斯表示和频谱-空间协同优化策略，解决超稀疏视角CT重建中的频谱偏置问题，在保留高频解剖细节的同时抑制伪影。


<details>
  <summary>Details</summary>
Motivation: 传统3DGS在超稀疏视角CBCT重建中存在频谱偏置，导致过度平滑和丢失高频解剖细节。小波变换虽能提供丰富高频信息，但物理X射线衰减的非负性与小波高频系数的双极性存在数学不匹配。

Method: 提出RGS方法：1）引入频谱解耦的高斯表示，将体积场分解为几何基成分和残差细节成分；2）将显式高频拟合转化为物理一致性的隐式残差补偿任务；3）设计频谱-空间协同优化策略，协调几何锚定与纹理细化。

Result: 在临床数据集上的实验表明，RGS能够重建出精细几何纹理的图像，成功解决了伪影抑制与细节保留之间的权衡，在复杂骨小梁和血管结构上优于现有神经渲染基线。

Conclusion: RGS通过频谱解耦表示和协同优化策略，有效解决了超稀疏视角CT重建中的频谱偏置问题，实现了物理一致性下的高质量高频细节重建。
...
TL;DR: 本文研究大型视觉语言模型（LVLM）在识别图像中文字概念时，文字的视觉样式（功能性与装饰性）会非预期地影响模型对该概念的属性描述，即存在“样式泄露”现象。


<details>
  <summary>Details</summary>
Motivation: 探究LVLM在正确识别文字概念后，文字的可读性样式（如黑色无衬线）与装饰性样式（如彩色手写体）是否会影响模型对该概念生成的属性描述，验证样式信息是否会非预期地渗入语义推理过程。

Method: 设计实验，让LVLM识别图像中的文字概念，并分别使用功能性文本样式（面向可读性）和装饰性样式（面向展示）呈现文字，比较模型对同一概念生成的属性描述是否存在差异。

Result: 即使LVLM能正确识别文字所指向的概念，文本样式仍然显著影响模型对该概念的属性描述，表明样式信息从视觉层面泄露到了语义推理层面。

Conclusion: LVLM存在非平凡的样式泄露问题，需要在多模态系统的评估和缓解策略中考虑样式感知，以提升模型的语义鲁棒性。
...
TL;DR: 提出RIHA框架，通过层次化对齐（段落/句子/单词级别）实现放射影像与报告的多级跨模态匹配，在IU-Xray和MIMIC-CXR数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法将放射学报告视为扁平序列，忽略了其结构化的章节和语义层次，导致跨模态对齐不精确，影响报告生成准确性。

Method: 构建视觉特征金字塔(VFP)和文本特征金字塔(TFP)提取多尺度特征，通过跨模态层次对齐模块(CHA)利用最优传输实现多级对齐，并在解码器中引入相对位置编码(RPE)增强词元级对齐。

Result: 在IU-Xray和MIMIC-CXR两个胸部X光基准数据集上，RIHA在自然语言生成和临床效能指标上均超越现有SOTA模型。

Conclusion: 层次化跨模态对齐策略能有效提升放射学报告生成的精度，验证了利用报告结构化语义信息进行多级对齐的有效性。
...
TL;DR: 本文提出World2Minecraft框架，将真实场景基于3D语义占据预测转换为可编辑的Minecraft环境，并构建大规模数据集MinecraftOcc（10万+图像，156个室内场景），解决数据污染和灵活性不足问题，支持具身智能下游任务如视觉语言导航。


<details>
  <summary>Details</summary>
Motivation: 现有具身智能仿真平台存在数据污染和灵活性有限的问题，且占据预测受限于数据稀缺和模型泛化能力差，亟需低成本、可扩展的数据构建方案。

Method: 提出World2Minecraft框架，基于3D语义占据预测将真实场景转换为结构化Minecraft环境；同时设计自动化、低成本的数据采集管线，构建MinecraftOcc大规模占据预测数据集。

Result: MinecraftOcc包含100,165张图像、156个室内场景；实验表明该数据集对现有数据形成关键补充，并对当前SOTA方法构成显著挑战。

Conclusion: World2Minecraft为个性化具身AI研究提供了可定制、可编辑的平台，同时改进占据预测有助于提升重建质量。
...
TL;DR: 提出ClipTBP框架，通过片段级对齐损失和边界感知损失，改善视频时刻检索中多答案片段间关系建模不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有模型仅计算片段级相似度，忽略了单个查询对应的多个答案片段之间的关系，容易受视觉相似但无关的上下文片段干扰。

Method: 提出ClipTBP框架，包含片段级对齐损失（学习答案片段间语义关系）和主边界损失与辅助边界损失（预测精确时间边界）。

Result: ClipTBP在多种现有模型上一致提升性能，在模糊查询场景下展现出更鲁棒的边界预测能力。

Conclusion: 通过显式建模多答案片段关系与边界感知学习，有效提升视频时刻检索的准确性和鲁棒性。
...
TL;DR: SECOS是一种面向开放世界半监督学习的新方法，通过引入外部知识来提取和对齐多模态语义表示，直接预测文本标签，无需后处理，在性能上超越现有方法最多5.4%。


<details>
  <summary>Details</summary>
Motivation: 现有开放世界半监督学习方法无法直接预测与候选文本标签具有语义对应关系的标签，因为新类样本缺乏显式监督训练，且缺少提取潜在语义信息的机制，不能满足实际应用需求。

Method: SECOS利用外部知识提取和对齐已知类和新类在跨模态上的语义表示，为新类训练提供显式监督信号，直接预测候选集中的文本标签，无需后处理。

Result: 即使现有方法在更宽松的后处理匹配设置下评估，SECOS无需此类帮助仍能超越它们最多5.4%。

Conclusion: SECOS有效解决了开放世界半监督学习中的语义标签预测问题，性能优于现有方法，代码已开源。
...
TL;DR: SPUR是一个面向科学实验图像感知、理解和推理的全面基准测试，包含4,264个QA对和1,084张专家精选图像。当前多模态大模型在该基准上表现远未达到专家水平，揭示了AI4S领域的关键瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有基准缺乏对科学实验图像中细粒度面板级感知、跨面板关系理解和专家级推理能力的系统评估，亟需一个全面的基准来检验MLLMs在科学图像理解上的实际水平。

Method: 构建包含4,264个QA对的SPUR基准，从三个维度（数值、形态、信息定位）评估面板级细粒度感知，利用平均14.3个面板的复杂图像评估跨面板关系理解，并通过五种实验范式评估定性和定量推理能力。

Result: 对20个MLLMs和四种多模态思维链方法的全面评估显示，当前模型在科学图像解释上显著低于专家水平。

Conclusion: 现有MLLMs在科学实验图像理解上远未达到专家级要求，这构成了AI4S研究的关键瓶颈。
...
TL;DR: SpaAct是一种通过后向动作推理和前向转换预测激活VLM动态空间感知能力的VLN训练框架，结合TriPA渐进式课程学习，在VLN-CE基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLM缺乏对导航任务中动态空间关系的感知能力，需要让VLM具备两种互补能力：向后推理已执行动作（为什么）和向前预测状态转移（如何）。

Method: 提出SpaAct框架，包含两个空间激活任务：动作回溯（Action Retrospection，从视觉转换推断动作序列）和未来帧选择（Future Frame Selection，根据历史和动作预测视觉变化）；同时设计TriPA三因素渐进自适应课程学习策略，按由易到难组织训练样本。

Result: 在标准VLN-CE基准上，SpaAct一致提升了基于VLM的导航性能，取得了最先进的结果。

Conclusion: 激活VLM的后向动作推理和前向状态预测能力是提升视觉-语言导航性能的有效途径，SpaAct框架通过轻量级监督实现了这一目标。
...
TL;DR: 本文提出了一个名为MSR的刚性-可变形混合配准框架，用于解决颈椎CT-MRI配准中局部解剖结构复杂、数据稀缺的问题，并发布了公开数据集R-D-Reg。


<details>
  <summary>Details</summary>
Motivation: 颈椎区域解剖结构复杂、变异性大，且椎动脉和脊髓易受损，精确的CT-MRI配准对术前规划至关重要。但现有研究对刚性-可变形混合建模探索不足，且缺乏高质量的多模态标注数据。

Method: 构建并发布了带全面标注的CT-MRI数据集R-D-Reg；提出MSR混合配准框架，包含刚性配准模块（单个椎体的独立局部刚性对齐）和可变形配准模块（MSL块，通过自适应门控融合Mamba全局建模与Swin Transformer局部建模），将两者变形场融合为混合场以保持局部解剖一致性。

Result: 构建了高质量的多模态标注数据集R-D-Reg；提出的MSR框架在颈椎CT-MRI配准任务上实现了刚性-可变形混合建模，有效保持了局部解剖结构的一致性。

Conclusion: MSR框架通过刚性-可变形混合机制较好地解决了复杂关节结构的配准难题，代码与数据集已开源，为相关研究提供了重要基础。
...
TL;DR: EviMem是一种结合证据缺口诊断与分层记忆架构的迭代检索框架，在时序和多跳对话记忆任务上显著超越现有方法，且延迟降低4.5倍。


<details>
  <summary>Details</summary>
Motivation: 现有长程对话记忆检索方法在处理时序性和多跳问题时面临挑战：单次检索无法覆盖分散在多个会话中的证据；已有的迭代方法虽能优化查询，但未能显式诊断检索集中缺失什么（即证据缺口），导致查询优化缺乏针对性。

Method: 提出EviMem框架，包含两个核心组件：1) IRIS——闭环迭代检索框架，通过充分性评估检测证据缺口、诊断缺失内容并驱动有针对性的查询优化；2) LaceMem——粗到细的分层记忆架构，支持细粒度的证据缺口诊断。

Result: 在LoCoMo数据集上，相比MIRIX，EviMem在时序问题上Judge Accuracy从73.3%提升至81.6%，在多跳问题上从65.9%提升至85.2%，同时延迟降低4.5倍。

Conclusion: 显式诊断证据缺口能有效提升长程对话记忆检索的准确性和效率，代码已开源。
...
TL;DR: 本文提出基于深度学习的自动分割方法（nnU-Net和Swin UNETR）用于CT图像上的放射学腹膜癌指数（rPCI）区域分割，nnU-Net达到Dice 0.82，接近观察者间一致性（0.88），为非侵入性腹膜转移评估奠定基础。


<details>
  <summary>Details</summary>
Motivation: 传统腹膜转移评估依赖有创的诊断性腹腔镜手术（sPCI），缺乏标准化的影像学评估手段，迫切需要一种非侵入性的CT影像评估方法。

Method: 采用nnU-Net和Swin UNETR两种深度学习模型，在62例CT扫描上（由三位临床研究者手动标注、两位放射科专家验证）进行rPCI区域自动分割，使用五折交叉验证评估性能。

Result: nnU-Net总体Dice为0.82，接近观察者间一致性（0.88），优于Swin UNETR（0.76），主要挑战为右侧腹和小肠区域的分割。
...
TL;DR: 本文针对视频快照压缩成像（SCI）的重建问题，提出了一种基于补丁级光线采样和Inter/Intra-Ray Transformer（RayFormer）的新方法，通过捕捉内容结构相似性并引入全变分先验，在模拟和真实场景中均实现了SOTA重建性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于NeRF的SCI重建方法采用随机光线采样策略，无法捕捉内容结构相似性，导致重建质量有限。

Method: 1) 提出补丁级光线采样策略，用于建模内容结构；2) 提出Inter- and Intra-Ray Transformer (RayFormer)，捕捉空间相邻点间的光线间相似性以及沿视线的相邻点间光线内相关性；3) 利用补丁级采样策略，将全变分先验融入目标函数以增强空间平滑性并抑制伪影。

Result: 在模拟和真实场景实验中，所提方法均达到最先进的（SOTA）重建性能。

Conclusion: 通过补丁级采样和RayFormer有效提升了SCI重建质量，结构相似性建模和全变分先验的引入是关键改进因素。
...
TL;DR: 本文提出一种新颖的预训练策略，引导模型在预训练时避免学习领域特定特征，从而缩小ImageNet与遥感图像之间的域差距。在四个遥感语义分割数据集上取得SOTA结果（iSAID: 67.4% mIoU, MFNet: 56.9% mIoU, PST900: 84.22% mIoU, Potsdam: 91.88% mF1），为统一基础模型奠定基础。


<details>
  <summary>Details</summary>
Motivation: 遥感图像分割中，基于ImageNet预训练的模型因域差距（场景和模态差异）性能受限；而建立大规模领域专用数据集成本高且泛化性差。因此需要一种简单有效的预训练策略来缓解域差距问题。

Method: 提出一种新颖的预训练策略，在预训练阶段引导模型避免学习预训练数据集的领域特定特征，从而提升预训练模型的泛化能力。模型先在ImageNet上预训练，然后在四个遥感分割数据集上微调。

Result: 在iSAID（67.4% mIoU）、MFNet（56.9% mIoU）、PST900（84.22% mIoU）、Potsdam（91.88% mF1）四个数据集上均达到SOTA精度。

Conclusion: 该策略有效缓解了域差距问题，为开发适用于计算机视觉和遥感的统一基础模型奠定了基础。
...
TL;DR: 本文提出面向越南语场景文本图像描述的语言感知融合框架，通过图结构（HSTFG和PhonoSTFG）解决声调语言中OCR错误、歧义边界和变音冲突问题，并构建了首个大规模越南语场景文本描述数据集ViTextCaps。


<details>
  <summary>Details</summary>
Motivation: 现有场景文本图像描述方法将文本视为语言无关信息，无法处理越南语作为声调语言的独特挑战：变音符号改变词义、OCR错误普遍、词边界模糊，亟需语言感知的多模态融合机制。

Method: 提出HSTFG（通用图融合框架，含学习空间注意力偏置）和PhonoSTFG（专门面向越南语语音学推理的图级融合框架），通过拓扑分析发现跨模态图边对场景文本融合有害。

Result: 构建ViTextCaps数据集（15,729张图像，74,970条描述），词汇分析显示52.8%的词汇存在变音冲突风险。

Conclusion: 越南语场景文本描述需要语言特定的多模态融合策略，图结构中的跨模态连接反而有害，而语音学知识注入可有效提升融合质量。
...
TL;DR: 本文提出Flatness-aware Prompt Pretraining (FPP)，通过在测试时提示微调前将提示初始化到损失平面的平坦区域，仅替换初始化方式即可同时提升模型校准质量和分类性能，无需额外计算开销。


<details>
  <summary>Details</summary>
Motivation: 已有TPT方法常产生校准不良的模型，现有正则化方法虽改善校准但会降低性能，因此需要一种能同时提升校准和性能的方法。

Method: 提出Flatness-aware Prompt Pretraining (FPP)，在测试时适应前将提示预训练至损失平面的平坦区域；仅替换现有TPT流程中的初始化步骤，不修改其他组件。

Result: FPP在提升校准质量的同时也能改善分类性能，且无需标注数据，测试时零额外计算开销。

Conclusion: 平坦区域初始化是兼顾校准与性能的关键，FPP是一种实用且高效的TPT改进方案。
...
TL;DR: VPDR是一种客户端隐私保护插件，通过方差自适应扰动和蒸馏引导裁剪正则化，在原型联邦学习中实现更好的隐私-效用权衡。


<details>
  <summary>Details</summary>
Motivation: 现有ProtoPFL中直接共享类原型存在隐私风险，而各向同性高斯扰动(IGPP)过度扰动判别性维度，且难以平衡裁剪阈值与表示保真度。

Method: 提出VPDR，包含两部分：1) 方差自适应原型扰动(VPP)，根据维度级类方差分配噪声，对判别性子空间添加更少噪声；2) 蒸馏引导裁剪正则化(DCR)，使特征范数自适应集中在预设裁剪阈值附近并保持预测一致性。

Result: 理论分析表明分组机制在相同隐私约束下提供不弱于各向同性基线的隐私保证。实验证明VPDR在隐私-效用权衡上优于IGPP，且不牺牲对抗攻击的鲁棒性。

Conclusion: VPDR作为客户端侧隐私插件，能无缝集成到现有ProtoPFL框架中，在保护隐私的同时保持语义可分离性，实现更优的个性化联邦微调性能。
...
TL;DR: HiMix是一个统一框架，通过混合驱动分布增强（MDA）和层次化伪影感知表示（HAR）模块，提升合成图像检测器对未知生成器的泛化能力，在多个基准测试中达到最先进性能。
...
Method: 提出HiMix框架，包含两个核心模块：1) Mixup驱动的分布增强（MDA），通过构建真实与伪造图像之间的连续过渡样本来扩展训练分布；2) 层次化伪影感知表示（HAR），通过跨层集成和由粗到细的特征融合，从全局和局部层面聚合伪影信息。

Result: 在多个基准测试上达到最先进性能，建立了良好分离的逻辑值，提升了对未见伪造图像的泛化能力。

Conclusion: HiMix通过扩展训练分布和促进伪影感知表示，有效提升了合成图像检测器的泛化性能。
...
TL;DR: TAVR是一种利用跨场景视频参考生成高保真说话头像的新框架，通过Token选择模块和三阶段训练策略，在自定义背景中合成高质量说话头像，并在新构建的158对跨场景视频基准上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有说话头像方法局限于静态参考图像的同一场景图像到视频生成，缺乏足够的时间与表情线索，难以在自定义背景中合成高保真说话头像。

Method: 提出TAVR框架，包含Token选择模块和三阶段训练方案：同场景视频预训练建立外观复制能力，跨场景参考微调实现跨场景适应，任务特定强化学习通过身份奖励最大化身份相似度。

Result: 构建了158个精心筛选的跨场景视频对作为新基准，实验表明TAVR在定量和定性上均持续超越现有基线方法，并已部署到生产环境。

Conclusion: TAVR通过灵活的推理时视频参考实现了跨场景说话头像生成的新范式，显著提升了自定义背景下的生成保真度。
...
TL;DR: TunnelMIND是一个无需训练的隧道检测框架，通过将粗粒度的语言引导缺陷提议转化为结构化缺陷实体，在可见光、探地雷达和道路缺陷任务上分别达到0.68、0.78和0.72的F1分数，实现了从粗略定位到工程评估的跨越。


<details>
  <summary>Details</summary>
Motivation: 现有无需训练的基础模型流水线在干扰严重的隧道场景中只能产生粗粒度的开放词汇提议，难以直接用于缺陷定位、测量、严重度分级和工程文档编制等实际需求。

Method: 提出TunnelMIND框架：(1) 通过密集视觉一致性在校准阶段将语言引导的缺陷提议转化为可靠提示；(2) 将生成的掩码重构为包含类别、位置、几何、严重度和上下文属性的结构化缺陷实体；(3) 在专家知识约束下映射为检索增强解释和工程可读报告。

Result: 在可见光、探地雷达和道路缺陷任务上，F1分数分别达到0.68、0.78和0.72。

Conclusion: TunnelMIND证明了无需训练的隧道检测可以从粗略定位发展到面向工程评估的结构化缺陷证据。
...
TL;DR: 本文提出动态聚类采样方法(DynamiCS)，通过降采样大数据簇并升采小数据簇，在降低VLM训练计算成本的同时提升长尾概念的学习效果。


<details>
  <summary>Details</summary>
Motivation: 现有高效VLM预训练方法在采样时可能过度移除稀有概念，导致长尾概念在训练数据中表示不足，影响模型对这些概念的捕捉能力。

Method: 提出DynamiCS动态聚类采样方法：对大数据簇进行降采样，对小数据簇进行升采样，且在每一轮训练中动态应用采样策略，同时保持语义簇的相对顺序而非完全扁平化语义分布。

Result: DynamiCS降低了VLM训练的计算成本，并在长尾概念上取得了性能优势。

Conclusion: 动态聚类采样方法在提升训练效率的同时，有效改善了长尾概念的学习效果，优于单纯扁平化语义分布的现有方法。
...
TL;DR: 本文提出了迄今为止最大、最多样化的野外视频虚拟试穿数据集TripVVT-10K，并基于此开发了TripVTT框架，采用Diffusion Transformer和稳定的人脸掩码先验替代脆弱的服装掩码，在真实场景中实现可靠的背景保留。同时建立了包含100个案例的TripVTT-Bench基准，在视频质量、试穿保真度等方面优于现有学术和商业系统。


<details>
  <summary>Details</summary>
Motivation: 现有视频虚拟试穿模型性能受限，主要因为大规模野外三元组数据稀缺以及掩码使用不当。缺乏视频级跨服装监督的真实数据，导致模型在真实场景中的泛化能力不足。

Method: 1) 构建TripVVT-10K数据集，提供视频级跨服装监督；2) 提出TripVTT框架，基于Diffusion Transformer架构，用简单稳定的人体掩码先验替代脆弱的服装掩码，实现背景保持；3) 建立TripVTT-Bench基准，涵盖多种服装、复杂环境和多人场景。

Result: TripVTT在视频质量、服装保真度方面优于现有学术和商业系统，对具有挑战性的野外视频展现出显著的泛化能力提升。
...
TL;DR: 本文评估了视觉语言模型（如VideoChatGPT、PandaGPT、VideoLLava）在社交媒体短视频主题检测中的零样本分类能力，并利用最小成本多割问题的聚类方法进行无监督视觉主题发现。实验表明，当前VLM无法有效检测气候变化相关类别，但ConvNeXt V2和DINOv2能产生有意义的聚类结果。


<details>
  <summary>Details</summary>
Motivation: 社交媒体短视频的迅猛发展深刻影响了公共话题讨论方式，亟需自动化视觉主题检测方法来理解数字内容中的主题结构。

Method: (1) 使用VideoChatGPT、PandaGPT、VideoLLava等VLM进行零样本图像分类，并以CLIP帧级分类为基线对比；(2) 将聚类问题建模为最小成本多割问题，进行无监督模式发现。

Result: VLM无法准确检测气候变化相关类别；ConvNeXt V2和DINOv2均能产生有意义聚类，其中DINOv2侧重风格差异和抽象类别，ConvNeXt V2则区分更细粒度特征。

Conclusion: 当前VLM在特定领域主题检测上能力有限，但基于图像嵌入模型的聚类方法在社交媒体视觉主题分析上具有实用价值。
...
TL;DR: FineState-Bench是一个评估大视觉语言模型在GUI交互中细粒度状态设定的基准，包含2209个跨平台实例和四阶段诊断指标。实验表明当前最优模型精确状态成功率仅22.8%，视觉定位能力仍有很大提升空间。


<details>
  <summary>Details</summary>
Motivation: 现有GUI交互评估存在覆盖有限、目标状态定义不精确、过度依赖最终任务成功率等问题，无法准确定位智能体在何处及为何失败。

Method: 构建FineState-Bench基准（2209个实例，覆盖桌面/网页/移动三平台、4类交互、23种UI组件），提出FineState-Metrics四阶段诊断pipeline（定位成功率、交互成功率、精确状态成功率），以及视觉诊断助手VDA用于对比分析视觉定位能力。

Result: 精确目标状态成功率（ES-SR@Int）在网页端最高32.8%，跨平台平均仅22.8%；引入VDA定位提示后，Gemini-2.5-Flash提升14.9个百分点。

Conclusion: 当前LVLMs在细粒度状态条件GUI交互中表现不足，视觉定位是主要瓶颈，该领域仍有较大提升空间。
...
TL;DR: Echo-α是一个面向超声影像解读的智能体多模态推理模型，通过调用并推理框架统一了病灶定位与临床推理能力，在肾脏和乳腺超声数据集上取得了优于基线方法的表现。


<details>
  <summary>Details</summary>
Motivation: 现有超声解读方法要么定位强但推理弱（专用检测器），要么推理灵活但定位不足（多模态大语言模型），缺乏同时具备两种能力的统一框架。

Method: 提出Echo-α模型，采用调用并推理框架，训练模型协调器官级检测器输出与全局视觉上下文，将证据转化为诊断决策；通过九任务监督课程学习+序列强化学习（不同奖励权衡）训练出Echo-α-Grounding（病灶定位）和Echo-α-Diagnosis（最终诊断）两个版本。

Result: 在多中心肾脏和乳腺超声基准上，Echo-α在定位和诊断上均优于基线。交叉中心测试集上，Echo-α-Grounding的F1@0.5达56.73%(肾)/43.78%(乳腺)，Echo-α-Diagnosis总体准确率达74.90%(肾)/49.20%(乳腺)。
...
TL;DR: 提出了一种结构感知的3D高斯溅射密集化框架，通过多尺度频率分析和各向异性分裂策略，实现了更快的收敛速度和更好的高频细节重建质量。
...
Method: 结合结构张量和拉普拉斯尺度空间分析进行多尺度频率分析，定义每高斯每轴的频率违规度量η；对高频轴进行各向异性分裂；引入多视角一致性准则聚合η观测值。

Result: 在标准基准测试上达到更优的重建质量，特别是在高频区域；显著加快收敛速度，跳过了基线方法所需的冗长迭代密集化阶段。

Conclusion: 通过显式比较高斯投影屏幕空间范围与局部纹理结构，实现了更高效、更高质量的场景重建。
...
TL;DR: TAFA-GSGC是一种可扩展学习的点云几何编解码器，通过单比特流和单模型实现多质量解码，支持最多9个可解码质量层级，相比PCGCv2在D1和D2指标上分别节省-4.99%和-5.92%的BD-Rate。


<details>
  <summary>Details</summary>
Motivation: 现有学习型编解码器大多针对固定率-失真点优化，码率自适应成本高，需要重新编码或维护多个比特流，缺乏高效的可扩展压缩方案。

Method: 提出TAFA-GSGC，结合分层残差细化与通道组熵编码，并设计目标对齐特征聚合（TAFA）模块以减少增强残差中的跨层冗余。

Result: 支持9个质量层级且单调提升，相比PCGCv2，D1和D2的BD-Rate分别节省-4.99%和-5.92%，RD性能相当或略优。

Conclusion: TAFA-GSGC实现了从单比特流和单模型进行多质量解码的可扩展点云几何压缩，兼顾压缩效率与率失真性能。
...
TL;DR: 本文系统综述了制造领域中3D重建技术的发展现状，涵盖传统方法和深度学习方法，通过对106篇文献的分析，将重建技术分为数据采集、点云生成、后处理与应用三大类，并指出当前研究缺乏统一框架。


<details>
  <summary>Details</summary>
Motivation: 制造领域对高精度3D重建需求日益增长，但现有技术分散、缺乏系统性综述，且深度学习方法与传统方法之间尚未形成统一框架，因此需要对该领域进行全面梳理。

Method: 系统综述106篇近期文献，将3D重建技术分为数据采集、点云生成、后处理与应用三类，统计各类技术的应用占比，并分析传统方法与深度学习的融合趋势。

Result: 非接触方法（结构光扫描和立体视觉）占主导，47%的应用聚焦于质量检测；深度学习增强了特征提取与匹配的精度和速度；应用分布：设计开发13%、加工8、工艺17%、装配22%、质量检测40%。

Conclusion: 当前技术在受控环境下可达亚毫米精度，但在反光表面和动态环境中仍有挑战；未来趋势是结合多种传感器和处理方法的混合系统，以克服单一技术的局限性。
...
TL;DR: 本文提出了一种层次化的视频美学评估框架（VA、VF、VP三个维度，15个细粒度标准），构建了大规模专家标注偏好数据集AesVideo-Bench，并训练了视频美学奖励模型AesRM系列（包括Base和CoT版本），通过三阶段渐进式训练方案，显著提升了视频生成的美学质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成研究主要关注视觉逼真度，而实际应用（如电影制作）需要更高的美学质量（如和谐色彩、电影级光影）。同时，现有视觉美学工作多聚焦于图像，且定义粗糙、缺乏系统评估，亟需针对视频美学的细粒度、可解释的评估框架。

Method: 提出层次化美学评估框架（VA、VF、VP三维度+15细粒度标准）；构建AesVideo-Bench基准（2500+视频对，专家标注）；训练AesRM-Base（直接预测偏好）和AesRM-CoT（生成CoT）；采用三阶段训练：原子美学能力学习、冷启动对齐、GRPO强化；引入自一致性CoT合成和基于CoT的过程奖励机制。

Result: AesRM在多个美学基准上优于基线方法，且具有更低的position bias；将Wan2.2与AesRM对齐后，在美学质量上相比现有美学奖励模型有显著提升。

Conclusion: 层次化细粒度评估结合三阶段训练策略能有效提升视频美学评估的准确性和可解释性，并为视频生成模型提供有效的美学对齐奖励信号，推动视频美学从粗粒度评价向系统化、细粒度方向进步。
...
### [55] [UHR-Net: An Uncertainty-Aware Hypergraph Refinement Network for Medical Image Segmentation](https://arxiv.org/abs/2604.28095)
*Shuokun Cheng,Jinghao Shi,Kun Sun*

Main category: cs.CV

TL;DR: 提出UHR-Net，通过不确定性导向的实例对比预训练和不确定性引导的超图精炼模块，解决病灶分割中边界模糊和小病灶分割不稳定的问题，在五个公开数据集上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 病灶与周围组织相似且边界模糊，导致边界/过渡区域预测不稳定；小病灶特征在多尺度提取中被稀释，造成欠分割或过分割。

Method: 1) UO-IC预训练策略：几何感知的复制-粘贴增强 + 病灶样背景困难负样本挖掘，提升实例级判别能力；2) UGHR模块：基于粗概率图生成熵不确定性图，引导超图精炼，将超边原型分为前景/背景两组以解耦高阶交互。

Result: 在五个公开基准数据集上，相比强基线方法取得一致性能提升。

Conclusion: UHR-Net通过不确定性感知的超图精炼有效改善了病灶分割中边界模糊和小病灶分割的难题。
...
TL;DR: 提出S$^2$VAE框架，使用Power Spherical潜变量分布进行几何优先的潜空间学习，在深度估计、相机位姿恢复和点云重建任务中优于传统高斯瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有视觉世界建模系统虽能生成合理的运动，但常无法保持底层3D几何结构和物理一致的相机动态，关键在于潜变量表示对几何结构的编码不足。

Method: 基于VGGT表示，提出使用Power Spherical潜变量分布的乘积作为变分自编码器的瓶颈，显式强制超球面结构以保留方向性和几何语义。

Result: 在深度估计、相机位姿恢复和点云重建任务中，几何对齐的超球面潜变量一致优于传统高斯瓶颈，特别是在高压缩比场景下优势明显。

Conclusion: 潜空间几何结构应作为物理基础视觉和世界模型的一等设计考量。
...
TL;DR: AdvDMD通过统一DMD蒸馏和强化学习，用对抗训练判别器作为奖励模型，实现少步生成性能超越教师模型。


<details>
  <summary>Details</summary>
Motivation: 现有蒸馏方法在采样步数受限时性能下降明显，而结合RL的蒸馏方法存在组合复杂性问题，需要更统一的解决方案。

Method: 将DMD2中的对抗训练判别器作为RL奖励模型，在去噪过程的中间和最终状态进行训练，采用统一的SDE反向模拟和差异化训练调度。

Result: 4步AdvDMD在SD3.5上超越原始40步模型，2步AdvDMD在Qwen-Image上优于TwinFlow。

Conclusion: 统一DMD与RL的方法能有效提升少步生成质量，且结构简洁高效。
...
TL;DR: 本文提出了首个端到端的任意骨架运动捕捉框架，通过引入参考姿态-旋转对解决姿态到旋转映射中的歧义性问题，并采用骨架感知的全局-局部图引导多头注意力模块，在降低旋转误差的同时实现了约20倍的推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有方法采用分解式流水线（Video-to-Pose + 解析IK），但关节位置无法完全确定旋转（如骨轴扭转存在歧义），且非可微的IK阶段使系统无法适应噪声预测或优化动画目标。

Method: 1) 首个完全端到端框架，Video-to-Pose和Pose-to-Rotation均可学习并联合优化；2) 引入目标资产的参考姿态-旋转对以锚定映射并定义旋转坐标系，将旋转预测转化为有约束的条件问题；3) 使用骨架感知的全局-局部图引导多头注意力模块进行联合级推理。

Result: 在Truebones Zoo和Objaverse上，旋转误差从约17度降至约10度，在未见骨架上达6.54度，推理速度比基于网格的流水线快约20倍。

Conclusion: 通过端到端学习和引入参考姿态-旋转对，有效解决了姿态到旋转映射的歧义问题，显著提升了任意骨架运动捕捉的精度和效率。
...
TL;DR: 本文提出3D-ReGen，一种基于初始3D形状条件的三维再生器，通过VecSet机制实现可控的三维增强、重建和编辑，在多项任务上达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成器多为一次性生成，从文本或图像直接生成3D物体，缺乏可控性。作者希望引入初始3D形状作为条件，实现更灵活可控的三维生成。

Method: 提出3D-ReGen框架，采用基于VecSet的新条件机制，使再生器能够以一致的细粒度细节更新或改进输入几何。通过自监督预文本任务和数据增强从现成3D数据集学习通用再生先验，无需额外标注。

Result: 在可控三维生成的多个任务（增强、重建、编辑）上达到最先进水平，兼顾了几何一致性和细粒度质量。

Conclusion: 3D-ReGen通过简单的条件再生框架统一支持多种3D任务，VecSet机制有效提升了生成质量与可控性。
...
TL;DR: 提出pHVI-ISPNet框架，基于HVI色彩空间，通过RAW域特征处理、小波特征传播、样本动态损失系数和特征分布损失项，在NTIRE 2025夜景摄影渲染挑战中实现CIE2000和LPIPS的最优结果。


<details>
  <summary>Details</summary>
Motivation: 夜景摄影中暗区与强光源并存导致极大对比度，现有方法因追求保真度指标而存在感知差距，影响视觉质量。

Method: 构建pHVI-ISPNet RAW-to-RGB框架，包含四项关键改进：RAW域特征处理、基于小波的特征传播、样本动态损失系数、基于特征分布的损失项。

Result: 在NTIRE 2025夜景摄影渲染挑战数据集上，取得CIE2000色差和LPIPS感知指标的双重最优结果。

Conclusion: 验证了以感知驱动设计的高质量夜景成像方法的有效性。
...
TL;DR: 提出PhyCo框架，通过物理监督微调、物理属性数据集和VLM引导优化，实现视频生成中连续、可解释的物理一致性控制。


<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在视觉外观上表现优异，但物理一致性严重不足（物体漂移、碰撞不真实、材质响应不符合物理规律），需要一种可控制、可解释的物理约束方法。

Method: 1）构建10万+仿真视频数据集，系统变化摩擦、恢复系数、变形和力参数；2）基于物理属性图的ControlNet对预训练扩散模型进行物理监督微调；3）VLM引导的奖励优化，用微调后的视觉语言模型进行物理查询并提供可微反馈。

Result: 在Physics-IQ基准上显著提升物理真实性，人类评估确认对物理属性更清晰、更忠实的控制，且无需推理时使用仿真器或几何重建。

Conclusion: 展示了一条通往物理一致、可控的生成式视频模型的可扩展路径，且能泛化到合成训练环境之外。
...
TL;DR: AEGIS是一个用于评估AI生成学术图像取证的综合基准，涵盖7大学科类别、25种生成模型和4种伪造策略，测试发现即使是GPT-5.1整体性能也仅达48.80%，专家模型定位IoU仅30.09%，揭示了当前学术图像取证的严重局限性。


<details>
  <summary>Details</summary>
Motivation: 现有基准无法充分反映AI生成学术图像取证的领域复杂性，缺乏对多样化伪造策略和多维度评估的综合考量，因此亟需一个更全面、更具挑战性的评估基准来诊断当前取证方法的真实水平。

Method: 构建了涵盖7大学科类别、39个细分子类型的学术图像数据集；模拟了4种常见学术伪造策略，使用25种生成模型生成图像；从检测、推理、定位三个维度对25个多模态大语言模型、9个专家模型和1个统一模型进行系统评估。

Result: GPT-5.1整体性能仅48.80%；专家模型定位精度IoU仅30.09%；11种生成模型导致平均取证准确率低于50%；MLLMs在文本伪影识别上达84.74%，专家模型在二元真实性检测上达79.54%。

Conclusion: AEGIS作为一个诊断性测试平台，揭示了当前学术图像取证技术落后于生成技术发展的根本性局限，不同模型家族之间存在互补优势。
...
TL;DR: 本文提出了一种无需屏气协议的支气管镜导航方法，通过患者特异性呼吸运动建模和网格锚定高斯溅射框架，实现实时变形感知重建，在RESPIRE模拟数据集上达到1.22 mm定位精度。


<details>
  <summary>Details</summary>
Motivation: 呼吸运动导致CT与体内解剖结构偏差5-20 mm，现有屏气协议难以复现且干扰临床流程，亟需一种无需屏气、连续感知呼吸运动的导航方法。

Method: 利用术前配对吸气和呼气CT扫描注册，将呼吸运动降维为单标量呼吸相位；嵌入网格锚定高斯溅射框架，用轻量级估计器从内窥镜RGB图像推断呼吸相位，实现连续变形感知重建。

Result: 在RESPIRE模拟数据集上，几何重建保真度高，训练速度快20倍以上，目标定位精度达1.22 mm（在3 mm临床耐受范围内），优于单CT基线方法。

Conclusion: 该方法有效消除了屏气协议需求，通过患者特异性呼吸建模显著提升支气管镜导航的定位精度和临床适用性。
...
TL;DR: 本文提出视觉生成应超越外观合成，迈向"智能视觉生成"，并建立五级分类法（原子生成、条件生成、上下文生成、智能体生成、世界模型生成），系统梳理了技术驱动因素与评估缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有视觉生成模型在照片真实感等方面进步显著，但在空间推理、持久状态、长程一致性和因果理解上仍存在不足，因此需要推动领域从外观合成向智能视觉生成转型。

Method: 提出五级分类法（原子生成→条件生成→上下文生成→智能体生成→世界模型生成）；分析关键技术驱动（流匹配、理解-生成统一模型、视觉表示改进、后训练、奖励建模、数据整理、合成数据蒸馏、采样加速）；结合基准评测、野外压力测试和专家约束案例进行综合评估。

Result: 发现当前评测过于侧重感知质量，忽略了结构、时间和因果层面的失败，导致对实际进展的过高估计。

Conclusion: 本研究以能力为中心的视角，为理解、评估和推进下一代智能视觉生成系统提供了路线图。
...
TL;DR: 本文提出FD-loss，通过解耦FD估计的种群大小与梯度计算的批次大小，成功将原本被认为不适合作为训练目标的Fréchet Distance用于优化生成模型，在多个表示空间中均能提升视觉质量，并发现FID指标可能错误排序视觉质量。


<details>
  <summary>Details</summary>
Motivation: Fréchet Distance (FD) 长期以来被认为无法作为训练目标进行有效优化，本文旨在证明在表示空间中FD可以被有效优化，从而提升生成模型性能。

Method: 提出FD-loss方法，核心思想是将FD估计所需的种群大小（如50k）与梯度计算所需的批次大小（如1024）解耦，使得FD能够在表示空间中作为可微分的训练目标进行优化。

Result: 1) 在Inception特征空间下，一步生成器在ImageNet 256x256上达到0.72 FID；2) FD-loss可将多步生成器转化为强一步生成器，无需教师蒸馏、对抗训练或逐样本目标；3) 发现FID可能错误排序视觉质量，现代表示空间可产生更好样本但Inception FID更差。

Conclusion: 本文希望推动将不同表示空间中的分布距离作为生成模型的训练目标和评估指标的进一步探索，并提出了多表示空间度量FDr$^k$。
...
TL;DR: GenWildSplat是一种前馈式框架，无需逐场景优化，能从稀疏、无位姿的互联网图像中重建3D场景，并在多变光照和瞬态遮挡条件下实现实时、高质量的渲染。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖逐场景优化（如外观嵌入或动态掩码），在稀疏视角和真实世界多变条件下泛化能力差，且评估场景有限，难以验证通用性。

Method: 提出GenWildSplat，前馈式框架，利用学习到的几何先验预测深度、相机参数和规范空间中的3D高斯；引入外观适配器调节目标光照条件，语义分割处理瞬态遮挡物体；采用课程学习策略在合成数据和真实数据上训练。

Result: 在PhotoTourism和MegaScenes基准上，实现了前馈式渲染质量的最优结果，支持实时推理且无需测试时优化。

Conclusion: GenWildSplat有效解决了稀疏、无位姿图像在复杂室外场景下的重建与渲染问题，具备良好的泛化能力和实时性能。
...
TL;DR: 提出HERMES++，一个统一的世界模型，同时实现3D场景理解与未来几何预测，通过BEV表示、LLM增强查询、当前到未来链接和联合几何优化等协同设计，在多个基准上取得领先性能。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶世界模型主要关注未来场景生成，缺乏全面的3D场景理解；而大语言模型虽有推理能力，但无法预测未来几何演化，导致语义理解与物理模拟之间存在显著鸿沟。

Method: 1) BEV表示将多视角空间信息统一为LLM兼容的结构；2) LLM增强的世界查询促进理解分支的知识迁移；3) 当前到未来链接（Current-to-Future Link）桥接时间鸿沟，将几何演化条件化于语义上下文；4) 联合几何优化策略整合显式几何约束与隐式潜在正则化。

Result: 在多个基准上，HERMES++在未来的点云预测和3D场景理解任务上均超越了专门化方法。

Conclusion: HERMES++有效弥合了语义理解与物理模拟之间的差距，验证了统一驾驶世界模型的可行性，代码和模型将开源。
