...
TL;DR: SafeScreen是一个安全优先的视频筛选框架，用于在开放域视频平台中为脆弱用户（如儿童、痴呆患者）提供个性化内容，同时强制执行个体化安全约束，确保内容安全性和适宜性。


<details>
  <summary>Details</summary>
Motivation: 开放域视频平台虽然提供丰富的个性化内容，但其以参与度优化的推荐算法可能让脆弱用户接触到不适当或有害内容。在儿童导向和护理环境（如痴呆护理）中，内容在展示前必须满足个体化安全约束。

Method: SafeScreen采用安全优先的方法，将安全作为前提条件，通过自动化流程顺序批准或拒绝候选视频。框架包含三个关键组件：(1) 基于用户档案提取个体化安全标准；(2) 通过自适应问题生成和多模态VideoRAG分析进行基于证据的评估；(3) 基于LLM的决策机制，在内容暴露前验证安全性、适宜性和相关性。

Result: 在痴呆护理回忆案例研究中，使用30个合成患者档案和90个测试查询进行评估。结果显示SafeScreen在80-93%的情况下优先考虑安全而非参与度，与YouTube的参与度优化排名形成明显差异，同时保持高水平的安全覆盖、合理性和基础性，得到基于LLM的评估和领域专家的验证。

Conclusion: SafeScreen提供了一个可解释的实时视频筛选框架，能够在不依赖预计算安全标签的情况下处理未策划的视频库，为脆弱用户提供安全、个性化的视频内容，在护理和教育应用中具有重要价值。
...
TL;DR: 提出基于SoC FPGA的智能摄像头平台，用于工业场景中喷射火焰的实时分割与表征，通过优化UNet模型实现边缘处理，将参数减少125倍，处理延迟降低7.5倍，达到30FPS性能。


<details>
  <summary>Details</summary>
Motivation: 解决工业环境中缺乏实时早期火灾分割与表征方案的问题，特别是在喷射火焰检测方面需要低延迟的边缘处理能力。

Method: 使用SoC FPGA平台（Ultra96），通过Vitis框架优化UNet分割模型，将全精度模型从750万参数压缩到59,095参数，并利用多线程和批归一化进一步优化，实现边缘实时处理。

Result: 模型参数减少125倍，处理延迟降低7.5倍，达到30FPS的实时性能，同时保持Dice Score评估指标的准确性。

Conclusion: 提出的智能摄像头平台能够有效实现工业喷射火焰的实时分割与表征，为工业消防安全管理提供了可行的边缘AI解决方案，该框架可扩展到其他消防安全应用。
...
TL;DR: 提出3D-Implicit Depth Emergence方法，通过几何自监督让3D感知作为涌现特性出现，而非显式编码，实现零延迟推理且消除深度和姿态依赖


<details>
  <summary>Details</summary>
Motivation: 现有方法在2D-3D表示融合上存在权衡问题，导致部署效果不佳。显式3D位置编码和外部3D基础模型嫁接方法都有局限性，需要重新思考3D知识在视觉语言模型中的集成方式

Method: 提出隐式几何涌现原理：通过细粒度几何验证器和全局表示约束等机制，构建信息瓶颈，迫使模型最大化视觉特征与3D结构之间的互信息，让3D感知在统一视觉表示中自然涌现

Result: 在多个3D场景理解基准测试中超越SOTA，推理延迟降低55%，同时在多样化下游任务中保持强大性能，实现零延迟开销的推理

Conclusion: 从外部嫁接转向隐式涌现代表了3D知识集成在视觉语言模型中的根本性重新思考，通过精心设计的辅助目标实现依赖无关的3D理解
...
Result: 在9个公开数据集和新收集的多视角无人机、步态分析数据集上评估，MoViD相比最先进方法减少姿态估计误差超过24.2%，在严重遮挡下保持鲁棒性能，训练数据减少60%，在NVIDIA边缘设备上实现15FPS实时推理。

Conclusion: MoViD通过解耦视角和运动特征，实现了视角不变的3D人体姿态估计，在减少训练数据需求的同时提升了泛化能力和推理效率，适合实时边缘部署。
...
TL;DR: 研究在严格限制下（仅上传紧凑嵌入向量+元数据）的星上系统，通过向量搜索对遥感图像进行危险分类，验证嵌入向量在多种遥感偏移场景下的有效性。


<details>
  <summary>Details</summary>
Motivation: 解决下行链路瓶颈问题，避免传输原始像素数据，通过仅上传紧凑嵌入向量和元数据实现星上危险优先处理。

Method: 使用OlmoEarth嵌入向量，在扩展的公共多任务基准测试（27个Sentinel-2 L2A场景，15个云站点，5个SpaceNet-2 AOI）上，比较kNN检索和类质心两种决策头在不同遥感偏移场景下的性能。

Result: 所有有效方法都依赖于相同的上传嵌入向量，但最优决策头是任务依赖的：kNN检索在云分类上显著优于质心方法（0.92 vs 0.91；p<0.01），而类质心在时序变化检测上占主导地位（0.85 vs 0.48；p<0.01）。

Conclusion: 仅嵌入向量上传是关键使能技术，一旦嵌入向量在星上，系统可以按任务选择最佳决策头而无需额外上行链路成本，每次查询的所有遥测数据小于1KB。
...
TL;DR: 该研究揭示了多模态大语言模型在连续体对象动态理解方面的显著缺陷，提出了两个基准测试任务，并开发了Scene Dynamic Field方法来提升模型的物理推理能力。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在图像和视频理解方面表现出色，但在理解物理世界方面存在明显不足，特别是在高级物理推理方面。当前MLLMs在理解连续体对象动态方面存在重大局限，需要专门的研究来填补这一空白。

Method: 研究首先引入了两个基准测试任务：下一帧选择（NFS）和时间一致性验证（TCV）来评估模型的物理理解能力。然后提出了Scene Dynamic Field（SDF）方法，该方法利用物理模拟器在多任务微调框架中提升模型的物理推理能力。

Result: 实验表明，即使是当前最先进的MLLMs在这些基础任务上也表现不佳。而SDF方法显著提升了性能，在流体任务上获得了高达20.7%的增益，并且在未见过的物理领域表现出强大的泛化能力。

Conclusion: 这项工作不仅揭示了当前MLLMs在物理理解方面的重要缺陷，而且提出了一种有前景的成本效益方法来开发更具物理基础的多模态大语言模型，为物理推理研究提供了新的方向。
...
TL;DR: HVG-3D是一个基于扩散模型的3D感知手物交互视频生成框架，通过3D ControlNet编码几何和运动线索，实现精确的时空控制。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖2D控制信号，缺乏空间表达能力，且难以充分利用合成的3D条件数据，限制了手物交互视频合成的质量和可控性。

Method: 提出基于扩散模型的架构，增强3D ControlNet编码3D输入的几何和运动线索；包含3D感知HOI视频生成扩散架构和混合管道构建输入与条件信号。

Result: 在TASTE-Rob数据集上实现最先进的空间保真度、时间一致性和可控性，能够有效利用真实和模拟数据，生成高保真、时间一致的视频。

Conclusion: HVG-3D通过3D控制信号实现了精确的时空控制，解决了2D控制信号的局限性，为手物交互视频合成提供了统一且有效的3D感知框架。
...
Result: 在基准数据集上与最先进聚类方法的广泛比较表明，该方法在鲁棒性、快速收敛性以及数据规模、聚类数量和图像上下文方面的灵活性上具有优越性。

Conclusion: IDCL通过引入密度信息的模型训练策略，有效解决了现有深度聚类方法在鲁棒性和误差累积方面的问题，为图像聚类提供了更优的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在细粒度任务中容易产生感知相关的幻觉，主要原因是其推理主要局限于语言领域，将视觉输入视为静态、与推理无关的前置内容，而不是动态参与者。模型作为被动观察者，无法重新审视视觉细节来支撑其不断演变的推理状态。
...
Conclusion: V-Reflection通过将多模态大语言模型转变为主动询问者，解决了细粒度任务中的感知幻觉问题，在推理过程中保持完全端到端的自回归解码，具有最优效率。
...
TL;DR: TreeGaussian：基于树引导的级联对比学习框架，通过构建多级对象树显式建模层次语义关系，解决3DGS方法在复杂场景中表示层次结构和捕获整体-部分关系的难题。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法难以表示层次化的3D语义结构和捕获复杂场景中的整体-部分关系，同时密集的成对比较和来自2D先验的不一致层次标签阻碍了特征学习，导致次优分割结果。

Method: 1) 构建多级对象树实现结构化学习；2) 两阶段级联对比学习策略，从全局到局部逐步细化特征表示；3) 一致性分割检测机制对齐跨视图分割模式；4) 基于图的去噪模块抑制不稳定高斯点。

Result: 通过开放词汇3D对象选择、3D点云理解和消融实验等广泛实验验证了方法的有效性和鲁棒性，显著提升了分割一致性和质量。

Conclusion: TreeGaussian框架成功解决了3DGS在层次语义表示方面的局限性，通过树引导的对比学习实现了更好的3D场景理解和分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 长距离人体运动生成是计算机视觉和图形学的重要挑战，特别是在需要流畅过渡不同语义运动域的应用中（如舞蹈编排），目前这一能力尚未得到充分探索

Method: 提出简单有效的推理时优化框架，受扩散基随机最优控制启发，通过控制能量目标函数显式正则化预训练扩散模型的过渡轨迹

Result: 在推理时优化该目标函数能够产生具有保真度和时间一致性的过渡运动

Conclusion: 这是首个为受控长距离人体运动生成提供显式过渡建模的通用框架
...
TL;DR: CardioSAM：结合SAM编码器和心脏专用解码器的混合架构，通过引入心脏特异性注意力模块和边界细化模块，在ACDC基准测试中实现93.39% Dice系数，超越nnU-Net和专家间一致性水平。


<details>
  <summary>Details</summary>
Motivation: 心脏磁共振图像的手动分割耗时且存在观察者间差异，现有深度学习模型（如SAM）虽然泛化能力强但缺乏临床所需的边界精度，需要开发更精确的心脏结构分割方法。

Method: 提出CardioSAM混合架构：使用冻结的SAM编码器进行通用特征提取，结合轻量级可训练的心脏专用解码器。解码器包含两个关键创新：1）心脏特异性注意力模块，融入解剖拓扑先验知识；2）边界细化模块，改善组织界面划分。

Result: 在ACDC基准测试中，CardioSAM达到Dice系数93.39%、IoU 87.61%、像素精度99.20%、HD95 4.2mm。相比nnU-Net提升3.89% Dice，超过报告的专家间一致性水平（91.2%）。

Conclusion: CardioSAM通过结合SAM的泛化能力和心脏专用解码器的解剖先验知识，实现了高精度的心脏结构分割，具有可靠的临床应用潜力，为心血管疾病的诊断和治疗提供了更准确的工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调方法（如LoRA）在多模态任务中存在局限性，它们在各模态内独立操作，无法有效捕捉跨模态交互，这限制了基础模型在多模态任务中的适应能力。

Method: 提出跨模态低秩适配（CoLA）框架，在标准模态内适配路径基础上，引入专门的跨模态适配路径，形成双路径设计，使单模态基础模型能有效适应多模态任务，同时避免模态特定学习与跨模态学习之间的干扰。

Result: 在视觉语言（RefCOCO、RefCOCO+、RefCOCOg）和视听（AVE、AVS）基准测试中，CoLA始终优于LoRA，分别获得约3%和2%的相对增益，同时保持参数效率。CoLA首次实现了视觉定位的多任务PEFT框架。

Conclusion: CoLA通过引入跨模态适配路径，有效解决了现有PEFT方法在多模态任务中的局限性，为高效多模态适应提供了创新解决方案，特别是在视觉定位任务中实现了首个多任务PEFT框架。
...
TL;DR: StoryBlender是一个基于3D的故事板生成框架，通过语义空间锚定、规范资产实例化和时空动态三阶段流程，解决了现有方法在跨镜头一致性和可编辑性方面的不足。
...
Method: 提出StoryBlender框架，采用三层级流程：1) 语义-空间锚定构建连续性记忆图，解耦全局资产和镜头特定变量；2) 规范资产实例化在统一坐标空间中实例化实体；3) 时空动态通过视觉指标实现布局设计和电影化演进。通过分层多智能体协调和验证循环实现空间幻觉的迭代自校正。

Result: 实验表明，StoryBlender在一致性和可编辑性方面显著优于基于扩散和3D锚定的基线方法。生成的本地3D场景支持对相机和视觉资产的直接精确编辑，同时保持稳定的多镜头连续性。

Conclusion: StoryBlender通过故事中心反射方案和分层智能体协调，实现了同时具备跨镜头一致性和显式可编辑性的3D故事板生成，为视觉叙事自动化提供了有效解决方案。
...
TL;DR: 该论文研究了大型视觉语言模型中的注意力汇聚现象，发现视觉汇聚分为两类，并提出了一种轻量级的层间汇聚门控方法来平衡全局推理和局部证据。
...
Method: 首先将视觉汇聚分为两类：视觉编码器传播的V-汇聚和深度LLM层中出现的L-汇聚。然后提出层间汇聚门控（LSG），这是一个轻量级即插即用模块，动态缩放V-汇聚和其他视觉标记的注意力贡献。LSG通过标准的下一个标记预测进行训练，无需任务特定监督，同时保持LVLM主干冻结。

Result: 分析揭示了基本的性能权衡：汇聚有效地编码全局场景级先验，但其主导地位会抑制局部感知所需的细粒度视觉证据。LSG在大多数层中改进了代表性多模态基准测试，有效平衡了全局推理和精确的局部证据。

Conclusion: 注意力汇聚在大型视觉语言模型中既不是纯粹的冗余伪影，也不是纯粹的全局先验，而是需要在不同层进行适当调节的机制。提出的LSG方法能够有效管理这种权衡，提升模型性能。
...
TL;DR: 本研究提出了一种基于预训练和基础模型的可扩展AI方法，用于自动检测面对面协作学习中的注视行为，无需人工标注数据，在复杂情境下表现出更好的跨配置鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统机器学习方法需要大量标注数据进行训练，且训练数据集往往无法涵盖教育情境中的所有情况，导致模型跨配置鲁棒性不足。本研究旨在解决这些挑战，开发无需人工标注的自动注视行为检测方法。

Method: 采用预训练模型组合：YOLO11用于人员跟踪，YOLOE-26（具备文本提示能力）用于教育相关物体检测，Gaze-LLE模型用于注视目标预测。整个方法无需人工标注数据。

Result: 提出的方法在视频数据中检测学生注视行为达到F1分数0.829，对笔记本电脑注视和同伴注视表现良好，但对其他注视目标表现较弱。相比其他监督学习方法，在复杂情境下表现出更优越和稳定的性能，具有更好的跨配置鲁棒性。

Conclusion: 该方法为支持学生在真实环境中进行协作学习提供了有前景的解决方案，通过预训练和基础模型的组合实现了无需人工标注的高效注视行为检测，在复杂教育情境中表现出良好的鲁棒性。
...
TL;DR: EgoMind是一个无需几何监督的链式思维框架，通过角色扮演描述和渐进空间分析实现多模态大语言模型的空间推理能力提升


<details>
  <summary>Details</summary>
Motivation: 现有方法要么需要引入3D先验或几何监督导致数据准备和对齐成本高昂，要么纯2D方法在多帧空间推理中因无法捕捉跨帧空间关系而表现不佳

Method: 提出EgoMind框架：1）角色扮演描述：跨帧构建连贯的语言场景图；2）渐进空间分析：逐步推理至任务特定问题。仅需5K自动生成的SFT样本和20K RL样本

Result: 在VSI-Bench、SPAR-Bench、SITE-Bench和SPBench等多个基准测试中取得有竞争力的结果

Conclusion: EgoMind有效增强了MLLMs的空间推理能力，展示了语言推理在空间认知中的潜力，且无需昂贵的几何监督
...
Method: 提出VitaTouch模型，使用模态特定编码器和双Q-Former提取语言相关的视觉和触觉特征，通过对比学习对齐模态并耦合视觉与触觉，构建了包含186个物体、52k图像和5.1k人工验证指令-答案对的多模态数据集VitaSet。

Result: 在HCT和TVL基准测试中取得最佳性能，在SSVTP上保持竞争力；在VitaSet上达到88.89%硬度准确率、75.13%粗糙度准确率和54.81%描述符召回率；材料描述任务语义相似度峰值达0.9009；通过LoRA微调，在2/3/5类缺陷识别中分别达到100%、96%、92%准确率，在100次实验室机器人试验中实现94%闭环识别准确率和94%端到端分拣成功率。

Conclusion: VitaTouch通过融合视觉、触觉和语言模态，有效解决了智能制造中材料属性识别和缺陷检测的挑战，在多个任务上表现出色，为智能质量检测提供了有效的多模态解决方案。
...
TL;DR: 该研究探讨了自动驾驶车辆感知系统的安全对齐评估与优化方法，提出了基于安全关键错误的评估指标和损失函数，并在单车辆、协同感知和端到端驾驶框架中验证了其有效性。
...
Method: 基于先前提出的安全导向指标NDS-USC和安全感知损失函数EC-IoU，研究在三个层面展开：1) 单车辆3D目标检测模型的安全对齐评估；2) 车辆-基础设施协同感知模型的安全导向评估；3) 将EC-IoU集成到端到端感知-规划框架SparseDrive中，实现安全感知的感知硬化。

Result: 1) 标准指标（如mAP和NDS）的改进不一定转化为安全导向指标NDS-USC的提升；2) EC-IoU安全感知微调能有效提高安全关键检测性能；3) 协同感知模型优于单车辆模型，对"零愿景"有潜在贡献；4) 安全感知感知硬化可将碰撞率降低近30%，直接提升系统级安全性。

Conclusion: 安全对齐的感知评估和优化为增强CAV安全性提供了实用路径，适用于单车辆、协同感知和端到端自动驾驶设置，是实现更安全自动驾驶系统的有效方法。
...
TL;DR: 该研究比较了9种代表性的表面重建方法在叶片表面重建中的性能，使用三个公开数据集评估了不同方法在精度、平滑度、抗噪性、缺失数据处理和计算成本等方面的权衡。


<details>
  <summary>Details</summary>
Motivation: 从3D点云准确重建叶片表面对农业表型分析等应用至关重要，但真实植物数据复杂，现有多种表面重建方法在叶片重建中的相对性能尚不清楚，需要系统比较以指导资源受限的机器人平台选择合适方法。

Method: 对9种代表性表面重建方法进行比较研究，包括参数化、三角化、隐式和基于学习的方法。在三个公开数据集（LAST-STRAW、Pheno4D、Crops3D）上进行评估，涵盖不同物种、传感器和感知环境，从清洁高分辨率室内扫描到噪声低分辨率田间设置。

Result: 分析揭示了不同方法在表面积估计精度、平滑度、抗噪性、缺失数据处理能力和计算成本之间的权衡。这些因素影响农业应用中机器人硬件的成本和约束。结果显示每种方法根据应用场景和资源约束具有不同的优势。

Conclusion: 研究结果为资源受限的机器人平台选择表面重建技术提供了实用指导，表明需要根据具体应用需求（精度、速度、抗噪性等）权衡选择合适的方法。
...
TL;DR: 本文介绍了RGxEStat工具，用于基因型与环境互作分析，包括显著性分析和稳定性分析，帮助育种者无需编程即可进行GxE研究


<details>
  <summary>Details</summary>
Motivation: GxE互作影响基因型在不同环境中的表现，降低表型预测性。深入分析GxE互作有助于识别基因优势或缺陷在特定环境下的表达或抑制，从而促进遗传选择和育种实践改进

Method: 提出两种GxE互作研究模型：基于混合效应模型的显著性分析（确定基因或GxE互作是否显著影响表型性状）和稳定性分析（研究基因与环境间的互作关系及基因型在不同环境中的相对优劣）。开发了RGxEStat轻量级交互工具，集成模型构建、求解和可视化功能

Result: 开发了RGxEStat工具，为育种者和农学家提供用户友好界面，无需学习复杂的SAS或R编程即可进行育种数据分析，显著加速研究周期。代码和数据集已在GitHub开源

Conclusion: RGxEStat工具通过简化GxE互作分析流程，使育种研究人员能够更高效地进行基因型与环境互作研究，促进精准育种和遗传改良
...
TL;DR: AC-LAM通过强制潜在动作空间具有加性组合结构，解决了现有潜在动作学习方法缺乏物理运动结构先验的问题，在模拟和真实桌面任务中表现优于现有方法。
...
Method: 提出加性组合潜在动作模型(AC-LAM)，在潜在动作空间上强制实施短时域内的场景级加性组合结构约束，鼓励潜在动作空间具有简单代数结构（恒等、逆、循环一致性），抑制非加性组合的信息。
...
<details>
  <summary>Details</summary>
Motivation: 遥感数据分析面临传感器模态多样性和时空动态性的挑战，混合专家模型通过动态路由输入到专门专家来解决这些问题，但缺乏该领域全面综述

Method: 提供系统性的综述方法，涵盖混合专家模型的基本原理、架构设计，以及在各种遥感任务中的应用

Result: 完成了遥感领域混合专家模型的首次全面综述，系统梳理了该技术在不同遥感任务中的应用现状

Conclusion: 混合专家模型是解决遥感数据多样性和动态性挑战的有效范式，该综述为未来研究提供指导并激发创新应用
...
### [24] [YOLOv11 Demystified: A Practical Guide to High-Performance Object Detection](https://arxiv.org/abs/2604.03349)
*Nikhileswara Rao Sulake*

Main category: cs.CV

TL;DR: YOLOv11是YOLO系列实时目标检测器的最新版本，通过引入新的架构模块改进特征提取和小目标检测性能，在保持实时性的同时实现了更高的精度。


<details>
  <summary>Details</summary>
Motivation: 开发新一代实时目标检测器，改进特征提取和小目标检测能力，同时保持YOLO系列的速度优势，为自动驾驶、监控和视频分析等应用提供更好的解决方案。

Method: 引入三个关键创新模块：C3K2块、SPPF（快速空间金字塔池化）和C2PSA（带空间注意力的跨阶段部分连接），这些模块增强了空间特征处理能力。模型包含backbone、neck和head组件，在标准基准测试中与先前YOLO版本进行比较。

Result: YOLOv11在平均精度（mAP）和推理速度方面均优于先前YOLO版本，实现了更高的准确性而不牺牲实时能力，在标准基准测试中表现出色。

Conclusion: YOLOv11通过创新的架构设计，在保持实时性能的同时显著提升了检测精度，特别适合自动驾驶、监控和视频分析等需要实时处理的应用场景，为后续研究提供了清晰的参考框架。
...
TL;DR: 研究发现多模态智能系统在多轮图像编辑中存在迭代退化问题，图像质量随编辑次数增加而显著下降，但现有图像质量评估指标无法检测这种退化。


<details>
  <summary>Details</summary>
Motivation: 虽然最新的图像编辑模型在单轮编辑中能忠实遵循指令并生成高质量图像，但研究者发现多轮编辑中存在一个关键弱点——图像质量的迭代退化。随着图像被重复编辑，微小伪影会累积，导致可见噪声严重积累，甚至无法遵循简单的编辑指令。

Method: 研究者引入了Banana100数据集，包含通过100次迭代编辑步骤生成的28,000张退化图像，涵盖多样纹理和图像内容。同时评估了21种流行的无参考图像质量评估（NR-IQA）指标对这些退化图像的检测能力。

Result: 令人担忧的是，图像质量评估器无法检测到这种退化。在21种流行的NR-IQA指标中，没有一个能一致地为严重退化的图像分配比干净图像更低的分数。生成器和评估器的双重失败可能威胁未来模型训练的稳定性和已部署智能系统的安全性。

Conclusion: 多轮编辑产生的低质量合成数据如果逃过质量过滤器，可能对多模态智能系统的稳定性构成威胁。研究者发布完整代码和数据以促进开发更鲁棒的模型，帮助减轻多模态智能系统的脆弱性。
...
TL;DR: KiToke是一种无需训练、与查询无关的视频令牌压缩方法，通过核基冗余度量全局估计令牌多样性，在极端压缩率下保持关键视觉信息，显著降低视频大语言模型的推理成本。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在视频理解任务上表现优异，但由于视觉令牌数量庞大导致推理成本高昂。现有压缩方法依赖局部或片段级启发式方法，无法有效捕获全局冗余信息。

Method: 提出KiToke方法：1) 使用核基冗余度量全局估计令牌多样性，实现内容自适应选择；2) 引入轻量级时间间隔构建与间隔感知令牌合并，保持时间连贯性；3) 无需训练且与查询无关，适用于极端令牌预算场景。

Result: 在多个视频理解基准测试和不同视频大语言模型骨干上的实验表明，KiToke始终优于现有的无需训练压缩方法，在低至1%的保留率下仍能保持优异性能。

Conclusion: KiToke通过全局冗余估计和内容自适应选择，有效减少了视频令牌的时空冗余，显著降低了视频大语言模型的推理成本，在极端压缩场景下仍能保持关键视觉信息。
...
TL;DR: 量化鲁棒性可通过权重空间中的可迁移方向（量化向量）实现零样本提升，无需接收方量化感知训练


<details>
  <summary>Details</summary>
Motivation: 量化感知训练（QAT）需要大量计算和训练数据，本文旨在寻找一种零样本、低成本的替代方案来提升模型对后训练量化（PTQ）的鲁棒性

Method: 通过简单的权重空间算术从捐赠任务中提取"量化向量"，将其应用于接收模型以增强对PTQ噪声的鲁棒性，无需接收方训练数据或QAT

Result: 在Vision Transformer模型上，该方法可将PTQ噪声鲁棒性提升高达60%，为极低比特部署提供了零样本、低成本的QAT替代方案

Conclusion: 量化鲁棒性不仅是任务特定训练的副产品，更是权重空间几何中可重用、可迁移的特征，可通过转移而非重新训练获得
...
TL;DR: 本研究提出了一种基于基础模型的自动化监测系统，用于群养猪只的长期跟踪，通过结合预训练视觉语言模型和轻量级任务特定逻辑，实现了高精度、可扩展且标签高效的监测方案。


<details>
  <summary>Details</summary>
Motivation: 在精准畜牧业中，现有监测系统主要依赖监督学习模型，需要大量标注数据、重复训练和农场特定调优。基础模型的发展为减少对任务特定监督学习的依赖提供了新机会，本研究旨在探索如何利用基础模型实现群养猪只的自动化监测。

Method: 采用基础模型为中心的流程：1) 使用Grounding-DINO进行目标检测；2) 针对夜间和遮挡问题集成时序跟踪逻辑；3) 使用Grounded-SAM2进行短期视频分割；4) 开发长期跟踪管道，包括初始化、跟踪、匹配、掩码细化、重识别和后处理质量控制。

Result: 在1,418张标注图像上建立基线检测性能；在550个一分钟视频片段中，80%以上的4,927个活动轨迹完全正确；在132分钟连续视频上，系统保持稳定身份识别，在132个均匀采样的地面真值帧上，获得平均区域相似度(J)0.83、轮廓精度(F)0.92、J&F 0.87、MOTA 0.99、MOTP 90.7%，无身份切换。

Conclusion: 本研究展示了如何将基础模型的先验知识与轻量级任务特定逻辑相结合，实现猪只生产中的可扩展、标签高效和长期监测，为精准畜牧业提供了新的技术路径。
...
TL;DR: 在无需微调模型权重的情况下，通过推理时的电路复制方法改进冻结自监督视觉基础模型在水下物种分类中的表现


<details>
  <summary>Details</summary>
Motivation: 水下物种自动分类面临标注成本高和环境变化导致模型泛化能力受限的问题。虽然冻结的自监督视觉基础模型嵌入已为海洋图像分类提供了强大的标签高效基线，但研究人员希望在不改变模型权重的情况下，通过推理时的方法进一步提升性能。

Method: 采用电路复制方法，该方法最初为大型语言模型设计，在推理时对选定的Transformer层进行两次前向传播。在AQUA20基准测试中使用冻结的DINOv3嵌入进行评估，采用两种设置：全局电路选择（整个数据集使用单一重复电路）和类别特定电路选择（每个物种可能获得不同的最优电路）。两种设置都使用简单的半监督下游分类器。

Result: 电路复制方法持续优于标准的冻结前向传播。在最大标签预算下，类别特定选择达到0.875的宏观F1分数，与完全监督的ConvNeXt基准（0.889）的差距缩小到1.4个点，且无需任何基于梯度的训练。四个物种的表现超过了完全监督的参考模型，其中章鱼提高了+12.1个F1点。在所有预算下，约75%的类别偏好类别特定电路，表明存在真正的类别依赖性效益。

Conclusion: 这是电路复制方法首次应用于计算机视觉领域，展示了在不改变模型权重的情况下，通过推理时优化可以显著提升冻结自监督视觉基础模型在水下物种分类中的性能，为解决标注成本高和环境变化问题提供了有前景的方向。
...
TL;DR: ExpressEdit是一个开源Photoshop插件，用于快速无伪影地编辑角色面部表情，比专有模型更快，并包含包含135个表情标签的数据库支持检索增强生成。


<details>
  <summary>Details</summary>
Motivation: 当前AI图像编辑模型在风格化表情编辑中会引入全局噪声和像素漂移，阻碍了专业图像编辑软件和工作流的集成，需要解决这一问题。

Method: 开发了完全开源的Photoshop插件ExpressEdit，避免了专有模型的常见伪影，并与Photoshop原生操作（如液化）稳健协同，同时构建了包含135个表情标签的数据库，支持检索增强生成。

Result: ExpressEdit在单个消费级GPU上3秒内无缝编辑表情，比流行专有模型显著更快，避免了常见伪影，能够与专业工作流良好集成。
...
TL;DR: 本研究提出了一种多模态框架，结合高分辨率卫星影像和Google街景图像，在有限标注条件下实现可扩展的详细城市树木检测。通过领域适应、半监督学习、主动学习和混合策略，显著减少了标注成本并提高了检测性能。


<details>
  <summary>Details</summary>
Motivation: 城市树木在环境可持续性和灾害缓解中具有重要作用，但精确的城市树木测绘面临传统野外调查劳动密集、标注成本高、在不同城市场景中泛化能力差等挑战。需要开发可扩展的自动化系统来克服这些限制。

Method: 1. 多模态框架：首先利用卫星影像定位候选树木，然后检索针对性的地面街景图像进行详细检测，减少低效的街道级采样。
2. 领域适应：将知识从现有标注数据集迁移到新的感兴趣区域。
3. 三种学习策略评估：半监督学习、主动学习以及两者结合的混合策略，使用基于transformer的检测模型。

Result: 混合策略取得了最佳性能，F1分数达到0.90，比基线模型提高了12%。半监督学习由于伪标注中的确认偏差而表现出渐进性能下降，而主动学习通过针对性的人工干预标注不确定或错误的预测，稳步改善结果。错误分析显示主动和混合策略减少了假阳性和假阴性。

Conclusion: 研究结果强调了多模态方法和指导性标注对于可扩展、标注高效的城市树木测绘的重要性，有助于加强可持续城市规划。混合策略在有限标注条件下提供了最佳的性能平衡。
...
TL;DR: 论文提出用户显著对象检测(UserSOD)任务，强调基于用户主动需求而非被动视觉刺激来检测显著对象，并指出该领域缺乏训练和测试数据集的主要挑战。


<details>
  <summary>Details</summary>
Motivation: 现有显著对象检测方法仅基于被动视觉刺激，忽略了用户主动需求在确定显著对象中的决定性作用。这种局限性不仅无法满足用户需求，还限制了下游任务的发展，如显著对象排序任务。

Method: 论文提出用户显著对象检测(UserSOD)任务，旨在检测与用户主动需求一致的显著对象。主要挑战是缺乏用于模型训练和测试的数据集。

Result: 论文提出了新的研究方向，但尚未提供具体的实验结果，主要贡献在于识别了现有方法的局限性并提出了新的任务框架。

Conclusion: 用户显著对象检测是一个重要的新研究方向，需要解决数据集缺乏的问题，以实现基于用户需求的显著对象检测，从而更好地满足用户需求并促进下游任务发展。
...
TL;DR: 本文提出了一种将预训练分数扩散模型直接集成到即插即用框架中的新方法，通过噪声注入改进严重病态逆问题的求解效果。


<details>
  <summary>Details</summary>
Motivation: 尽管即插即用方法和分数扩散模型都依赖于去噪器，但如何在不依赖反向扩散采样的前提下，系统地将分数扩散模型作为先验集成到即插即用框架中仍不明确。

Method: 建立即插即用的分数解释，提出随机生成即插即用框架，通过噪声注入更好地利用分数扩散模型的生成先验，并提供了理论分析证明噪声注入优化高斯平滑目标并促进逃离严格鞍点。

Result: 在多线圈MRI重建和大掩码自然图像修复等挑战性逆任务上，相比传统即插即用方法有持续改进，性能与基于扩散的求解器相当。

Conclusion: 提出的随机生成即插即用框架成功将预训练分数扩散模型集成到即插即用算法中，通过噪声注入机制在严重病态逆问题上实现了更好的鲁棒性和性能。
...
TL;DR: PortraitCraft是一个统一的肖像构图理解和生成基准，包含5万张带多级标注的真实肖像图像，支持构图评分、属性推理、视觉问答和可控生成等任务。


<details>
  <summary>Details</summary>
Motivation: 现有数据集主要关注粗粒度美学评分、通用图像美学或无约束肖像生成，缺乏对结构化肖像构图分析和可控肖像生成的系统性研究，限制了该领域的发展。

Method: 构建包含约5万张精心策划的真实肖像图像数据集，提供结构化多级监督：全局构图评分、13个构图属性标注、属性级解释文本、视觉问答对和面向构图的文本描述。基于此数据集建立构图理解和构图感知生成两个互补的基准任务。

Result: 建立了PortraitCraft基准，包含标准化评估协议和代表性多模态模型的参考基线结果，为细粒度肖像理解、可解释美学评估和可控肖像生成提供全面基准。

Conclusion: PortraitCraft填补了现有研究空白，为未来肖像构图分析和可控生成研究提供了系统性的基准平台，推动了该领域的结构化发展。
...
TL;DR: TABLeT是一种新的fMRI分析方法，使用预训练的2D自然图像自编码器将3D fMRI体积压缩为连续令牌，通过Transformer编码器实现长序列建模，在多个大规模基准测试中表现优异，同时显著提高了计算和内存效率。


<details>
  <summary>Details</summary>
Motivation: fMRI四维信号的高维度特性使得建模长距离时空动态面临挑战。现有的基于体素的方法虽然性能良好且可解释性强，但受限于巨大的内存需求，只能捕捉有限的时间窗口。

Method: 提出TABLeT方法：1）使用预训练的2D自然图像自编码器将3D fMRI体积压缩成紧凑的连续令牌；2）使用简单的Transformer编码器进行长序列建模；3）开发自监督的掩码令牌建模方法进行预训练。

Result: 在UK-Biobank、Human Connectome Project和ADHD-200等多个大规模基准测试中，TABLeT在多项任务上优于现有模型，同时在相同输入条件下比最先进的基于体素方法显著提高了计算和内存效率。自监督预训练进一步提升了各种下游任务的性能。

Conclusion: TABLeT为大脑活动的可扩展和可解释的时空建模提供了一种有前景的方法，能够有效处理fMRI数据的高维度特性，同时保持计算效率。
...
TL;DR: MuPD是一个多模态病理学生成基础模型，通过扩散变换器将组织学、分子RNA谱和临床文本嵌入共享潜在空间，支持多种跨模态合成任务，在多个病理学任务上显著优于领域特定模型。
...
Method: 使用具有解耦跨模态注意力的扩散变换器，将H&E染色组织学、分子RNA谱和临床文本嵌入共享潜在空间。在3400万组织学图像块、160万文本-组织学对和1080万RNA-组织学对上进行预训练。

Result: 在文本条件和图像到图像生成中，FID分数降低50%，少样本分类准确率提高47%；在RNA条件组织学生成中，FID降低23%；作为虚拟染色器，标记物相关性提高37%。

Conclusion: 单一统一的多模态生成模型在异质病理学模态上预训练，能够显著优于专业替代方案，为多模态组织病理学提供了可扩展的计算框架。
...
Method: 提出ComPrivDet方法，通过重用I帧推理结果来检测压缩视频中的隐私对象。利用压缩域线索识别新对象出现，然后跳过P帧和B帧检测，或使用轻量级检测器高效精炼检测结果。

Result: ComPrivDet在隐私人脸检测中保持99.75%的准确率，在隐私车牌检测中保持96.83%的准确率，同时跳过超过80%的推理。相比现有压缩域检测方法，平均准确率提高9.84%，延迟降低75.95%。

Conclusion: ComPrivDet通过在压缩视频中高效检测隐私对象，实现了高精度和低延迟的平衡，为大规模视频分析中的隐私保护提供了实用解决方案。
...
TL;DR: 提出CSRS方法解决多模态大语言模型自进化中多数投票选择伪黄金答案的偏差问题，通过回溯重推理机制、软化频率奖励和视觉语义扰动提升推理性能
...
Method: 提出连续软化回溯重采样(CSRS)：1)回溯重推理机制(RRM)从锚点重新推理以探索长尾推理路径；2)软化频率奖励(SFR)用连续信号替代二元奖励，基于答案频率校准奖励；3)视觉语义扰动(VSP)确保模型优先考虑数学逻辑而非视觉表面特征

Result: CSRS显著提升了Qwen2.5-VL-7B在MathVision等基准测试上的推理性能，在几何任务的无监督自进化中取得了最先进(SOTA)结果

Conclusion: CSRS方法通过改进反馈信号质量，有效解决了多模态大语言模型自进化中的偏差问题，提升了模型的推理能力和稳定性
...
TL;DR: ART模型通过时间感知关系图和自适应交互剪枝机制，高效准确地预测行人轨迹


<details>
  <summary>Details</summary>
Motivation: 现有基于图或Transformer的方法存在计算开销大或难以捕捉多样化、时变的人类交互特征的问题

Method: 提出自适应关系Transformer（ART），包含时间感知关系图（TARG）来显式捕捉成对交互的演变，以及自适应交互剪枝（AIP）机制来高效减少冗余计算

Result: 在ETH/UCY和NBA基准测试中，ART实现了最先进的准确性，同时具有高计算效率

Conclusion: ART模型能够有效平衡预测准确性和计算效率，为机器人相关应用提供了实用的行人轨迹预测解决方案
...
TL;DR: DreamPRVR提出了一种粗到细的表征学习范式，用于部分相关视频检索，通过全局语义寄存器增强上下文感知，解决查询模糊性和局部噪声问题。
...
Method: 采用粗到细表征学习范式：1) 生成全局上下文语义寄存器作为粗粒度高亮；2) 通过文本监督的截断扩散模型迭代精炼；3) 文本语义结构学习构建良好文本潜在空间；4) 寄存器通过寄存器增强的高斯注意力块与视频标记自适应融合。

Result: 大量实验表明DreamPRVR在性能上超越了现有最先进方法。

Conclusion: DreamPRVR通过粗到细表征学习和全局语义寄存器，有效提升了部分相关视频检索的准确性和鲁棒性。
...
TL;DR: 本文提出了LaPR框架，通过标签感知的提示检索来提升视觉上下文学习性能，解决了现有方法忽视标签一致性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉上下文学习(VICL)的提示检索方法主要关注图像相似性而忽视标签一致性，这可能导致检索到视觉相似但标签不一致的提示，从而降低VICL性能。研究发现查询与提示之间的标签一致性对VICL结果有重要影响。

Method: 提出LaPR框架：1)设计图像-标签联合表示来显式融入标签信息；2)引入混合专家机制处理测试时查询标签不可用的情况，每个专家捕获特定标签模式；3)设计交替优化策略，包括VICL性能引导的对比损失和标签引导的对比损失。

Result: 在上下文分割、检测和着色任务上，LaPR都显示出显著且一致的性能提升。该框架在不同特征提取器和跨折叠场景中泛化良好。

Conclusion: LaPR框架证明了标签信息在VICL提示检索中的重要性，通过标签感知的检索策略有效提升了视觉上下文学习的性能。
...
TL;DR: 本文提出了一种基于视觉大语言模型的方法，通过Set-of-Mark提示增强视觉定位能力，利用用户注视轨迹理解意图，并采用逆指数采样策略处理视频帧，显著提升了第一人称视角下的人-物交互预测性能。


<details>
  <summary>Details</summary>
Motivation: 在智能辅助系统中，预测人-物交互对于指导用户日常活动和理解其短期与长期目标至关重要。现有方法在视觉定位和用户意图理解方面存在局限性，需要更有效的解决方案。

Method: 1. 使用Set-of-Mark提示增强视觉大语言模型的视觉定位能力；2. 通过用户最近注视点形成的轨迹理解用户意图；3. 引入新颖的逆指数采样策略捕捉交互前的时序动态。

Result: 在HD-EPIC第一人称数据集上的实验表明，该方法超越了现有最先进方法，展示了其模型无关的特性。

Conclusion: 该方法通过改进视觉定位、意图理解和时序动态捕捉，有效提升了第一人称视角下人-物交互预测的性能，为智能辅助系统提供了更准确的交互预测能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决新型传感器（如事件相机和4D雷达）的数据稀缺问题，支持跨传感器组合的系统性研究，促进不同融合策略和传感器组合的研究。

Method: 构建包含多种传感器的驾驶数据集，建立统一的3D和2D基准测试，提出融合框架将传感器特定线索集成到统一特征空间，提高3D检测在多变天气和光照下的鲁棒性。

Result: 提供了包含精确2D/3D边界框标注和车辆里程计的数据集，建立了跨传感器家族和家族内直接比较的基准，报告了代表性单模态和多模态方法的基线结果。

Conclusion: DSERT-RoLL数据集通过集成多种传感器并建立统一基准，为新型传感器研究提供了重要资源，提出的融合框架能有效提升3D检测在复杂环境下的性能。
...
TL;DR: 该论文研究了科学图像领域的长尾识别问题，提出SciLT框架通过自适应特征融合和双重监督学习，在基础模型微调有限的情况下提升头尾类别的平衡性能。


<details>
  <summary>Details</summary>
Motivation: 现有长尾识别研究主要局限于自然图像领域，而科学图像具有独特的视觉特征和监督信号。在基础模型预训练和微调数据分布差异大的情况下，需要探索科学长尾识别的有效方法。

Method: 提出SciLT框架，利用自适应特征融合和双重监督学习，联合利用倒数第二层和最后一层特征，实现头尾类别的平衡性能。

Result: 在三个科学基准测试上的实验表明，SciLT框架一致优于现有方法，为科学长尾识别建立了强大实用的基准。

Conclusion: SciLT框架通过多级表示利用，为适应基础模型到具有显著领域偏移的科学数据提供了有价值的指导，解决了科学长尾识别中的性能不平衡问题。
...
TL;DR: 该论文揭示了深度学习图像水印中"编码器-噪声层-解码器"架构对已知原始攻击的脆弱性，并提出ResGuard增强模块来提升水印鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习图像水印方法主要关注随机信道失真的鲁棒性，但忽视了攻击者拥有原始-水印图像对时的已知原始攻击威胁。这种攻击能几乎完全去除水印而保持视觉质量，暴露了当前架构的严重安全漏洞。

Method: 提出ResGuard插件模块，通过残差特异性增强损失强制嵌入残差与宿主图像紧密耦合，提高图像依赖性。同时引入辅助KOA噪声层，在训练中注入残差式扰动，使解码器在更强的嵌入不一致性下保持可靠性。

Result: ResGuard显著提升了KOA鲁棒性，将平均水印提取准确率从59.87%提高到99.81%。该模块可轻松集成到现有框架中，有效抵御基于残差估计的已知原始攻击。

Conclusion: 深度学习水印系统需要更强的图像依赖性来抵御已知原始攻击。ResGuard通过增强残差特异性提供了一种有效的防御方案，为构建更安全的图像水印系统提供了重要思路。
...
TL;DR: FunFact是一个从RGB-D图像构建概率开放词汇功能3D场景图的框架，通过联合概率推理解决场景功能关系中的模糊性问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D场景理解方法通常孤立地考虑物体对之间的功能关系，无法捕捉人类用于解决模糊性的场景级相互依赖性，需要更全面的功能场景理解方法。

Method: FunFact首先构建对象和部件中心的3D地图，使用基础模型提出语义上合理的功能关系候选，将其转换为因子图变量，并受LLM常识先验和几何先验约束，进行联合概率推理。

Result: 在SceneFun3D、FunGraph3D和FunThor数据集上的实验表明，FunFact提高了节点和关系发现的召回率，显著减少了模糊关系的校准误差。

Conclusion: 整体概率建模对功能场景理解具有显著优势，FunFact框架通过联合推理功能关系及其边缘概率，实现了更好的校准置信度评分。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决交通视频理解的复杂问题，需要结合结构化场景表示和多模态推理，以实现可解释的决策过程。

Method: 从路边视频构建交通场景图（包括检测、跟踪和车道提取），然后基于工具对符号图查询和视觉输入进行推理，采用ReAct方法处理大型语言模型的交错推理轨迹和工具调用。

Result: 在TUMTraffic VideoQA数据集样本上的实验表明，SGTA在多种问题类型上达到了有竞争力的准确率，同时提供了透明的推理步骤。

Conclusion: 将结构化场景表示与多模态智能体相结合，在交通视频理解方面具有巨大潜力，SGTA框架为此提供了有效的方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频数值推理基准通常局限于狭窄领域（如重复运动），或将简单计数视为表面回归任务，无法评估真实世界多媒体内容中的多步数值逻辑。需要测试视觉语言模型是否真正理解现实世界动态，因为准确的数值推理需要对时间事件、物体持久性和组合逻辑的深刻把握。

Method: 创建VidNum-1.4K基准，包含1379个严格人工标注的视频-问题对，涵盖高度多样化的环境。采用三级层次结构：从直接视觉感知到基于视频的组合数值推理，要求模型执行基于时间证据的算术运算、比较和逻辑推理。

Result: 评估多种最先进的视觉语言模型显示显著的推理差距：Gemini-3.1-pro勉强达到60%准确率阈值，代表性开源模型在25%-45%范围内挣扎。这表明当前模型缺乏稳定的"内部世界模型"。

Conclusion: VidNum-1.4K是一个具有挑战性的诊断测试平台，可用于下一代数值视频智能的发展，揭示了当前视觉语言模型在真实数值推理能力上的局限性。
...
TL;DR: XSeg是最大的X射线违禁品分割数据集，包含98,644张图像和295,932个实例掩码，涵盖30个常见违禁品类别。提出了基于SAM的自适应点SAM（APSAM）标注模型，通过能量感知编码器和自适应点生成器解决跨域泛化和堆叠物体检测问题。


<details>
  <summary>Details</summary>
Motivation: 当前X射线违禁品检测方法主要依赖边界框标注，由于缺乏像素级监督和真实世界数据，限制了模型的泛化能力和性能。需要构建大规模分割数据集并开发高效标注工具来解决这些问题。

Method: 1. 构建XSeg数据集：从公共数据集和合成数据中收集图像，通过自定义数据清洗管道过滤低质量样本；2. 提出APSAM标注模型：基于SAM构建，引入能量感知编码器增强掩码解码器初始化，提高对重叠物品的敏感性；3. 设计自适应点生成器：允许用户仅通过单个粗略点提示获得精确掩码标签。

Result: XSeg是目前最大的X射线违禁品分割数据集，包含98,644张图像和295,932个实例掩码，涵盖30个常见违禁品类别。APSAM在XSeg上表现出优越性能，显著提高了标注效率和准确性。

Conclusion: XSeg数据集和APSAM标注模型为X射线违禁品分割研究提供了重要资源，解决了现有方法在像素级监督、真实数据获取和堆叠物体检测方面的局限性，有望推动公共安全领域的技术进步。
...
TL;DR: 该研究提出了一种基于图学习的黑色素瘤检测方法，使用超像素集成图(SEG)和超像素层次图(SHG)两种图表示，结合手工和学习的边权重分配技术，在ISIC2017数据集上取得了99.00%的准确率和99.59%的AUC值。


<details>
  <summary>Details</summary>
Motivation: 传统图信号处理(GSP)中的图结构和边权重通常通过统计和计算方法设置，而图结构学习方法能提供更可靠和灵活的数据表示。本研究旨在将图学习方法应用于皮肤镜图像的黑色素瘤检测，探索不同的图表示和权重分配技术对检测性能的影响。

Method: 1. 使用两种图表示：超像素集成图(SEG)和超像素层次图(SHG)，在多个层次(20、40、60、80、100个节点)生成超像素图；2. 探索两种边权重分配技术：手工高斯权重和基于优化的学习权重；3. 基于纹理、几何和颜色超像素特征分配图节点信号；4. 研究图边阈值化(25%、50%、75%)对性能的影响；5. 在ISIC2017数据集上进行实验评估，使用数据增强缓解类别不平衡问题。

Result: 实验结果表明，使用纹理节点信号的超像素集成图(SEG)结合学习权重的方法取得了最佳性能，准确率达到99.00%，AUC达到99.59%。图边阈值化分析显示，适当的边剪枝可以改善检测性能。

Conclusion: 基于图学习的黑色素瘤检测方法在皮肤镜图像分析中表现出色，特别是超像素集成图与学习权重和纹理特征的组合。该方法为医学图像分析提供了一种有效的图表示学习框架，能够捕捉皮肤病变的复杂结构特征。
...
TL;DR: 提出一个紧凑的头发重建流程，通过聚类头发丝为代表性发片并共享纹理码本，结合3D高斯泼溅渲染，大幅减少存储和渲染成本，同时保持视觉质量。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯泼溅方法虽然能实现逼真效果，但需要数百万个基元，导致高存储和渲染成本。观察到头发在发型中具有结构和视觉相似性，可以利用这种相似性进行优化。

Method: 1. 将头发丝聚类为代表性发片；2. 将这些发片分组到共享纹理码本中；3. 将此结构与3D高斯泼溅渲染集成；4. 提出生成先验加速方法从图像集重建初始头发丝几何。

Result: 实现了4倍的头发丝重建时间减少，渲染性能相当，内存占用降低超过200倍。

Conclusion: 该方法通过利用头发结构相似性，显著减少了高保真头发重建的计算和存储需求，同时保持了视觉质量，为高效头发建模提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法通常只能控制相机运动或物体动态中的一种，或者依赖模糊的2D线索，这些线索会将相机引起的视差与真实物体运动纠缠在一起。需要一种能够统一控制相机轨迹和物体动态的方法。
...
Result: 广泛的实验和用户研究表明，SymphoMotion在视觉保真度、相机可控性和物体运动准确性方面显著优于现有方法，为统一运动控制设定了新的基准。

Conclusion: SymphoMotion建立了一个统一的运动控制框架，能够同时控制相机轨迹和物体动态，通过整合显式相机路径、几何感知线索和3D轨迹嵌入，实现了更连贯、更具表现力的视频生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有Sora2等专有模型在多参考角色视频生成方面取得进展，但学术研究有限。当参考图像外观高度相似时，模型容易出现参考混淆问题，语义相似的token会降低模型检索正确上下文的能力

Method: 提出PoCo（Position Embedding as a Context Controller），将位置编码作为语义检索之外的额外上下文控制。通过利用token的边信息，PoCo实现精确的token级匹配，同时保持隐式的语义一致性建模

Result: 基于PoCo开发了多参考多镜头视频生成模型，能够可靠地控制视觉特征极其相似的角色。大量实验表明，与各种基线相比，PoCo提高了跨镜头一致性和参考保真度

Conclusion: PoCo通过位置嵌入作为上下文控制器，有效解决了多参考图像相似时的混淆问题，提升了多角色多镜头视频生成的质量和一致性
...
TL;DR: SA-DSVN是一个3D卷积架构，用于使用宇宙射线μ子断层扫描对钢筋混凝土结构缺陷进行体素级分割。该方法联合处理散射运动学（9通道）和次级电磁簇射多重性（40通道），通过交叉注意力融合，相比传统方法显著提升了缺陷检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统μ子断层扫描方法（如POCA、MLSD）仅依赖μ子散射角，忽略了次级电磁簇射多重性这一重要特征。本研究旨在开发一种能够充分利用μ子与物质相互作用产生的多种信号特征的深度学习架构，以提高钢筋混凝土结构缺陷检测的准确性和效率。

Method: 提出SA-DSVN（Scattering and Shower Deep Voxel Network）3D卷积架构，包含两个独立的编码器流：一个处理散射运动学（9个通道），另一个处理次级电磁簇射多重性（40个通道）。两个流通过交叉注意力机制融合。使用云原生Geant4仿真框架Vega生成训练数据，包含450万个μ子事件，分布在900个体积中，涵盖蜂窝状缺陷、剪切断裂、腐蚀空洞和分层四种缺陷类型，嵌入密集的7x7钢筋笼中。

Result: 在60个独立仿真的验证体积上，模型达到96.3%的体素准确率，各缺陷类型的Dice分数在0.59-0.81之间，体积级检测灵敏度达到100%，每个体积推理时间仅10毫秒。消融研究表明，仅簇射多重性流就贡献了大部分判别能力，将缺陷平均Dice从0.535（仅散射）提升到0.685（仅簇射）。

Conclusion: 次级电磁簇射多重性是一个先前未被充分利用但非常有效的特征，对于学习的μ子断层扫描重建具有重要意义。SA-DSVN架构通过联合处理散射和簇射信息，显著提升了钢筋混凝土结构缺陷检测的性能，为无损检测领域提供了新的解决方案。
...
TL;DR: ICBench是一个新的大规模图像描述基准，包含12个内容类别，使用10个先进MLLM在2K图像上生成了40K个短长描述，并通过人类主观研究获得细粒度评估分数，同时提出了基于图像-文本-图像框架的自动评估指标ITIScore。


<details>
  <summary>Details</summary>
Motivation: 现有图像描述基准存在描述长度多样性有限、缺乏最新先进MLLM、人类标注不足等问题，这可能导致偏见并限制对现代MLLM性能的全面评估。

Method: 1) 构建ICBench基准：覆盖12个内容类别，使用10个先进MLLM在2K图像上生成短长描述，共40K个描述；2) 进行广泛人类主观研究，获得细粒度评估维度的平均意见分数；3) 提出ITIScore自动评估指标，基于图像-文本-图像框架，通过重建一致性衡量描述质量。

Result: 实验结果表明：1) ICBench基准提供了全面评估MLLM图像描述能力的资源；2) ITIScore自动指标与人类判断有很强的一致性；3) ITIScore在其他公共描述数据集上展现出稳健的零样本泛化能力。

Conclusion: ICBench解决了现有基准的局限性，提供了更全面的图像描述评估框架，ITIScore作为自动评估指标与人类判断高度一致，数据集和模型将在发表后发布。
...
TL;DR: M2StyleGS是一种实时3D风格迁移方法，使用3D高斯泼溅作为3D表示，结合CLIP多模态知识作为参考风格，通过精确特征对齐解决异常变换问题，支持文本或图像作为参考输入。


<details>
  <summary>Details</summary>
Motivation: 传统3D风格迁移方法依赖固定参考图像，但在虚拟/增强现实等实际应用中，用户需要更灵活的输入方式，包括文本描述和多样化图像。

Method: 使用3D高斯泼溅作为3D表示，CLIP多模态知识作为参考风格；采用子分割流进行精确特征对齐，加强CLIP文本-视觉组合特征到VGG风格特征的映射；引入观察损失确保风格化场景与参考风格匹配，以及抑制损失减少参考颜色信息在解码过程中的偏移。

Result: M2StyleGS能够使用文本或图像作为参考生成风格增强的新视角序列，实验显示在视觉质量上表现更好，一致性比先前工作提升高达32.92%。

Conclusion: 该方法实现了实时3D风格迁移，支持多模态输入，通过创新的特征对齐和损失函数设计解决了传统方法的局限性，在视觉质量和一致性方面显著优于现有方法。
...
TL;DR: 提出了一种自动化框架，利用粗糙的CityGML建筑模型作为几何先验，通过整合MLS数据来精化建筑立面几何，同时保持拓扑有效性


<details>
  <summary>Details</summary>
Motivation: 传统基于机载激光扫描的CityGML建筑模型在立面几何精度上存在显著缺陷，需要与高精度移动激光扫描数据集成来恢复详细立面几何，但现有重建方法会丢弃现有语义信息且依赖完整数据覆盖

Method: 使用粗糙模型作为几何先验，通过表面匹配识别过时表面，采用二进制整数优化从候选数据中选择最优面片，并在优化中强制执行硬约束以确保拓扑有效性

Result: 该方法有效修正了立面错位，将云到网格均方根误差降低了约36%，实现了厘米级对齐，同时保证了几何的严格水密性和流形性
...
TL;DR: MoScale是一个分层自回归框架，通过从粗到细的时间分辨率生成运动，解决了标准下一令牌预测在文本条件运动生成中的对齐问题，实现了SOTA性能和高训练效率。


<details>
  <summary>Details</summary>
Motivation: 标准自回归模型虽然训练稳定高效，但其下一令牌预测机制与文本条件运动生成所需的时间结构不匹配，需要更好的层次化因果结构来处理长距离运动结构。

Method: 提出MoScale框架：1）采用从粗到细的分层自回归生成，在最粗尺度提供全局语义并逐步细化；2）引入跨尺度层次化细化改进每尺度初始预测；3）采用尺度内时间细化进行选择性双向重新预测。

Result: MoScale在文本到运动生成任务上达到SOTA性能，具有高训练效率，能够有效随模型规模扩展，并能零样本泛化到多样化的运动生成和编辑任务。

Conclusion: MoScale通过分层自回归框架解决了文本条件运动生成中的时间对齐问题，在有限文本-运动数据下表现出鲁棒性，为运动生成提供了有效的解决方案。
...
TL;DR: 提出基于注意力分布熵的补丁剪枝方法，通过保留低熵（注意力集中）的补丁、移除高熵（注意力分散）的冗余补丁来降低Transformer计算成本，并扩展到Rényi熵以提供更灵活的剪枝策略。
...
Method: 提出基于注意力分布Shannon熵的补丁重要性评估准则：低熵补丁（注意力集中）被保留为重要补丁，高熵补丁（注意力分散）被视为冗余而被移除。进一步扩展到Rényi熵，强调注意力分布的峰值，支持根据任务需求和计算限制调整剪枝策略。

Result: 在细粒度图像识别任务中，该方法在保持准确率的同时显著减少了计算量。通过Rényi熵调整剪枝策略进一步提升了性能，改善了准确率与计算效率之间的权衡。

Conclusion: 基于熵的补丁剪枝方法有效降低了Transformer的计算成本，同时保持了模型性能。Rényi熵的引入提供了更灵活的剪枝策略，能够根据具体任务需求平衡准确率和计算效率。
...
TL;DR: EyeBench-V2是一个用于眼底图像增强模型的基准测试框架，旨在解决现有评估方法在临床相关性、统一评估协议和可操作指导方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有眼底图像增强模型的评估存在三个主要问题：1) 传统去噪指标（如PSNR、SSIM）无法捕捉临床相关特征；2) 缺乏针对配对和非配对增强方法的统一评估协议；3) 需要提供可操作的指导来推动临床对齐增强模型的发展。

Method: 提出EyeBench-V2基准测试框架，包含三个核心贡献：1) 通过下游任务进行多维临床对齐评估；2) 专家指导的评估设计，包括新数据集和结构化人工评估协议；3) 提供可操作见解的任务导向分析。

Result: 该基准测试能够全面评估增强模型在血管分割、糖尿病视网膜病变分级、未见噪声模式泛化能力和病变分割等临床任务上的表现，同时通过专家评估关注病变结构变化、背景颜色偏移和人工结构引入等临床关键方面。

Conclusion: EyeBench-V2填补了增强模型性能与临床效用之间的差距，为临床研究人员提供决策依据，同时识别当前方法的局限性，指导下一代增强模型的设计。
...
TL;DR: 论文提出了ActivityForensics，首个针对视频中人类活动伪造定位的大规模基准数据集，包含6000多个伪造视频片段，并提出了Temporal Artifact Diffuser（TADiff）作为基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频伪造检测基准主要关注外观层面的伪造（如人脸交换、物体移除），而随着视频生成技术的发展，出现了活动层面的伪造（修改人类动作以扭曲事件语义），这类伪造具有高度欺骗性，严重威胁媒体真实性和公众信任。

Method: 提出了ActivityForensics基准数据集，包含6000多个伪造视频片段，这些片段与视频上下文无缝融合，具有高视觉一致性。同时提出了Temporal Artifact Diffuser（TADiff）基线方法，通过基于扩散的特征正则化器来暴露伪造痕迹。

Result: 建立了全面的评估协议，涵盖域内、跨域和开放世界设置，并对多种最先进的伪造定位方法进行了基准测试，为未来研究提供了基础。

Conclusion: ActivityForensics填补了活动层面视频伪造检测的空白，TADiff方法简单有效，该基准将促进视频伪造检测领域的研究发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有多任务三元组损失方法依赖静态权重来平衡不同标注类型的监督，但静态权重需要调参且无法考虑任务在塑造共享表示时的交互作用

Method: 提出任务引导的多标注三元组损失，通过互信息标准选择跨任务信息量最大的三元组，修改影响表示的样本而非调整损失幅度

Result: 在航空野生动物数据集上的实验表明，相比多种三元组损失设置，任务引导选择在分类和回归任务上均表现更优，能产生更有效的共享表示

Conclusion: 任务感知的三元组选择能产生更有效的共享表示，为下游任务提供更好的基础，解决了静态权重方法的局限性
...
TL;DR: 提出了一种半监督知识蒸馏框架，将预训练视觉基础模型压缩为紧凑专家模型，使用有限标注数据和大量未标注数据，在实例分割任务上实现高效部署。


<details>
  <summary>Details</summary>
Motivation: 基础模型感知能力强但计算量大难以部署，且适应新任务需要昂贵的标注成本。需要一种方法在有限标注下压缩基础模型，同时保持高性能。

Method: 三阶段框架：1) 通过自训练和对比校准进行领域适应；2) 通过统一多目标损失进行知识转移；3) 学生模型精炼以减轻伪标签偏差。核心是实例感知的像素级对比损失，融合掩码和类别分数来提取信息负样本并增强实例间边界。

Result: 在Cityscapes和ADE20K数据集上，约11倍小的学生模型相比零样本基础模型教师提升+11.9和+8.6 AP，相比适应后的教师提升+3.4和+1.5 AP，并超越最先进的半监督知识蒸馏方法。

Conclusion: 提出的半监督知识蒸馏框架能有效压缩视觉基础模型，在有限标注下实现高性能实例分割，通过对比学习策略更好地利用未标注数据，为轻量级部署提供解决方案。
...
### [64] [Interpreting Video Representations with Spatio-Temporal Sparse Autoencoders](https://arxiv.org/abs/2604.03919)
*Atahan Dokme,Sriram Vishwanath*

Main category: cs.CV

TL;DR: 本文首次系统研究视频表示上的稀疏自编码器，发现标准SAE会破坏时间连贯性，提出时空对比目标和Matryoshka层次分组方法恢复时间连贯性，并在多个任务上取得改进。


<details>
  <summary>Details</summary>
Motivation: 标准稀疏自编码器在视频表示中会破坏时间连贯性：硬TopK选择导致跨帧特征分配不稳定，使自相关降低36%。需要开发能够保持时间连贯性的视频SAE方法。

Method: 提出时空对比目标函数和Matryoshka层次分组方法，通过对比损失权重在重建保真度和时间连贯性之间实现可调权衡。

Result: 对比SAE特征在动作分类上比原始特征提升+3.9%，文本-视频检索提升达2.8xR@1。因果消融证实对比训练将预测信号集中到少量可识别特征中。

Conclusion: 不同配置在不同目标上表现优异：重建保真度、时间连贯性、动作判别或可解释性。跨骨干分析显示标准单语义性指标存在骨干对齐伪影。
...
TL;DR: SafeCtrl：一种基于检测-抑制范式的区域感知安全控制框架，用于文本到图像扩散模型，通过精确定位风险区域并局部抑制有害内容，在安全性和内容保真度之间取得更好平衡。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像扩散模型面临生成视觉有害内容（如色情、暴力、恐怖图像）的挑战。现有安全干预方法存在两个关键局限：1）安全性与上下文保真度之间的严重权衡，移除不安全概念会降低安全内容的保真度；2）易受对抗性攻击，安全机制容易被绕过。

Method: 提出SafeCtrl框架，采用检测-抑制范式：1）检测模块：使用注意力引导机制精确定位特定风险区域；2）抑制模块：通过图像级直接偏好优化（DPO）进行优化，仅在检测区域内中和有害语义，将不安全对象转换为安全替代品，同时保持周围上下文完整。

Result: 在多个风险类别上的广泛实验表明，SafeCtrl在安全性和保真度之间的权衡优于现有最先进方法。关键的是，该方法对对抗性提示攻击表现出更强的韧性。

Conclusion: SafeCtrl为负责任生成提供了一个精确且鲁棒的解决方案，通过区域感知的安全控制框架，在保持内容保真度的同时有效防止有害内容生成，并增强了对对抗性攻击的抵抗力。
...
TL;DR: CM-GLasso：一种跨模态图套索方法，通过文本-视觉对齐策略和交叉注意力蒸馏机制，在共享潜在空间中学习可解释的多模态表示，同时解耦不变和类别特定的依赖结构。
...
Method: 提出跨模态图形套索（CM-GLasso），结合新颖的文本-视觉对齐策略和统一的视觉-语言编码器，将多模态特征严格对齐到共享潜在空间。引入交叉注意力蒸馏机制，将高维补丁压缩为显式语义节点，提取空间感知的跨模态先验。将定制的GLasso估计和公共-特定结构学习统一到通过交替方向乘子法优化的联合目标中。

Result: 在涵盖自然和医学领域的八个基准测试上进行广泛实验，CM-GLasso在生成式分类和密集语义分割任务中建立了新的最先进性能。

Conclusion: CM-GLasso克服了传统多模态图估计的基本限制，通过联合优化框架同时解耦不变和类别特定的精度矩阵，无需多步误差累积，为可解释多模态表示学习提供了有效解决方案。
...
TL;DR: VLA-Forget：一种针对视觉-语言-动作模型的混合遗忘框架，通过选择性编辑和分层遗忘，在消除不安全行为的同时保持感知、推理和任务执行能力


<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作模型在机器人操作中面临新的遗忘挑战：需要移除不安全、虚假或隐私敏感的行为，同时不损害感知、语言理解和动作控制能力。传统遗忘方法在VLA模型中效果有限，因为不良知识分布在感知、对齐和推理/动作多个层次中。

Method: 提出VLA-Forget混合遗忘框架，结合感知层的比例感知选择性编辑和跨模态特异性，以及推理/动作层的分层选择性遗忘。通过三阶段优化目标：针对性遗忘、感知保留和推理保持，对视觉编码器、投影器和上层动作生成变换器块进行分阶段更新。

Result: 相比强遗忘基线，VLA-Forget在遗忘效果上提升10%，感知特异性保留提升22%，推理和任务成功率保持提升9%，量化后恢复率降低55%。

Conclusion: VLA-Forget为VLA模型提供了一种有效的遗忘框架，能够在消除特定行为的同时保持模型的整体功能，解决了传统方法在具身环境中的局限性。
...
TL;DR: GAPL提出了一种基于二阶统计量的提示学习方法，通过Gram矩阵增强视觉语言模型的适应能力，相比仅依赖一阶空间特征的方法，能更好地应对领域偏移和局部噪声。


<details>
  <summary>Details</summary>
Motivation: 现有参数高效的提示学习方法主要关注将文本提示与一阶视觉特征（空间特征图）对齐，虽然对细粒度语义区分有效，但这些空间纠缠的特征对领域偏移和局部噪声高度敏感，仅依赖一阶信息不足以实现鲁棒的适应。
...
Conclusion: GAPL通过结合局部语义对齐和全局结构一致性，利用二阶统计信息增强了视觉语言模型的提示学习能力，提高了对领域偏移的鲁棒性。
...
TL;DR: HMAT是一种用于古代壁画修复的统一框架，结合了动态滤波和Transformer架构，通过掩码感知和风格融合模块处理多样化的退化形态，在保持原始区域真实性的同时重建缺失结构。


<details>
  <summary>Details</summary>
Motivation: 古代壁画作为宝贵的文化遗产，因环境暴露、材料老化和人类活动而遭受严重退化。修复这些艺术品具有挑战性，因为它需要同时重建大型缺失结构并严格保护未受损的真实区域。

Method: 提出了混合掩码感知Transformer（HMAT）框架，集成了掩码感知动态滤波用于局部纹理建模和Transformer瓶颈用于长距离结构推断。引入了掩码条件风格融合模块来处理退化的多样性形态，设计了带有硬门控跳跃连接的教师强制解码器，以强制有效区域的保真度并将重建集中在缺失区域。

Result: 在DHMural数据集和精心策划的九色鹿数据集上评估HMAT，实验结果表明该方法在竞争性性能方面与最先进方法相当，同时产生更结构一致和视觉忠实的修复结果。

Conclusion: HMAT为文化遗产壁画的数字修复提供了有效的解决方案，能够产生结构连贯且视觉忠实的修复结果。
...
TL;DR: 多模态排版攻击研究：通过协调音频、视觉和文本扰动，显著提升对多模态大语言模型的攻击成功率（83.43% vs 34.93%），揭示了跨模态脆弱性。
...
Method: 引入多模态排版攻击方法，系统研究跨多个模态的排版攻击如何负面影响MLLMs。分析音频、视觉和文本扰动之间的相互作用，展示协调多模态攻击比单模态攻击更具威胁性。

Result: 在多个前沿MLLMs、任务以及常识推理和内容审核基准测试中，多模态攻击成功率显著高于单模态攻击（83.43% vs 34.93%），证实了多模态排版攻击的有效性。
...
TL;DR: OASIC方法通过同时处理遮挡的两个根本原因（可见信息丢失和遮挡物干扰模式）来改善严重遮挡下的物体识别。该方法在测试时通过掩码移除遮挡物干扰，并在训练时使用随机掩码增强模型对不同遮挡程度的适应能力。


<details>
  <summary>Details</summary>
Motivation: 严重遮挡是计算机视觉中的主要挑战，其根本原因包括：(1)可见信息的丢失；(2)遮挡物产生的干扰模式。现有方法未能同时有效处理这两个问题。

Method: 提出OASIC方法：1）测试时通过掩码移除遮挡物干扰模式（独立于遮挡类型）；2）训练时使用随机掩码增强对不同遮挡程度的适应；3）在测试时估计遮挡程度，并选择针对该程度优化的模型进行分类。

Result: 实验显示，结合灰度掩码和自适应模型选择，OASIC在遮挡图像上的AUC_occ比标准训练提高+18.5，比在未遮挡图像上微调提高+23.7。模型能够准确估计遮挡程度，且针对特定遮挡程度优化的模型在相似程度的测试中表现最佳。

Conclusion: OASIC通过同时处理遮挡的两个根本原因，并利用遮挡程度估计进行自适应模型选择，显著提高了严重遮挡情况下的物体识别性能。该方法展示了遮挡程度感知的重要性以及针对特定遮挡程度优化模型的有效性。
...
TL;DR: HOIGS提出了一种新方法，通过交叉注意力机制显式建模人-物交互变形，使用HexPlane和CHS分别提取人和物特征，显著提升了交互场景的重建质量。


<details>
  <summary>Details</summary>
Motivation: 现有高斯泼溅方法要么依赖人体姿态先验而忽略动态物体，要么将所有运动近似为单一场，难以捕捉交互丰富的动态场景。需要显式建模人-物交互变形来解决这一局限。

Method: 提出HOIGS方法，通过交叉注意力HOI模块显式建模人-物交互引起的变形。使用异构特征提取：HexPlane用于人体，立方Hermite样条用于物体。通过集成这些特征有效捕捉相互依赖的运动。

Result: 在多个数据集上的综合实验表明，HOIGS在遮挡、接触和物体操纵等场景中显著优于现有最先进的人体中心和4D高斯方法，提升了变形估计质量。

Conclusion: 显式建模人-物交互对于高保真重建至关重要，HOIGS通过异构特征提取和交叉注意力机制有效捕捉交互动态，为复杂交互场景重建提供了新思路。
...
TL;DR: 1.x-Distill是一种突破整数步限制的分数步蒸馏框架，首次实现1.x步生成，在SD3-Medium和SD3.5-Large上以1.67和1.74有效NFE达到更好质量和多样性，相比原始28x2 NFE采样提速33倍。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然能生成高质量文本到图像结果，但其迭代去噪过程计算成本高昂。现有的Distribution Matching Distillation (DMD)方法在减少到两步或更少步骤时会出现多样性崩溃和保真度下降的问题，需要突破整数步限制，建立更实用的分数步蒸馏框架。

Method: 1.x-Distill包含三个关键技术：1) 分析教师CFG在DMD中被忽视的作用，引入简单有效的修改来抑制模式崩溃；2) 提出阶段化聚焦蒸馏，通过多样性保持的分布匹配学习粗粒度结构，再通过推理一致对抗蒸馏细化细节；3) 设计轻量级补偿模块用于蒸馏-缓存协同训练，将块级缓存自然融入蒸馏流程。

Result: 在SD3-Medium和SD3.5-Large上的实验表明，1.x-Distill超越了先前少步方法，分别在1.67和1.74有效NFE下实现了更好的质量和多样性，相比原始28x2 NFE采样速度提升高达33倍。

Conclusion: 1.x-Distill首次建立了分数步蒸馏框架，突破了整数步限制，使1.x步生成成为蒸馏扩散模型的实用方案，在极少数步骤下实现了高质量、高多样性的图像生成，显著提升了推理效率。
...
Method: 提出ATSS方法，识别AI生成视频中的异常时间自相似性指纹。该方法构建视觉、文本和跨模态相似性矩阵来量化时间异常，使用专用Transformer编码器编码这些矩阵，并通过双向交叉注意力融合模块整合，有效建模模态内和模态间动态。

Result: 在GenVideo、EvalCrafter、VideoPhy和VidProM四个大规模基准测试上的广泛实验表明，ATSS在AP、AUC和ACC指标上显著优于最先进方法，在不同视频生成模型上表现出优异的泛化能力。

Conclusion: ATSS通过检测AI生成视频中的异常时间自相似性，提供了一种有效的多模态检测框架，能够捕捉生成视频的底层逻辑，显著提升检测性能，为数字取证提供了有力工具。
...
TL;DR: TORA框架通过从预训练的3D编码器中提取关系结构，注入到流匹配主干中，实现拓扑优先的表示对齐，加速收敛并提升装配精度


<details>
  <summary>Details</summary>
Motivation: 现有的流匹配方法在3D形状装配中学习点状速度场，但缺乏对跨部件交互的明确指导，无法有效驱动部件运动

Method: 提出TORA框架：1）通过token-wise余弦匹配注入教师表示的几何描述符；2）使用中心核对齐损失匹配师生表示的相似性结构；3）系统探索不同3D编码器的影响

Result: 零推理开销下实现：收敛速度提升6.9倍，分布内精度提高，域偏移下鲁棒性增强；在5个基准测试中达到SOTA，零样本迁移到未见数据集表现突出

Conclusion: 几何和接触为中心的教师属性（而非语义分类能力）决定对齐效果，后期transformer层对齐最有效；TORA为3D形状装配提供高效拓扑指导
...
TL;DR: DINO-VO是一种端到端单目视觉里程计系统，通过可微分自适应补丁选择器和多任务特征提取模块，结合可微分光束法平差，在合成、室内和室外环境中实现强场景泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉里程计系统通常依赖启发式特征提取策略，这在大规模室外环境中会降低精度和鲁棒性。需要解决特征提取质量不足和场景泛化能力有限的问题。

Method: 1. 引入可微分自适应补丁选择器，提高提取补丁的质量；2. 集成多任务特征提取模块；3. 结合可微分光束法平差模块，利用逆深度先验；4. 端到端学习外观和几何信息，桥接特征学习和状态估计之间的差距。

Result: 在TartanAir、KITTI、Euroc和TUM数据集上的大量实验表明，DINO-VO在合成、室内和室外环境中表现出强大的泛化能力，实现了最先进的跟踪精度。

Conclusion: DINO-VO通过端到端学习框架和自适应特征选择机制，显著提升了视觉里程计系统的泛化能力和跟踪精度，为单目视觉里程计提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要密集的多视角相机阵列（数十甚至上百个相机）来建模动态场景，这在实际应用中成本高昂且不便携。本文旨在解决从极稀疏相机（最少4个便携相机）视频中恢复4D动态场景的挑战。

Method: 提出4C4D框架，核心创新是引入神经衰减函数作用于高斯不透明度，增强4D高斯的几何建模能力。该设计通过让4DGS梯度更多地关注几何学习，缓解了4DGS中几何与外观建模之间的固有不平衡问题。

Result: 在具有不同相机重叠度的稀疏视角数据集上进行广泛实验，结果显示4C4D在性能上优于现有技术，实现了高质量的4D高斯溅射和时序一致的新视角渲染。

Conclusion: 4C4D成功解决了从极稀疏相机视频中恢复4D动态场景的难题，通过神经衰减函数有效增强了几何建模能力，为实际应用中的便携式动态场景捕捉提供了可行方案。
...
TL;DR: 本文提出了一种基于YOLOv11检测器和BoT-SORT/ByteTrack跟踪器的离线实时交通监控系统，通过Qt桌面界面实现，在多种场景下达到66.67-95.83%的计数准确率。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能驱动的计算机视觉技术进步，交通监控系统得到显著增强。本文旨在开发一个不依赖云端的离线实时交通监控系统，为智慧城市建设提供现代化解决方案。

Method: 采用预训练的YOLOv11目标检测器结合BoT-SORT/ByteTrack多目标跟踪算法，使用PyTorch/OpenCV实现，并封装在基于Qt的桌面用户界面中，形成完整的CNN处理流水线。

Result: 系统在不同场景下达到66.67-95.83%的计数准确率。类别检测方面：汽车精度0.97-1.00，召回率0.82-1.00，F1分数0.90-1.00；卡车精度1.00，召回率0.70-1.00，F1分数0.82-1.00。在正常天气条件下表现稳健。

Conclusion: 通过集成轻量级模型和易用的云独立界面，该系统展示了AI驱动交通监控系统的能力，为未来智慧城市的现代化和发展做出了贡献。
...
TL;DR: HGR是一个用于具身智能体的导航框架，通过可修正的假设节点和依赖感知图记忆，结合语义预测进行定向探索，并在发现错误时进行级联修正，提高长期导航的可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有基于图的导航系统将未探索区域视为语义未知，导致前沿搜索效率低下。视觉语言模型可以预测前沿语义，但错误预测会嵌入记忆并传播，造成结构性错误累积，仅靠置信度衰减无法解决。需要一种既能利用语义预测进行定向探索，又能在新证据矛盾时系统撤回错误的框架。

Method: 提出假设图精炼（HGR）框架：1）语义假设模块，估计前沿的上下文条件语义分布，根据目标相关性、旅行成本和不确定性对探索目标进行排序；2）验证驱动的级联修正，将现场观察与预测语义进行比较，发现不匹配时撤回被反驳节点及其所有下游依赖节点，使图能够通过剪除错误子图而收缩。

Result: 在GOAT-Bench上达到72.41%的成功率和56.22%的SPL，在A-EQA和EM-EQA问答基准上表现一致提升。诊断分析显示级联修正消除了约20%的结构冗余假设节点，将错误区域的重复访问减少了4.5倍，镜面和透明表面占修正预测错误的67%。

Conclusion: HGR通过将前沿预测表示为可修正的假设节点，结合验证驱动的级联修正机制，能够在长期导航中保持记忆可靠性，有效解决语义预测错误传播问题，提高具身智能体的探索效率。
...
<details>
  <summary>Details</summary>
Motivation: 开发在真实世界恶劣条件下（特别是极端低光和烟雾退化环境）鲁棒的3D重建管道，解决现有方法在复杂退化场景下的性能限制。

Method: 基于RealX3D基准测试，组织NTIRE 2026 3D恢复与重建挑战赛，收集279名注册参与者的33个有效提交方法，系统评估这些方法与现有最先进基线的性能对比。

Result: 挑战赛结果显示在恶劣条件下3D重建取得了显著进展，分析揭示了顶级方法的共同设计原则，并提供了处理3D场景退化的有效策略见解。

Conclusion: 该挑战赛推动了3D重建在恶劣条件下的技术进步，识别了有效的设计模式，为未来研究提供了重要参考和方向。
...
TL;DR: 提出了一种针对空间非均匀曝光校正的新范式，通过空间自适应调制权重和不确定性引导的损失函数，有效处理图像中同时存在的多种曝光误差。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的曝光校正面临空间非均匀退化的根本挑战，单张图像中经常同时存在多种曝光误差。然而现有方法大多基于均匀假设，使用全局聚合的调制信号和全局尺度的重建损失，忽视了不同区域的空间变化校正需求。

Method: 1) 引入空间信号编码器预测空间自适应调制权重，指导多个查找表进行图像变换；2) 基于HSL的补偿模块提升色彩保真度；3) 提出不确定性启发的非均匀损失，根据局部恢复不确定性动态分配优化焦点。

Result: 大量实验表明，该方法在定性和定量性能上都优于现有最先进方法，能够更好地处理真实世界中的空间非均匀曝光误差。

Conclusion: 本文提出了一种专门针对空间非均匀性的曝光校正新范式，通过空间自适应调制架构和不确定性引导的优化策略，有效解决了传统方法在处理复杂曝光误差时的局限性。
...
TL;DR: OP-GRPO：首个为流匹配模型设计的离线策略GRPO框架，通过重用高质量轨迹、序列级重要性采样修正和轨迹截断，将训练效率提升至仅需原方法34.2%的训练步数


<details>
  <summary>Details</summary>
Motivation: GRPO在提升流匹配模型生成质量方面表现出色，但其在线策略训练范式导致样本效率低下。需要开发更高效的训练方法来减少训练成本同时保持生成质量。

Method: 1. 主动选择高质量轨迹并自适应地将其纳入回放缓冲区以供后续训练重用；2. 提出序列级重要性采样修正，在保持GRPO裁剪机制完整性的同时确保策略更新的稳定性；3. 通过截断后期去噪步骤的轨迹来缓解后期步骤产生的病态离线策略比率问题。

Result: 在图像和视频生成基准测试中，OP-GRPO仅需平均34.2%的训练步数就能达到与Flow-GRPO相当或更优的性能，在保持生成质量的同时显著提升训练效率。

Conclusion: OP-GRPO是首个为流匹配模型设计的离线策略GRPO框架，通过创新的轨迹重用、重要性采样修正和轨迹截断技术，有效解决了GRPO样本效率低下的问题，实现了训练效率的显著提升。
...
TL;DR: 提出了一种用于遥感回归任务的不确定性感知测试时自适应框架，专门针对地表温度时空融合，通过仅更新融合模块并利用不确定性指导，在未见地理区域实现显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 深度学习模型在遥感应用中面临跨地理区域泛化困难的问题，主要原因是训练区域与目标区域之间的域偏移。现有测试时自适应方法主要针对分类任务，不适用于回归任务，特别是时空融合这种回归任务。

Method: 提出不确定性感知测试时自适应框架：1）仅更新预训练时空融合模型的融合模块；2）利用认知不确定性指导自适应过程；3）结合土地利用和土地覆盖一致性约束；4）加入偏差校正机制；5）无需源数据或标记的目标样本。

Result: 在四个不同气候的目标区域（意大利罗马、埃及开罗、西班牙马德里、法国蒙彼利埃）进行实验，使用在法国奥尔良预训练的模型。结果显示RMSE和MAE分别平均提升24.2%和27.9%，即使使用有限的无标记目标数据和仅10个TTA训练周期。

Conclusion: 该不确定性感知测试时自适应框架能有效解决遥感回归任务中的域偏移问题，显著提升模型在未见地理区域的性能，为遥感回归任务的域自适应提供了有效解决方案。
...
TL;DR: 本文提出了一种双曲空间共嵌入框架，通过蕴含关系而非简单配对对齐来建模字体与印象描述之间的对应关系，量化了印象的风格特异性。


<details>
  <summary>Details</summary>
Motivation: 字体形状能唤起广泛的印象，但字体与印象描述之间并非一一对应关系：有些印象与多种字体风格兼容，而另一些印象则强烈限制了可能的字体集合。这种分级约束强度被称为风格特异性，需要更好的建模方法。
...
Result: 在MyFonts数据集上的实验显示，相比强的一对一基线方法，双向检索性能得到提升。遍历和标签级分析表明，学习到的空间捕捉了从模糊到更风格特异性印象的连贯进展，并提供了有意义的数据驱动风格特异性量化。

Conclusion: 双曲共嵌入框架通过蕴含关系有效建模字体-印象对应关系，提供了解释性的几何度量来衡量印象对字体风格的约束强度，实现了对风格特异性的数据驱动量化。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多视图多标签学习方法主要关注视图缺失情况，但双缺失场景（视图和标签都不完整）研究不足。现有方法依赖对比学习或信息瓶颈理论，但缺乏显式结构约束，难以捕捉稳定且具有判别性的共享语义。

Method: 1) 通过多视图共享码本和跨视图重建学习离散一致表示，在有限的共享码本嵌入中对齐不同视图并减少特征冗余；2) 设计权重估计方法评估各视图保留标签相关结构的能力，据此分配权重以提升融合预测质量；3) 提出融合教师自蒸馏框架，用融合预测指导视图特定分类器训练，并将全局知识反馈到单视图分支中。

Result: 在五个基准数据集上与先进方法进行了广泛的比较实验，充分证明了所提方法的有效性。代码已在GitHub上开源。

Conclusion: 该方法通过结构化的一致表示学习机制和融合教师自蒸馏框架，有效解决了多视图多标签学习中的双缺失问题，提高了模型的泛化能力和预测质量。
...
TL;DR: GENFIG1是一个评估生成式AI模型能力的基准，要求模型根据论文标题、摘要、引言和图表说明生成能够清晰表达论文核心思想的"Figure 1"图表。


<details>
  <summary>Details</summary>
Motivation: 科学论文中的"Figure 1"通常是研究核心思想的视觉总结，这些图表视觉简洁但概念丰富，需要大量人工迭代才能制作得当，这凸显了科学视觉传达的难度。因此需要建立一个基准来评估AI模型在科学可视化方面的能力。

Method: 从顶级深度学习会议论文中收集数据，进行严格质量控制，并引入与专家判断相关性良好的自动评估指标。要求模型根据论文标题、摘要、引言和图表说明生成"Figure 1"图表。

Result: 评估了一系列代表性模型在GENFIG1基准上的表现，发现即使对于性能最佳的系统，该任务也提出了显著挑战。

Conclusion: GENFIG1基准为多模态AI的未来进展奠定了基础，展示了科学可视化任务对AI系统的挑战性，需要模型具备科学理解与视觉合成的耦合推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型大多是离线的，不适合需要持续观察和及时响应的实时视频流。虽然已有流式VideoLLMs，但当前方法通常依赖解耦的触发-响应管道或仅限于字幕式叙述，限制了开放式问答和长时程交互的效果。

Method: AURA是一个端到端的流式视觉交互框架，集成了上下文管理、数据构建、训练目标和部署优化，支持稳定的长时程流式交互。它采用统一的VideoLLM持续处理视频流，支持实时问答和主动响应。

Result: AURA在流式基准测试中达到了最先进的性能，并支持实时演示系统，在两张80G加速器上以2FPS运行，包含ASR和TTS功能。作者发布了AURA模型和实时推理框架以促进未来研究。

Conclusion: AURA通过端到端的流式视觉交互框架解决了现有VideoLLMs在实时视频流处理中的局限性，为开放式问答和长时程交互提供了有效的解决方案，并展示了实际部署的可行性。
...
TL;DR: GraphicDesignBench (GDB) 是首个针对专业平面设计任务的全方位基准测试套件，包含50个任务，涵盖布局、排版、信息图表、模板与设计语义、动画五个维度，评估现有AI模型在专业设计核心挑战上的表现。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注自然图像理解或通用文本到图像合成，缺乏针对专业平面设计独特挑战的评估标准。专业设计需要将沟通意图转化为结构化布局、渲染精确排版、操作分层组合、生成有效矢量图形以及动画推理等能力。
...
Result: 评估显示当前模型在专业设计的核心挑战上表现不足：复杂布局的空间推理、准确的矢量代码生成、细粒度排版感知、动画的时间分解等问题仍未解决。虽然高层语义理解已可实现，但当任务需要精度、结构和组合意识时，差距显著扩大。

Conclusion: GDB为追踪AI系统成为有能力的设计协作伙伴的进展提供了严谨、可复现的测试平台。当前模型在专业设计的关键能力上仍有明显差距，该基准测试有助于推动相关研究发展。
...
TL;DR: DriveVA是一个自动驾驶世界模型，通过共享潜在生成过程联合解码未来视觉预测和动作序列，提升跨域泛化能力和视频-轨迹一致性。


<details>
  <summary>Details</summary>
Motivation: 当前基于世界模型的规划方法在跨数据集和传感器配置的泛化能力有限，且规划与场景演化的松散耦合导致视频-轨迹一致性差。

Method: 提出DriveVA模型，使用DiT-based解码器联合预测未来动作序列（轨迹）和视频，继承大规模视频生成模型的运动动力学先验，并引入视频延续策略增强长时一致性。

Result: 在NAVSIM上获得90.9 PDM分数；在nuScenes上平均L2误差和碰撞率分别降低78.9%和83.3%；在CARLA v2的Bench2drive上分别降低52.5%和52.4%。

Conclusion: DriveVA通过联合解码视觉预测和动作序列，实现了优异的跨域泛化能力和规划-场景演化一致性，显著提升了自动驾驶系统的泛化性能。
...
TL;DR: HighFM：首个针对高时间分辨率多光谱地球观测数据的基础模型，利用2TB+ SEVIRI气象卫星数据，通过改进的SatMAE掩码自编码框架学习稳健的时空表征，在云掩码和活跃火灾检测任务中优于传统方法和现有地理空间基础模型。


<details>
  <summary>Details</summary>
Motivation: 气候变化相关灾害日益频繁和严重，需要实时监测、早期预警和知情决策。现有基础模型主要依赖高分辨率但低重访率的卫星影像，不适合快速演变现象和时间关键型应急响应。需要开发适用于高时间分辨率数据的基础模型。

Method: 提出HighFM方法：1）利用超过2TB的Meteosat第二代（MSG）平台的SEVIRI影像数据；2）改进SatMAE掩码自编码框架，学习稳健的时空表征；3）增强原始架构，加入细粒度时间编码以捕捉短期变化；4）在云掩码和活跃火灾检测任务上进行微调。

Result: SEVIRI预训练的Vision Transformers在平衡准确率和IoU指标上均优于传统基准方法和近期地理空间基础模型，在两项任务中均取得一致性的性能提升。

Conclusion: 高时间分辨率地球静止数据在实时地球观测中具有巨大潜力，为灾害检测和追踪的基础模型提供了可扩展的路径。HighFM展示了利用密集时间序列数据构建专用基础模型的价值。
...
Method: 1) 使用运动感知模块分割和移除动态区域；2) 利用扩散模型修复遮挡区域，提供伪真实监督；3) 为每个高斯基元引入可学习的真实性标量，动态调整渲染过程中的不透明度，实现真实性感知的渲染和监督。

Result: 在DAVIS数据集和自建的Trajectory-Match数据集上的实验表明，GA-GS在静态场景重建方面达到最先进的性能，特别是在具有大规模、持续遮挡的挑战性场景中表现优异。

Conclusion: GA-GS通过生成辅助和真实性感知渲染，有效解决了动态物体遮挡下的静态场景重建问题，为相关应用提供了更完整的场景重建能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统CLIP方法将所有不匹配样本视为同等负样本，忽略了地理空间中的空间自相关特性（地理学第一定律），导致地理定位效果受限。

Method: 提出空间加权CLIP框架：1）引入位置作为文本表示编码地理坐标；2）用基于测地距离的空间加权软标签替代one-hot InfoNCE目标；3）使用邻域一致性正则化保持嵌入空间中的局部空间结构。
...
TL;DR: RLER提出了一种视频推理的双范式方法，将证据生成与可靠答案获取解耦，通过强化学习训练产生结构化证据，在推理时通过证据加权选举提高可靠性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有大型多模态模型的视频推理通常是单次推理，直接返回答案而不验证推理是否与证据对齐，缺乏可靠性和可解释性。

Method: 提出RLER双范式：RLER-Training使用群体相对强化学习和三个任务驱动奖励（帧敏感奖励、思维透明度奖励、抗重复奖励）优化策略；RLER-Inference使用免训练编排器生成多样化候选，解析答案和引用帧，基于证据一致性、置信度、透明度和非冗余性评分，进行证据加权选举。

Result: 在8个代表性基准测试中，RLER在所有基准上都达到了最先进水平，相比基础模型平均提升6.3%，每个问题平均使用3.1个候选，在计算和质量之间取得了良好平衡。
...
TL;DR: 该论文提出了UENR-600K数据集，这是一个包含60万对1080p帧的大规模物理基础夜间视频去雨数据集，使用虚幻引擎模拟3D雨滴粒子，显著提升了模型在真实夜间雨景中的泛化能力。
...
Method: 使用虚幻引擎在虚拟环境中将雨模拟为3D粒子，确保照片级真实感和物理真实的雨滴，捕捉颜色折射、场景遮挡、雨帘等正确细节。基于此高质量数据，通过适配Wan 2.2视频生成模型建立新的最先进基线，将去雨视为视频到视频生成任务，利用强大的生成先验来弥合模拟到真实的差距。

Result: 创建了包含60万对1080p帧的UENR-600K数据集，建立了新的最先进基线。广泛的基准测试表明，在该数据集上训练的模型在真实世界视频中的泛化能力显著提升，几乎完全弥合了模拟到真实的差距。

Conclusion: UENR-600K数据集通过物理基础的3D雨模拟解决了夜间视频去雨的关键挑战，显著提升了模型在真实夜间雨景中的性能，为夜间视频去雨研究提供了高质量的数据基础和有效的基线方法。
...
TL;DR: 该研究提出了BoxComm数据集，专注于拳击比赛的解说生成，首次探索了格斗体育的自动解说任务，并设计了类别条件生成和节奏评估两种新评测方法。


<details>
  <summary>Details</summary>
Motivation: 现有体育解说生成基准主要集中在团队运动（如足球、篮球），完全忽略了格斗体育。格斗体育具有独特挑战：关键动作在毫秒间发生且视觉差异微妙但语义决定性，专业解说中战术分析比例远高于团队运动。

Method: 提出了BoxComm数据集（445场世界拳击锦标赛视频，超过52K条专业解说句子），设计了结构化解说分类法（逐场播放、战术、背景三类），并提出了两种新评测：类别条件生成（评估模型能否生成指定类型的准确解说）和节奏评估（评估自由生成解说的时间节奏和类型分布）。

Result: 实验表明当前最先进的多模态大语言模型在两项评测上都表现不佳。进一步提出了EIC-Gen基线方法，通过检测击打事件提供结构化动作线索，获得了稳定提升，突显了感知短暂微妙事件对格斗体育解说的重要性。

Conclusion: 该研究填补了格斗体育解说生成基准的空白，揭示了当前模型在格斗体育解说任务上的局限性，提出的评测方法和改进基线为未来研究提供了重要方向。
...
<details>
  <summary>Details</summary>
Motivation: 虚拟现实需要详细且可定制的3D手部模型，但现有生成方法昂贵、繁琐且缺乏可定制性。虽然零样本文本到3D合成技术有所进展，但在手部模型生成上表现不佳，存在结构不自然、视角不一致和细节丢失等问题。

Method: 1. 使用MANO手部模型初始化提供手部结构先验；2. 引入手部骨架引导的扩散过程确保视角和姿态一致性；3. 提出新颖的矫正手部形状引导损失，确保所有视角收敛到一致的分布模式而不产生几何变形。
...
TL;DR: HSA-DINO是一个参数高效的语义增强框架，用于提升开放词汇目标检测在领域转移场景下的性能，通过多尺度提示库和语义感知路由器实现从粗到细的语义增强。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测方法在大规模预训练后能在通用场景表现良好，但在领域转移的下游任务中性能严重下降。这源于领域特定任务中类别标签稀缺且语义弱，以及现有模型无法捕捉粗粒度类别标签之外的辅助语义。

Method: 提出HSA-DINO框架：1）多尺度提示库利用图像特征金字塔捕获层次化语义，选择领域特定的局部语义提示，从粗到细逐步丰富文本表示；2）语义感知路由器在推理时动态选择适当的语义增强策略，防止参数更新降低预训练模型的泛化能力。
...
TL;DR: Chorus是一种利用请求间相似性的缓存方法，用于加速视频扩散模型推理，相比仅利用单请求内相似性的现有方法，在4步蒸馏模型上实现了高达45%的加速。
...
Method: Chorus采用三阶段缓存策略：1）对相似请求的潜在特征进行完全复用；2）在中间去噪步骤中对特定潜在区域进行请求间缓存，并结合Token-Guided Attention Amplification技术来改善生成视频与条件提示之间的语义对齐；3）将完全复用的适用性扩展到后期去噪步骤。

Result: Chorus在工业级4步蒸馏模型上实现了高达45%的加速，而先前的单请求内缓存方法在这些模型上效果不佳。

Conclusion: 通过利用请求间相似性，Chorus能够有效加速视频扩散模型的推理过程，特别是在现有方法效果有限的蒸馏模型上表现出显著优势。
...
Method: 提出了系统评估框架，在多种下游任务（零样本图像分类、语义分割、视觉问答）上评估了多种VLMs（CLIP、鲁棒CLIP、BLIP2、SigLIP2），使用精心策划的对抗数据集（排版攻击、ImageNet-A、自然语言诱导的对抗样本）。

Result: 分析表明：1）鲁棒CLIP模型可能放大自然对抗漏洞；2）CLIP模型在自然语言诱导的对抗样本上性能显著下降；3）提供了可解释性分析以识别失败模式。

Conclusion: 该研究揭示了视觉语言模型在自然对抗场景下的脆弱性，希望研究结果能启发未来在鲁棒和公平的多模态模式识别方面的研究。
...
TL;DR: VicoEdit是一种无需训练和反演的可视化上下文感知图像编辑方法，通过直接转换源图像并采用后验采样增强一致性，性能优于现有训练方法。
...
Method: 提出VicoEdit方法：1) 无需训练和反演，直接基于可视化上下文将源图像转换为目标图像；2) 设计基于概念对齐的后验采样方法增强编辑一致性。

Result: 实验结果表明，这种无需训练的方法在编辑性能上甚至优于最先进的基于训练的模型。

Conclusion: VicoEdit通过消除反演需求和增强一致性，提供了一种高效且有效的可视化上下文感知图像编辑解决方案。
...
Method: 提出基于补丁增强和跨视图正则化的统一防御框架：1) 结合补丁级数据增强与跨视图输出差异正则化，利用后门响应对非语义扰动的异常不变性，主动拉开原始视图和扰动视图的输出分布；2) 通过施加输出熵约束避免防御过程中对模型的过度抑制，确保正常指令生成质量。

Result: 在三个模型、两个任务和六种攻击的实验结果表明，所提出的防御方法能有效降低攻击成功率，同时保持高水平的正常文本生成能力。

Conclusion: 该工作使得大规模多模态模型能够在现实低频率中毒和隐蔽触发场景中安全、可控地部署，为多模态大语言模型的后门防御提供了有效解决方案。
...
TL;DR: 本文提出"因陀罗表示假说"，认为单模态基础模型的表示正在收敛，反映现实中的共享关系结构。通过范畴论中的V-富集Yoneda嵌入形式化这一假说，定义了样本间的关系轮廓表示，并在跨模型和跨模态场景中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有研究发现单模态基础模型倾向于学习收敛的表示，但这些表示本质上是样本的独立内部抽象，表达能力有限。作者受到哲学隐喻"因陀罗网"的启发，认为这些表示实际上在收敛到反映现实共享关系结构的隐含表示。

Method: 使用范畴论中的V-富集Yoneda嵌入形式化因陀罗表示假说，将因陀罗表示定义为每个样本相对于其他样本的关系轮廓。采用角距离实例化这一表示，并在涉及视觉、语言和音频的跨模型和跨模态场景中进行评估。

Result: 大量实验表明，因陀罗表示在不同架构和模态中持续增强了鲁棒性和对齐性，为单模态基础模型的无训练对齐提供了理论基础和实践框架。

Conclusion: 因陀罗表示假说为理解单模态基础模型的收敛表示提供了新的理论视角，提出的关系轮廓表示方法能够有效提升跨模型和跨模态的对齐性能，具有重要的理论和实践意义。
...
TL;DR: Saliency-R1框架通过新颖的显著性映射技术提升视觉语言模型的推理忠实度和可解释性，无需额外计算开销，并利用GRPO对齐模型关注区域与关键视觉证据。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型存在过度依赖文本线索而非视觉证据的倾向，以及产生无根据或虚构回答的风险，这影响了其可信度。

Method: 提出Saliency-R1框架：1）新颖的显著性映射技术，高效突出对生成token有贡献的关键图像区域；2）扩展用于追踪视觉信息在推理过程中的流向；3）使用显著性图与人工标注边界框的重叠作为奖励函数；4）应用GRPO对齐显著部分与关键区域。

Result: 实验表明Saliency-R1提高了推理的忠实度、可解释性和整体任务性能。

Conclusion: Saliency-R1框架有效解决了VLMs的信任问题，通过增强视觉证据的关注和推理过程的透明度，提升了模型的可靠性和性能。
...
TL;DR: MedROI是一个与编解码器无关的ROI中心框架，通过丢弃医学影像中的背景体素来提高压缩效率，无需修改现有编解码器架构。
...
Method: 使用轻量级基于强度的阈值分割提取组织边界框，丢弃背景体素，存储54字节元数据用于空间恢复，然后将裁剪后的ROI使用任何现有2D/3D编解码器压缩。

Result: 在200个T1加权脑MRI上测试6种编解码器配置，MedROI显著提高了压缩比和编码/解码时间，同时保持ROI内可比较的重建质量（HEIF在压缩比增益方面是主要例外）。

Conclusion: MedROI是一个即插即用的ROI中心框架，能显著提高医学影像压缩效率，兼容现有编解码器，无需架构修改或重新训练。
...
TL;DR: 提出StableTTA方法解决集成方法中的聚合策略冲突问题，提升预测稳定性和效率，在ImageNet-1K上实现显著精度提升，轻量架构超越ViT性能同时大幅减少参数和计算成本。


<details>
  <summary>Details</summary>
Motivation: 集成方法虽然能提升预测性能，但通常以增加内存使用和计算复杂度为代价。本文发现聚合策略中存在冲突，这会负面影响预测稳定性，因此需要一种训练免费的方法来改善聚合稳定性和效率。

Method: 提出StableTTA（训练免费测试时增强）方法，通过改进聚合策略来解决集成中的冲突问题，无需额外训练过程，直接提升模型聚合的稳定性和效率。

Result: 在ImageNet-1K上获得10.93-32.82%的top-1准确率提升，33个模型达到超过95%准确率，多个模型超过96%。轻量架构在top-1准确率上超越ViT 11.75%，同时使用不到5%的参数，计算成本减少约89.1%（以GFLOPs计）。

Conclusion: StableTTA方法有效解决了集成方法中的聚合冲突问题，显著提升了预测稳定性和效率，使轻量级架构能够在资源受限设备上实现高精度推理，为实际部署提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉SLAM中的相对相机位姿估计面临噪声对应点的挑战，经典方法依赖随机假设采样和迭代估计，而学习方法通常缺乏明确的几何结构。需要一种结合几何结构和学习能力的方法来提高估计的准确性和鲁棒性。

Method: 将相对位姿估计重新表述为极线对应图上的关系推理问题：匹配关键点作为节点，相邻点通过边连接。通过图操作（剪枝、消息传递、池化）估计四元数旋转、平移向量和本质矩阵。使用包含5个部分的损失函数进行优化：(i)与真值的L2差异，(ii)估计与真值本质矩阵的Frobenius范数，(iii)奇异值差异，(iv)航向角差异，(v)尺度差异。使用无检测器的密集匹配方法LoFTR进行特征匹配。

Result: 在室内外基准测试上的实验表明，相比经典方法和学习引导方法，该方法对密集噪声和大基线变化具有更好的鲁棒性，突出了全局关系共识的有效性。

Conclusion: 通过将相对位姿估计重新表述为图关系推理问题，结合几何结构和学习能力，能够更有效地处理噪声对应点，在视觉SLAM中实现更准确和鲁棒的相对位姿估计。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学基础模型大多孤立分析放射影像，忽略了临床实践中对比前后图像评估变化的关键任务。对于胸部X光片，捕捉时间间隔变化至关重要，因为放射科医生不仅需要评估发现的静态外观，还需要评估它们如何随时间演变。

Method: 提出TILA（时序反转感知学习与对齐）框架，使用时间反转（反转图像对）作为监督信号，增强现有时序视觉语言模型对方向性变化的敏感性。该框架在预训练、微调和推理阶段集成反转感知目标，通过显式学习时序顺序来补充传统外观建模。

Result: 在公共数据集和真实医院队列上的实验表明，TILA在应用于多种现有架构时，能持续改进进展分类和时间嵌入对齐性能。同时提出了MS-CXR-Tretrieval检索评估集和统一的评估协议。

Conclusion: TILA是一个简单而有效的框架，通过时间反转监督信号显著增强了医学视觉语言模型对时序变化的敏感性，为临床影像分析中的时间变化评估提供了重要改进。
...
Method: 提出TAPE：两阶段适应框架，通过参数高效微调将适应过程解耦为领域对齐和任务拟合。领域适应阶段在掩码图像建模的背景下应用PEFT进行医学图像领域适应。

Result: 在通用（MAE）和专用（RETFound）基础模型上应用于视网膜层分割，TAPE展示了优越的参数效率，并在多种病理情况下实现了最先进的泛化性能。

Conclusion: TAPE框架有效解决了医学图像分析中的领域适应问题，为资源受限的临床环境提供了高效的OCT/OCTA图像分析解决方案。
...
TL;DR: 本文系统评估了文本到图像扩散模型中概念遗忘方法的效果，发现遗忘效果与模型组合生成能力之间存在权衡关系：强遗忘方法会损害属性绑定、空间推理和计数能力，而保留组合结构的方法则遗忘效果不足。


<details>
  <summary>Details</summary>
Motivation: 现有的事后遗忘方法主要评估遗忘成功率，但对模型整体生成能力的影响了解不足。本文旨在通过组合文本到图像生成的视角，系统研究概念遗忘的全面影响。

Method: 以Stable Diffusion 1.4中的裸露内容移除为例，使用T2I-CompBench++和GenEval等评估基准，对多种最先进的遗忘方法进行系统实证研究。

Result: 研究发现遗忘效果与组合完整性之间存在一致权衡：实现强遗忘的方法在属性绑定、空间推理和计数方面显著退化；而保留组合结构的方法则无法提供稳健的遗忘效果。
...
TL;DR: 提出PR-IQA框架，用于评估扩散模型生成的稀疏视角新视图，无需真实数据，通过部分参考图像质量评估提升3D高斯溅射重建质量


<details>
  <summary>Details</summary>
Motivation: 扩散模型可用于稀疏视角新视图合成，但生成的图像存在光度学和几何不一致性，直接用于监督会损害3D重建质量

Method: 提出PR-IQA框架：1) 在重叠区域计算几何一致的部分质量图；2) 通过跨注意力机制结合参考视图上下文，将部分质量图补全为密集全图像质量图

Result: PR-IQA在无需真实数据监督的情况下达到全参考级别的准确性，优于现有IQA方法；集成到3DGS管道中能有效过滤不一致性，产生更优的3D重建和新视图合成结果

Conclusion: PR-IQA框架能有效评估扩散生成视图的质量，通过质量感知的3DGS方法显著提升稀疏视角3D重建效果，无需真实数据监督
...
TL;DR: Firebolt-VL：一种高效视觉语言模型，用液态基础模型解码器替代Transformer，通过Token-Grid相关模块增强视觉定位，实现细粒度理解与线性时间推理


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型计算成本高，难以部署到资源受限场景；现有方法依赖Transformer交叉注意力，二次复杂度影响效率；小模型难以精确捕捉细粒度视觉区域，导致细粒度推理任务性能下降

Method: 1) 用液态基础模型解码器替代Transformer解码器；2) 提出Token-Grid相关模块，计算文本token与图像patch的轻量级相关性，通过状态空间模型和FiLM条件进行调制

Result: 在多个基准测试中，Firebolt-VL实现了准确、细粒度的理解，同时显著提升了效率，能够选择性强调与文本提示相关的视觉区域，保持线性时间推理
...
TL;DR: 该研究通过分析15种物理特征，筛选出5种核心特征（拉普拉斯方差、索贝尔统计、残差噪声方差等），将其与语义信息结合集成到CLIP模型中，显著提升了AI生成图像检测性能，在多个基准测试中达到SOTA水平。


<details>
  <summary>Details</summary>
Motivation: 随着AIGC技术的快速发展，真实与合成图像的界限变得模糊，现有的深度伪造检测器往往过度拟合特定生成模型，存在适应性危机。需要重新审视区分自然图像与AI生成图像的内在物理特征。

Method: 1. 在20多个由不同GAN和扩散模型生成的数据集上全面探索15种物理特征；2. 提出新颖的特征选择算法，识别出5种在所有测试数据集中都表现出一致判别能力的核心物理特征；3. 将这些特征转换为文本编码值，并与语义描述结合，指导CLIP中的图像-文本表示学习。

Result: 在多个Genimage基准测试中达到最先进的性能，在Wukong和SDv1.4等数据集上获得接近完美的准确率（99.8%）。通过将像素级真实性与语义理解相结合，为可信赖的视觉语言建模开辟了新方向。

Conclusion: 该工作开创了使用物理基础特征进行可信视觉语言建模的先河，为缓解大型多模态模型中的幻觉和文本不准确问题开辟了新方向，通过像素级真实性与语义理解的结合，显著提升了AI生成图像检测的鲁棒性和准确性。
...
TL;DR: GLA提出了一种针对自动驾驶视觉语言模型的隐蔽后门攻击方法，使用涂鸦式视觉触发器和跨语言文本触发器，在仅10%投毒比例下实现90%攻击成功率，且不降低模型在干净任务上的性能。
...
Result: 在DriveVLM上的实验表明，GLA仅需10%投毒比例即可达到90%攻击成功率(ASR)和0%误报率(FPR)。更隐蔽的是，后门不会削弱模型在干净任务上的性能，反而提高了BLEU-1等指标，使传统基于性能下降的检测方法难以识别攻击。
...
TL;DR: InCTRLv2是一个新颖的少样本通用异常检测与分割框架，通过双分支架构从异常-正常判别和正常偏离语义两个互补视角进行异常感知，在多个数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前大多数异常检测方法是针对特定领域的专家模型，需要大量目标数据集训练，难以泛化到未见过的数据集。为了解决这一限制，通用异常检测范式应运而生，旨在学习单一通用模型来检测跨领域的异常而无需重新训练。

Method: InCTRLv2在原有InCTRL基础上扩展，采用双分支框架：1）主分支使用判别式异常分数学习，利用正常和异常数据学习语义引导的异常-正常空间；2）辅助分支使用单类异常分数学习，仅使用正常数据学习语义空间中的广义正常模式。两个分支都由大规模视觉语言模型编码的丰富视觉-文本语义先验指导。

Result: 在十个异常检测数据集上的广泛实验表明，InCTRLv2在各种设置下，在异常检测和分割任务中都实现了最先进的性能。

Conclusion: InCTRLv2通过双分支架构从异常-正常判别和正常偏离语义两个互补视角进行异常感知，为通用异常检测提供了有效的解决方案，显著提升了跨领域异常检测的泛化能力。
...
TL;DR: 该论文针对当前AI生成视频检测方法的局限性，提出了一个包含超过14万视频的大规模数据集和一个基于Qwen2.5-VL Vision Transformer的原生尺度检测框架，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型的快速发展使得合成媒体高度逼真，引发了关于错误信息传播的重大社会担忧。当前检测方法存在严重局限性：依赖固定分辨率调整和裁剪等预处理操作，这些操作不仅丢弃了细微的高频伪造痕迹，还导致空间失真和显著信息丢失。此外，现有方法通常在过时的数据集上训练和评估，无法捕捉现代生成模型的复杂性。

Method: 首先构建了一个大规模数据集，包含来自15个最先进的开源和商业生成器的超过14万视频，以及专门用于评估超逼真合成内容的Magic Videos基准。然后提出了一个基于Qwen2.5-VL Vision Transformer的新型检测框架，该框架原生支持可变空间分辨率和时间长度，有效保留了传统预处理中通常丢失的高频伪影和时空不一致性。

Result: 广泛的实验表明，该方法在多个基准测试中实现了卓越性能，突显了原生尺度处理的关键重要性，并为AI生成视频检测建立了强大的新基准。

Conclusion: 通过引入大规模数据集和原生尺度检测框架，解决了当前AI生成视频检测方法的局限性，为应对日益复杂的合成媒体提供了更有效的检测解决方案。
...
TL;DR: 提出Flow Divergence Sampler (FDS)框架，通过计算边缘速度场的散度来量化样本间速度冲突，并在每个求解步骤前优化中间状态，从而提升生成质量。
...
Method: 提出Flow Divergence Sampler (FDS)，这是一个无需训练的框架，在每个求解步骤前优化中间状态。关键发现是这种误导的严重程度可以通过边缘速度场的散度来量化，该散度在推理过程中可计算。FDS利用这一信号将状态引导至较少歧义区域。

Result: FDS作为一个即插即用框架，与标准求解器和现成的流模型骨干兼容，在各种生成任务中（包括文本到图像合成和逆问题）持续提升生成保真度。

Conclusion: 通过量化边缘速度场的散度来识别和缓解样本间速度冲突，FDS能够有效提升基于流模型的生成质量，且无需额外训练。
...
TL;DR: Synthesis4AD提出了一种利用大规模高保真合成异常数据来提升3D异常检测性能的端到端范式，通过3D-DefectStudio平台生成几何真实的缺陷，并引入多模态大语言模型实现可扩展的知识驱动异常合成。


<details>
  <summary>Details</summary>
Motivation: 工业3D异常检测面临异常样本稀缺和长尾分布的根本限制，这严重制约了检测性能。现有方法难以获得足够多样化的异常数据来训练鲁棒的检测模型。

Method: 1. 开发3D-DefectStudio平台，基于可控合成引擎MPAS注入几何真实的缺陷，生成点级异常掩码；2. 引入多模态大语言模型解释产品设计信息，自动生成可执行的异常合成指令；3. 提出基于空间分布归一化和几何保真数据增强的训练管道，缓解Point Transformer对绝对坐标的敏感性。

Result: 在Real3D-AD、MulSen-AD和真实工业零件数据集上的大量实验表明，该方法取得了最先进的性能。MPAS合成方法和3D-DefectStudio交互系统将在GitHub上公开。
...
TL;DR: ZeD-MAP：一种将零样本扩散深度模型转换为度量一致SLAM式映射管线的框架，通过集群级束调整实现无人机图像实时深度重建


<details>
  <summary>Details</summary>
Motivation: 无人机超高分辨率图像的实时深度重建对灾害响应等时间关键地理空间任务至关重要，但面临宽基线视差、大图像尺寸、低纹理/镜面表面、遮挡和严格计算约束等挑战。现有零样本扩散模型虽能快速预测但缺乏度量精度和时间一致性。

Method: 提出ZeD-MAP框架，将测试时扩散深度模型转换为度量一致的SLAM式映射管线。通过将流式无人机帧分组为重叠集群，周期性束调整产生度量一致的位姿和稀疏3D连接点，再将其重投影到选定帧中作为扩散深度估计的度量指导。

Result: 在约50米高度采集的地面标记飞行数据（GSD约0.85厘米/像素）验证显示，该方法达到亚米级精度：水平面误差约0.87米，垂直方向误差约0.12米，单帧处理时间在1.47-4.91秒之间。

Conclusion: 束调整为基础的度量指导提供了与经典摄影测量方法相当的精度一致性，同时显著加速处理速度，实现了实时3D地图生成，为时间关键地理空间应用提供了可行解决方案。
...
TL;DR: 该论文提出DenZa-Gaussian方法，通过3D高斯泼溅技术改进稀疏视角ADF-STEM断层扫描，解决了传统方法在有限视角下的重建质量下降问题。


<details>
  <summary>Details</summary>
Motivation: 传统ADF-STEM断层扫描需要多视角倾斜系列图像进行3D重建，但增加视角会延长电子束暴露时间，可能损伤剂量敏感材料并引入漂移和错位问题。稀疏视角采集在实际中经常需要，但传统方法在有限视角下重建质量会显著下降，产生伪影和结构保真度降低。

Method: 论文将3D高斯泼溅技术适配到ADF-STEM领域，包含三个关键组件：1) 将局部散射强度建模为可学习的标量场denza，解决3DGS与ADF-STEM成像物理不匹配问题；2) 引入系数γ稳定不同倾斜角度的散射，通过散射视角归一化确保denza一致性；3) 加入包含2D傅里叶振幅项的损失函数，抑制稀疏视角重建中的缺失楔形伪影。

Result: 在45视角和15视角倾斜系列上的实验表明，DenZa-Gaussian方法能够产生高保真度的重建结果和2D投影，这些投影与原始倾斜图像更紧密对齐，在稀疏视角条件下表现出优越的鲁棒性。

Conclusion: DenZa-Gaussian方法通过改进的3D高斯泼溅技术，有效解决了ADF-STEM稀疏视角断层扫描中的重建质量问题，在保持样本完整性的同时实现了高质量3D重建，为剂量敏感材料的纳米尺度分析提供了更好的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 世界模型作为人工智能的重要研究方向，目前缺乏清晰统一的定义，阻碍了该领域的研究进展和应用发展。作者旨在填补这一空白，为世界模型研究提供标准化框架。

Method: 基于世界模型的发展演变，提出了明确定义：以感知为中心、具备交互和长期记忆能力的模型或框架，用于理解和预测复杂世界。系统分类了世界模型的核心能力，并开发了OpenWorldLib框架，将不同任务的模型集成到统一框架中，实现高效复用和协作推理。

Result: 成功开发了OpenWorldLib框架，为世界模型研究提供了标准化工具。提出了清晰的世界模型定义和系统化的能力分类，有助于统一研究方向和促进模型复用。

Conclusion: OpenWorldLib为世界模型研究提供了重要的标准化框架和理论基础，提出的定义和分类有助于统一该领域的研究方向。作者还提供了对未来世界模型研究方向的深入思考和分析，为后续研究提供了有价值的参考。
...
TL;DR: 提出自适应KV缓存量化方法，通过数据驱动的控制器动态分配不同精度（2/4/8/FP16位），在减少内存和延迟的同时保持接近FP16的推理精度。


<details>
  <summary>Details</summary>
Motivation: LLM在移动和边缘设备部署面临KV缓存内存和带宽开销大的挑战，现有固定精度量化方案浪费比特在低重要性token上，同时过度压缩重要token，导致精度下降。

Method: 提出自适应KV缓存量化框架，提取token频率、质量分数、注意力方差和基于熵的不确定性等轻量特征，通过紧凑的数据驱动控制器动态选择KV精度（2/4/8/FP16位）。

Result: 在SmolLM-135M/360M/1.7B模型上，相比静态KV量化，解码延迟降低17.75%（SmolLM-360M在HellaSwag上），精度提升7.60分，仅比FP16推理低0.30分。

Conclusion: 自适应KV缓存量化通过动态精度分配，在减少内存和延迟的同时保持接近FP16的推理精度，为边缘设备部署LLM提供了有效的解决方案。
...
Method: 采用强化学习框架，训练提问者智能体根据候选VLM的响应自适应生成查询以引出错误答案，通过关注细粒度视觉细节和不同技能组合逐步增加问题复杂度

Result: 发现了36个新的失败模式，展示了框架在各种模型组合上的泛化能力

Conclusion: 提出的RL框架能够自动、系统地发现VLM的盲点，为模型评估和改进提供了可扩展的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 人类绘画是渐进式的：先规划全局布局，再绘制粗略草图，检查并细化细节，每个步骤都基于不断演变的视觉状态。然而，在文本-图像交错数据集上训练的统一多模态模型是否也能想象中间状态链？这促使研究者探索过程驱动的图像生成方法。

Method: 提出过程驱动的图像生成范式，将合成过程分解为交错的思想和行动推理轨迹。采用多步迭代方法，每轮包含四个阶段：文本规划、视觉草图、文本反思和视觉细化。通过密集的逐步监督，对视觉中间状态实施空间和语义一致性约束，对文本中间状态保持先验视觉知识同时识别和纠正违反提示的元素。

Result: 在多个文本到图像生成基准测试中进行了实验验证，证明该方法能够实现显式、可解释且可直接监督的生成过程。

Conclusion: 过程驱动的图像生成方法成功地将图像合成分解为多步迭代过程，使生成过程更加透明、可控，并为多模态模型的渐进式推理能力提供了新的研究方向。
...
### [124] [MinerU2.5-Pro: Pushing the Limits of Data-Centric Document Parsing at Scale](https://arxiv.org/abs/2604.04771)
...
TL;DR: MINERUPRO通过数据工程和训练策略优化，在不改变1.2B参数架构的情况下，将文档解析性能提升到SOTA水平，证明了数据质量比架构创新更重要


<details>
  <summary>Details</summary>
Motivation: 当前文档解析方法主要关注模型架构创新，而训练数据的系统工程研究不足。研究发现不同架构和参数规模的SOTA模型在相同困难样本上表现出一致的失败模式，表明性能瓶颈源于训练数据的共同缺陷而非架构本身

Method: 提出MINERUPRO方法，核心是围绕覆盖率、信息量和标注准确性共同设计的数据引擎：1）多样性和难度感知采样将训练数据从不足1000万扩展到6550万样本；2）跨模型一致性验证利用异构模型输出一致性评估样本难度并生成可靠标注；3）Judge-and-Refine管道通过渲染-验证迭代校正提高困难样本标注质量。采用三阶段渐进训练策略：大规模预训练、困难样本微调和GRPO对齐

Result: 在OmniDocBench v1.6评估中达到95.69分，比相同架构基线提升2.71分，超越所有现有方法（包括参数超过200倍的大模型）。修复了OmniDocBench v1.5中的元素匹配偏差，并引入Hard子集建立更具区分性的评估协议

Conclusion: 通过系统性的数据工程和训练策略优化，可以在不改变模型架构的情况下显著提升文档解析性能，证明了数据质量对模型性能的关键作用，为文档解析领域提供了新的研究方向
...
### [125] [CLEAR: Unlocking Generative Potential for Degraded Image Understanding in Unified Multimodal Models](https://arxiv.org/abs/2604.04780)
*Xiangzhao Hao,Zefeng Zhang,Zhenyu Zhang,Linhao Yu,Yao Chen,Yiqian Zhang,Haiyun Guo,Shuohuan Wang,Yu Sun*

Main category: cs.CV

TL;DR: CLEAR框架通过连接多模态模型的生成与理解能力，提升对退化图像（模糊、噪声、压缩、光照不佳）的鲁棒性，同时保持干净图像性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的图像退化（模糊、噪声、压缩、光照不佳）严重削弱了多模态理解能力。现有的统一多模态模型虽然结合了理解和生成能力，但未能利用自身的生成能力来处理退化输入，这源于两个因素：现有训练机制从未要求模型在推理过程中调用生成能力，以及标准的解码-重新编码路径不支持有效的联合优化。

Method: CLEAR框架通过三个渐进步骤连接生成与理解能力：1）在退化感知数据集上进行监督微调，建立"先生成后回答"的推理模式；2）潜在表示桥接，用直接可优化的连接替代解码-重新编码的迂回路径；3）交错GRPO，一种在答案正确性奖励下联合优化文本推理和视觉生成的强化学习方法。构建了MMD-Bench基准，涵盖六个标准多模态基准的三个退化严重程度级别。

Result: 实验表明，CLEAR显著提高了对退化输入的鲁棒性，同时保持了干净图像的性能。分析进一步表明，移除像素级重建监督会导致具有更高感知质量的中间视觉状态，这表明任务驱动的优化与视觉质量是自然对齐的。

Conclusion: CLEAR框架成功地将多模态模型的生成与理解能力连接起来，有效解决了现实世界中图像退化对多模态理解的影响。该方法不仅提高了对退化输入的鲁棒性，还揭示了任务驱动的优化与视觉质量之间的自然对齐关系，为多模态模型的鲁棒性研究提供了新思路。
...
### [126] [AvatarPointillist: AutoRegressive 4D Gaussian Avatarization](https://arxiv.org/abs/2604.04787)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在从单张肖像图像生成动态4D高斯化身方面存在局限性，需要一种能够自适应调整点密度和数量、同时实现逼真动画的新方法。

Method: 使用解码器专用的Transformer自回归生成用于3D高斯泼溅的点云，动态调整点密度和总数，同时预测每点的绑定信息用于动画。生成后通过专用高斯解码器将点转换为可渲染的高斯属性。

Result: 实验验证AvatarPointillist能够生成高质量、逼真且可控的化身，自回归生成器与解码器之间的条件交互显著提高了保真度。

Conclusion: 该方法代表了化身生成的新范式，自回归公式能够生成高质量、逼真且可控的动态4D高斯化身，代码将开源以促进未来研究。
...
### [127] [Multi-Modal Sensor Fusion using Hybrid Attention for Autonomous Driving](https://arxiv.org/abs/2604.04797)
...
Result: 在VoD数据集上，MMF-BEV始终优于单模态基线（相机或雷达单独），并在所有物体类别、完整标注区域和近程感兴趣区域中，与现有融合方法相比达到竞争性结果。

Conclusion: MMF-BEV通过可变形注意力机制有效融合雷达和相机模态，传感器贡献分析提供了模态互补性的可解释证据，两阶段训练策略稳定了学习过程，为自动驾驶3D目标检测提供了有效的多模态融合解决方案。
...
### [128] [E-VLA: Event-Augmented Vision-Language-Action Model for Dark and Blurred Scenes](https://arxiv.org/abs/2604.04834)
...
<details>
  <summary>Details</summary>
Motivation: 传统基于帧的视觉-语言-动作模型在极端低光、运动模糊、黑裁剪等感知退化条件下表现脆弱，需要更鲁棒的感知方案

Method: 提出事件增强VLA框架，直接利用事件流中的运动和结构线索，而非重建图像；构建开源遥操作平台和RGB-事件-动作数据集；设计轻量级事件集成策略，包括事件窗口化和融合方法

Result: 简单的事件累积图叠加RGB图像即可显著提升鲁棒性：20lux下Pick-Place成功率从0%提升至60%，使用事件适配器达90%；严重运动模糊下Pick-Place从0%提升至20-25%，Sorting从5%提升至32.5%

Conclusion: 事件驱动感知能有效集成到VLA模型中，为实现超越传统帧成像的鲁棒具身智能提供了系统证据
...
Result: 实验结果表明"少即是多"：通过有意降低视觉输入并提供有针对性的结构提示，DDP使VLMs能够绕过干扰性纹理，在具有挑战性的视觉基准上实现更优的推理准确性

Conclusion: 策略性降低图像保真度并引导模型关注结构信息，可以有效提升视觉语言模型在复杂视觉任务中的表现
...
TL;DR: 提出了一种粗到细的指令条件交互生成框架，通过动态感知策略和碰撞感知引导实现实时的人-物-场景交互生成，并采用混合训练策略解决数据稀缺问题。
...
Method: 1) 提出粗到细的指令条件交互生成框架，与一致性模型的迭代去噪过程对齐；2) 采用动态感知策略，利用前一步精炼的轨迹更新场景上下文；3) 引入碰撞感知引导，减少物理伪影；4) 设计混合训练策略，通过向HOI数据集注入体素化场景占用合成伪HOSI样本，并与高保真HSI数据联合训练。

Result: 在HOSI和HOI生成任务上均达到最先进性能，对未见场景具有强泛化能力，支持实时生成。
...
TL;DR: 本文首次系统研究了自动驾驶中视觉语言模型的灾难性遗忘问题，提出了首个专门量化该问题的基准数据集，并设计了Drive Expert Adapter框架，通过在提示空间而非权重空间进行适配，有效解决了性能提升与知识保留的权衡问题。
...
Method: 提出了Drive Expert Adapter（DEA）框架，将适配从权重空间转移到提示空间。DEA基于场景特定线索动态路由推理到不同的知识专家，在不破坏模型基础参数的情况下提升驾驶任务性能。同时创建了包含18万场景的大规模数据集，建立了首个专门量化自动驾驶中灾难性遗忘的基准。

Result: 实验表明，该方法在驾驶任务上达到了最先进的性能，同时有效缓解了灾难性遗忘，保留了视觉语言模型作为自动驾驶变革力量所必需的泛化能力。数据集和模型已在FidelityDrivingBench发布。

Conclusion: 本文首次系统研究了自动驾驶中视觉语言模型的灾难性遗忘问题，提出的DEA框架通过在提示空间进行适配，成功解决了性能提升与知识保留的权衡，为自动驾驶系统保留了视觉语言模型的泛化能力这一核心价值。
...
TL;DR: 该论文提出了一种通用的楼层平面图表示方法FML，并基于此开发了能够处理多种条件生成任务的Transformer模型FMLM，在RPLAN数据集上超越了之前的任务特定方法。


<details>
  <summary>Details</summary>
Motivation: 现有的自动住宅平面图生成方法存在局限性：早期基于约束满足或组合优化的方法缺乏多样性和灵活性，而最近的生成模型由于表示方法不够优化，难以在异构条件任务（如从场地边界、房间邻接图或部分布局生成）中实现良好泛化。

Method: 提出了Floorplan Markup Language (FML)，这是一种通用表示方法，将楼层平面图信息编码为结构化语法，将整个平面图生成问题转化为下一个令牌预测任务。基于FML开发了Transformer生成模型FMLM，能够生成高保真且功能性的平面图。

Result: 在RPLAN数据集上的综合实验表明，FMLM作为单一模型，超越了之前任务特定的最先进方法，能够生成高质量的平面图。

Conclusion: FML提供了一种有效的通用表示方法，FMLM模型展示了在多样化条件任务中生成高质量住宅平面图的能力，为自动平面图生成提供了更灵活和强大的解决方案。
...
TL;DR: 本文提出了一种基于patch级别的幻觉检测框架，通过分析细粒度token级交互来检测大型视觉语言模型中的幻觉问题，相比现有全局检测方法具有显著优势。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在视觉推理任务中表现优异，但容易产生幻觉。现有检测方法主要依赖粗粒度的全局图像度量，这种方法存在局限：幻觉token可能在许多局部区域表现出微弱但广泛分散的相关性，这些相关性聚合起来会形成欺骗性的高整体相关性，从而逃避当前的全局幻觉检测器。

Method: 基于"忠实对象token必须强烈地基于特定图像区域"的关键观察，引入patch级别的幻觉检测框架，分析模型层间的细粒度token级交互。发现幻觉token的两个特征签名：(1)产生扩散、非局部化的注意力模式；(2)未能与任何视觉区域表现出有意义的语义对齐。基于这些发现，开发了一种轻量级、可解释的检测方法，利用patch级统计特征结合隐藏层表示。

Result: 该方法在token级幻觉检测中达到高达90%的准确率，证明了细粒度结构分析在检测幻觉方面的优越性。

Conclusion: 通过分析patch级别的细粒度结构特征，可以更有效地检测大型视觉语言模型中的幻觉问题，相比全局检测方法具有明显优势，为幻觉检测提供了新的方向。
...
TL;DR: Free-Range Gaussians：一种多视角重建方法，通过流匹配预测非像素、非体素对齐的3D高斯分布，仅需4张图像即可生成高质量3D重建，在未观测区域能合成合理内容。
...
Conclusion: Free-Range Gaussians通过生成式重建和高效的高斯表示，解决了现有方法在未观测区域的局限性，实现了高质量的多视角3D重建。
...
TL;DR: HorizonWeaver是一个用于自动驾驶场景编辑的框架，通过大规模数据集、语言引导掩码和联合训练方法，解决了密集驾驶场景中的多粒度编辑、语义保持和领域迁移挑战。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶安全需要可扩展的现实驾驶场景生成，但现有图像编辑方法在处理密集、安全关键的驾驶布局时存在困难，需要解决多粒度编辑、丰富语义保持和领域迁移三大挑战。
...
Conclusion: HorizonWeaver为复杂驾驶场景提供了可扩展的、逼真的指令驱动编辑框架，显著提升了自动驾驶场景编辑的质量和实用性。
...
TL;DR: ClickAIXR是一个在XR中通过控制器点击选择真实世界物体，然后使用本地视觉语言模型处理物体图像并回答自然语言问题的设备端框架。


<details>
  <summary>Details</summary>
Motivation: 现有XR系统主要依赖云端AI（如ChatGPT）或基于注视的选择（如GazePointAR），存在隐私、延迟和交互模糊性问题。ClickAIXR旨在通过设备端视觉语言模型和控制器点击选择，提供更精确、隐私保护且透明的XR交互体验。

Method: 集成设备端视觉语言模型与控制器基础的对象选择范式，用户通过控制器精确点击XR中的真实世界物体，选择的物体图像由本地VLM处理，通过文本和语音回答自然语言问题。使用Magic Leap SDK（C API）实现，基于ONNX进行本地VLM推理。

Result: 与Gemini 2.5 Flash和ChatGPT 5相比的用户研究表明，ClickAIXR的延迟适中，用户体验可接受。点击式对象选择结合设备端AI能够推进可信赖、隐私保护的XR交互。

Conclusion: ClickAIXR展示了点击式对象选择与设备端AI结合在推进可信赖、隐私保护XR交互方面的潜力，解决了基于注视或语音接口的模糊性问题，并通过设备端推理提高了透明度。
...
TL;DR: 本文提出了一个用于图像空间编辑的完整评估框架，包括基准测试数据集、合成训练数据集和基线模型，旨在解决现有模型在细粒度空间操作方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前图像空间编辑模型在细粒度空间操作方面能力不足，缺乏专门的评估标准，需要建立一个完整的评估框架来推动该领域的发展。

Method: 提出了三个主要贡献：1) SpatialEdit-Bench基准测试，通过视角重建和构图分析联合评估感知合理性和几何保真度；2) SpatialEdit-500k合成数据集，使用可控Blender管道生成，提供精确的地面真实变换；3) SpatialEdit-16B基线模型，基于该数据构建的细粒度空间编辑模型。

Result: 该方法在通用编辑任务上表现有竞争力，在空间操作任务上显著优于现有方法，为图像空间编辑提供了有效的评估和训练框架。

Conclusion: 通过建立完整的评估基准、大规模合成数据集和基线模型，本文为图像空间编辑领域提供了重要的基础设施，有助于推动细粒度空间操作技术的发展。
...
TL;DR: DeltaWorld是一种高效的生成式世界模型，通过预测视觉基础模型特征差异的"delta token"来生成多样化的未来预测，相比现有方法参数减少35倍，计算量减少2000倍。


<details>
  <summary>Details</summary>
Motivation: 视频世界建模中预测多样化未来状态是一个核心挑战。判别式世界模型产生确定性预测，隐式地平均了可能的未来；而现有的生成式世界模型计算成本高昂。最近研究表明，在视觉基础模型的特征空间中预测未来，比在像素重建优化的潜在空间中预测需要更少的世界模型参数，但大多数方法仍是判别式的。

Method: 提出DeltaTok tokenizer，将连续帧之间的VFM特征差异编码为单个连续的"delta" token；以及DeltaWorld生成式世界模型，在这些token上操作以高效生成多样化的合理未来。delta token将视频从三维时空表示减少为一维时间序列，实现可处理的多假设训练，并行生成多个未来，只监督最佳预测。

Result: 在密集预测任务上的实验表明，DeltaWorld预测的未来更接近真实世界结果，同时参数比现有生成式世界模型少35倍以上，FLOPs减少2000倍。推理时单次前向传播即可产生多样化预测。

Conclusion: DeltaWorld通过delta token的紧凑表示和高效的多假设训练，实现了计算高效的多样化未来预测，在保持预测质量的同时大幅降低了计算成本。
...
TL;DR: Vero是一个完全开源的视觉语言模型家族，通过构建600K样本的多样化RL数据集和任务路由奖励机制，在多个视觉推理任务上达到或超越现有开源模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前最强的视觉语言模型虽然展现出广泛的视觉推理能力，但其训练方法被锁在专有的强化学习流程和非公开数据中，缺乏透明度。研究团队希望开发完全开源的视觉推理模型，揭示成功的训练方法。

Method: 构建Vero-600K数据集（包含59个数据集的600K样本），设计任务路由奖励机制处理异构答案格式，在六个广泛任务类别上扩展RL数据和奖励，从Qwen3-VL-8B-Instruct基础模型开始训练。

Result: Vero在30个挑战性基准测试套件VeroEval上平均提升3.7-5.5个百分点，在23/30个基准上超越Qwen3-VL-8B-Thinking，无需额外专有思维数据。系统消融实验表明广泛数据覆盖是RL扩展的主要驱动力。

Conclusion: 构建强大的跨领域视觉推理器需要广泛的数据覆盖，Vero展示了完全开源方法可以达到或超越现有模型的性能，所有数据、代码和模型均已开源。
...
TL;DR: 该研究提出了一种通过直接学习文本编码器输出的提示嵌入来解锁预训练扩散模型内在修复能力的方法，避免了微调或控制模块，实现了高性能的All-in-One修复。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型修复方法主要依赖微调或Control-Net风格模块来利用预训练扩散模型的先验知识，但作者发现这些预训练模型本身就具有修复能力，只是需要通过适当的方式解锁。

Method: 1. 直接在文本编码器输出端学习提示嵌入来解锁模型的修复行为；2. 采用扩散桥公式训练提示，对齐训练和推理动态，确保从噪声退化状态到干净图像的一致性去噪路径；3. 在预训练的WAN视频模型和FLUX图像模型上应用轻量级学习提示。

Result: 该方法在多种退化类型上实现了竞争性性能和泛化能力，避免了模型微调和特定修复控制模块的使用，将预训练扩散模型转化为高性能修复模型。

Conclusion: 预训练扩散模型本身就具有修复能力，可以通过学习提示嵌入而非文本提示来解锁；采用扩散桥公式确保训练稳定性；该方法为利用预训练扩散模型进行All-in-One修复提供了一种轻量高效的解决方案。
...
TL;DR: SimpleProc：一种基于NURBS和基本位移纹理模式的完全程序化多视图立体训练数据生成器，仅需少量规则即可生成有效训练数据，在小规模（8000张）上优于手动采集数据，大规模（352k张）上性能媲美甚至超越692k手动采集数据。


<details>
  <summary>Details</summary>
Motivation: 探索多视图立体（MVS）程序化规则的设计空间，解决手动采集训练数据成本高、规模受限的问题，寻求更高效、可扩展的训练数据生成方法。

Method: 提出SimpleProc完全程序化生成器，使用非均匀有理B样条（NURBS）以及基本位移和纹理模式，通过少量规则驱动生成多视图立体训练数据。

Result: 在8000张图像规模上，SimpleProc生成的数据优于同规模游戏和真实物体手动采集数据；当扩展到352,000张图像时，性能与692,000张手动采集图像相当甚至在某些基准测试中超越。

Conclusion: 程序化数据生成是多视图立体训练的有效替代方案，能够以更小的数据规模达到甚至超越大规模手动采集数据的性能，为计算机视觉任务提供了高效、可扩展的数据生成方法。
...
TL;DR: 大型视觉语言模型（VLM）的端到端延迟主要受输出token数量的瓶颈影响，研究发现大型模型用更少的输出token就能达到小型模型需要大量token才能实现的性能，因此提出了多智能体推理框架来提升效率。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型通常使用大型语言模型作为解码器，通过自回归方式顺序生成响应token，导致输出token数量成为端到端延迟的主要瓶颈。不同模型需要不同数量的输出token才能达到可比性能，需要研究如何优化这一效率问题。

Method: 1. 在模拟数据上对VLM不同组件的延迟进行全面分析；2. 在多样化真实世界基准测试中进行实证研究；3. 提出多智能体推理框架：保持大型模型生成简短响应，但在必要时从小型模型转移关键推理token。

Result: 实验表明：1. 输出token较少的大型模型比输出序列较长的小型模型更高效；2. 实证研究证实大型模型能用显著更少的输出token达到更好或可比的性能；3. 通过重用小型模型的推理token，多智能体框架能接近大型模型自身推理的性能。

Conclusion: 输出token数量是VLM延迟的关键瓶颈，大型模型在效率上具有优势。提出的多智能体推理框架通过结合大小模型的优势，在保持大型模型高效性的同时，通过转移小型模型的关键推理token来提升性能，验证了该方法的有效性。
...
TL;DR: LoMa通过数据驱动方法改进局部特征匹配，结合大规模多样化数据、现代训练方法、扩展模型容量和计算资源，在多个基准测试中显著超越现有方法


<details>
  <summary>Details</summary>
Motivation: 局部特征匹配作为3D视觉系统的基础组件，其发展滞后于现代数据驱动方法。现有方法仅在少数中等规模数据集上训练，而新方法如前馈重建模型已从大规模数据集中获益。同时，当前基准测试主要基于成功的3D重建稀疏视图，评估局限于相对简单的图像对，导致基准饱和

Method: 提出LoMa方法，从数据驱动角度重新审视局部特征匹配：1）结合大规模多样化数据混合；2）采用现代训练方法；3）扩展模型容量；4）增加计算资源。为解决基准饱和问题，收集1000个极具挑战性的互联网图像对构建HardMatch数据集，并通过人工标注获得真实对应关系
...
Conclusion: 通过数据驱动方法重新审视局部特征匹配，结合大规模数据、现代训练、扩展模型和计算资源，LoMa在多个挑战性基准上显著超越现有方法，为解决基准饱和问题贡献了HardMatch数据集
...
TL;DR: Vanast是一个统一框架，能够直接从单张人体图像、服装图像和姿态引导视频生成服装转移的人类动画视频，解决了传统两阶段方法中的身份漂移、服装变形和前后不一致问题。


<details>
  <summary>Details</summary>
Motivation: 传统虚拟试穿和姿态驱动动画的两阶段流程存在身份漂移、服装变形和前后不一致的问题，需要统一的端到端解决方案来生成连贯的服装转移动画。

Method: 构建大规模三元组监督数据，包括生成身份保持的换装图像、捕获完整上下装三元组、组装多样化的野外三元组；采用双模块视频扩散Transformer架构稳定训练，保持预训练生成质量，提高服装准确性、姿态遵循和身份保持能力。

Result: Vanast能够生成高保真、身份一致的动画视频，支持零样本服装插值，适用于广泛的服装类型，在服装准确性、姿态遵循和身份保持方面表现优异。

Conclusion: Vanast通过统一框架解决了传统两阶段方法的局限性，实现了从单张图像直接生成连贯的服装转移动画，为虚拟试穿和动画生成提供了更有效的解决方案。
