...
TL;DR: SITH是一个无需数据、无需训练的可解释性框架，通过分析CLIP视觉transformer的权重空间来理解注意力头的语义机制，支持精确的模型编辑和适应研究。


<details>
  <summary>Details</summary>
Motivation: 现有基于激活的可解释性方法存在数据集依赖、易受数据偏见影响、只能提供粗粒度解释等问题，需要开发更直接、更精确的模型内部机制分析方法。

Method: SITH框架直接分析CLIP视觉transformer的权重空间，将每个注意力头的value-output矩阵分解为奇异向量，通过COMP算法将其解释为稀疏、语义连贯的人类可理解概念组合。

Result: SITH能够产生连贯、忠实的内在头级解释，通过重建保真度和可解释性实验验证，支持精确的权重空间模型编辑，改善下游性能，并能研究模型适应机制。

Conclusion: SITH提供了一种无需数据、基于权重的可解释性方法，能够深入理解视觉语言模型的内部机制，支持精确模型编辑，并揭示了微调主要通过重新加权稳定语义基而非学习全新特征。
...
TL;DR: ReDiPrune是一种无需训练的视觉token剪枝方法，在视觉-语言投影器之前直接选择信息丰富的视觉token，在减少计算量的同时提升多模态大语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型计算成本高，因为Transformer需要处理大量视觉token。现有方法通常在投影后剪枝，这会损失细粒度空间和语义信息。

Method: ReDiPrune在视觉编码器输出和投影器之间插入，通过轻量级规则为每个token评分，综合考虑文本条件相关性和最大-最小多样性，选择既与查询相关又不冗余的token。

Result: 在4个视频和5个图像基准测试中，ReDiPrune持续改善了准确率-效率权衡。例如在EgoSchema上，仅保留15%视觉token就能获得+2.0%绝对准确率提升，同时减少6倍以上TFLOPs计算量。

Conclusion: ReDiPrune是一种即插即用的训练免费token剪枝方法，通过直接在视觉编码器输出中选择信息丰富的token，有效平衡了多模态大语言模型的准确性和计算效率。
...
TL;DR: 该论文针对统一多模态模型中的上下文学习问题，提出了一个六层能力导向的分类法来诊断演示示例的功能角色，构建了大规模数据集UniICL-760K和评估基准UniICL-Bench，并设计了轻量级的上下文自适应原型调制器来稳定少样本适应。


<details>
  <summary>Details</summary>
Motivation: 上下文学习虽然能实现免训练适应，但对示例选择和格式高度敏感，在统一多模态模型中这种敏感性因跨模态干扰和认知需求差异而加剧，导致效果非单调且高度依赖任务。需要系统诊断这些行为并提升上下文学习的稳定性。

Method: 1) 提出六层能力导向分类法，从基础感知到高阶辨别对演示示例功能进行分类；2) 构建UniICL-760K大规模语料库，包含15个子任务的8-shot上下文学习片段；3) 创建UniICL-Bench用于严格评估；4) 设计上下文自适应原型调制器，作为轻量级即插即用模块稳定少样本适应。

Result: 在UniICL-Bench上的评估显示，该方法在大多数理解型上下文学习任务上优于更大参数的多模态大语言模型基线，取得了高度竞争力的统一结果。

Conclusion: 通过认知框架、大规模数据集和轻量级架构干预，有效诊断并提升了统一多模态模型中上下文学习的稳定性和效果，为多模态上下文学习提供了系统化的解决方案。
...
TL;DR: BCMDA框架通过虚拟域桥接和原型对齐解决混合域半监督医学图像分割中的域偏移和标注不足问题，在有限标注下取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 混合域半监督医学图像分割面临两个主要挑战：1) 标注数据与未标注数据之间的分布差异阻碍有效知识迁移；2) 从未标注数据中学习效率低下导致严重的确认偏误。

Method: 提出双向相关图域适应框架：1) 通过虚拟域桥接进行知识迁移，利用双向相关图合成标注和未标注图像，采用固定比例和渐进动态MixUp策略生成虚拟图像；2) 使用原型对齐和伪标签校正缓解确认偏误，通过可学习原型余弦相似度分类器实现虚拟域与真实域之间的双向原型对齐。

Result: 在三个公开多域数据集上的实证评估表明该方法具有优越性，特别是在非常有限的标注样本下仍能表现出色。

Conclusion: BCMDA框架有效解决了混合域半监督医学图像分割中的域偏移和标注不足问题，通过虚拟域桥接和原型对齐机制实现了更好的知识迁移和特征表示。
...
TL;DR: LLaVA-LE是一个专门用于月球表面和次表面特征分析的视觉语言模型，通过构建大规模月球数据集LUCID（包含96k图像-描述对和81k问答对）并进行两阶段微调，显著提升了在月球地形分析任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态视觉语言模型在行星科学领域的应用尚未充分探索，主要障碍是缺乏大规模的行星图像与科学描述配对数据集。本研究旨在开发专门用于月球探索的视觉语言模型，以支持月球表面和次表面特征分析。

Method: 1. 构建LUCID数据集：包含96k高分辨率全色图像与详细地形描述，以及81k从约20k图像中生成的问答对；2. 两阶段微调：第一阶段进行领域特定地形描述的概念对齐，第二阶段进行指令调优的视觉问答训练。

Result: LLaVA-LE相比基础LLaVA模型获得了3.3倍的整体性能提升，相比第一阶段模型提升了2.1倍，推理得分达到1.070，超过了评判者自身的参考得分，证明了领域特定多模态数据和指令调优的有效性。

Conclusion: 通过构建大规模月球数据集和专门的两阶段训练方法，LLaVA-LE显著提升了在月球地形分析任务上的性能，展示了领域特定多模态数据和指令调优在推进行星探索视觉语言模型发展中的重要作用。
...
TL;DR: 提出室内场景中的相似物体检测任务，利用相同和近似的物体对之间的重复和互补线索，通过多视角图像区分物体对为相同、相似或不同。


<details>
  <summary>Details</summary>
Motivation: 现有的3D物体理解和生成方法虽然效果显著，但往往忽略了真实场景中普遍存在的信息源：重复物体。室内场景中经常出现相同或相似的物体，这些重复模式可以作为强大的感知线索，但现有方法未能充分利用这一信息。

Method: 提出了Lookalike3D，一个多视角图像变换器，通过利用大型图像基础模型的强语义先验，有效区分相同、相似和不同的物体对。同时构建了3DTwins数据集，包含基于ScanNet++的76k手动标注的物体对。

Result: 在3DTwins数据集上，该方法相比基线提升了104%的IoU。实验表明该方法能够改进下游任务，如联合3D物体重建和部件共分割，将重复和相似物体转化为一致、高质量3D感知的强大线索。

Conclusion: 室内场景中的相似物体检测是一个有价值的研究方向，Lookalike3D方法通过利用多视角图像和大型基础模型的语义先验，能够有效识别相同和相似的物体对，为3D感知任务提供了新的强大线索。
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅在实时渲染方面表现出色，但其在精确几何测量方面的潜力尚未充分开发。当前的点测量方法依赖于昂贵的立体工作站或不完整、不准确的3D网格，需要更高效、准确的测量方案。

Method: 利用3DGS作为新颖视图合成器，渲染精确的源视图并平滑插值中间视图。用户可以在不同视图中直观选取共轭点，通过三角测量这些共轭点来生成精确的3D点测量。该方法支持多视图交会（超过两个视图）以提高测量精度。

Result: 该方法在无人机航拍数据集上实现了厘米级精度（RMSE 1-2厘米）。在薄结构测量中，RMSE为0.037米，优于网格方法的0.062米。在网格重建不佳的尖锐角点处，该方法成功测量所有点（RMSE 0.013米），而网格方法完全失败。

Conclusion: 3DGS不仅可用于高质量视图合成，还能实现高精度几何测量。该方法无需昂贵立体工作站或专业操作员立体视觉能力，通过基于Web的应用验证了概念，在标准硬件上达到或超过传统立体测量方法的精度。
...
TL;DR: QuatRoPE是一种新颖的位置编码方法，用于3D空间推理任务，通过线性复杂度编码对象间空间关系，并引入IGRE机制保护LLM原有能力。


<details>
  <summary>Details</summary>
Motivation: 由于3D场景-语言配对数据有限，难以从头训练具有强推理能力的模型。现有方法存在两个问题：绝对位置编码难以从过早融合的特征中提取空间关系；显式编码所有空间关系的方法（二次复杂度）可扩展性差。
...
Result: QuatRoPE的3D坐标整体向量编码保证了高度的空间一致性，保持了场景几何完整性。实验证明该方法有效。

Conclusion: QuatRoPE解决了3D空间推理中位置编码的局限性，通过线性复杂度的关系编码和隔离机制，在保持LLM原有能力的同时提升了空间推理性能。
...
TL;DR: 基于面部关键点的轻量级视线估计方法在跨域评估中表现出与深度CNN相当的泛化能力，为高效、可解释、隐私友好的边缘应用提供了可能。


<details>
  <summary>Details</summary>
Motivation: 基于深度CNN的视线估计方法虽然准确，但计算成本高且缺乏可解释性；基于面部关键点的几何方法作为轻量级替代方案，其性能极限和泛化能力在现代基准测试中尚未得到充分探索。

Method: 提出标准化流程从三个大规模数据集(Gaze360、ETH-XGaze、GazeGene)提取和归一化面部关键点，训练轻量级回归模型：极限梯度提升树、整体多层感知机(MLP)以及设计用于捕捉双眼几何结构的孪生MLP。
...
TL;DR: 本文提出了一种用于3D高斯泼溅的自监督置信度框架，通过动态平衡光度与几何监督，结合改进的外观模型，实现了无界网格的高效高质量提取。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅虽然加速了从姿态图像中提取网格的过程，但在具有丰富视角依赖效应的场景中，网格提取仍然困难。现有方法依赖多视图技术、迭代网格提取或大型预训练模型，牺牲了3DGS的固有效率。
...
TL;DR: 该研究利用语义模糊图像作为可解释性探针，通过心理物理学框架在CLIP嵌入空间中插值生成连续模糊图像谱，比较人类和机器学习分类器在概念边界上的差异。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探索当视觉证据模糊时，人类大脑如何决定看到什么，以及机器学习分类器是否在相同位置划定概念边界。经典的鸭兔错觉揭示了人类在模糊视觉输入下的决策过程，但机器模型是否以相同方式处理这种模糊性尚不清楚。

Method: 提出心理物理学框架，在CLIP嵌入空间中插值概念，生成连续模糊图像谱。通过精确测量人类和机器分类器放置语义边界的位置和方式，比较两者的差异。使用CLIP模型进行图像合成和分类。

Result: 结果显示机器学习分类器更偏向看到"兔子"，而人类更接近用于合成的CLIP嵌入。引导尺度对人类敏感性的影响比对机器分类器更强。机器与人类在概念边界划分上存在系统性差异。

Conclusion: 受控模糊性可作为诊断工具，弥合人类心理物理分析、图像分类和生成图像模型之间的差距。该框架为人类-模型对齐、鲁棒性、模型可解释性和图像合成方法提供了新见解。
...
TL;DR: OpenCap Monocular：通过单部智能手机视频估计3D骨骼运动学和动力学，实现可扩展的生物力学评估


<details>
  <summary>Details</summary>
Motivation: 传统生物力学分析需要在专业实验室进行，成本高、耗时长，限制了临床转化。需要开发可扩展、准确的生物力学评估工具。

Method: 通过单部智能手机视频，使用WHAM单目姿态估计模型获取3D人体姿态，通过优化方法进行精炼，计算生物力学约束骨骼模型的运动学，并通过物理模拟和机器学习估计动力学。

Result: 与基于标记的运动捕捉和测力板数据相比，OpenCap Monocular在运动学方面误差低（旋转自由度平均绝对误差4.8°，骨盆平移3.4厘米），比仅回归的计算机视觉基线在旋转精度上提升48%，平移精度提升69%。在动力学方面，步行的地面反作用力估计精度与之前的两相机系统相当或更好。

Conclusion: OpenCap Monocular能够以临床意义精度估计重要的动力学结果，已通过智能手机应用、网页应用和安全云计算部署，实现免费、可访问的单智能手机生物力学评估。
...
TL;DR: TIGeR模型通过统一的地理时空嵌入空间，将图像、地理位置和时间信息融合，支持地理定位、拍摄时间预测和地理时间感知检索，在多个任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 数字取证、城市监控和环境分析等实际应用需要同时处理视觉外观、地理位置和时间信息。现有方法主要关注单一任务（地理定位或时间预测），缺乏对复杂查询（如相同位置但不同时间的图像检索）的支持。

Method: 提出TIGeR模型，基于多模态transformer架构，将图像、地理位置和时间映射到统一的geo-temporal嵌入空间。模型支持灵活的单模态或多模态查询配置，使用相同表示完成三项任务：地理定位、拍摄时间预测和地理时间感知检索。

Result: 构建了包含450万训练样本和8.6万高质量评估样本的数据集。实验表明TIGeR在多个任务上显著优于基线方法和SOTA：时间预测（年份+16%，日期+8%），地理时间感知检索召回率提升14%。模型能更好地保持位置身份特征，减少外观变化的影响。

Conclusion: 统一的地理时空建模方法能有效支持复杂的地理时间感知检索任务，TIGeR模型通过融合多模态信息，实现了基于场景"何时何地"而非单纯视觉相似性的检索，为实际应用提供了更强大的能力。
...
TL;DR: DRoPS：利用静态预扫描作为几何先验，通过网格化表面对齐的高斯原语和CNN运动参数化，显著提升动态场景重建质量
...
Method: 1) 使用静态预扫描作为显式几何和外观先验；2) 将高斯原语组织成锚定在物体表面的像素网格；3) 利用CNN参数化运动，注入强隐式正则化并关联相邻点运动

Result: 在渲染质量和3D跟踪精度方面显著优于当前最先进方法，特别是在极端新视角和高度关节运动场景下

Conclusion: DRoPS通过静态预扫描先验、网格化表面对齐模型和CNN运动参数化，有效约束解空间并确保几何一致性，显著提升动态场景重建性能
...
TL;DR: AVControl是一个轻量级可扩展框架，基于LTX-2音频视觉基础模型，通过独立的LoRA适配器实现多种控制模态的视频音频生成，无需架构修改。


<details>
  <summary>Details</summary>
Motivation: 现有视频音频生成控制方法要么训练单一固定控制集的大型模型，要么需要为每个新模态进行昂贵的架构修改，缺乏轻量级可扩展的解决方案。

Method: 基于LTX-2联合音频视觉基础模型，每个控制模态作为独立的LoRA适配器在并行画布上训练，将参考信号作为注意力层的额外令牌，仅需LoRA适配器无需架构变更。

Result: 在VACE基准测试中，在深度和姿态引导生成、修复和扩展方面优于所有基线，在相机控制和音频视觉基准上表现竞争性；支持多种独立训练模态，计算和数据效率高。

Conclusion: AVControl提供了一个轻量级、可扩展的框架，能够高效支持多种控制模态的视频音频生成，每个模态仅需少量数据和训练步骤，优于传统大型模型方法。
...
TL;DR: 提出Calibri方法，通过引入单个学习缩放参数和进化算法优化DiT组件，仅修改约100个参数即可显著提升文本到图像生成模型的性能并减少推理步骤
...
Conclusion: Calibri是一种参数高效的方法，通过优化校准DiT组件显著提升生成质量，展示了通过最小参数修改实现显著性能改进的潜力
...
TL;DR: 提出一种基于可解释AI指导的编码器塑形框架，通过XAI场引导模型关注关键区域，提升复杂腹主动脉瘤CT图像分割的准确性和可靠性


<details>
  <summary>Details</summary>
Motivation: 复杂腹主动脉瘤CT图像分割常因模型关注无关结构或忽略薄层低对比度目标而失败，需要引导模型关注正确区域

Method: 计算密集的基于归因的编码器关注图（XAI场），通过两种方式使用：1）对齐预测概率质量与XAI场；2）将XAI场输入轻量级细化路径和置信度先验，在推理时调制logits

Result: 相比基础SAM设置，在临床验证的挑战性病例上获得显著改进，表明通过XAI指导明确优化编码器关注是复杂场景中可靠分割的有效原则

Conclusion: 将归因指导集成到表示和解码中是提升复杂医学图像分割性能的实用有效方法，特别适用于失败风险高的场景
...
<details>
  <summary>Details</summary>
Motivation: 现有大规模视觉语言模型依赖数十亿样本数据集，这成为研究进展的重大障碍。虽然近期工作通过提升监督质量来缩小差距，但每个方法只解决了对比预训练中的部分弱点。

Method: GoldiCLIP采用多方面的训练框架，结合三个关键创新：(1) 文本条件自蒸馏方法，对齐文本无关和文本条件特征；(2) 集成解码器的编码器，通过视觉问答目标使编码器超越标题式查询；(3) 基于不确定性的加权机制，自动平衡所有异构损失。

Result: 仅用3000万图像训练（比领先方法少300倍数据），GoldiCLIP在数据高效方法中达到最先进水平：在MSCOCO检索上提升2.2分，细粒度检索提升2.0分，基于问题的检索提升5.9分，同时与数十亿规模模型保持竞争力。

Conclusion: GoldiCLIP通过找到监督信号的恰当平衡，显著降低了视觉语言模型训练的数据需求，为数据高效的多模态学习提供了有前景的解决方案。
...
TL;DR: 该论文收集了针道伤口感染数据集，提出了一种基于注意力的深度学习模型来分类针道图像，区分有感染迹象和无明显并发症的针道，模型性能优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 针道是外部环境与肢体内部环境的接口，容易发生感染，导致患者疼痛和发病率增加。改善针道感染的识别和管理可以显著提升使用外固定器患者的体验。

Method: 收集并制作针道伤口感染数据集，提出基于注意力的深度学习模型，强调相关区域并减少针的干扰。引入高效冗余重建卷积（ERRC）方法来增强特征图丰富性同时减少参数数量。

Result: 模型性能优于基线方法，AUC达到0.975，F1分数为0.927，仅需5.77M参数。深度学习能够基于视觉感染迹象区分针道，与医疗专业人员评估一致。

Conclusion: 深度学习在仅基于视觉感染迹象区分针道方面具有潜力，与医疗专业人员评估相符，但需要更多数据进行进一步验证。
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击研究主要集中在密度图模型之间的可迁移性，但跨范式攻击（密度图与点回归模型之间）尚未探索。考虑到人群计数领域的安全影响，需要研究能够同时攻击两种主流架构的对抗攻击方法。

Method: 提出综合多任务损失优化框架：对点回归模型使用场景密度特定的高置信度逻辑抑制；对密度图方法使用峰值目标密度图抑制；结合模型无关的感知约束确保扰动有效且对人眼不可察觉。

Result: 攻击效果显著，平均使平均绝对误差增加7倍，同时保持有竞争力的视觉质量；成功迁移到7个最先进的人群模型，迁移比在0.55到1.69之间；在攻击效果和不可察觉性之间取得良好平衡。

Conclusion: 该研究首次实现了跨范式对抗攻击，能够同时威胁密度图和点回归两种人群计数架构，揭示了现有模型的安全脆弱性，为开发更鲁棒的防御方法提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: 长轨迹视频生成是世界建模的关键但具有挑战性的任务。现有视频扩散模型（VDMs）可扩展性有限，自回归模型虽然能无限展开但存在视觉漂移和可控性差的问题。需要一种既能保持结构稳定性又能实现高保真生成的解决方案。

Method: 提出DCARL分治自回归框架：1）使用专门的关键帧生成器（无时间压缩训练）建立长范围、全局一致的结构锚点；2）插值生成器以自回归方式合成密集帧，采用重叠片段，利用关键帧提供全局上下文，单个干净的前一帧确保局部连贯性。

Result: 在大规模互联网长轨迹视频数据集上训练，在视觉质量（更低的FID和FVD）和相机一致性（更低的ATE和ARE）方面优于最先进的自回归和分治基线方法，能够稳定生成长达32秒的高保真长轨迹视频。

Conclusion: DCARL通过结合分治方案的结构稳定性和VDMs的高保真生成能力，有效解决了长轨迹视频生成中的视觉漂移和可控性问题，为世界建模提供了可靠的长视频生成解决方案。
...
TL;DR: WAFT-Stereo是一种基于warping的立体匹配方法，无需cost volumes，在多个基准测试中表现最佳，速度提升1.8-6.7倍


<details>
  <summary>Details</summary>
Motivation: 传统立体匹配方法通常依赖cost volumes，但这种方法计算复杂且效率较低。作者希望证明cost volumes不是高性能立体匹配的必要组件，可以通过更高效的warping操作替代。

Method: 提出WAFT-Stereo方法，采用基于warping的架构替代传统的cost volumes。该方法简化了立体匹配流程，通过warping操作直接建立图像间的对应关系，提高了计算效率。

Result: 在ETH3D、KITTI和Middlebury公开基准测试中排名第一，在ETH3D基准上将zero-shot误差降低了81%，同时比竞争方法快1.8-6.7倍。

Conclusion: WAFT-Stereo证明了cost volumes不是高性能立体匹配的必要组件，可以通过更简单高效的warping方法实现更好的性能和速度，为立体匹配领域提供了新的设计思路。
...
TL;DR: 该研究对20种前沿多模态大语言模型在神经影像诊断任务上进行了全面基准测试，评估了它们在MRI和CT图像上的诊断性能、结构化输出能力及计算效率。
...
Method: 使用涵盖多发性硬化、中风、脑肿瘤、其他异常和正常对照的MRI和CT数据集，要求模型同时生成诊断、亚型、成像模态、专业序列和解剖平面等多重输出。采用多阶段框架确保公平比较，控制选择偏差，从四个维度评估：带弃权的判别分类、校准、结构化输出有效性和计算效率。

Result: 技术成像属性（如模态和平面）基本解决，但诊断推理特别是亚型预测仍具挑战性。肿瘤分类最可靠，中风中等可解，多发性硬化和罕见异常仍困难。少样本提示可提升性能但增加计算成本。Gemini-2.5-Pro和GPT-5-Chat诊断性能最强，Gemini-2.5-Flash效率-性能权衡最佳。开源模型中MedGemma-1.5-4B表现最突出，在少样本提示下接近多个专有模型的零样本性能。

Conclusion: 研究为多模态大语言模型在神经影像中的性能、可靠性和效率权衡提供了实用见解，支持该领域的标准化评估，有助于指导临床决策支持系统的开发和应用。
...
Method: 提出CORA模型，采用病理中心、合成驱动的自监督框架。通过解剖引导的病变合成引擎，模型被明确训练检测模拟的血管异常，使表征学习偏向临床相关的疾病特征而非主导的背景解剖结构。在12,801个未标注CCTA体积上进行训练，并将成像编码器与大型语言模型结合形成多模态框架。

Result: 在来自9家独立医院的多中心数据集上评估，CORA在诊断和解剖任务（包括斑块表征、狭窄检测和冠状动脉分割）上始终优于最先进的3D视觉基础模型，性能提升高达29%。多模态框架显著改善了30天主要不良心脏事件的风险分层。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感视觉定位方法局限于单一传感器域（光学或SAR），限制了实际应用。需要开发能够处理跨域数据的框架，以提升模型的泛化能力和实用性。

Method: 提出OptiSAR-Net++框架，包含：1) 补丁级低秩适应专家混合(PL-MoE)用于跨域特征解耦；2) 基于CLIP的对比范式结合动态对抗负采样，将生成式回归转化为跨模态匹配；3) 文本引导双门融合模块(TGDF-SSA)和区域感知辅助头增强语义-视觉对齐和空间建模。

Result: 在首个大规模跨域基准数据集OptSAR-RSVG和DIOR-RSVG上达到最先进性能，在定位精度和效率方面均有显著优势。

Conclusion: OptiSAR-Net++成功解决了跨域遥感视觉定位的挑战，通过高效的特征解耦、计算优化和语义增强机制，为实际应用提供了有效的解决方案。
...
TL;DR: 该研究提出了一种结合自监督学习、鲁棒时序建模和可扩展数据标注策略的垂体瘤手术视频阶段识别框架，在测试集上达到90%准确率，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 准确的手术阶段识别对于分析手术流程、支持术中决策以及实现数据驱动的手术教育和绩效评估改进至关重要。

Method: 1) 开发协作在线平台供外科医生上传手术视频、接收自动阶段分析并贡献数据；2) 在251个未标记PTS视频上使用自监督框架预训练ResNet-50模型；3) 在81个标记手术数据集上使用包含焦点损失、渐进层解冻和动态采样的改进训练方案进行微调。

Result: 在保留测试集上达到90%的准确率，优于当前最先进方法，并在不同手术案例中表现出强大的泛化能力。

Conclusion: 该框架通过结合自监督表示学习、鲁棒时序建模和可扩展数据标注策略，实现了准确的手术阶段识别，同时协作平台促进了大规模数据收集和持续模型改进。
...
TL;DR: 自监督学习在膝关节骨关节炎诊断和预后建模中的应用评估：图像自监督学习在诊断任务中表现不一，多模态图像-文本自监督学习因训练数据偏倚在诊断任务中效果有限，但在预后预测任务中显著优于ImageNet预训练


<details>
  <summary>Details</summary>
Motivation: 评估自监督学习（SSL）相对于ImageNet预训练初始化在膝关节骨关节炎诊断和预后建模中的改进效果，特别关注图像自监督和多模态图像-文本自监督两种方法

Method: 比较两种SSL方法：(1) 图像自监督学习：在OAI、MOST和NYU队列的膝关节X光片上预训练；(2) 多模态图像-文本自监督学习：在未筛选的医院膝关节X光片（配放射科医生印象报告）上预训练。评估诊断任务（KL分级预测）和预后任务（4年结构发生和进展预测）

Result: 诊断任务：图像自监督在线性探测（冻结编码器）时提高准确性，但在完全微调时未超越ImageNet预训练；多模态自监督因训练数据严重偏倚（93% KL 3级）未能改善分级性能。预后任务：多模态初始化显著优于ImageNet基线，在外部验证中表现更好（MOST AUROC：0.701 vs. 0.599，10%标记数据）

Conclusion: 未筛选的医院图像-文本数据由于严重偏倚可能不适合学习诊断任务，但当下游任务与预训练数据分布一致时，为预后建模提供了强有力的信号。自监督学习的有效性取决于预训练数据与下游任务的对齐程度
...
TL;DR: 该论文提出了一个大规模的真实世界物体偏振反射和材料数据集，用于解决逆渲染中材料建模的挑战，显著提升了材料分离、光照保真度和几何一致性。


<details>
  <summary>Details</summary>
Motivation: 准确建模真实世界材料的光反射是逆渲染的核心挑战，主要由于缺乏真实的测量反射数据。现有方法严重依赖合成数据集，这些数据集光照简化且材料真实性有限，导致模型难以泛化到真实世界图像。

Method: 引入了一个大规模的真实世界物体偏振反射和材料数据集，使用配备交叉/平行偏振的8相机、346光源的光照舞台捕获。数据集涵盖218个日常物体，跨越五个采集维度：多视角、多光照、偏振、反射分离和材料属性，产生超过120万张高分辨率图像，包含漫反射-镜面反射分离以及分析推导的漫反射反照率、镜面反射反照率和表面法线。

Result: 使用该数据集训练和评估了最先进的逆渲染和正向渲染模型，在固有分解、重光照和稀疏视角3D重建方面展示了显著改进，包括材料分离、光照保真度和几何一致性方面的提升。

Conclusion: 这项工作为基于物理的材料理解建立了新的基础，并实现了超越合成训练机制的真实世界泛化能力，有望推动逆渲染和材料建模领域的发展。
...
Method: 1. 第一阶段：构建基于条件流匹配的预测器，加入轨迹交互图模块建模细粒度视觉空间交互，增强上下文编码；2. 第二阶段：进行Flow-GRPO后训练，将确定性流展开重新表述为随机ODE-to-SDE采样以实现轨迹探索，使用复合奖励结合视角感知的社会合规性和地图感知的物理可行性。
...
TL;DR: 提出基于自回归扩散模型的无限时长原始注视轨迹生成框架，用于视频中的长期注视预测


<details>
  <summary>Details</summary>
Motivation: 传统显著性图和扫描路径方法无法捕捉原始注视的细粒度时间动态，且现有模型局限于短期窗口（3-5秒），无法捕获真实世界内容中的长期行为依赖关系

Method: 使用自回归扩散模型生成具有连续空间坐标和高分辨率时间戳的注视轨迹，模型基于显著性感知的视觉潜在空间进行条件生成

Result: 定量和定性评估表明，该方法在长期时空准确性和轨迹真实性方面显著优于现有方法

Conclusion: 提出的生成框架能够预测任意长度视频中的无限时长原始注视轨迹，解决了传统方法的局限性
...
TL;DR: 提出TIES方法，通过动态平衡注意力大小和层间排名一致性，减少视觉语言动作模型中的视觉token数量，降低推理延迟


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言动作模型在处理密集视觉token时存在显著的推理延迟问题。现有的token减少方法主要依赖注意力大小作为静态选择标准，但研究发现高注意力token是任务依赖的，甚至可能降低策略性能

Method: 提出TIES框架，通过tau引导的层间高效选择，利用层间token排名一致性作为动态指导，自适应地平衡注意力大小和排名一致性，无需额外训练即可实现鲁棒的token选择

Result: 在CogACT + SIMPLER基准测试中，TIES将平均成功率提高了6%，同时减少了78%的token使用量，并在不同的解码器和基准测试中表现出强大的泛化能力

Conclusion: TIES通过动态的层间token排名一致性指导，有效解决了视觉语言动作模型中的token减少问题，在保持性能的同时显著降低了推理延迟
...
TL;DR: xLARD是一个自校正框架，利用多模态大语言模型通过可解释的潜在奖励指导图像生成，解决复杂提示对齐问题


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成在复杂提示对齐方面存在挑战，特别是细粒度语义和空间关系。生成过程的feed-forward特性使得在完全理解输出前就需要预测对齐，而评估生成图像相对更容易。基于这种不对称性，提出了xLARD框架

Method: xLARD使用多模态大语言模型通过Explainable LAtent RewarDs指导生成。引入轻量级校正器，基于模型生成参考的结构化反馈来精炼潜在表示。关键组件是从潜在编辑到可解释奖励信号的可微分映射，使得不可微分的图像级评估能够提供连续的潜在级指导

Result: 在多样化的生成和编辑任务实验中，xLARD提高了语义对齐和视觉保真度，同时保持了生成先验

Conclusion: xLARD通过自校正机制使模型能够在生成过程中理解、评估和校正自身，有效解决了复杂提示对齐问题，为文本到图像生成提供了新的改进方向
...
TL;DR: PASDiff是一种无需训练的物理感知语义扩散方法，用于解决真实世界低光人脸图像的多重退化问题，通过逆强度加权和Retinex理论恢复可见度，利用风格无关结构注入重建面部细节。


<details>
  <summary>Details</summary>
Motivation: 真实世界低光人脸图像存在多重退化问题（低光照、模糊、噪声、低可见度等），现有级联方法存在严重误差累积，通用联合模型缺乏明确的面部先验且难以恢复清晰面部结构。

Method: 提出PASDiff（物理感知语义扩散）方法：1）利用逆强度加权和Retinex理论引入光度约束，可靠恢复可见度和自然色彩分布；2）提出风格无关结构注入（SASI），从现成面部先验中提取结构同时过滤其固有光度偏差，将身份特征与物理约束无缝融合；3）构建WildDark-Face基准数据集。

Result: 大量实验表明，PASDiff显著优于现有方法，在自然光照、色彩恢复和身份一致性之间实现了优越的平衡。

Conclusion: PASDiff通过物理感知语义扩散方法有效解决了真实世界低光人脸图像的多重退化问题，无需训练即可实现高质量的面部恢复。
...
TL;DR: MoE-GRPO：基于强化学习的专家路由优化框架，用于提升视觉语言混合专家模型的路由多样性，解决确定性top-K路由导致的专家过拟合问题


<details>
  <summary>Details</summary>
Motivation: 混合专家模型通过稀疏激活参数降低Transformer计算成本，但广泛采用的确定性top-K路由机制可能忽略更优的专家组合，导致专家过拟合，需要提升专家选择的多样性

Method: 提出MoE-GRPO强化学习框架，将专家选择建模为序列决策问题，使用组相对策略优化学习自适应路由策略；引入模态感知路由指导，通过抑制不常激活的专家来增强训练稳定性

Result: 在多模态图像和视频基准测试中，MoE-GRPO持续优于标准top-K路由及其变体，通过促进更多样化的专家选择来缓解专家过拟合，实现任务级专家专业化

Conclusion: MoE-GRPO通过强化学习优化专家路由，有效解决了确定性路由的局限性，提升了混合专家视觉语言模型的性能和专家选择多样性
...
TL;DR: 提出基于MAML的K-shot 3D左心房壁分割框架，通过元训练结合辅助任务和边界感知损失，在少样本情况下实现薄壁结构的准确分割。


<details>
  <summary>Details</summary>
Motivation: 左心房壁的LGE-MRI分割面临薄壁几何结构、低对比度和专家标注稀缺的挑战，需要开发能够在小样本情况下准确分割的方法。

Method: 采用模型无关元学习框架，在元训练中结合左心房壁主任务和左右心房腔辅助任务，使用边界感知复合损失函数强调薄结构准确性。

Result: 在5-shot情况下，MAML的Dice得分为0.64（vs. 0.52），HD95为5.70mm（vs. 7.60mm）；在20-shot时接近全监督参考（0.69 vs. 0.71 DSC）。在未见域偏移下仍保持稳健性能。
...
TL;DR: 本文提出了首个事件流视频异常检测框架EWAD，构建了多个事件流基准数据集，通过事件密度感知采样、密度调制时序建模和RGB到事件的知识蒸馏机制，显著提升了事件流异常检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 3D LGE-MRI中左心房壁分割面临挑战：壁薄、解剖结构复杂、对比度低，准确分割对壁厚映射和纤维化量化至关重要。

Method: 提出C2W-Tune两阶段框架：第一阶段使用3D U-Net（ResNeXt编码器+实例归一化）预训练分割左心房腔室；第二阶段通过渐进式层解冻策略将权重转移到壁分割任务，保留心内膜特征的同时进行壁特异性优化。
...
Conclusion: 基于解剖学的任务转移与受控微调相结合，显著提高了3D LGE-MRI中薄左心房壁分割的边界准确性，为临床壁厚映射和纤维化量化提供了可靠工具。
...
TL;DR: 提出了一种基于3D高斯泼溅的动态场景重建新方法，通过视图空间射线分组策略保持高斯分布的时间一致性，无需外部先验即可实现更真实的物理运动建模。


<details>
  <summary>Details</summary>
Motivation: 当前基于3D高斯泼溅的动态场景重建方法在建模真实运动方面存在挑战，特别是对于单目视频数据集，大多数方法无法将高斯的运动与现实世界的物理动力学对齐。这种不对齐会破坏局部几何结构，导致重建质量下降，且现有方法过度依赖光流或2D轨迹等外部先验来强制时间一致性。

Method: 提出了一种新颖的方法来明确保持4D场景中高斯分布的时间局部几何结构。核心思想是引入视图空间射线分组策略，将同一射线相交的高斯进行聚类，仅考虑α混合权重超过阈值的高斯。然后对这些组施加约束以保持一致的空间分布，有效保持其局部几何结构。这种方法通过确保局部几何随时间保持稳定，强制了更物理合理的运动模型，消除了对外部指导的依赖。

Result: 将方法集成到两个不同的基线模型中，在具有挑战性的单目数据集上进行广泛实验。结果表明，该方法显著优于现有方法，实现了卓越的时间一致性和重建质量。

Conclusion: 提出的视图空间射线分组策略能够有效保持动态3D场景重建中高斯分布的局部几何结构，无需依赖外部先验即可实现更物理合理的运动建模，显著提升了时间一致性和重建质量。
...
TL;DR: 提出了一种分布式车辆控制方法，用于紧急车辆快速通行，仅使用局部信息实现实时决策，无需预训练且具有自然适应性


<details>
  <summary>Details</summary>
Motivation: 现有方法（集中式数学求解器和强化学习）存在计算成本高和可扩展性差的问题，无法在实际大规模场景中有效确保紧急车辆快速通行同时最小化对普通车辆的影响

Method: 提出可扩展的分布式车辆控制方法，车辆仅使用局部信息在线分布式调整驾驶行为；证明局部信息方法近似等价于全局信息方法；进一步提出分布式冲突解决机制保证车辆安全

Result: 仿真实验基于真实交通数据集，相比现有方法，该方法实现更快的决策速度、对普通车辆影响更小，在不同交通密度和道路配置下保持更强的可扩展性

Conclusion: 分布式方法克服了集中式方法的高计算成本和可扩展性限制，提供了确定性的安全保证，无需预训练且能自然适应变化的交通条件
...
TL;DR: SGREC提出了一种基于查询驱动场景图的零样本指代表达理解方法，通过场景图作为结构化中介，结合视觉语言模型和大型语言模型的优势，实现了高精度且可解释的目标定位。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：基于CLIP等视觉语言模型的方法难以捕捉细粒度视觉细节和复杂对象关系；大型语言模型虽然擅长高级语义推理，但无法直接将视觉特征抽象为文本语义。需要一种既能理解复杂视觉关系又能提供可解释推理的方法。

Method: 提出SGREC方法：1) 使用视觉语言模型构建查询驱动的场景图，显式编码空间关系、描述性标题和对象交互；2) 利用场景图作为结构化中介，弥合低级图像区域和高级语义理解之间的鸿沟；3) 让大型语言模型从场景图提供的结构化文本表示中推断目标对象，并提供详细解释确保推理过程的可解释性。

Result: 在多个零样本REC基准测试中取得最佳性能：RefCOCO val (66.78%)、RefCOCO+ testB (53.43%)、RefCOCOg val (73.28%)，展示了强大的视觉场景理解能力。

Conclusion: SGREC通过查询驱动场景图作为结构化中介，有效结合了视觉语言模型和大型语言模型的优势，在零样本指代表达理解任务中实现了高精度和可解释性，为视觉语言理解提供了新思路。
...
TL;DR: Few TensoRF结合TensorRF的高效张量表示与FreeNeRF的频率驱动少样本正则化，通过引入频率和遮挡掩码提升稀疏视角下的重建质量和稳定性，在保持快速训练的同时显著提升PSNR指标。
...
Result: 在Synthesis NeRF基准测试中，PSNR从21.45dB提升到23.70dB，微调版本达到24.52dB，训练时间保持10-15分钟；在THuman 2.0数据集上，仅用8张输入图像就达到27.37-34.00dB的竞争性性能。

Conclusion: Few TensoRF是一种高效且数据有效的实时3D重建解决方案，在保持TensorRF快速训练的同时，显著提升了稀疏视角下的重建质量，适用于多样化场景。
...
TL;DR: 提出GDPO-Listener框架，通过自回归流匹配架构和分组奖励解耦策略优化，解决了虚拟人交互中听者动作的"回归均值"问题，实现了高表达性的说话和倾听动作生成，并支持语义文本控制。


<details>
  <summary>Details</summary>
Motivation: 当前虚拟人生成方法在说话头部动作上表现良好，但在听者动作上存在"回归均值"问题，容易退化为静态面部，且缺乏复杂非语言动作的参数空间。

Method: 1. 引入自回归流匹配架构实现稳定监督学习；2. 应用分组奖励解耦策略优化，通过对不同FLAME参数组进行独立的奖励归一化，明确激励高方差表达性生成；3. 支持显式语义文本控制以实现可定制响应。

Result: 在Seamless Interaction和DualTalk数据集上的广泛评估表明，该方法在长期运动方差、视觉表达性和语义可控性方面优于现有基线方法。

Conclusion: GDPO-Listener框架成功解决了虚拟人交互中听者动作的"回归均值"问题，实现了高表达性的说话和倾听动作生成，并提供了语义可控性，为虚拟人合成提供了更自然的交互体验。
...
TL;DR: VideoTIR：基于强化学习的多级工具包调用方法，通过Zero-RL和SFT冷启动、TAGPO策略优化和沙盒轨迹合成，解决长视频理解中的幻觉问题，提升准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在长视频理解中存在幻觉问题，主要源于文本和视觉token的不平衡。虽然基于SFT的工具调用方法可以缓解此问题，但需要大量细粒度高质量数据且工具调用轨迹受限。

Method: 提出VideoTIR方法：1）利用强化学习鼓励正确使用多级工具包；2）探索Zero-RL和SFT冷启动策略；3）提出工具包动作分组策略优化(TAGPO)减少冗余调用；4）开发沙盒轨迹合成框架生成高质量轨迹数据。

Result: 在三个长视频QA基准测试上进行广泛实验，证明了方法的有效性和效率。

Conclusion: VideoTIR通过强化学习驱动的多级工具包调用机制，显著提升了长视频理解的准确性和效率，有效缓解了幻觉问题。
...
TL;DR: CARE是一个无需训练的、可控的医学图像修复框架，通过双潜在分支策略和风险感知自适应控制器，在保持数据保真度和解剖结构一致性的同时，实现可调节的修复效果。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像修复方法通常需要针对特定任务重新训练，且难以控制重建保真度与先验驱动增强之间的权衡。过度激进的修复可能引入幻觉细节或改变诊断重要结构，这在临床环境中尤其成问题。

Method: CARE采用双潜在修复策略：一个分支强制数据保真度和解剖一致性，另一个分支利用生成先验恢复缺失或退化信息。风险感知自适应控制器根据修复不确定性和局部结构可靠性动态调整各分支贡献，实现保守或增强导向的修复模式。

Result: 在噪声和不完整的医学成像场景中，CARE实现了强大的修复质量，同时更好地保留了临床相关结构，减少了不合理重建的风险。

Conclusion: CARE为更安全、更可控、更易于部署的医学图像修复提供了实用步骤，通过训练自由的方法实现了结构保存与先验引导增强之间的平衡控制。
...
TL;DR: GeoNDC是一种可查询的神经数据立方体，将大规模地球观测数据编码为连续的时空隐式神经场，支持按需查询和连续时间重建，实现高效压缩和查询。


<details>
  <summary>Details</summary>
Motivation: 卫星地球观测积累了海量时空数据，但现有存储方式为离散的栅格文件，导致存储、传输和查询成本高昂，需要更高效的数据表示方法。

Method: 提出GeoNDC查询式神经数据立方体，将行星尺度的地球观测数据编码为连续的时空隐式神经场，支持直接时空查询和连续时间重建。

Result: 在20年全球MODIS数据上实现95:1压缩比（0.44GB），保持高光谱保真度（R²>0.98，RMSE=0.021）；在Sentinel-2数据上能恢复云遮挡下的动态变化（R²>0.85）；在HiGLASS生物物理产品上达到接近完美的精度（R²>0.98）。
...
<details>
  <summary>Details</summary>
Motivation: 现有在线4D重建方法虽然高效且视觉质量好，但无法学习反映真实场景动态的每个高斯点的运动。在没有明确运动线索的情况下，外观和运动仅通过光度损失优化，导致每个高斯点的运动追逐像素残差而非真实的3D运动。

Method: 1) 利用稀疏关键视图上的光流作为轻量级运动线索，超越光度监督来正则化每个高斯点的运动；2) 学习每个高斯点的运动偏移场，以协调投影3D运动与跨视图和时间观察到的光流之间的差异；3) 引入每个高斯点的运动置信度，区分动态和静态高斯点，并加权高斯属性残差更新，从而抑制静态区域的冗余运动。

Result: 大量实验表明，MoRGS在在线方法中实现了最先进的重建质量和运动保真度，同时保持了可流式处理的性能。

Conclusion: MoRGS通过显式建模每个高斯点的运动并引入光流监督和运动置信度机制，有效提升了在线4D重建的质量和运动保真度，解决了现有方法中运动学习不准确的问题。
...
Method: 引入几何感知的视频到视频生成器，通过渲染包含深度、法线、不透明度和协方差的Gaussian primitive视频缓冲区，生成时间一致且无伪影的帧；同时开发了模拟多样化退化模式的伪影合成流程。

Result: 在新视角合成基准测试中达到最先进性能，高效变体在21FPS实时运行的同时保持相似性能，支持交互式3D应用。

Conclusion: GaussFusion通过几何感知的视频生成有效改善了3DGS重建质量，减少了常见伪影，在保持高质量的同时实现了实时性能，为交互式3D应用提供了实用解决方案。
...
TL;DR: 提出了一种用于高能推进剂燃烧实时监测的闭环事件-SVE测量系统，结合空间变曝光相机和神经形态事件相机，解决了烟雾遮挡、高动态范围条件下的微秒级3D燃烧测量难题。


<details>
  <summary>Details</summary>
Motivation: 高能推进剂燃烧实时监测面临极端高动态范围、微秒级粒子运动和浓密烟雾的挑战，传统成像方法容易产生饱和、运动模糊和不稳定的粒子提取问题。

Method: 开发了闭环事件-SVE测量系统，结合SVE相机和立体事件相机对。SVE分支采用烟雾感知融合策略生成HDR图，事件相机分支利用HDR参考抑制烟雾伪影，通过立体事件3D管道估计分离高度和等效粒子尺寸。

Result: 系统最大校准误差0.56%，在硼基推进剂实验中获得了多模态等效半径统计，能够捕捉传统传感器难以观测的快速分离瞬态现象。

Conclusion: 该框架为烟雾遮挡HDR条件下的微秒级3D燃烧测量提供了实用且校准一致的解决方案，能够有效监测高能推进剂燃烧过程。
...
TL;DR: 提出一种从单目视频进行高质量动态高斯溅射的方法，通过SE(3)B样条运动基函数显式建模高斯粒子的连续位置和方向变形，并采用自适应控制机制提升计算效率和复杂运动建模能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在从单目视频进行动态高斯溅射时，对复杂运动的建模能力有限，计算效率不高，且容易受到长间隔运动干扰和训练视角过拟合的影响。

Method: 1. 使用SE(3)B样条运动基函数显式建模动态高斯的连续位置和方向变形；2. 设计自适应控制机制动态调整运动基函数和控制点数量；3. 开发软分割重建策略减轻长间隔运动干扰；4. 采用多视角扩散模型提供多视角线索避免训练视角过拟合。

Result: 大量实验表明，该方法在新视角合成任务上优于现有最先进方法，能够生成更高质量的动态场景重建结果。

Conclusion: 提出的SE(3)B样条运动建模框架结合自适应控制、软分割重建和多视角扩散模型，有效提升了从单目视频进行动态高斯溅射的质量和效率，为动态场景重建提供了新的解决方案。
...
TL;DR: GIFT是一种无需训练的视频帧选择框架，通过评估帧的不可替代性来选择关键帧，解决了现有方法因贪心决策和分离评估相关性与多样性而导致的局部最优问题。


<details>
  <summary>Details</summary>
Motivation: 视频大语言模型在处理密集帧时计算成本过高，现有关键帧选择方法存在贪心决策和分离评估相关性与多样性的问题，容易陷入局部最优并选择无关噪声帧。

Method: 提出GIFT框架：1) 引入定向多样性量化条件相关性的帧独特性，形成统一的不可替代性评分；2) 采用预算感知细化策略，通过自适应迭代过程先选择最高不可替代性的核心帧集，再随着预算扩展优先构建关键时间上下文。

Result: 在LLaVA-Video-7B上的长视频基准测试中，相比均匀采样，GIFT实现了平均最大12.5%的性能提升。
...
TL;DR: Z-Erase：首个针对单流扩散Transformer（如Z-Image）的概念擦除方法，解决了直接应用现有方法导致生成崩溃的问题


<details>
  <summary>Details</summary>
Motivation: 概念擦除是文本到图像模型的重要安全机制，但在新兴的单流扩散Transformer架构中研究不足。现有方法直接应用于这种架构会导致生成崩溃，需要专门解决方案

Method: 提出流解耦概念擦除框架，将单流模型解耦更新；引入拉格朗日引导的自适应擦除调制算法，平衡擦除与保留的敏感权衡；提供严格的收敛性分析

Result: Z-Erase成功克服了生成崩溃问题，在广泛任务中实现了最先进的性能

Conclusion: Z-Erase是首个针对单流文本到图像模型的概念擦除方法，通过解耦框架和约束算法有效解决了该架构下的概念擦除挑战
...
TL;DR: 本文提出Token-Reweighting (ToR)策略，解决多模态大语言模型中感知相关token和推理相关token的耦合问题，通过在RLVR训练中动态重加权关键token来提升性能。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型的响应中感知相关token（用于视觉内容定位）和推理相关token（用于构建推理链）交织在一起，这两种token类型体现了不同但相互依赖的能力。单独优化任一种token都表现不佳，需要解决这种内在耦合问题。

Method: 提出Token-Reweighting (ToR)策略，通过识别感知和推理两种类型的关键token，在RLVR训练过程中动态重加权这些token。该策略是即插即用的，可应用于现有方法（如GRPO和DAPO）之上。

Result: 在多个多模态推理基准测试中，ToR策略在现有方法基础上带来了一致的性能提升，实现了最先进的性能，同时保证了准确的视觉定位和连贯的推理。

Conclusion: ToR策略通过显式建模感知和推理token的相互依赖性，有效解决了多模态大语言模型中RLVR训练的挑战，为多模态推理任务提供了有效的优化方法。
...
TL;DR: 本文提出分层因果丢弃（HCD）方法，通过通道级因果掩码强制特征稀疏性，分离因果特征与伪特征，在表示层面进行因果干预，以解决OOD泛化中的捷径学习问题。
...
Conclusion: HCD通过表示层面的因果干预有效解决了OOD泛化中的捷径学习问题，在分离因果特征与伪特征方面优于现有方法。
...
TL;DR: CLVA是一种无需训练的方法，通过跨层视觉锚点抑制幻觉，利用中层特征纠正深层注意力漂移，在多种架构和基准测试中表现优异且计算开销小。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在物体幻觉问题，现有方法缺乏对最终模型阶段注意力漂移的充分可解释性。研究发现幻觉源于深层注意力回归到早期层的视觉噪声，而输出可靠性依赖于在中间层而非最终层获取视觉锚点。

Method: 提出CLVA（跨层视觉锚点），这是一种无需训练的方法，通过强化关键中层特征同时抑制回归噪声，利用从注意力动态中捕获的必需锚点将深层注意力拉回正确的视觉区域。

Result: 在多种架构和基准测试中评估该方法，展示了出色的性能表现，且没有显著增加计算时间和GPU内存消耗。

Conclusion: CLVA通过跨层视觉锚点有效解决了多模态大语言模型的物体幻觉问题，提供了一种高效且无需训练的解决方案，通过纠正深层注意力漂移来提升模型可靠性。
...
TL;DR: THEMIS是一个新颖的多任务基准测试，专门评估多模态大语言模型在真实学术场景中的视觉欺诈推理能力，包含4000多个问题，涵盖7种场景和5种欺诈类型，对16个领先MLLMs的测试显示最佳模型准确率仅56.15%。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试与真实世界学术欺诈的复杂性存在差距，需要开发一个能够全面评估多模态大语言模型在复杂视觉欺诈推理能力的基准测试。

Method: THEMIS基准包含三个主要创新：1) 基于真实撤稿案例构建的7种场景4000多个问题，包含60.47%复杂纹理图像；2) 系统覆盖5种欺诈类型和16种细粒度操作；3) 建立欺诈类型到5个核心视觉欺诈推理能力的映射，实现多维度能力评估。

Result: 在16个领先的MLLMs上测试，即使表现最好的GPT-5模型总体准确率也只有56.15%，表明该基准测试具有很高的挑战性。

Conclusion: THEMIS基准测试填补了现有基准与真实世界学术欺诈复杂性之间的关键差距，能够揭示不同模型在视觉欺诈推理方面的具体优势和弱点，有望推动MLLMs在复杂真实世界欺诈推理任务中的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言系统是被动的静态观察者：只能描述像素、不能行动、无法在分布偏移下安全改进。这种被动性限制了可泛化的、物理基础的视觉智能。学习需要通过行动而非静态描述来实现，这在精心策划的数据之外至关重要。

Method: Pixelis采用三阶段训练：1）监督微调学习像素工具语法，使用思维链-行动轨迹和掩码模仿损失；2）好奇心-一致性奖励微调，优化双驱动目标结合预测误差好奇心和相邻步骤一致性；3）像素测试时强化学习，通过检索邻居、投票完整轨迹进行无标签适应，使用KL-to-EMA安全控制约束漂移。

Result: 在六个公开图像和视频基准测试中，Pixelis相比相同的8B基线模型获得一致改进：平均相对增益为+4.08%（在VSI-Bench上达到+6.03%），同时产生更短、可审计的工具链，并在测试时学习中保持走廊内的KL散度。

Conclusion: 在像素空间内行动而非抽象标记中，将多模态感知锚定在物理世界中，将视觉推理与可操作结果联系起来，并实现无需外部反馈的具身适应。这为构建更通用、物理基础的视觉智能提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 多模态学习依赖大规模标注数据，但标注成本高昂。现有主动学习方法在多模态设置中通常假设模态重要性稳定且选择规则固定，无法适应模态相对价值和样本难度随训练动态变化的特性。
...
Conclusion: RL-MBA框架通过强化学习动态适应多模态学习中的模态贡献变化和样本难度变化，实现了更有效的样本选择策略，为多模态主动学习提供了新的解决方案。
...
TL;DR: 提出多阶段强化学习框架MSRL，通过文本偏好数据学习通用奖励推理能力，逐步迁移到多模态任务，解决多模态奖励模型训练数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于RLVR的多模态奖励模型训练依赖昂贵的人工标注多模态偏好数据，难以规模化扩展。需要一种能在有限多模态数据下实现可扩展强化学习的方法。

Method: 提出多阶段强化学习框架MSRL：1）从大规模文本偏好数据学习通用奖励推理能力；2）通过基于描述的强化学习阶段将能力迁移到多模态任务；3）完全多模态强化学习阶段；4）引入跨模态知识蒸馏提升偏好泛化能力。

Result: MSRL显著提升生成式多模态奖励模型性能：在VL-RewardBench上从66.6%提升到75.9%，在GenAI-Bench上从70.2%提升到75.7%，且无需额外多模态偏好标注。

Conclusion: MSRL框架有效解决了多模态奖励模型训练的数据稀缺问题，实现了可扩展的强化学习，显著提升了模型在视觉理解和生成任务上的性能。
...
TL;DR: 本文提出了一种基于莫尔干涉模式的轻量级数据增强方法，通过解析公式实时生成结构化纹理扰动，无需外部数据且计算成本极低，显著提升了图像分类模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前数据增强方法主要依赖扩散模型合成或复杂的特征混合策略，这些方法计算开销大或需要外部数据集。本文探索基于解析干涉模式的程序化增强方法，以提供更高效实用的增强方案。

Method: 提出一种轻量级增强方法，使用封闭形式的数学公式程序化生成莫尔干涉纹理。这些模式直接在内存中合成，计算成本极低（每张图像0.0026秒），在训练期间与训练图像混合后立即丢弃，实现无需存储的外部数据自由增强流程。

Result: 在Vision Transformers上的广泛实验表明，该方法在多个基准测试（包括ImageNet-C、ImageNet-R和对抗性基准）中持续提升鲁棒性，优于标准增强基线和现有的外部数据自由增强方法。

Conclusion: 解析干涉模式为数据驱动的生成式增强方法提供了实用且高效的替代方案，证明了程序化增强在提升模型鲁棒性方面的有效性。
...
TL;DR: AnyDoc是一个统一的文档生成框架，通过HTML/CSS格式处理多种文档生成任务，使用大规模合成数据集DocHTML（265,206个样本，111个类别，32种风格），并采用高度感知强化学习解决内容溢出问题。


<details>
  <summary>Details</summary>
Motivation: 现有文档生成方法存在局限性：1）现有数据集覆盖范围有限且规模小；2）缺乏统一的文档表示格式；3）文档生成任务多样化但缺乏统一框架；4）训练过程中存在内容溢出问题。

Method: 1）建立可扩展的数据合成管道，自动生成HTML/CSS格式文档，创建DocHTML数据集；2）基于该数据集微调多模态大语言模型，实现三种文档生成任务：意图到文档、文档反渲染、元素到文档；3）引入高度感知强化学习（HARL）后训练过程，通过基于预测与目标文档高度差异的奖励函数惩罚溢出问题。

Result: 1）创建了包含265,206个文档样本、覆盖111个类别和32种风格的DocHTML数据集；2）AnyDoc在三种文档生成任务上均优于通用多模态大语言模型和任务特定基线；3）HARL有效缓解了内容溢出问题，提升了整体性能。

Conclusion: AnyDoc通过统一的HTML/CSS表示、大规模合成数据集和高度感知强化学习，成功构建了一个能够处理多种文档生成任务的框架，在文档生成领域取得了显著进展，为AI驱动的文档创建提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管3D视觉基础模型在零样本几何估计方面表现出色，但直接应用于无需姿态信息的新视角合成任务仍然具有挑战性。需要将强大的几何先验有效适配到高质量、无需姿态的新视角合成中。

Method: 提出两个关键技术贡献：1) 自一致姿态对齐(SCPA)，通过训练时反馈循环确保像素对齐监督，解决姿态-几何不一致问题；2) 评分式不透明度匹配(ROM)，利用稀疏视角新视角合成教师模型的局部3D几何一致性知识，过滤退化的基元。

Result: 在大规模基准测试中，该方法在重建质量方面显著优于最先进的无需姿态新视角合成方法。
...
<details>
  <summary>Details</summary>
Motivation: 全景语义分割在自动驾驶和虚拟现实等应用中至关重要，但面临两个关键挑战：全景投影的几何畸变和密集标注的高成本。虽然无监督领域适应（UDA）提供了一种替代方案，但许多实际任务要求更严格的源数据不可用（SFUDA）约束，这加剧了领域偏移问题，导致不可靠的伪标签和性能下降，特别是对少数类别。
...
Conclusion: DAPASS框架有效解决了源数据不可用情况下的全景语义分割领域自适应问题，通过创新的去噪和对抗对齐策略，显著提升了性能，特别是在处理几何畸变和类别不平衡方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 传统的鲁棒主成分分析（RPCA）假设稀疏前景与低秩背景是相加关系，但在许多实际应用中，稀疏前景实际上会遮挡或替换低秩背景元素，这种不匹配需要新的方法来解决。

Method: 提出了鲁棒主成分完成（RPCC）框架，通过确定稀疏成分的支持集来间接识别稀疏成分，采用完全概率贝叶斯稀疏张量分解，并通过变分贝叶斯推理进行求解。

Result: 该方法在合成数据上实现了接近最优的估计，在真实彩色视频数据集上实现了鲁棒的前景提取，在高光谱数据集上实现了有效的异常检测，收敛到硬分类器，无需后处理阈值。

Conclusion: RPCC框架有效地解决了传统RPCA中前景遮挡背景的不匹配问题，通过变分贝叶斯推理实现了无需阈值的稀疏成分识别，在多个实际应用中表现出色。
...
TL;DR: 本文介绍了EgoXtreme数据集，这是一个用于第一人称视角6D物体姿态估计的大规模数据集，专门针对工业维护、体育和紧急救援等极端场景，包含严重运动模糊、动态光照和视觉遮挡等挑战。
...
Method: 创建了EgoXtreme数据集，包含三个具有挑战性的场景（工业维护、体育和紧急救援），通过极端光照、重度运动模糊和烟雾引入严重的感知模糊性。

Result: 评估显示现有通用姿态估计器在极端条件下泛化能力失效，特别是在低光照情况下。简单的图像恢复（如去模糊）在极端条件下没有积极改善，而基于跟踪的方法表现出性能提升，表明在快速运动场景中使用时间信息是有意义的。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态深度伪造检测方法主要依赖合成伪造数据进行训练，这会导致数据集和生成器偏差，限制了模型的扩展性和对未见篡改的鲁棒性。
...
Conclusion: 自监督学习是多模态深度伪造检测的可扩展范式，能够减少对合成伪造数据的依赖，提高模型的泛化能力和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有解耦数据集蒸馏方法主要依赖粗粒度类别标签监督，在细粒度数据集上会导致：(1)保留大的类内差异而忽略细微的类间差异；(2)同一类内样本过于相似，限制了局部判别线索，损害识别性能。

Method: FD²包含两个阶段：预训练阶段使用反事实注意力学习聚合判别表示更新类别原型；蒸馏阶段采用细粒度特征约束对齐样本与其类别原型并排斥其他类别，同时使用相似性约束使同类样本的注意力多样化。

Result: 在多个细粒度和通用数据集上的实验表明，FD²能够无缝集成到解耦数据集蒸馏框架中，在大多数设置下提升了性能，显示出强大的可迁移性。

Conclusion: FD²通过定位判别区域和构建细粒度表示，有效解决了细粒度数据集蒸馏中的挑战，为细粒度视觉识别任务提供了更高效的数据压缩方案。
...
TL;DR: 本文提出用排序优化替代二元DPO来提升视觉语言模型的对齐效果，特别针对视频文本对齐任务，通过生成有序的退化字幕链进行训练，发现需要微调视觉编码器才能有效。


<details>
  <summary>Details</summary>
Motivation: 标准DPO的二元"赢家通吃"方法对于视觉语言模型存在不足，因为即使某个回答不如另一个受欢迎，它可能仍然忠实于视觉内容。DPO缺乏这种细微差别，过度强调获胜回答而忽略了"失败"回答可能仍保持高视觉保真度。

Method: 提出排序优化作为替代方案，更精确地定位回答对视觉输入的忠实度。专注于视频文本对齐任务，通过重复字幕退化生成具有挑战性的完全有序字幕链，用于大规模训练。

Result: 排序优化在长内容生成和评估方面优于二元DPO。重要的是，发现这些方法需要微调视觉编码器才能有效，这挑战了DPO纯粹是语言重加权过程的观点。

Conclusion: 对于视觉语言模型，排序优化比二元DPO更适合处理视觉保真度的细微差别，并且视觉编码器的微调对于对齐效果至关重要，表明视觉语言对齐需要更全面的方法。
...
TL;DR: 提出了一种语义解缠的统一3D异常检测模型，通过解耦语义表示来解决多类别统一模型中的类别间纠缠问题，在多个数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 在3D异常检测中，统一模型虽然能提高可扩展性，但存在类别间纠缠问题，导致模型在重建时采用错误的语义先验，产生不可靠的异常分数。

Method: 提出了语义解缠的统一3D异常检测模型，包含三个关键组件：1) 粗到细的全局标记化形成实例级语义身份；2) 类别条件对比学习解耦类别语义；3) 几何引导解码器进行语义一致的重建。

Result: 在Real3D-AD和Anomaly-ShapeNet数据集上的实验表明，该方法在统一模型和类别特定模型上都达到了最先进的性能，分别将对象级AUROC提高了2.8%和9.1%。

Conclusion: 提出的语义解缠统一模型有效解决了多类别3D异常检测中的类别间纠缠问题，提高了统一3D异常检测的可靠性。
...
TL;DR: SportSkills是一个针对体育技能学习的大规模视频数据集，包含36万+教学视频和63万+视觉演示，覆盖55种运动，能够理解精细动作差异，并首次提出基于错误的视频检索任务。


<details>
  <summary>Details</summary>
Motivation: 现有大规模视频数据集主要关注一般人类活动，缺乏针对物理技能学习的精细活动覆盖。为了支持体育技能学习，需要专门的数据集来理解动作的细微差别并提供个性化反馈。

Method: 构建SportSkills数据集，包含36万+教学视频和63万+视觉演示，配有解释动作技巧的教学旁白。基于该数据集训练模型，并首次提出基于错误的视频检索任务，即根据用户执行动作的错误来检索相应的教学视频片段。

Result: SportSkills数据集使模型能够理解精细动作差异，相比传统活动中心数据集，相同模型的性能提升高达4倍。专业教练的正式评估表明，检索方法显著提升了视频模型为用户查询个性化视觉指导的能力。

Conclusion: SportSkills是首个面向物理技能学习的大规模体育数据集，不仅提升了动作理解的精细度，还开创了基于错误的视频检索新任务，为个性化技能学习反馈提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有自监督学习方法强调语义感知，但在实例定位任务上迁移效果差，需要完全微调才能获得良好性能。实例感知是3D感知的基本组成部分，需要弥合这一差距以推进真正的3D基础模型发展。
...
Conclusion: PointINS通过几何感知的自监督学习成功增强了点云表示的实例感知能力，显著提升了3D实例分割性能，推动了向真正支持所有下游任务的3D基础模型的发展。
...
TL;DR: ET-SAM是一个基于SAM的高效统一场景文本检测和布局分析框架，通过轻量级点解码器和联合训练策略，在减少推理延迟的同时提升性能。
...
Method: 提出ET-SAM框架，包含两个解码器：1）轻量级点解码器生成词热力图以减少前景点数量，消除过多点提示并加速推理；2）设计联合训练策略，利用具有异构文本级标注的现有数据；3）引入三组可学习任务提示来缓解不同数据集间的差异。

Result: 相比之前的SAM架构，ET-SAM实现了约3倍的推理加速，在HierText上获得竞争性性能，在Total-Text、CTW1500和ICDAR15上平均提升11.0%的F分数。

Conclusion: ET-SAM通过减少点提示数量和设计联合训练策略，有效解决了基于SAM方法的推理延迟和数据利用率问题，在保持性能的同时显著提升了效率。
...
### [72] [Bilingual Text-to-Motion Generation: A New Benchmark and Baselines](https://arxiv.org/abs/2603.25178)
*Wanjiang Weng,Xiaofeng Tan,Xiangbo Shu,Guo-Sen Xie,Pan Zhou,Hongsong Wang*

Main category: cs.CV

TL;DR: BiHumanML3D是首个双语文本到动作生成基准数据集，通过LLM辅助标注和人工修正构建。研究者提出了BiMD模型，采用跨语言对齐策略，显著提升了双语输入下的动作生成质量。
...
Conclusion: 该研究证明了双语数据集和跨语言对齐策略对跨语言动作合成的关键必要性和有效性，为跨语言文本到动作生成提供了可靠的数据集和有效的对齐方法。
...
TL;DR: VolDiT：首个基于纯Transformer的3D扩散模型，用于体医学图像合成，通过体素块嵌入和全局自注意力直接处理3D数据，并引入时间步门控控制适配器实现精确空间引导。


<details>
  <summary>Details</summary>
Motivation: 现有3D医学图像生成方法大多依赖卷积U-Net架构，这些架构存在局部偏置和有限感受野的问题，限制了可扩展性、全局上下文整合和灵活条件控制。

Method: 提出VolDiT：1）通过体素块嵌入和全局自注意力直接处理3D令牌；2）引入时间步门控控制适配器，将分割掩码映射为可学习的控制令牌，在去噪过程中调制Transformer层。

Result: 在高分辨率3D医学图像合成任务中，相比基于U-Net的3D潜在扩散模型，VolDiT展现出改进的全局一致性、更优的生成保真度和增强的可控性。

Conclusion: 完全基于Transformer的扩散模型为体医学图像合成提供了灵活的基础框架，在全局建模和可控生成方面具有优势。
...
### [74] [AnyID: Ultra-Fidelity Universal Identity-Preserving Video Generation from Any Visual References](https://arxiv.org/abs/2603.25188)
...
<details>
  <summary>Details</summary>
Motivation: 现有身份保持视频生成方法通常针对单一身份参考进行设计和优化，这限制了创作灵活性，无法适应多样化的真实世界输入格式。单一参考源构成一个不适定场景，导致模型难以在新颖背景下忠实再现身份。
...
### [75] [CardioDiT: Latent Diffusion Transformers for 4D Cardiac MRI Synthesis](https://arxiv.org/abs/2603.25194)
...
TL;DR: CardioDiT是一个基于扩散变换器的4D潜在扩散模型，用于合成短轴电影心脏MRI，通过统一建模时空维度来生成更一致的心脏动态图像。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理电影心脏MRI（CMR）这种时空同步的3D体积数据时，通常将空间和时间分离建模，或通过解剖掩码等辅助机制强制时间一致性。这些策略引入了结构偏差，可能限制全局上下文整合，导致微妙的时空不连续或生理上不一致的心脏动态。

Method: 提出CardioDiT，一个完全4D的潜在扩散框架：1）使用时空VQ-VAE将2D+t切片编码为紧凑的潜在表示；2）使用扩散变换器将潜在表示联合建模为完整的3D+t体积，在整个生成过程中耦合空间和时间维度。
...
### [76] [TacSIm: A Dataset and Benchmark for Football Tactical Style Imitation](https://arxiv.org/abs/2603.25199)
*Peng Wen,Yuting Wang,Qiurui Wang*

Main category: cs.CV

TL;DR: TacSIm是一个用于足球战术风格模仿的大规模数据集和基准，专注于准确复制真实世界球队战术行为，而不仅仅是优化进球或胜率等奖励目标。


<details>
  <summary>Details</summary>
Motivation: 当前足球模仿研究主要关注优化奖励目标（如进球数或胜率），而较少关注准确复制真实世界的球队战术行为。需要建立一个能够评估战术风格模仿能力的基准。

Method: TacSIm将英超比赛转播画面中所有22名球员的起始位置和动作投影到标准球场坐标系中，提供明确的风格模仿任务和评估协议。使用空间占用相似性和运动向量相似性来测量战术风格模仿。

Result: 在统一的虚拟环境中运行多个基线方法生成全队行为，支持对战术协调性的定量和视觉评估。通过从转播到模拟的统一数据和指标，建立了严谨的足球风格对齐战术模仿任务基准。

Conclusion: TacSIm为足球战术风格模仿任务建立了严格的基准，支持空间和时间相似性评估，填补了当前研究在准确复制真实世界战术行为方面的空白。
...
### [77] [CIV-DG: Conditional Instrumental Variables for Domain Generalization in Medical Imaging](https://arxiv.org/abs/2603.25202)
...
<details>
  <summary>Details</summary>
Motivation: 医学AI中的跨站点泛化受到选择偏倚的根本性损害，患者人口统计学特征（如年龄、严重程度）非随机决定医院分配，导致站点特定变异与诊断标签之间的虚假相关性。传统的域泛化方法主要针对图像级分布偏移，无法解决这种结构性混杂问题。

Method: 提出CIV-DG因果框架，利用条件工具变量分离病理语义与扫描器诱导的伪影。通过放宽标准工具变量方法的严格随机分配假设，适应医院选择由患者人口统计学特征内生驱动的复杂临床场景。采用深度广义矩方法架构，使用条件批评器最小化矩违规并在人口统计学层内强制实施工具-误差正交性。

Result: 在Camelyon17基准和大规模胸部X射线数据集上的广泛实验表明，CIV-DG显著优于领先的基线方法，验证了条件因果机制在解决医学AI结构性混杂问题中的有效性。

Conclusion: CIV-DG框架通过条件工具变量有效解决了医学AI中的结构性混杂问题，为处理医院选择偏倚和提升跨站点泛化性能提供了有效的因果方法，在医学AI鲁棒性方面具有重要价值。
...
TL;DR: PCGR框架通过构建概率概念图进行可解释的多模态虚假信息检测，将检测任务转化为结构化概念推理，实现高精度和强鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 多模态虚假信息日益严重，传统检测器存在黑盒不透明、对新型操纵手段脆弱等问题，需要可解释且能适应演化的检测框架。

Method: 采用"构建-推理"范式：首先构建包含多模态大语言模型自动发现和验证的高层概念的概念图，然后通过分层注意力机制在概念图上进行推理，生成可解释的证据链。

Result: PCGR在多模态虚假信息检测精度和鲁棒性方面达到最先进水平，在粗粒度检测和细粒度操纵识别上都优于现有方法。

Conclusion: PCGR框架通过结构化概念推理成功解决了多模态虚假信息检测的可解释性和适应性挑战，为应对不断演化的虚假信息提供了有效解决方案。
...
TL;DR: FreeLOC是一个无需训练、层自适应的框架，通过视频相对位置重编码和分层稀疏注意力技术，解决预训练视频扩散模型在生成长视频时出现的分布外问题，显著提升时间一致性和视觉质量。
...
Method: 提出FreeLOC框架，包含两个核心技术：1) 视频相对位置重编码(VRPR)，通过多层次重新编码时间相对位置来对齐预训练分布；2) 分层稀疏注意力(TSA)，在不同时间尺度上结构化注意力密度以保持局部细节和长程依赖。还引入了层自适应探测机制，识别每个transformer层对这些分布外问题的敏感性。

Result: 大量实验表明，该方法显著优于现有的无需训练方法，在时间一致性和视觉质量方面达到了最先进的结果。

Conclusion: FreeLOC通过解决帧级相对位置和上下文长度分布外问题，有效提升了预训练视频扩散模型在生成长视频时的性能，无需额外训练即可实现高质量长视频生成。
...
Method: 两阶段流程：1) 多视图检测与匹配，通过模板渲染和预训练特征提取器生成掩码提议；2) 姿态估计，通过迭代精炼和特征度量评分，最后使用多视图遮挡感知轮廓配准进行最终优化

Result: 在MVPSP数据集上达到毫米级精度，与监督方法相当，同时保持对未见器械的完全泛化能力

Conclusion: 提出了一种无需任务特定训练、结合先进基础模型、多视图几何和轮廓优化的灵活流程，实现了手术器械的高精度6D姿态估计
...
TL;DR: 本文开发了一个用于常见皮肤病检测的公开数据集，包含孟加拉国五种流行皮肤病（接触性皮炎、白癜风、湿疹、疥疮、癣）的1612张图像，并应用多种机器学习模型进行分类性能评估。


<details>
  <summary>Details</summary>
Motivation: 皮肤病是全球性的重大公共卫生问题，在孟加拉国等人口密集国家，皮肤科专家和诊断设备严重不足，缺乏适当的检测和治疗可能导致严重的健康后果甚至死亡。人工智能和机器学习技术为通过图像处理和计算机视觉检测皮肤病提供了可能。

Method: 开发了一个公开可用的皮肤病检测数据集，包含五种常见皮肤病（接触性皮炎、白癜风、湿疹、疥疮、癣）的1612张图像（其中250张为原始图像，其余为增强图像）。数据直接从孟加拉国Faridpur医学院门诊患者处收集。在数据集上应用了多种机器学习和深度学习模型进行分类性能评估。

Result: 数据集包含302张皮炎、381张湿疹、301张疥疮、316张癣和312张白癜风图像。虽然数据是区域性收集的，但所选疾病在南亚等许多国家都很常见，使该数据集在全球机器学习皮肤病学应用中具有潜在价值。研究还报告了多种机器学习模型的分类性能。

Conclusion: 该研究开发了一个有价值的皮肤病检测公开数据集，并展示了机器学习模型在该数据集上的应用潜力。期望这项研究能够吸引从事自动化疾病诊断领域的机器学习和深度学习研究人员及从业者的关注。
...
<details>
  <summary>Details</summary>
Motivation: 现有的变换对抗攻击（TAAs）在分类任务中表现出良好的可迁移性，但在结构化任务（如语义分割、目标检测）中效果不佳甚至失败。这是因为结构化任务的标签具有空间结构，而传统TAAs在应用空间变换时未能同步调整标签，导致空间错位和错误梯度。

Method: 提出统一的空间对齐框架（SAF），包含空间对齐（SA）算法，在应用空间变换时同步变换标签和输入，确保空间结构的一致性。将变换分为非空间变换和空间变换两类，针对结构化任务的特点设计专门的标签同步机制。
...
Conclusion: 空间对齐对于结构化任务的变换对抗攻击至关重要，提出的SAF框架通过同步变换标签和输入解决了空间错位问题，显著提升了攻击的可迁移性和效果，为结构化任务的对抗攻击提供了新思路。
...
TL;DR: 该论文提出了一种用于LiDAR点云无损压缩的高效预测编码框架，通过几何重稠密化和跨尺度特征传播模块实现实时压缩性能
...
Method: 提出紧凑表示框架，包含两个轻量级模块：1) 几何重稠密化模块迭代稠密化编码的稀疏几何，提取稠密尺度特征后稀疏化用于预测编码；2) 跨尺度特征传播模块利用多分辨率级别的占用线索引导分层特征传播，实现跨尺度信息共享。此外引入整数推理管道确保跨平台一致性

Result: 实验表明该方法在实时速度下实现了有竞争力的压缩性能

Conclusion: 提出的框架通过紧凑表示和轻量级模块设计，解决了LiDAR点云压缩中的稀疏性挑战，实现了高效的无损压缩
...
TL;DR: V2U4Real是首个大规模真实世界多模态数据集，用于车辆与无人机协同感知，包含超过56K LiDAR帧、56K多视角图像和700K标注3D边界框，覆盖城市街道、校园和乡村道路等多种交通场景。


<details>
  <summary>Details</summary>
Motivation: 现有车辆间(V2V)和车路协同(V2I)感知系统受限于地面视角，无法完全解决大规模遮挡和复杂环境中的长距离感知问题，需要车辆与无人机(V2U)的跨视角协同感知来提升感知能力。

Method: 通过地面车辆和无人机配备多视角LiDAR和RGB相机采集数据，构建包含城市街道、大学校园和乡村道路的多样化交通场景数据集，并建立单智能体3D目标检测、协同3D目标检测和目标跟踪的基准测试。

Result: 数据集包含超过56,000个LiDAR帧、56,000个多视角相机图像和700,000个标注的3D边界框，涵盖四个类别。对多个先进模型的综合评估证明了V2U协同在增强感知鲁棒性和长距离感知方面的有效性。

Conclusion: V2U4Real数据集填补了车辆与无人机协同感知研究的数据空白，为跨视角协同感知提供了首个大规模真实世界基准，展示了V2U协同在提升感知系统性能方面的潜力。
...
Method: 引入MacroData数据集（40万个样本，每个最多包含10个参考图像），系统组织在四个互补维度：定制化、插图、空间推理和时间动态；同时提出MacroBench基准（4000个样本）用于评估生成一致性

Result: 在MacroData上微调显著提升了多参考图像生成性能，消融研究揭示了跨任务协同训练的好处以及处理长上下文复杂性的有效策略

Conclusion: MacroData和MacroBench为解决多参考图像生成的数据瓶颈提供了系统解决方案，数据集和基准将公开发布
...
TL;DR: 本文提出了一种基于头部敏感度得分的两阶段稀疏化方法，用于解决VGGT中全局注意力层的二次计算成本问题，通过量化头部稀疏化敏感度并重新分配注意力预算来减少精度损失。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏化加速技术虽然能降低VGGT中全局注意力层的二次计算成本，但往往导致显著的精度下降。作者假设这种精度下降源于头部稀疏化敏感度的异质性，而现有方法对所有头部应用统一的稀疏模式。

Method: 提出两阶段稀疏化流程：1）使用新指标HeSS（头部敏感度得分）在小规模校准集上量化头部稀疏化敏感度；2）在推理阶段进行HeSS引导的稀疏化，根据预计算的HeSS重新分配总注意力预算，为敏感头部分配更密集的注意力，为更鲁棒的头部分配更稀疏的注意力。

Result: 实验表明HeSS能有效捕捉头部稀疏化敏感度，并证实全局注意力层中的注意力头部确实具有异质性敏感度特征。该方法在高稀疏度下有效缓解了性能下降，在不同稀疏化水平下表现出强鲁棒性。

Conclusion: 通过量化并利用头部稀疏化敏感度的异质性，提出的HeSS引导稀疏化方法能在保持计算效率的同时显著减少精度损失，为VGGT等模型的稀疏化加速提供了更有效的解决方案。
...
TL;DR: InstanceAnimator是一个基于扩散Transformer的框架，用于多实例草图视频着色，解决了现有方法在用户控制灵活性、实例可控性和细节保真度方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在三个核心限制：1) 由于过度依赖单一参考帧导致用户控制不灵活；2) 在多角色场景中实例可控性差导致错位；3) 细粒度区域细节保真度下降。这些问题限制了多实例草图视频着色的实用性和质量。

Method: 提出三个创新模块：1) 画布引导条件，允许自由放置参考元素和背景，消除工作流程碎片化；2) 实例匹配机制，将实例特征与草图集成，确保对多个角色的精确控制；3) 自适应解耦控制模块，将角色、背景和文本条件的语义特征注入扩散过程，增强细节保真度。

Result: 大量实验表明，InstanceAnimator在用户控制、视觉质量和实例一致性方面实现了卓越的多实例着色效果，相比现有方法具有显著优势。

Conclusion: InstanceAnimator通过创新的画布引导、实例匹配和自适应控制模块，成功解决了多实例草图视频着色中的关键挑战，为用户提供了前所未有的控制灵活性，同时保持了高质量的视觉效果和实例一致性。
...
TL;DR: 提出CLIP-RD关系知识蒸馏框架，通过垂直关系蒸馏和交叉关系蒸馏方法，解决现有CLIP蒸馏方法未能建模师生嵌入间多向关系依赖的问题


<details>
  <summary>Details</summary>
Motivation: CLIP模型需要大量计算和内存资源，需要将其能力蒸馏到轻量级学生模型中。现有CLIP蒸馏方法未能显式建模师生嵌入间的多向关系依赖，限制了学生模型保持教师编码的结构关系的能力

Method: 提出关系知识蒸馏框架CLIP-RD，包含两种新方法：垂直关系蒸馏（VRD）在分布层面强制师生蒸馏强度跨模态一致性；交叉关系蒸馏（XRD）对跨模态师生相似度分布施加双向对称性约束

Result: CLIP-RD通过联合建模多向关系结构，促进学生嵌入几何与教师嵌入几何的忠实对齐，性能优于现有方法0.8个百分点

Conclusion: 通过显式建模师生嵌入间的多向关系依赖，CLIP-RD框架能够更有效地将CLIP的能力蒸馏到轻量级模型中，提升学生模型的性能表现
...
TL;DR: 本文提出PTM-ST框架，通过分阶段教师建模和基于捷径的轨迹构建策略，解决多模态数据集蒸馏中跨阶段性能差距和教师轨迹不稳定的问题。
...
Conclusion: PTM-ST框架通过分阶段蒸馏策略有效解决了多模态数据集蒸馏中的关键挑战，提升了蒸馏过程的稳定性和表达能力，取得了显著的性能改进。
...
TL;DR: FSGNet：一种结合频率感知和语义引导机制的轻量级红外小目标检测框架，通过多方向交互注意力、多尺度频率感知模块和全局语义引导流，解决了U-Net在特征传递中的语义退化问题，提升了小目标的精确定位能力。


<details>
  <summary>Details</summary>
Motivation: 虽然基于U-Net架构的红外小目标检测取得了显著进展，但U-Net在将深层高级特征传递到浅层时存在语义退化问题，限制了小目标的精确定位能力。需要解决这一限制以提升检测性能。

Method: 提出FSGNet框架，包含三个核心模块：1）多方向交互注意力模块，在编码器中捕获细粒度方向特征；2）多尺度频率感知模块，利用快速傅里叶变换过滤目标相似杂波；3）全局池化模块捕获高级语义信息，通过全局语义引导流传播到各解码器阶段。

Result: 在四个公开IRSTD数据集上的大量实验表明，FSGNet实现了优越的检测性能，同时保持了高效率，突出了其实际适用性和鲁棒性。

Conclusion: FSGNet通过频率感知和语义引导机制有效解决了U-Net的语义退化问题，实现了红外小目标的精确检测，具有轻量级、高效的特点，在实际应用中具有良好前景。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视觉基础模型的编码器-仅分割方法需要微调编码器，牺牲了多任务编码器共享的优势，而冻结编码器的方法又存在速度慢的问题

Method: 提出Plain Mask Decoder (PMD)，一个简单的基于Transformer的分割解码器，直接在冻结的VFM特征上操作，形成Plain Mask Transformer (PMT)模型

Result: 在图像分割基准测试中，PMT达到冻结编码器SOTA性能，速度提升约3倍；在视频分割中，性能与完全微调方法相当，比冻结编码器SOTA快8倍

Conclusion: PMT成功平衡了编码器-仅设计的简单快速与冻结VFM特征的多任务共享优势，为大规模部署提供了实用解决方案
...
TL;DR: LaMP是一个双专家视觉-语言-动作框架，通过将密集3D场景流作为潜在运动先验嵌入，提升机器人操作性能。相比直接回归动作的现有VLA模型，LaMP通过流匹配运动专家和策略预测动作专家的门控交叉注意力对齐，在多个仿真和真实世界实验中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型直接从2D语义视觉特征回归动作，迫使它们隐式学习复杂的3D物理交互。这种隐式学习策略在遇到不熟悉的空间动态时会性能下降。需要一种能显式建模3D运动先验的方法来提升机器人操作的鲁棒性。

Method: LaMP采用双专家架构：1）运动专家通过流匹配生成一步部分去噪的3D场景流；2）动作专家预测策略，通过门控交叉注意力与运动专家的隐藏状态对齐，无需完整的多步重建。这种设计使模型能利用3D运动先验而不需要完全重建。

Result: 在LIBERO、LIBERO-Plus和SimplerEnv-WidowX仿真基准测试以及真实世界实验中，LaMP始终优于评估的VLA基线，在相同训练预算下达到最高平均成功率。在LIBERO-Plus OOD扰动下，LaMP表现出更好的鲁棒性，比最强基线平均提升9.7%。

Conclusion: LaMP通过将3D场景流作为潜在运动先验嵌入，有效解决了现有VLA模型在复杂3D物理交互中隐式学习的局限性。双专家架构和门控交叉注意力机制使模型能更好地适应空间动态变化，显著提升了机器人操作的性能和鲁棒性。
...
TL;DR: 该论文提出了一个分层框架来提升视觉语言模型的3D空间理解能力，通过自动生成大规模3D空间VQA数据进行监督微调，并开发了结合RGB-D点云输入的VLM模型，在多个空间理解基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 实现类似人类的3D空间智能需要视觉语言模型从2D观察推断3D结构、识别3D空间中的物体属性和关系，并进行高级空间推理。现有VLMs在这方面存在不足，需要系统性的方法来提升3D空间理解能力。

Method: 1) 提出分层框架将3D空间理解学习分解为四个渐进复杂层次：几何感知到抽象空间推理；2) 构建自动化流水线处理约500万图像和4500万物体，生成多样化的3D空间VQA对用于VLM监督微调；3) 开发RGB-D VLM，将度量尺度点云图作为辅助输入以增强空间理解。

Result: 在多个空间理解和推理基准测试中达到最先进性能，超越了专门的3D空间模型和大型专有系统如Gemini-2.5-pro和GPT-5。分析显示分层任务级别之间存在明确依赖关系。

Conclusion: 提出的分层框架和自动数据生成方法有效提升了VLMs的3D空间智能，多级任务设计促进了3D空间智能的涌现，为构建更强大的空间感知AI系统提供了新见解。
...
TL;DR: VideoWeaver是首个多模态多视角视频到视频翻译框架，通过共享4D潜在空间和扩散时间步训练实现视角一致的多视角翻译，支持机器人学习中的世界随机化。


<details>
  <summary>Details</summary>
Motivation: 现有视频到视频翻译方法只能处理单视角，而具身AI任务通常使用多个同步摄像头。独立应用单视角模型会导致视角间外观不一致，标准Transformer架构由于跨视角注意力的二次成本无法扩展到多视角设置。

Method: 1. 首先训练为基于流的单视角V2V模型；2. 通过空间基础模型Pi3将所有视角锚定在共享的4D潜在空间中，确保视角一致性；3. 在不同扩散时间步训练不同视角，使模型学习联合和条件视角分布；4. 支持基于现有视角的自回归新视角合成。

Result: 在单视角翻译基准上达到或优于最先进水平，首次实现物理和风格一致的多视角翻译，包括对机器人学习至关重要的具有挑战性的自我中心视角和异构相机设置。

Conclusion: VideoWeaver首次实现了多视角一致的视频到视频翻译，为机器人策略在不同环境中的迁移提供了有效工具，支持具身AI任务中的世界随机化需求。
...
TL;DR: DC-Reg：一种基于DC规划和BnB的全局最优点云配准框架，通过整体凹下界估计实现快速收敛，对噪声和异常值具有强鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有点云配准方法在处理部分重叠和大错位时面临挑战：同时估计变换和对应关系的方法虽然对非刚性变形鲁棒，但其非凸耦合目标导致启发式方法陷入局部最优，而现有全局求解器因下界松弛导致收敛时间过长

Method: 提出DC-Reg框架，基于DC规划推导出变换-对应耦合目标的整体凹下界估计器，通过线性分配问题在搜索框顶点计算紧致下界，支持2D相似变换和3D刚性配准（后者使用旋转不变特征提高效率）

Result: 在合成数据和3DMatch基准测试中，DC-Reg相比最先进的全局技术实现显著更快的收敛速度，对极端噪声和异常值具有优越的鲁棒性

Conclusion: DC-Reg通过整体DC分解捕获变换与对应之间的联合结构交互，为分支定界搜索提供紧致下界，解决了全局最优点云配准中的收敛效率和鲁棒性问题
...
TL;DR: CIAR是一个云-设备协作框架，通过设备端自验证和不确定性量化来解决自回归图像生成模型的延迟问题，实现了2.18倍加速并减少70%云端请求


<details>
  <summary>Details</summary>
Motivation: 自回归图像生成模型虽然性能接近扩散模型，但其计算密集和顺序处理的特性导致设备端部署时产生破坏性延迟，阻碍了实际应用

Method: 提出CIAR框架：1) 设备端令牌不确定性量化器，采用连续概率区间加速处理；2) 区间增强解码模块，通过分布对齐训练策略保持视觉保真度和语义一致性；3) 利用视觉合成的两个关键特性：大词汇量和空间冗余性

Result: CIAR实现了2.18倍的加速，减少了70%的云端请求，同时保持了与现有方法相当的图像质量

Conclusion: CIAR通过云-设备协作框架有效解决了自回归图像生成模型的延迟问题，为设备端高效视觉合成提供了可行方案
...
TL;DR: GridVAD是一个无需训练的零样本视频异常检测框架，通过"提议-定位-传播"原则，使用视觉语言模型生成异常候选描述，然后通过空间和时间模块进行定位和跟踪，实现像素级异常分割。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型作为开放集推理器在视频监控异常检测中直接使用时存在脆弱性：缺乏校准的异常先验会导致漏检和误报。问题不在于VLM本身，而在于如何使用它。

Method: 提出GridVAD框架：1) VLM在分层网格表示上生成自然语言异常提议；2) 自一致性整合过滤幻觉提议；3) Grounding DINO将提议定位到边界框；4) SAM2在异常区间内传播为密集掩码。固定每片段M+1次VLM调用。

Result: 在UCSD Ped2数据集上，GridVAD获得最高像素级AUROC(77.59)，超过部分微调的TAO(75.11)，在对象级RBDC上比其他零样本方法高出5倍以上。SCC提供可控的精度-召回权衡，GridVAD比均匀每帧查询效率高2.7倍。

Conclusion: VLM应作为异常提议者而非直接检测器，GridVAD通过"提议-定位-传播"原则实现了无需训练的高效零样本视频异常检测，在像素级和对象级指标上均表现优异。
...
TL;DR: AdaSFormer：一种针对室内单目语义场景补全的串行化Transformer框架，通过自适应感受野、中心相对位置编码和卷积调制层归一化等技术，在复杂室内场景中实现了最先进的性能。
...
Method: 提出AdaSFormer框架，包含三个关键设计：1）具有可学习偏移的自适应串行化Transformer，可动态调整感受野；2）中心相对位置编码，捕捉空间信息丰富性；3）卷积调制层归一化，桥接卷积和Transformer特征的异构表示。

Result: 在NYUv2和Occ-ScanNet数据集上的大量实验表明，AdaSFormer实现了最先进的性能。

Conclusion: AdaSFormer通过创新的串行化Transformer设计有效解决了室内单目语义场景补全的挑战，在复杂室内场景中表现出色，代码已开源。
...
TL;DR: KGFP是一种基于表示的监控框架，通过检测目标检测器内部特征与视觉基础模型嵌入之间的语义不对齐来预测功能失效，显著提升安全关键检测的召回率。


<details>
  <summary>Details</summary>
Motivation: 安全关键环境中的目标检测器可能无声地失效（如漏检行人、工人等），传统OOD检测方法只关注识别陌生输入，不能直接预测检测器本身的功能失效。

Method: 提出知识引导失效预测（KGFP），采用双编码器架构和角度距离度量，测量目标检测器内部特征与视觉基础模型嵌入之间的语义不对齐，当两者出现分歧时产生高角度信号来标记不安全图像。

Result: 在COCO行人检测任务中，KGFP作为选择性预测门将接受图像中行人的召回率从64.3%提升到84.5%（5%假阳性率），在六个COCO-O视觉域中保持强劲性能，大幅优于OOD基线方法。

Conclusion: KGFP能有效预测目标检测器的功能失效，通过监测内部特征与基础模型嵌入之间的语义不对齐，为安全关键应用提供了可靠的运行时监控框架。
...
TL;DR: 该研究针对真实世界图像修复任务，构建大规模数据集并训练开源模型以缩小与闭源模型的性能差距，同时提出包含464张真实退化图像和专门评估指标的基准测试集。


<details>
  <summary>Details</summary>
Motivation: 现有修复模型受限于训练数据的规模和分布，在真实世界场景中泛化能力差；而大型闭源模型虽表现优异但需要大量数据和计算成本，因此需要开发高性能的开源解决方案。

Method: 构建覆盖9种常见真实世界退化类型的大规模数据集，训练先进的开放源码模型，并引入包含464张真实退化图像的RealIR-Bench基准测试集及专门评估指标。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶领域应用高光谱成像技术面临诸多挑战：非受控的变光照条件、大景深范围、动态场景中的快速移动物体，以及实时性要求和嵌入式平台有限的计算资源。这些因素共同决定了技术选择和算法开发的需求。

Method: 分析自动驾驶中基于高光谱成像的视觉系统研究技术，使用最新版HSI-Drive数据集的实验结果作为案例进行探讨。

Result: 通过HSI-Drive数据集的实验，展示了高光谱成像在自动驾驶应用中的具体技术探索和算法开发成果。

Conclusion: 高光谱成像在自动驾驶中的应用需要综合考虑环境挑战和计算限制，通过选择合适的技术和开发定制算法来充分利用光谱和空间信息。
...
<details>
  <summary>Details</summary>
Motivation: 野生动物长期行为监测对保护生物学和进化生物学至关重要，但缺乏覆盖多种计算机视觉任务的综合数据集，难以从个体动物提取有生物学意义的测量数据

Method: 1. 创建CHIRP数据集：来自瑞典拉普兰野生西伯利亚松鸦种群，支持重识别、动作识别、2D关键点估计、目标检测和实例分割；2. 开发CORVID方法：基于彩色腿环分割和分类的鸟类个体识别管道，使用概率匹配方法；3. 引入应用特定基准测试：使用生物学相关指标（喂食率、共现率）评估模型在实际应用中的表现

Result: CORVID在应用特定基准测试中优于现有最先进的重识别方法，CHIRP数据集为计算机视觉任务提供了综合基准

Conclusion: 该工作为从伦理批准的生物学研究中策划真实世界数据集提供了蓝图，有助于弥合计算机视觉研究与生物学应用之间的差距
...
TL;DR: 本文提出了一种解决视频生成中"运动-视觉质量困境"的新方法TQD，通过时间步感知的质量解耦，在不完美的数据上训练也能超越传统方法在优质数据上的表现。


<details>
  <summary>Details</summary>
Motivation: 视频生成模型严重依赖同时具备高视觉质量和高运动质量的"黄金数据"，但研究发现视觉质量和运动强度存在负相关关系，难以获得同时具备两方面优势的数据，这构成了"运动-视觉质量困境"。

Method: 提出时间步感知质量解耦(TQD)方法：首先分析视频扩散模型的分层学习动态，对质量退化样本进行基于梯度的分析；发现质量不平衡数据在适当时间步能产生与黄金数据相似的梯度；基于此引入训练过程中的时间步选择概念，修改数据采样分布以更好地匹配模型学习过程：运动丰富的数据偏向更高时间步采样，高视觉质量数据偏向更低时间步采样。

Result: 实验表明TQD能够仅在不平衡的分离数据上训练就超越传统方法在更好数据上的表现，挑战了视频生成中完美数据的必要性；同时该方法在高质量数据上训练时也能提升模型性能，展示了在不同数据场景下的有效性。

Conclusion: TQD方法通过时间步感知的质量解耦，有效解决了视频数据整理中的运动-视觉质量困境，证明了即使使用不完美的数据也能训练出高性能的视频生成模型，为视频生成领域的数据利用提供了新思路。
...
TL;DR: 本文提出了首个羽毛球全场比赛密集标注数据集BFMD，包含19场比赛、1687个回合和16751个击球事件，并开发了基于VideoMAE的多模态字幕生成框架，通过语义反馈机制提升击球描述质量。


<details>
  <summary>Details</summary>
Motivation: 现有羽毛球数据集主要关注短片段或特定任务标注，缺乏全场比赛数据和密集多模态标注，这限制了准确击球描述生成和比赛级别战术分析的能力。

Method: 1) 构建BFMD数据集，包含19场广播比赛，涵盖单打和双打，提供分层标注；2) 开发基于VideoMAE的多模态字幕生成框架，引入语义反馈机制，利用击球语义指导字幕生成。

Result: 实验结果表明，多模态建模和语义反馈机制相比仅使用RGB的基线方法显著提升了击球字幕质量。数据集还展示了在全场比赛战术模式时间演化分析方面的潜力。

Conclusion: BFMD数据集填补了羽毛球全场比赛密集标注数据的空白，提出的多模态字幕生成框架有效提升了击球描述准确性，为比赛级别战术动态分析提供了重要基础。
...
Result: 在HD-EPIC和Arti4D数据集上相比基线方法有显著提升，提取的关节结构有益于下游任务
...
<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型在地球观测中通常忽略关键的"垂直"维度，限制了它们在复杂遥感几何和灾害场景中的推理能力，因为物理空间结构往往比平面视觉纹理更重要。

Method: 1) 开发可扩展的VLM驱动数据生成管道，利用系统提示工程和元数据提取；2) 构建两个互补基准：GeoHeight-Bench用于相对高度分析，GeoHeight-Bench+用于更全面的地形感知推理；3) 提出GeoHeightChat，首个高度感知的遥感LMM基线模型。

Result: 提出的基线模型证明，将视觉语义与隐式注入的高度几何特征相结合，能有效缓解"垂直盲点"，成功在现有光学模型中开启了交互式高度推理的新范式。

Conclusion: 该研究填补了地球观测大模型中高度感知能力的空白，通过创新的评估框架和基线模型，为遥感理解中的垂直维度推理提供了新的研究方向和技术基础。
...
TL;DR: 该论文提出了两种层次感知的多模态学习方法，通过编码生物分类的层次结构来提高大规模生物多样性识别的准确性，在噪声和缺失模态条件下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有生物多样性识别方法通常将分类学视为扁平标签空间，忽略了生物分类的层次结构，导致在噪声和缺失模态条件下鲁棒性不足。需要开发能够编码层次结构并支持灵活模态融合的方法。

Method: 提出了两种端到端变体：1) CLiBD-HiR：引入层次信息正则化（HiR）来塑造跨分类级别的嵌入几何，产生结构化和噪声鲁棒的表征；2) CLiBD-HiR-Fuse：额外训练轻量级融合预测器，支持仅图像、仅DNA或联合推理，并对模态损坏具有弹性。

Result: 在大规模生物多样性基准测试中，相比强大的多模态基线方法，该方法将分类学分类准确性提高了超过14%，在部分和损坏的DNA条件下获得特别大的增益。

Conclusion: 明确编码生物层次结构并结合灵活融合是构建实用生物多样性基础模型的关键，该方法为生态学、保护和环境监测提供了更可靠的大规模生物多样性识别工具。
...
<details>
  <summary>Details</summary>
Motivation: CT广泛应用导致肺结节检出增多，但现有深度学习方法未能全面整合全局和局部特征，且缺乏临床验证。

Method: 开发基于Transformer的DeepFAN模型，在超过10K病理确认的结节上训练，并进行多读者多病例临床试验评估其对初级放射科医生的辅助效果。

Result: DeepFAN在内部测试集AUC为0.939，临床试验数据集AUC为0.954。12位读者平均性能显著提升：AUC提高10.9%，准确率提高10.0%，敏感性提高7.6%，特异性提高12.6%。结节级别读者间诊断一致性从一般提升到中等。
...
TL;DR: 本文对9个开源多模态大语言模型在面部验证任务中的公平性进行基准测试，发现面部专用模型FaceLLM-8B表现最佳，但公平性模式与传统面部识别系统不同，且最准确的模型不一定最公平。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）最近被探索用作面部验证系统，但其人口统计学公平性尚未得到充分研究。本文旨在评估这些模型在不同种族和性别群体中的公平性表现。

Method: 研究评估了9个来自6个模型系列的开源MLLMs（参数规模2B-8B），使用IJB-C和RFW面部验证协议，覆盖4个种族群体和2个性别群体。通过等错误率和多个操作点的真匹配率测量验证准确性，并使用4个基于FMR的公平性指标量化人口统计学差异。
...
Conclusion: 多模态大语言模型在面部验证任务中存在人口统计学公平性问题，其偏见模式与传统面部识别系统不同。面部专用模型表现更好，但模型准确性不一定与公平性相关，需要专门的公平性评估方法。
...
TL;DR: LanteRn框架让大型多模态模型能够在潜在空间中进行视觉推理，通过生成和关注连续视觉思维嵌入，避免了像素空间的不必要计算，提高了视觉推理效率。


<details>
  <summary>Details</summary>
Motivation: 当前大型多模态模型在视觉推理方面存在挑战，大多数模型将感知内容转化为文本，这限制了需要细粒度空间和视觉理解的任务。现有方法要么依赖外部模块，要么在像素空间中推理导致不必要的计算开销。

Method: LanteRn框架增强视觉语言变换器，使其能够在推理过程中生成和关注连续视觉思维嵌入。采用两阶段训练：监督微调将视觉特征映射到潜在状态，然后通过强化学习将潜在推理与任务级效用对齐。

Result: 在三个感知中心基准测试（VisCoT、V*和Blink）上评估，观察到视觉基础和细粒度推理方面的一致改进。

Conclusion: 内部潜在表示为更高效的多模态推理提供了有前景的方向，通过潜在空间中的视觉推理可以避免像素空间的不必要计算，提高推理效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有跨视角地理定位方法普遍采用基于对比学习的图像检索框架，存在三个主要问题：1）依赖大批量和硬负样本挖掘；2）忽略地图的几何结构；3）街景和卫星图像覆盖不匹配问题，导致检索目标模糊且空间推理受限

Method: 提出Just Zoom In方法，将CVGL重新定义为自回归缩放问题。从粗粒度卫星视图开始，模型通过一系列缩放决策选择目标分辨率的终端卫星单元，无需对比损失或硬负样本挖掘。同时引入包含众包街景和高分辨率卫星图像的真实基准

Result: 在提出的真实基准上，Just Zoom In达到最先进性能：在50米内Recall@1提升5.5%，在100米内Recall@1提升9.6%，优于最强的对比检索基线

Conclusion: 结果表明，从粗到细的顺序空间推理方法对跨视角地理定位是有效的，为CVGL提供了新的替代框架
...
TL;DR: Trace框架：通过单锚帧设计目标轨迹，实现视频中物体运动路径编辑，保持场景内容不变
...
Method: 两阶段流水线：1)跨视角运动变换模块，将首帧路径设计映射到相机运动下的帧对齐边界框轨迹；2)运动条件视频重合成模块，按轨迹重新生成物体并保持输入视频其余内容

Result: 在多样化真实世界视频上的实验表明，相比最近的图像到视频和视频到视频方法，本方法产生更连贯、真实且可控的运动编辑效果

Conclusion: Trace框架提供了一种实用、易用的物体运动路径编辑方法，用户只需在单锚帧设计期望轨迹，即可生成时间一致的编辑视频，有效解决了相机运动下的物体轨迹编辑挑战
...
TL;DR: 提出Hybrid Memory新范式解决视频世界模型中动态主体消失/扭曲问题，构建HM-World数据集并开发HyDRA架构，显著提升动态主体一致性和生成质量


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型将环境视为静态画布，当动态主体离开视线再出现时，常出现冻结、扭曲或消失的问题，需要新的记忆机制来处理静态背景和动态主体的不同需求

Method: 提出Hybrid Memory范式，要求模型同时作为静态背景的精确档案员和动态主体的警觉追踪器；构建HM-World数据集（59K高保真片段，解耦相机和主体轨迹）；开发HyDRA架构，将记忆压缩为token并使用时空相关性驱动的检索机制

Result: 在HM-World数据集上的广泛实验表明，该方法在动态主体一致性和整体生成质量方面显著优于现有最先进方法

Conclusion: Hybrid Memory范式有效解决了动态主体在离开视线期间的运动连续性问题，HyDRA架构通过选择性关注相关运动线索，成功保持了隐藏主体的身份和运动
...
TL;DR: 该论文提出两种简单改进方法解决视觉语言模型的组合性学习问题：使用短概念中心化描述和对齐，以及引入参数自由的跨模态注意力池化，在保持基础能力的同时显著提升组合性任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有对比视觉语言模型存在组合性表示学习能力有限的问题。先前方法通常通过生成特定训练数据获取困难负样本来解决，但这些方法往往只针对单一基准，缺乏泛化性，且会严重损害模型的基础能力（如零样本学习和检索性能），实用性受限。

Method: 提出两种核心改进：1）使用标准NLP工具获取短概念中心化描述片段并与图像对齐；2）引入参数自由的跨模态注意力池化机制，从图像编码器中提取概念中心化的视觉嵌入。结合简单的辅助对比损失，在不增加推理成本的情况下提升性能。

Result: 在标准组合性基准测试中取得了最先进的性能，同时保持甚至增强了模型的零样本学习和检索能力。该方法具有很好的泛化性，不损害基础视觉语言能力。

Conclusion: 通过解决长训练描述不需要组合性表示以及全局池化导致绑定信息丢失这两个根本问题，提出的简单改进方法能有效提升视觉语言模型的组合性学习能力，同时保持其实用性和泛化性。
...
TL;DR: AnyHand是一个大规模合成数据集，包含250万张单手和410万张手-物体交互的RGB-D图像，旨在提升3D手部姿态估计的性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有真实世界数据集覆盖有限，先前合成数据集缺乏遮挡、手臂细节和深度信息，限制了3D手部姿态估计的性能提升。需要大规模、多样化的合成数据来解决这一瓶颈。

Method: 创建包含2.5M单手和4.1M手-物体交互RGB-D图像的AnyHand数据集，提供丰富的几何标注。同时开发轻量级深度融合模块，可轻松集成到现有RGB模型中。

Result: 在RGB-only设置下，使用AnyHand扩展训练集显著提升了FreiHAND和HO-3D基准测试性能；在未微调情况下，模型在域外HO-Cap数据集上表现出更强的泛化能力。RGB-D模型在HO-3D基准上达到最优性能。

Conclusion: AnyHand数据集有效解决了3D手部姿态估计的数据瓶颈，通过大规模合成数据和深度融合模块，显著提升了模型性能和泛化能力，为RGB和RGB-D手部姿态估计提供了有力支持。
...
TL;DR: 本文提出PixelSmile，一个用于细粒度面部表情编辑的扩散框架，通过完全对称联合训练解耦表情语义，结合强度监督和对比学习实现精确线性控制。
...
Conclusion: PixelSmile有效实现了连续、可控、细粒度的表情编辑，自然支持平滑的表情混合，解决了面部表情编辑中的语义重叠问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注自然图像合成，无法系统评估模型在现实商业设计任务中的结构化、多约束要求下的表现，需要专门的商业视觉内容生成基准。

Method: 创建BizGenEval基准，包含5种文档类型、4个能力维度、20个评估任务、400个精心设计的提示和8000个人工验证的检查清单问题，对26个主流图像生成系统进行大规模基准测试。

Result: 基准测试结果显示，当前生成模型与专业视觉内容创作需求之间存在显著能力差距，现有模型难以满足商业设计任务中的复杂视觉和语义约束。
...
TL;DR: SlotVTG：一个轻量级框架，通过对象中心学习提升多模态大语言模型在视频时序定位任务中的域外泛化能力


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视频时序定位任务中表现良好，但其粗粒度的识别能力不足以进行细粒度的时间理解，需要任务特定的微调。这种微调导致模型记忆数据集特定的捷径，而非真正基于视觉内容进行定位，导致域外泛化能力差。

Method: 提出SlotVTG框架，引入轻量级槽适配器，通过槽注意力将视觉标记分解为抽象槽并重建原始序列。使用自监督视觉模型的对象性先验促进语义连贯的槽形成，以最小成本引导MLLMs进行对象中心、输入接地的视觉推理。

Result: 在标准VTG基准测试的跨域评估中，该方法显著提高了域外鲁棒性，同时以最小开销保持了具有竞争力的域内性能。

Conclusion: SlotVTG通过对象中心学习有效解决了MLLMs在视频时序定位中的域外泛化问题，提供了一种低成本、高效的解决方案。
...
TL;DR: LIGHT：一种基于数据驱动的扩散模型，通过异步去噪调度和跨模态引导生成逼真的人-物交互动画，无需手工设计的接触先验


<details>
  <summary>Details</summary>
Motivation: 生成逼真的人-物交互动画具有挑战性，需要同时建模动态人体动作和多样化的物体几何形状。现有基于扩散的方法通常依赖手工设计的接触先验或人为施加的运动学约束来改善接触质量。

Method: 提出LIGHT方法：1）基于扩散强迫技术，将表示分解为模态特定组件；2）为不同组件分配个性化噪声水平，采用异步去噪调度；3）通过跨注意力机制，让较干净的组件引导较嘈杂的组件，实现无需辅助分类器的引导；4）使用广泛的合成物体几何数据进行训练，增强接触语义对几何多样性的不变性。

Result: 实验表明，基于节奏的引导比传统的无分类器引导更有效地模拟接触先验的益处，同时实现了更高的接触保真度、更逼真的人-物交互生成，以及对未见物体和任务的更强泛化能力。

Conclusion: LIGHT提供了一种数据驱动的替代方案，通过去噪节奏本身产生的引导减少了对手工设计先验的依赖，能够生成更逼真、接触感知的人-物交互动画。
...
TL;DR: PSDesigner是一个模仿人类设计师工作流程的自动化图形设计系统，通过收集主题相关素材并自主推断和执行工具调用来操作设计文件，解决了现有系统灵活性不足的问题。
...
Method: 基于多个专用组件构建PSDesigner系统，收集主题相关素材，自主推断和执行工具调用来操作设计文件。创建CreativePSD数据集，包含大量高质量PSD设计文件和操作轨迹，让模型学习专家设计流程。

Result: 大量实验表明，PSDesigner在多样化图形设计任务中优于现有方法，使非专业人士能够方便地创建生产质量的设计。

Conclusion: PSDesigner通过模仿人类设计师的创意工作流程，解决了自动化设计系统的局限性，为非专业人士提供了强大的图形设计工具。
...
TL;DR: MegaFlow是一个用于零样本大位移光流估计的简单而强大的模型，它利用预训练的视觉先验进行全局匹配，无需特定任务架构设计或领域微调


<details>
  <summary>Details</summary>
Motivation: 现有的大位移光流估计方法通常依赖迭代局部搜索和/或领域特定微调，这严重限制了它们在大位移和零样本泛化场景中的性能

Method: 将光流估计制定为全局匹配问题，利用预训练的全局Vision Transformer特征捕获大位移，然后进行少量轻量级迭代细化以提高亚像素精度

Result: 在多个光流基准测试中实现了最先进的零样本性能，在长程点跟踪基准测试中也表现出高度竞争力的零样本性能，展示了强大的可迁移性

Conclusion: MegaFlow为通用运动估计提供了一个统一的范式，通过利用预训练的视觉先验实现零样本大位移光流估计，无需复杂的任务特定架构设计
...
TL;DR: 本文提出了Vega模型，这是一个统一的视觉-语言-世界-动作模型，用于基于指令的自动驾驶生成和规划，通过构建大规模驾驶数据集InstructScene并采用自回归和扩散范式实现多模态交互。
...
Method: 首先构建包含约10万个场景的大规模驾驶数据集InstructScene，标注了多样化驾驶指令及对应轨迹；然后提出Vega模型，采用自回归范式处理视觉输入和语言指令，扩散范式生成未来预测和轨迹；通过联合注意力实现模态间交互，不同模态使用独立的投影层以增强能力。

Result: 大量实验表明，该方法不仅实现了优越的规划性能，还展现出强大的指令跟随能力，为更智能和个性化的驾驶系统铺平了道路。

Conclusion: Vega模型通过统一的多模态架构和创新的训练方法，成功解决了现有自动驾驶系统在个性化指令跟随方面的局限性，推动了智能驾驶系统向更灵活、更人性化的方向发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有参考到视频生成方法虽然引入了高级语义或跨模态特征来缓解VAE潜在空间中的像素级信息泄露，但仍然存在模态不匹配导致的复制粘贴伪影和多主体混淆问题，需要在文本可控性和参考保真度之间取得更好平衡。

Method: 提出RefAlign表示对齐框架，核心是参考对齐损失函数：将同一主体的参考特征和视觉基础模型特征拉近以提高身份一致性，同时将不同主体的对应特征推远以增强语义区分性。该策略仅在训练时应用，不增加推理时间开销。

Result: 在OpenS2V-Eval基准测试中，RefAlign在TotalScore指标上优于当前最先进的方法，验证了显式参考对齐在参考到视频生成任务中的有效性。

Conclusion: 显式参考对齐是解决参考到视频生成中模态不匹配问题的有效策略，能够在保持文本可控性的同时提高参考保真度，且不增加推理计算成本。
...
TL;DR: MuRF是一种无需训练的多分辨率融合策略，通过处理图像在多个分辨率下的特征并融合，提升视觉基础模型的推理性能。
...
Method: 提出Multi-Resolution Fusion (MuRF)策略，通过冻结的视觉基础模型处理图像在多个分辨率下的特征，然后将这些特征融合成统一表示。该方法无需训练，适用于各种架构。

Result: MuRF在广泛的计算机视觉任务中表现出有效性，主要应用于DINOv2模型，并成功推广到对比学习模型如SigLIP2，证明了其通用性。

Conclusion: MuRF是一种简单而普遍有效的策略，能够利用多分辨率特征的协同作用，作为视觉表示的基本训练免费增强方法，适用于不同的视觉基础模型家族。
...
TL;DR: LGTM提出了一种前馈式3D高斯泼溅框架，通过预测紧凑的高斯基元配合每个基元的纹理，将几何复杂度与渲染分辨率解耦，实现了无需场景优化的4K新视角合成。
...
Method: 提出LGTM框架，预测紧凑的高斯基元并配合每个基元的纹理，将几何复杂度与渲染分辨率解耦，从而克服分辨率缩放障碍。

Result: 实现了无需场景优化的高保真4K新视角合成，这是前馈方法之前无法达到的能力，同时使用显著更少的高斯基元。

Conclusion: LGTM通过解耦几何复杂度和渲染分辨率，克服了前馈式3D高斯泼溅方法的分辨率缩放限制，为高分辨率新视角合成提供了可行的解决方案。
...
TL;DR: ShotStream是一种新颖的因果多镜头视频生成架构，通过将任务重新定义为基于历史上下文的下一个镜头生成，实现交互式叙事和高效实时帧生成，在单GPU上达到16 FPS。


<details>
  <summary>Details</summary>
Motivation: 当前双向架构在多镜头视频生成中存在交互性有限和延迟高的问题，无法满足实时交互式叙事的需求。需要一种能够支持动态指令输入、保持镜头间一致性并降低延迟的解决方案。
...
Result: ShotStream能够生成连贯的多镜头视频，延迟低于1秒，在单GPU上达到16 FPS。在质量上匹配或超越了较慢的双向模型，为实时交互式叙事铺平了道路。

Conclusion: ShotStream通过因果架构、双缓存记忆机制和两阶段蒸馏策略，成功解决了多镜头视频生成中的交互性、一致性和延迟问题，实现了高效、连贯的实时视频生成，推动了交互式叙事技术的发展。
