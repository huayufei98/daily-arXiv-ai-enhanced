...
Motivation: 现有步态识别研究主要基于轮廓和解析表示，但轮廓稀疏缺乏内部结构细节，解析方法依赖上游人体解析器导致性能不稳定。需要探索新的步态表示方法。

Method: 从结构视角重新审视步态表示，提出SKETCH作为新的视觉模态，通过边缘检测器从RGB图像中无标签地提取高频结构线索。进一步提出SKETCHGAIT框架，包含两个独立流进行模态特定学习，以及轻量级早期融合分支捕捉结构互补性。

Result: 在SUSTech1K和CCPG数据集上的实验验证了所提模态和框架的有效性：SketchGait在SUSTech1K上达到92.9%的Rank-1准确率，在CCPG上达到93.1%的平均Rank-1准确率。

Conclusion: SKETCH作为一种新的步态识别视觉模态，能够无标签地提取密集部分级结构线索，与标签引导的解析方法在语义上解耦且结构上互补，SKETCHGAIT框架有效利用了这种互补性，显著提升了步态识别性能。
...
TL;DR: DreamCAD是一个多模态生成框架，能够直接从点云监督生成可编辑的BRep（边界表示），无需CAD特定标注，并创建了最大的CAD标注数据集CADCap-1M。
...
Conclusion: DreamCAD通过点级监督生成可编辑BRep，无需CAD特定标注，结合CADCap-1M数据集，推动了可扩展CAD生成和文本到CAD研究的发展。
...
TL;DR: ABRA提出对抗性批次表示增强方法，通过参数化特征统计量为结构化不确定性，在表示空间中合成最坏情况批次扰动，解决细胞图像分析中的生物批次效应问题。
...
Method: 将生物批次缓解问题构建为领域泛化问题，提出对抗性批次表示增强（ABRA）。通过参数化特征统计量为结构化不确定性，在表示空间中主动合成最坏情况生物批次扰动，使用严格的角几何边界保持细粒度类别可区分性，并引入协同分布对齐目标防止表示坍塌。

Result: 在RxRx1和RxRx1-WILDS大规模基准测试上进行广泛评估，结果表明ABRA在siRNA扰动分类任务上建立了新的最先进性能。

Conclusion: ABRA通过对抗性增强方法有效缓解生物批次效应，提高了深度学习模型在细胞图像分析中的泛化能力，为高通量筛选中的批次效应问题提供了有效的解决方案。
...
TL;DR: 提出PFS（后融合稳定器）模块，通过处理BEV中间特征来提升现有融合检测器在域偏移和传感器故障下的鲁棒性，无需修改架构或重新训练。
...
Method: 提出轻量级PFS模块，在现有检测器的中间BEV表示上操作，生成精炼的特征图供原始检测头使用。通过稳定域偏移下的特征统计、抑制传感器退化影响的区域、自适应恢复弱化线索的残差校正来实现鲁棒性提升。

Result: 在nuScenes基准测试中，PFS在多种故障模式下取得SOTA结果：相机丢失鲁棒性提升+1.2%，低光性能提升+4.4% mAP，同时保持轻量级（仅3.3M参数）。

Conclusion: PFS作为近恒等变换设计，在保持性能的同时显著提升现有融合检测器在相机和LiDAR损坏下的鲁棒性，易于集成到已部署系统中。
...
TL;DR: CBM-Suite框架解决了概念瓶颈模型的四个核心问题：概念相关性预评估指标、线性化问题、精度差距以及视觉主干和VLM影响的系统性研究。


<details>
  <summary>Details</summary>
Motivation: 现有概念瓶颈模型存在四个基本限制：缺乏预评估概念相关性的指标、线性化问题导致模型绕过概念瓶颈、与不透明模型相比存在精度差距、缺乏对不同视觉主干和视觉语言模型影响的系统性研究。

Method: 提出CBM-Suite框架：1) 基于熵的指标量化概念集的内在适用性；2) 在概念激活和分类器之间插入非线性层解决线性化问题；3) 使用线性教师探针引导的蒸馏损失缩小精度差距；4) 系统性分析不同视觉编码器、视觉语言模型和概念集对精度和可解释性的影响。

Result: 广泛评估表明，CBM-Suite能够产生更准确的模型，并为改进基于概念的可解释性提供见解。

Conclusion: CBM-Suite框架系统性地解决了概念瓶颈模型的关键挑战，通过新指标、非线性层、蒸馏损失和系统性分析，提高了模型精度并增强了可解释性。
...
Method: 提出iFID：对每个数据样本，在潜在空间中检索其最近邻，插值潜在表示后解码，计算解码样本与原始数据集的FID。同时分析rFID与扩散细化阶段质量相关，iFID与扩散导航阶段质量相关

Result: iFID是首个与扩散gFID强相关的指标，皮尔逊线性相关和斯皮尔曼秩相关达到约0.85。rFID与扩散细化阶段质量相关，iFID与扩散导航阶段质量相关

Conclusion: iFID能有效预测扩散模型的生成质量，为评估扩散模型提供了新的可靠指标，并解释了重建指标与生成质量负相关的原因
...
TL;DR: 论文提出Implicit Error Counting (IEC)方法，用于解决缺乏理想参考答案的任务中的强化学习奖励设计问题，通过在虚拟试穿领域验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR和RaR方法依赖于理想参考答案来生成评估标准，但许多现实任务允许多种有效输出且缺乏单一理想答案，这种无参考设置是当前后训练方法的空白。

Method: 提出Implicit Error Counting (IEC)方法：不是检查响应在哪些方面正确，而是枚举其在任务相关维度上的错误，应用严重性加权评分，并通过隐式分数发射和组校准将其转换为校准的每方面奖励。

Result: 在虚拟试穿领域验证：IEC在MDressBench上优于RaR（CEC: 5.31 vs. 5.60）；在VITON-HD和DressCode上，在8个感知指标中的6个上匹配或超越6个基线方法。

Conclusion: 当理想答案不可用时，计数错误比构建评分标准提供更强的信号，IEC方法填补了无参考设置下的奖励设计空白。
...
TL;DR: SemVID是一个无需训练的视觉token剪枝框架，专门针对视频时序定位任务，通过保留关键证据和跨帧连接性，在仅使用12.5%视觉token的情况下保持95.4%的定位精度，实现5.8倍推理加速。


<details>
  <summary>Details</summary>
Motivation: 视频时序定位需要处理长视频，使用视频语言模型成本过高。现有的视觉token剪枝方法直接应用于VTG任务会导致性能大幅下降，因为VTG依赖于边界敏感的证据和跨帧推理链。

Method: 提出SemVID框架，基于两个VTG特定剪枝原则：证据保留和连接强度。首先平衡查询相关性和帧间变化分配每帧token预算，然后选择三类token：对象token用于多样化关键证据，运动token捕捉有意义的转换并作为跨帧中继，少量上下文token保持场景连续性。

Result: 在VTG基准测试中，SemVID实现了强大的精度-效率权衡，仅使用12.5%视觉token即可保留95.4%的mIoU，提供高达5.8倍的预填充加速，在相同预算下始终优于先前方法。

Conclusion: SemVID通过识别VTG特定的剪枝原则并构建具有互补语义角色的紧凑token子集，为视频时序定位提供了高效且有效的训练自由剪枝解决方案。
...
TL;DR: OWL是一种基于视觉运动线索的感知函数，通过分析运动过程中的局部视觉扩张和刚性旋转线索，实现无需先验知识的3D场景重建和相机朝向估计。


<details>
  <summary>Details</summary>
Motivation: 解决运动过程中3D感知的复杂挑战，特别是如何在缺乏环境先验知识、运动物体信息和相机运动信息的情况下，仅通过视觉运动线索实现可靠的3D感知。

Method: 提出OWL感知函数，基于两个基本视觉运动线索：1) 注视点附近点的局部视觉扩张感知；2) 刚性物体相对于注视点的旋转感知。该方法直接从原始视觉运动图像序列中提取测量值，无需先验知识，采用最小化的像素级并行计算。

Result: 仿真实验表明，OWL能够实现3D物体的几何恒常性，仅凭视觉运动线索就能实现缩放3D场景重建。该方法能够获得相机与任何可见3D点之间的相对瞬时方向范围和方向平移关系，而无需显式测量或先验知识。

Conclusion: OWL为机器感知提供了新的视角，弥合了理论概念与机器人、自主导航实际应用之间的差距，可能作为下一代自主系统的构建模块，对自然感知、行为心理学和神经功能理解也有潜在贡献。
...
TL;DR: MultiHaystack是首个评估大规模跨模态检索与推理的基准，包含46,000+多模态检索候选和747个可验证问题，揭示当前MLLMs在异构多模态检索中的瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估单模态理解，检索限于小规模、单模态候选集，简化了搜索空间并高估了端到端可靠性，无法反映真实世界中从大规模异构多模态语料库中检索证据的需求。

Method: 构建MultiHaystack基准，包含超过46,000个跨文档、图像和视频的多模态检索候选，以及747个开放但可验证的问题。每个问题都基于检索池中唯一的验证证据项，需要跨模态的证据定位和细粒度推理。

Result: 模型在提供对应证据时表现良好，但从完整语料库中检索证据时性能急剧下降。最强检索器E5-V的Recall@1仅为40.8%，GPT-5等最先进MLLMs的推理准确率从提供证据时的80.86%降至top-5检索下的51.4%。

Conclusion: 异构多模态检索是MLLMs的主要瓶颈，MultiHaystack作为有价值的测试平台，揭示了小规模评估掩盖的底层限制，促进多模态系统中检索中心化的进展。
...
TL;DR: 本文提出了Audiovisual Geolocation框架，通过可解释的感知和推理解决地理定位问题，结合视觉和听觉线索实现高精度全球定位。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在基于图像的定位方面有所改进，但精确的全球地理定位仍然面临挑战，主要因为视觉景观的固有模糊性和听觉线索的未充分利用。

Method: 提出三阶段框架：1) 感知阶段使用混合自回归稀疏自编码器将噪声音频分解为语义基础的"声学原子"；2) 多模态推理阶段使用通过组相对策略优化微调的MLLM合成这些原子与视觉特征；3) 精度预测阶段在S^2流形上使用黎曼流匹配。

Result: 实验表明该框架显著优于单模态基线，可解释的声景感知提供了关键的正交信号，结合多模态推理可实现高精度全球定位。

Conclusion: 听觉线索为地理定位提供了重要补充信息，结合视觉特征的多模态方法能够显著提高全球定位的精度和可靠性。
...
TL;DR: 提出自动将ImageNet训练集转换为多标签数据集的方法，无需人工标注，通过自监督视觉Transformer进行无监督物体发现，生成高质量多标签标注，提升模型性能


<details>
  <summary>Details</summary>
Motivation: 原始ImageNet基准采用单标签假设，但许多图像包含多个物体，导致标签噪声并限制学习信号的丰富性。多标签标注能更准确地反映现实世界视觉场景，让模型学习更丰富、更鲁棒的表征

Method: 使用自监督视觉Transformer进行无监督物体发现，选择与原始标签对齐的区域训练轻量级分类器，然后应用于所有区域生成跨数据集的连贯多标签标注

Result: 生成的多标签标注在定性评估中与人类判断高度一致。相比传统单标签方案，使用多标签监督训练的模型在架构间获得更好的域内准确率（ReaL上最高+2.0 top-1准确率，ImageNet-V2上+1.5），并在下游任务上表现出更强的可迁移性（COCO上最高+4.2 mAP，VOC上+2.3 mAP）

Conclusion: 准确的多标签标注对于提升分类性能和表征学习都至关重要。提出的自动化流程成功将ImageNet训练集转换为高质量多标签数据集，无需人工标注，为视觉模型训练提供了更丰富的监督信号
...
Conclusion: 该方法能够自动创建手术时间线和叙述，减少外科医生手动视频审查和标注的需求，为工具辅助手术提供更高效的时间线生成方案
...
TL;DR: Icarus是一个全天气天空模型，能够学习全动态范围物理捕获户外图像，实现用户可控的太阳和云层定位，提供前所未有的准确性和真实感。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习天空模型在高分辨率下无法准确建模14EV+类别不平衡的太阳区域，导致视觉质量差、光照传输、阴影和色调失真。需要更准确、真实的全天气天空模型来替代物理捕获和参数化天空模型。

Method: 提出Icarus模型，能够学习全动态范围物理捕获户外图像的曝光范围，支持用户条件生成环境贴图，允许直观定位太阳和云层，并扩展当前最先进技术以实现用户控制的大气形成纹理。

Result: Icarus可与全动态范围物理捕获户外图像或参数化天空模型互换，在基于图像的光照中提供前所未有的准确性、真实感、光照方向性（阴影）和色调。

Conclusion: Icarus是一个创新的全天气天空模型，解决了现有深度学习天空模型在太阳区域建模的局限性，为户外场景渲染提供了高质量、用户可控的解决方案。
...
TL;DR: LayerBind：一种无需训练的即插即用方法，通过分层建模和绑定实现文本到图像生成中的精确区域布局和遮挡控制


<details>
  <summary>Details</summary>
Motivation: 现有区域控制方法存在数据偏见、图像质量下降和遮挡顺序控制困难的问题。研究发现空间布局和遮挡关系在去噪早期阶段就已确定，因此重新排列早期潜在结构足以修改最终输出

Method: 采用两阶段方案：1）实例初始化阶段，利用多模态联合注意力中的上下文共享机制，创建每个实例的分支，在早期步骤中按层顺序融合形成具有预设布局的统一潜在表示；2）语义护理阶段，通过分层注意力增强机制强化区域细节并保持遮挡顺序

Result: LayerBind在定性和定量评估中均表现出色，能够实现精确的区域控制和遮挡顺序管理，同时支持可编辑工作流程，如更改实例或重新排列可见顺序
...
TL;DR: BM25-V：基于视觉词稀疏自编码和BM25评分的高效图像检索方法，通过稀疏索引实现高召回率，可作为密集重排的前置检索器。


<details>
  <summary>Details</summary>
Motivation: 密集图像检索虽然准确，但可解释性差、归因困难，且在大规模场景下计算成本高。需要一种高效、可解释且能保持高召回率的检索方法。

Method: 使用稀疏自编码器（SAE）从Vision Transformer的patch特征中提取稀疏视觉词激活，然后应用Okapi BM25评分。利用BM25的逆文档频率（IDF）权重抑制常见低信息词，强调罕见判别性词。通过稀疏倒排索引进行高效检索。

Result: 在七个基准测试中，BM25-V的Recall@200 ≥ 0.993，仅需对每个查询重排K=200个候选就能恢复接近密集检索的准确率（平均差距0.2%）。SAE在ImageNet-1K上训练后能零样本迁移到七个细粒度基准，检索决策可归因于特定视觉词及其IDF贡献。

Conclusion: BM25-V提供了一种高效、可解释的图像检索方法，可作为密集重排的高召回率前置检索器，在保持准确性的同时显著降低计算成本，并支持检索决策的可归因分析。
...
TL;DR: EventGeM：一种用于事件相机视觉地点识别的全局到局部特征融合管道，通过ViT提取全局特征进行初始匹配，再用MaxViT检测局部特征进行重排序，最后用深度估计模型进行结构相似性比较，在多个数据集上达到SOTA性能且能实时运行。


<details>
  <summary>Details</summary>
Motivation: 事件相机因其稀疏激活和高时间分辨率在机器人导航和定位任务中日益流行，但需要在小而频繁的时间尺度上进行精确定位，同时考虑能耗问题。现有事件相机的地点识别方法仍有改进空间。

Method: 1. 使用预训练的ViT-S/16从事件直方图图像中提取全局特征进行初始匹配；2. 使用预训练的MaxViT检测局部特征关键点，基于2D单应性变换和RANSAC进行重排序；3. 使用预训练的视觉基础模型进行深度估计，比较参考图像和查询图像之间的结构相似性进行进一步重排序优化。

Result: 在多个基准数据集和不同光照条件下，EventGeM相比当前最佳的事件相机地点识别方法达到了最先进的定位性能，同时能够在各种计算架构上实时运行。在真实机器人平台上使用事件流进行在线定位的部署验证了其实际应用能力。

Conclusion: EventGeM通过全局到局部特征融合管道，结合深度估计的结构相似性比较，为事件相机视觉地点识别提供了高性能的解决方案，在保持实时性的同时显著提升了定位精度。
...
Conclusion: LIPAR框架有效弥合了传统压缩算法与现代生成管道之间的差距，通过利用时间冗余性显著提升视频生成效率，同时保持生成质量。
...
TL;DR: Cog2Gen3D是一个3D认知引导的扩散框架，通过语义和几何表示的融合来解决3D生成中的空间几何约束问题，显著提升了生成结果的语义保真度和几何合理性。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型在2D图像生成上成功，但在3D生成中面临空间几何约束缺失的挑战。现有方法通常使用几何特征作为条件，但只能建模相对关系，容易出现绝对几何尺度不一致的问题。作者认为语义信息和绝对几何信息共同赋能3D认知，从而实现对物理世界的可控3D生成。

Method: 提出了Cog2Gen3D框架，包含三个关键设计：1) 认知特征嵌入：将不同模态编码为语义和几何表示，并进一步提取逻辑表示；2) 3D潜在认知图：将不同表示结构化为双流语义-几何图，并通过基于公共的交叉注意力融合它们；3) 认知引导的潜在扩散：利用融合的3D认知图作为条件，引导潜在扩散过程进行3D高斯生成。

Result: 在基于Marble World Labs构建的验证子集上进行的大量实验表明，Cog2Gen3D在语义保真度和几何合理性方面显著优于现有方法。3D认知图确保了3D生成的物理合理性和结构合理性。

Conclusion: Cog2Gen3D通过融合语义和几何表示的3D认知图，成功解决了3D生成中的空间几何约束问题，实现了对物理世界的可控3D生成，在语义保真度和几何合理性方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 视频稳定面临几何鲁棒性与全帧一致性的根本权衡：2D方法因激进裁剪而受限，3D技术则因脆弱优化流程在极端运动下失效。需要一种能在所有场景下可靠工作的方法。

Method: VS3R框架协同前馈3D重建与生成式视频扩散，联合估计相机参数、深度和掩码以确保全场景可靠性；引入混合稳定渲染模块融合语义和几何线索实现动态一致性；使用双流视频扩散模型通过结构引导与语义锚点协同恢复遮挡区域并修正伪影。
...
TL;DR: 该研究构建了TumorCoT数据集和TumorChain框架，通过多模态链式推理实现从CT影像到病理预测的临床肿瘤分析，提高了诊断准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 临床肿瘤分析需要从影像发现到病理结论的逐步推理，链式思维（CoT）能够提高可追溯性并减少诊断错误，但目前缺乏大规模的多模态推理基准和框架。

Method: 构建了包含150万CoT标注的TumorCoT数据集，并提出TumorChain多模态交错推理框架，结合3D影像编码器、临床文本理解和器官级视觉语言对齐，通过跨模态对齐和迭代因果推理实现病理预测。

Result: 在病灶检测、印象生成和病理分类任务上均优于基线方法，在DeepTumorVQA基准上表现出强泛化能力，提高了可追溯性并减少了幻觉风险。

Conclusion: 多模态链式推理框架为临床肿瘤分析提供了可靠且可解释的解决方案，有望在临床实践中提升肿瘤诊断的准确性和可追溯性。
...
TL;DR: PatchCue是一种基于图像块的视觉提示范式，通过将图像分割成块状区域来增强视觉语言模型的推理能力，相比像素级和点级提示更有效且符合人类感知习惯。
...
Method: 提出PatchCue，一种基于图像块的视觉提示范式：1）将图像分割成块状区域；2）采用两阶段训练方法：冷启动监督微调输出块级提示，然后通过过程监督提示奖励的强化学习指导中间视觉推理步骤。

Result: 在多个视觉语言模型和多样化基准测试（包括通用视觉问答、复杂推理和文档理解）上的广泛实验表明，PatchCue能持续提升整体模型性能。块级提示优于像素级边界框和点级提示。

Conclusion: PatchCue提供了一种更有效且认知对齐的视觉推理范式，通过块级视觉提示显著增强了视觉语言模型的视觉推理能力，与现代视觉语言模型的块标记化输入更好地对齐。
...
TL;DR: 该论文系统评估了合成新视角在视频地点识别中的应用，发现小规模合成视角能提升识别效果，而视角变化幅度不如添加视角数量和数据集图像类型重要。


<details>
  <summary>Details</summary>
Motivation: 合成新视角的生成对机器人导航有积极影响：地面机器人场景可生成空中视角引导无人机，空中视角可生成地面视角帮助无人机识别地面机器人看到的地点。这有助于跨平台地点识别。

Method: 使用5个公开VPR图像数据库和7种典型图像相似度方法，系统评估合成新视角在VPR中的效果。研究不同规模合成视角添加对识别性能的影响。

Result: 对于小规模合成视角添加，新视角能改善VPR识别统计结果。对于大规模添加，视角变化幅度的重要性不如添加视角数量和数据集图像类型重要。

Conclusion: 合成新视角在VPR中具有应用价值，特别是小规模添加时能提升识别性能。实际应用中应更关注视角数量和数据集特性，而非单纯追求视角变化幅度。
...
TL;DR: CylinderSplat：一种用于全景3D高斯溅射的前馈框架，通过圆柱形Triplane表示解决稀疏视图场景中的遮挡问题，在单视图和多视图全景新视角合成中达到最先进效果。


<details>
  <summary>Details</summary>
Motivation: 现有的前馈3D高斯溅射方法在全景图像应用中面临挑战，多视图成本体积在稀疏视图场景中难以解决遮挡问题，而标准的笛卡尔Triplane表示无法有效捕捉360°场景的固有几何结构，导致失真和混叠。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖隐式有向距离场和事后布局优化，无法直接生成连贯且可直接用于艺术创作的网格。PixARMesh旨在解决这一问题，通过统一模型实现从单张图像到完整3D场景网格的直接重建。
...
Conclusion: PixARMesh通过统一的自回归模型成功实现了从单张RGB图像到完整3D室内场景网格的直接重建，克服了现有方法的局限性，为下游应用提供了可直接使用的艺术级网格。
...
TL;DR: InnoAds-Composer：单阶段电商海报生成框架，通过重要性分析和路由机制实现主题、文字、风格三条件高效控制，解决传统多阶段方法存在的主题保真度差、文字不准确、风格不一致问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的产品海报生成方法通常依赖多阶段流程，难以同时控制主题、文字和风格，且存在主题保真度差、文字不准确、风格不一致三大问题，需要更高效的解决方案。

Method: 提出单阶段框架InnoAds-Composer，采用三条件控制令牌（主题、字形、风格）；通过层和时间步重要性分析，将每个条件仅路由到最敏感位置，缩短有效令牌序列；设计文本特征增强模块（TFEM），结合字形图像和裁剪特征提升中文文本渲染准确性；构建首个同时包含主题、文字、风格条件的高质量电商产品海报数据集和基准。

Result: 大量实验表明，InnoAds-Composer在显著优于现有产品海报生成方法的同时，没有明显增加推理延迟，有效解决了主题保真度、文字准确性和风格一致性问题。

Conclusion: InnoAds-Composer通过创新的三条件控制机制和重要性路由策略，实现了高效的单阶段电商海报生成，为多条件可控图像合成提供了新的解决方案，并通过首个综合数据集为后续研究建立了基准。
...
Conclusion: CollabOD框架有效解决了无人机图像中小目标检测的关键问题，通过协作设计平衡了检测精度和计算效率，代码已开源
...
TL;DR: 该论文提出Art3D框架，将2D转3D从几何重建范式转向艺术化视差合成，旨在复制专业3D电影的艺术效果而非追求物理精度。


<details>
  <summary>Details</summary>
Motivation: 现有2D转3D方法虽然几何准确但艺术性不足，无法复制专业3D电影的沉浸感和情感共鸣体验。传统方法将艺术意图（如零平面偏移、局部深度塑造）误认为是数据噪声或歧义。

Method: 提出Art3D框架，采用双路径架构解耦全局深度参数（宏观意图）和局部艺术效果（视觉笔触），通过间接监督从专业3D电影数据中学习艺术化视差合成。

Result: 实验表明Art3D在复制关键局部出屏效果和与电影3D内容的全局深度风格对齐方面展现出潜力，为艺术驱动转换工具奠定了基础。

Conclusion: 需要从物理准确视差估计转向艺术一致视差合成的新范式，Art3D框架为此提供了初步探索，为艺术化2D转3D工具开发奠定了基础。
...
TL;DR: Pano3DComposer：一种高效的前馈框架，通过Object-World Transformation Predictor将图像到3D模型生成的局部坐标对象转换为世界坐标，实现全景图像到3D场景的快速生成


<details>
  <summary>Details</summary>
Motivation: 当前组合式图像到3D场景生成方法存在迭代布局优化耗时、联合对象-布局生成不灵活、依赖有限视场角图像等问题，无法高效生成完整的360度环境

Method: 1. 提出Object-World Transformation Predictor模块，将现成图像到3D模型生成的对象从局部坐标转换为世界坐标；2. 基于VGGT架构改进为Alignment-VGGT，使用目标对象裁剪、多视角对象渲染和相机参数预测变换；3. 采用伪几何监督训练解决生成对象与真实对象之间的形状差异；4. 引入Coarse-to-Fine对齐机制，通过场景渲染反馈迭代优化几何一致性

Result: 在合成和真实世界数据集上，该方法在图像/文本到3D任务中实现了优越的几何精度，在RTX 4090 GPU上约20秒即可生成高保真3D场景

Conclusion: Pano3DComposer通过解耦对象生成和布局估计，提供了一种高效的全景图像到3D场景生成方案，解决了现有方法的局限性
...
TL;DR: 该论文提出了CORE-Seg框架，通过语义引导提示适配器将推理与分割结合，用于复杂病灶分割，在ComLesion-14K基准上取得了37.06%的平均Dice分数，比次优基线提升14.89%。
...
Result: 在ComLesion-14K基准上取得了37.06%的平均Dice分数，比次优基线提升14.89%，同时将失败率降低到18.42%，达到了最先进的性能。

Conclusion: 该研究通过将推理能力整合到医学图像分割中，成功解决了复杂病灶分割的挑战，为医学图像分析提供了更智能、可解释的解决方案。
...
TL;DR: BlackMirror是一个黑盒文本到图像模型后门检测框架，通过分析生成图像中部分语义模式的稳定性来识别后门攻击，无需训练即可部署。


<details>
  <summary>Details</summary>
Motivation: 现有基于图像相似度的后门检测方法假设后门触发生成具有强一致性，但难以应对新兴的视觉多样性后门攻击。研究发现后门攻击中只有部分语义模式被稳定操纵，其余内容保持多样或良性。

Method: BlackMirror包含两个组件：MirrorMatch将视觉模式与对应指令对齐以检测语义偏差；MirrorVerify评估这些偏差在不同提示下的稳定性，以区分真正后门行为与良性响应。

Result: 综合实验表明，BlackMirror能够准确检测各种攻击，是一个通用的、无需训练的框架，可作为即插即用模块部署在模型即服务应用中。

Conclusion: BlackMirror通过分析部分语义模式的稳定性，有效解决了黑盒文本到图像模型后门检测的挑战，特别是针对视觉多样性的新型后门攻击。
...
<details>
  <summary>Details</summary>
Motivation: 传统VAE存在重建与生成之间的差距问题，且计算成本较高。作者希望开发一种既能保持高质量重建，又能提升生成质量，同时降低参数和计算成本的替代方案。

Method: RAC采用Rectified Flow思想，通过流时间步进行多步解码，解码路径是直线且可校正的。模型支持双向推理，解码器通过时间反转可作为编码器使用（因此称为Coder而非编码器或解码器），减少了41%的参数。生成式解码方法允许模型沿路径校正潜在变量。

Result: 实验表明，RAC在重建和生成质量上都超越了最先进的VAE模型，同时计算成本降低了约70%。

Conclusion: RAC是一种高效的自编码器替代方案，通过Rectified Flow的多步解码和双向推理机制，在提升性能的同时显著降低了计算成本，部分解决了VAE中重建与生成之间的差距问题。
...
TL;DR: 本文提出了RAID数据集，这是一个专门用于研究驾驶员风险感知和情境风险评估的大规模数据集，包含4,691个标注视频片段，并提出了一个弱监督风险对象识别框架。


<details>
  <summary>Details</summary>
Motivation: 实现零碰撞移动是智能车辆系统的关键目标，这需要理解驾驶员的风险感知——一个由驾驶员对外部刺激的自愿反应和周围道路使用者对自车的注意力形成的复杂认知过程。目前缺乏专门用于研究驾驶员风险感知和情境风险评估的大规模数据集。

Method: 1. 引入RAID数据集：包含4,691个标注视频片段，涵盖多样化交通场景，标注包括驾驶员意图操作、道路拓扑、风险情况、驾驶员反应和行人注意力等。
2. 提出弱监督风险对象识别框架：通过建模驾驶员意图操作与反应之间的关系来识别潜在风险源。
3. 分析行人注意力在风险评估中的作用。

Result: 实验评估表明，该方法在RAID和HDDS数据集上分别比现有最先进方法性能提升了20.6%和23.1%。

Conclusion: RAID数据集为驾驶员风险感知研究提供了有价值的资源，提出的弱监督风险对象识别框架有效提升了风险识别性能，行人注意力分析进一步增强了风险评估能力。
...
TL;DR: TAR-ViTPose：一种用于视频2D人体姿态估计的时序聚合与恢复视觉Transformer，通过聚合跨帧的时序线索提升静态ViT表示，实现更鲁棒准确的姿态估计。


<details>
  <summary>Details</summary>
Motivation: 现有基于ViT的姿态估计器为静态图像设计，独立处理每帧，忽略了视频序列中的时序连贯性，导致在运动模糊、遮挡或失焦等挑战性场景中预测不稳定。

Method: 提出TAR-ViTPose，包含两个核心模块：1）关节中心时序聚合（JTA），为每个关节分配可学习的查询token，选择性地关注相邻帧中对应区域；2）全局恢复注意力（GRA），将聚合的时序特征恢复回当前帧的token序列，丰富姿态表示同时保持全局上下文。

Result: 在PoseTrack2017基准测试上相比单帧基线ViTPose提升+2.3 mAP，优于现有最先进的视频方法，同时在真实应用中实现更高的运行时帧率。

Conclusion: TAR-ViTPose通过有效的时序聚合和恢复机制，显著提升了视频2D人体姿态估计的性能和鲁棒性，同时保持了高效的推理速度。
...
<details>
  <summary>Details</summary>
Motivation: 现有NeRF和3DGS方法虽然渲染质量好，但依赖耗时的逐场景优化，限制了实时部署。新兴的前馈高斯泼溅方法效率高，但缺乏仿真所需的显式流形几何结构。
...
TL;DR: OD-RASE框架通过本体论驱动的方法检测导致交通事故的道路结构，并生成基础设施改进方案，旨在提升自动驾驶系统的安全性。
...
Method: 提出OD-RASE框架：1)基于道路交通系统专业领域知识形式化本体论；2)使用大规模视觉语言模型生成基础设施改进方案；3)通过本体论驱动数据过滤增强可靠性；4)自动标注改进方案到事故前道路图像构建新数据集；5)引入基线方法结合LVLM和扩散模型生成改进方案和改进后道路环境图像。

Result: 实验表明，本体论驱动数据过滤能够高精度预测导致事故的道路结构和相应的改进计划，构建了新的数据集。
...
### [37] [Adaptive Radial Projection on Fourier Magnitude Spectrum for Document Image Skew Estimation](https://arxiv.org/abs/2603.05942)
...
<details>
  <summary>Details</summary>
Motivation: 文档倾斜估计是文档处理系统的关键任务，直接影响后续处理步骤。随着数字化时代的发展，需要更准确可靠的倾斜估计方法。

Method: 提出新颖的倾斜估计方法：在2D离散傅里叶幅度谱上应用自适应径向投影，提取文档图像的主导倾斜角度。同时引入高质量评估数据集DISE-2021。

Result: 实验结果表明，所提方法具有鲁棒性和可靠性，在所有对比方法中表现最优。

Conclusion: 该方法在文档倾斜估计任务中表现出色，提供了傅里叶方法的全面改进分析，源代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 生成式真实世界图像超分辨率存在语义和结构幻觉问题，即输出看起来清晰但与低分辨率输入证据不忠实。基于偏好的强化学习虽然适合，但面临三个主要障碍：缺乏退化鲁棒的LR参考忠实度信号、rollout组优化瓶颈导致优势崩溃，以及真实退化数据覆盖有限。

Method: 提出LucidNFT框架：1) LucidConsistency - 退化鲁棒的语义评估器，使LR锚定的忠实度可测量和优化；2) 解耦优势归一化策略，在融合前保留每个LR条件rollout组内的目标对比，防止优势崩溃；3) LucidLR - 大规模真实世界退化图像集合，支持鲁棒的RL微调。

Result: 实验表明，LucidNFT能持续改进基于流的真实世界超分辨率基线，在多样真实世界场景中实现更好的感知-忠实度权衡，并保持稳定的优化动态。
...
TL;DR: VINE是一个统一的少样本分割框架，通过联合建模结构一致性和前景判别性来优化类别特定原型，特别针对视角变化和复杂结构场景。


<details>
  <summary>Details</summary>
Motivation: 少样本分割在有限监督下泛化到新类别时面临结构错位和跨视角不一致的挑战，特别是在外观或视角变化较大的情况下。

Method: 提出VINE框架：1) 在骨干特征上构建空间-视角图，空间图捕捉局部几何拓扑，视角图连接不同视角特征传播视角不变的结构语义；2) 从支持-查询特征差异中推导判别性先验，捕获类别特定对比度；3) 通过掩码交叉注意力逐步集成前景增强的SAM特征和结构丰富的ResNet特征，生成类别一致原型作为SAM解码器的自适应提示。

Result: 在多个少样本分割基准测试上的广泛实验验证了VINE的有效性和鲁棒性，特别是在具有视角偏移和复杂结构的挑战性场景下。

Conclusion: VINE通过联合建模结构一致性和前景判别性，能够有效处理少样本分割中的结构错位和跨视角不一致问题，在挑战性场景下表现出色。
...
### [40] [OVGGT: O(1) Constant-Cost Streaming Visual Geometry Transformer](https://arxiv.org/abs/2603.05959)
...
<details>
  <summary>Details</summary>
Motivation: 现有几何基础模型使用全对全注意力机制，虽然重建质量好但计算成本呈二次方增长，只能处理短序列；因果注意力变体如StreamVGGT虽然支持单次流式处理，但KV缓存会不断增长，在数百帧内就会耗尽GPU内存，无法实现真正的长序列流式推理

Method: 提出OVGGT框架，结合两种技术：1) 自选择缓存：利用FFN残差幅度压缩KV缓存，同时保持与FlashAttention完全兼容；2) 动态锚点保护：保护坐标关键令牌不被驱逐，抑制长轨迹中的几何漂移

Result: 在室内、室外和超长序列基准测试中，OVGGT能够在固定VRAM限制内处理任意长度的视频，同时实现最先进的3D几何精度

Conclusion: OVGGT通过训练免费的方法解决了长序列视频3D重建中的内存和计算瓶颈，实现了真正的流式推理能力
...
### [41] [Exploring Open-Vocabulary Object Recognition in Images using CLIP](https://arxiv.org/abs/2603.05962)
*Wei Yu Chen,Ying Dai*

Main category: cs.CV

TL;DR: 提出基于两阶段策略的开放词汇目标识别框架：先分割后识别，无需复杂重训练和标注，使用CLIP生成嵌入并通过SVD构建共享表示空间，在多个数据集上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标识别方法存在系统复杂度高、训练成本大、泛化能力有限等问题，需要更简洁有效的解决方案

Method: 采用两阶段策略：目标分割+识别。裁剪目标区域后，使用CLIP生成图像和文本嵌入，引入CNN/MLP方法提取特征并对齐视觉特征与文本嵌入，通过SVD构建共享表示空间，最后通过嵌入相似度匹配进行识别

Result: 在COCO、Pascal VOC和ADE20K数据集上的实验表明，无需训练的CLIP编码（无SVD）实现了最高的平均AP，超越了当前SOTA方法，同时CNN/MLP图像编码显示出潜力

Conclusion: 提出的框架有效解决了现有方法的局限性，无需复杂训练和标注，在多个数据集上表现出色，为开放词汇目标识别提供了简洁高效的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有预训练视觉模型难以直接应用于3D骨架数据，因为数据格式差异大；同时大规模骨架数据集稀缺，且需要在不增加额外模型分支的情况下融入多模态动作识别。

Method: 提出Skeleton-to-Image Encoding (S2I)表示方法：基于身体部位语义对关节进行分区排列，将骨架序列转换为标准尺寸的图像格式，从而能够利用预训练视觉模型进行自监督骨架表示学习。

Result: 在NTU-60、NTU-120和PKU-MMD数据集上的实验表明，该方法在自监督骨架表示学习方面有效且泛化能力强，特别是在跨格式评估的挑战性设置下表现优异。

Conclusion: S2I方法首次实现了将预训练视觉模型知识有效迁移到骨架分析中，为异构骨架数据提供了统一的图像格式表示，解决了骨架数据格式差异和数据集稀缺的核心问题。
...
### [43] [CR-QAT: Curriculum Relational Quantization-Aware Training for Open-Vocabulary Object Detection](https://arxiv.org/abs/2603.05964)
...
<details>
  <summary>Details</summary>
Motivation: 开放词汇目标检测模型通常体积庞大，难以部署在资源受限设备上。虽然量化可以提供压缩，但极端低比特（如4位）量化会严重破坏细粒度视觉-语言对齐和区域间关系结构。

Method: 提出CR-QAT框架：1) 课程QAT（CQAT）将模型分区进行渐进量化，通过错误隔离确保稳定优化；2) 文本中心关系KD（TRKD）构建文本锚定的成对相似度矩阵，全面传递教师模型的多维关系知识。

Result: 在LVIS和COCO零样本基准测试中，CR-QAT在激进低比特设置下始终优于现有QAT基线，相对AP提升分别达到38.9%和40.9%。

Conclusion: CR-QAT框架通过结合渐进量化和关系知识蒸馏，有效缓解了开放词汇目标检测在极端量化下的性能退化，为资源受限设备上的部署提供了实用解决方案。
...
TL;DR: ProCap：将变化描述从静态图像对比转变为动态过程建模的两阶段框架，通过过程编码器学习变化过程动态，使用可学习过程查询推断潜在过程表示，在三个数据集上验证有效性。
...
Method: 两阶段框架：第一阶段训练过程编码器从关键帧学习变化过程，通过自动生成中间帧使隐含过程动态显式化，然后采样关键帧减少冗余，通过标题条件掩码重建任务学习潜在动态；第二阶段将训练好的编码器集成到编码器-解码器模型中，引入可学习过程查询来推断潜在过程表示，避免依赖显式帧带来的计算开销和视觉噪声敏感性，整个模型通过标题损失端到端训练。

Result: 在三个数据集上的实验证明了ProCap的有效性。

Conclusion: ProCap通过将变化建模从静态图像比较重新定义为动态过程建模，能够更好地理解和描述变化过程，代码和预训练模型已开源。
...
TL;DR: DynUAV是一个新的无人机视角多目标跟踪基准，专注于动态场景下的挑战，包含剧烈自运动和复杂表观轨迹，包含42个视频序列和170万+标注框


<details>
  <summary>Details</summary>
Motivation: 现有无人机视角MOT基准缺乏真实动态场景的复杂性，主要包含可预测的相机动态和线性运动模式，无法反映无人机快速机动带来的观测挑战

Method: 构建DynUAV基准，包含42个视频序列，超过170万个边界框标注，涵盖车辆、行人以及挖掘机、推土机、起重机等工业类别，特别强调剧烈自运动带来的挑战

Result: 对最先进跟踪器在DynUAV上的综合评估显示它们在动态条件下的检测和关联方面存在局限性，特别是在处理尺度变化、视角变化和运动模糊方面表现不足

Conclusion: DynUAV建立了一个严格的基准，预计将作为具有挑战性的测试平台推动真实世界无人机视角MOT的进展，所有资源将公开提供
...
TL;DR: 该报告探讨了巴基斯坦制造的手术器械及其制造缺陷，通过深度学习架构（YOLOv8、ResNet-152、EfficientNet-b4）进行自动化缺陷检测，旨在提升医疗器械质量标准。


<details>
  <summary>Details</summary>
Motivation: 在医疗保健领域，手术器械的质量直接关系到手术安全和患者生命。即使是微小的制造缺陷也可能导致致命后果，因此需要精确的制造和质量控制。报告旨在通过自动化缺陷检测技术提升巴基斯坦制造的手术器械质量标准。

Method: 1. 与巴基斯坦Sialkot手术器械集群的行业领导者Daddy D Pro和Dr. Frigz International合作
2. 收集了4,414张高分辨率图像构建新数据集
3. 采用深度学习架构包括YOLOv8、ResNet-152和EfficientNet-b4进行自动化缺陷检测
4. 通过自动光学检测（AOI）工具进行质量保证

Result: 报告识别了巴基斯坦制造的各种手术器械及其相关缺陷，包括裂纹、锈蚀和结构不规则等关键缺陷。通过深度学习技术实现了对制造缺陷的自动化检测和分析。

Conclusion: 通过整合行业专业知识和尖端技术，特别是深度学习架构，能够有效提升手术器械制造的质量标准。这种自动化缺陷检测方法不仅为制造商、医疗专业人员和监管机构提供了宝贵资源，还能确保更可靠的医疗环境，最终保障患者安全。
...
TL;DR: MM-ISTS是一个基于视觉-文本大语言模型的多模态框架，用于不规则采样时间序列预测，通过融合时间、视觉和文本模态来提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 现有不规则采样时间序列预测方法仅利用历史观测数据进行预测，缺乏对上下文语义和细粒度时间模式的学习能力，需要更全面的多模态解决方案。
...
Conclusion: MM-ISTS通过融合时间、视觉和文本模态，成功解决了不规则采样时间序列预测中的上下文语义学习和细粒度模式识别问题，为IST预测提供了有效的多模态框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度基础模型在透视图像上训练表现良好，但在360°图像上泛化能力差，因为透视和全景域之间存在显著的几何差异。完全微调这些模型通常需要大量全景数据。
...
Conclusion: 通过将重点从互补投影融合转向在保持预训练透视先验下的全景域适应，RePer-360实现了高效的全景深度估计，显著减少了数据需求并提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前高质量视觉特效(VFX)制作需要专业知识和昂贵生产流程，现有AIGC系统面临特效数据稀缺、超自然或风格化效果建模困难等挑战，且通常需要逐特效微调，限制了其扩展性和对新特效的泛化能力。

Method: EffectMaker采用统一推理-生成框架：1) 多模态大语言模型解释高级特效语义并推理如何适应目标主体；2) 扩散变换器通过上下文学习从参考视频中捕捉细粒度视觉线索；3) 语义-视觉双路径引导机制实现准确可控的特效一致合成；4) 构建包含130k视频、3k特效类别的EffectData数据集。

Result: 实验表明EffectMaker在视觉质量和特效一致性方面优于现有最先进基线方法，提供了一个可扩展且灵活的自定义VFX生成范式。

Conclusion: EffectMaker通过语义-视觉双路径引导机制，无需逐特效微调即可实现准确、可控、特效一致的自定义视觉特效生成，解决了现有AIGC系统在VFX生成中的扩展性和泛化性限制问题。
...
TL;DR: MOSIV框架通过可微分模拟器从视频中优化连续、逐对象的材料参数，解决了多对象系统识别问题，显著提升了接地精度和长期模拟保真度。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对单对象场景或固定材料原型的离散分类，无法有效处理多对象系统识别问题，需要新的框架来优化连续、逐对象的材料参数。

Method: 提出MOSIV框架，使用可微分模拟器直接优化连续、逐对象的材料参数，通过视频导出的几何目标进行指导，并创建了包含接触丰富的多对象交互的合成基准数据集。

Result: 在新建的基准测试中，MOSIV相比适应基线显著提高了接地精度和长期模拟保真度，成为该新任务的强基线方法。

Conclusion: 对象级细粒度监督和几何对齐目标对于复杂多对象设置中的稳定优化至关重要，MOSIV为多对象系统识别提供了有效解决方案。
...
TL;DR: StruVis框架通过结构化视觉表示增强文本到图像生成，避免中间图像生成的计算成本，同时保持视觉感知能力


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像生成推理框架存在缺陷：纯文本推理缺乏视觉上下文，容易忽略空间和视觉元素；文本-图像交错推理虽然增强视觉基础但计算成本高，且受限于生成器的表示能力
...
Conclusion: StruVis作为生成器无关的推理框架，能够无缝集成到多种文本到图像生成器中，高效提升其在推理型文本到图像生成任务中的性能
...
Method: 提出OA-SORT框架，包含三个核心组件：遮挡感知模块（OAM）使用高斯地图分析物体遮挡状态并减少背景影响；遮挡感知偏移（OAO）和偏置感知动量（BAM）利用遮挡状态信息来缓解成本混淆并抑制估计不稳定性。

Result: 在DanceTrack、SportsMOT和MOT17数据集上的综合评估验证了遮挡处理的重要性。在DanceTrack测试集上，OA-SORT达到63.1% HOTA和64.2% IDF1。将遮挡感知框架集成到其他四个跟踪器中，平均提升HOTA 2.08%和IDF1 3.05%。

Conclusion: OA-SORT框架有效解决了2D MOT中的遮挡问题，提出的遮挡感知机制具有可重用性，能够显著提升多目标跟踪性能，为遮挡处理提供了有效的解决方案。
...
TL;DR: 该研究针对高维超列在图像分割中的计算复杂度问题，提出使用分层子采样和集成学习方法，在脑肿瘤数据集上显著提升了小样本情况下的分割性能。


<details>
  <summary>Details</summary>
Motivation: 受生物视觉启发的高维超列虽然能用于像素分类，但由于计算复杂度随训练集大小线性增长，实际应用受限。本研究旨在解决这一计算挑战。

Method: 采用分层子采样处理VGG16生成的超列，并研究集成学习方法（堆叠和投票集成）在稀疏超列上的性能。

Result: 在脑肿瘤数据集上，当N≤20的极低样本情况下，简单的逻辑回归分类器最有效。在10%分层子采样率下，N=20时最佳平均Dice分数为0.66，比标准多尺度UNet基线显著提升24.53%。

Conclusion: 分层子采样和集成学习能有效处理超列的计算复杂度问题，在小样本情况下显著优于传统方法，为超列的实际应用提供了可行方案。
...
TL;DR: 该论文提出了一种数据驱动的方法来解决文本到图像模型中排版控制不足的问题，通过构建大规模排版数据集FontUse和训练模型来更好地遵循排版相关的提示词。


<details>
  <summary>Details</summary>
Motivation: 当前文本到图像模型在生成高质量图像方面表现出色，但在控制排版方面存在明显不足：模型经常忽略或弱化用户请求的排版样式。排版是视觉传达的重要元素，特别是在设计相关应用中，这种限制影响了模型的实用性和用户体验。

Method: 1. 构建专门针对排版的大规模数据集FontUse，包含约70K图像，使用分割模型和多模态大语言模型自动标注用户友好提示词、文本区域位置和OCR识别字符串；2. 提示词明确结合字体样式（如衬线体、手写体、优雅字体）和使用场景（如婚礼邀请函、咖啡店菜单）；3. 使用这些标注对现有图像生成模型进行微调，无需修改架构；4. 引入基于Long-CLIP的评估指标来衡量生成排版与请求属性的一致性。

Result: 实验表明，使用该管道训练的模型在多样化提示词和布局下，生成的文本渲染比竞争基线更符合提示词要求。评估指标验证了生成排版与请求属性之间的一致性得到显著改善。

Conclusion: 该研究通过数据中心的标注管道解决了文本到图像模型中排版控制的挑战，构建的FontUse数据集和训练方法使模型能够更一致地解释样式和使用场景条件，提高了排版生成的质量和可控性。源代码已开源。
...
Method: 提出GvU方法：1）基于token级别的内在文本-图像对齐奖励机制，让模型同时作为教师和学生，用理解分支评估自身输出并指导生成；2）设计自监督强化学习框架，让模型通过基于理解的内在奖励信号迭代提升生成质量，无需外部监督。
...
TL;DR: GenHOI是一种轻量级增强方法，通过时间平衡和空间选择的方式向预训练视频生成模型注入参考对象信息，显著提升手-物交互视频合成的物理合理性和对象一致性。


<details>
  <summary>Details</summary>
Motivation: 当前手-物交互视频合成面临两大挑战：现有HOI重演方法局限于域内场景，无法泛化到复杂野外环境；而通用视频编辑模型虽然鲁棒性更强，但在HOI特定问题上（如对象外观不一致）表现不佳。

Method: GenHOI采用两种核心技术：1) Head-Sliding RoPE - 为参考标记分配头部特定的时间偏移，均匀分布其跨帧影响，缓解3D RoPE的时间衰减问题；2) 两级空间注意力门 - 将对象条件注意力集中在HOI区域，并自适应调整强度，在保持背景真实性的同时增强交互保真度。

Result: 在未见过的野外场景上进行广泛定性和定量评估表明，GenHOI在物理合理性、对象一致性和交互保真度方面显著优于当前最先进的HOI重演方法和通用视频编辑方法。

Conclusion: GenHOI通过轻量级的时间平衡和空间选择机制，有效解决了手-物交互视频合成中的关键挑战，实现了在复杂野外场景下的高质量HOI视频生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶VLA模型存在窄策略限制问题，模仿学习阶段倾向于限制探索，导致后续强化学习阶段因反馈多样性不足而过早饱和，无法充分发挥模型潜力。

Method: 提出Curious-VLA两阶段框架：1) IL阶段使用可行轨迹扩展策略生成多个物理有效轨迹，采用逐步归一化轨迹表示适应多样化数据；2) RL阶段采用自适应多样性感知采样优先选择高多样性样本，引入带焦点风格加权的跨度驾驶奖励来扩大奖励值范围，提高对驾驶质量的敏感性。

Result: 在Navsim基准测试中，Curious-VLA取得最先进结果（PDMS 90.3，EPDMS 85.4），Best-of-N PDMS达到94.8，有效释放了VLA模型的探索潜力。

Conclusion: Curious-VLA通过解决探索-利用困境，显著提升了自动驾驶VLA模型的性能，证明了多样化数据生成和自适应采样策略在解锁模型探索潜力方面的有效性。
...
TL;DR: 研究通过分析视觉语言模型在自动驾驶场景中的中间激活，发现某些视觉概念（如物体存在）被线性编码，而空间概念（如物体方向）仅被隐式编码，并识别出感知失败和认知失败两种故障模式。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在自动驾驶应用中越来越普遍，但它们经常在简单但关键的视觉问题上失败，且失败原因尚不清楚。研究旨在通过分析模型中间激活来理解这些失败的根本原因。

Method: 创建仅针对特定视觉概念不同的反事实图像集，使用四个最先进的视觉语言模型的激活训练线性探针，评估视觉概念是否被线性编码，并分析视觉信息在模型中的流动瓶颈。

Result: 发现物体存在等概念被明确线性编码，而物体方向等空间概念仅被隐式编码；识别出两种故障模式：感知失败（视觉信息未线性编码）和认知失败（信息存在但未能与语言语义正确对齐）；物体距离增加会快速降低相应视觉概念的线性可分性。

Conclusion: 该研究提高了对视觉语言模型在自动驾驶相关简单视觉任务中失败案例的理解，揭示了视觉信息在模型中的编码方式和故障机制，为改进模型性能提供了重要见解。
...
TL;DR: TempoSyncDiff是一个参考条件潜在扩散框架，通过师生蒸馏实现高效音频驱动说话头生成，减少推理步骤并提高稳定性


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型在说话头生成中存在高推理延迟、时间不稳定（闪烁和身份漂移）以及音频-视觉对齐不完美的问题，需要在实际计算受限环境下实现实用的说话头生成

Method: 采用参考条件潜在扩散框架，使用师生蒸馏方法：用标准噪声预测目标训练的扩散教师指导轻量级学生去噪器，结合身份锚定和时间正则化减少身份漂移和闪烁，使用音素级音频条件控制唇部运动

Result: 在LRS3数据集上的实验显示，蒸馏扩散模型能保留强教师的大部分重建行为，同时实现显著降低的延迟推理，包括CPU和边缘计算测量，证明了边缘部署的可行性
...
TL;DR: 本文提出情感连续说话人脸生成（EC-TFG）新任务，旨在生成能反映情感变化的说话视频，并开发了TIE-TFG模型来管理动态情感变化。


<details>
  <summary>Details</summary>
Motivation: 现有说话人脸生成方法虽然能生成自然的面部运动，但通常只能表达固定目标情感，缺乏像人类那样展现连续变化自然表情的能力，无法真实反映情感变化。

Method: 提出TIE-TFG模型，采用时间密集型情感波动建模技术，通过生成与输入文本对应的情感变化序列来驱动合成视频中的连续面部表情变化。

Result: 广泛评估表明，该方法在产生平滑情感过渡方面表现出色，能够在不同情感状态下保持高质量的视觉效果和运动真实性。

Conclusion: EC-TFG任务和TIE-TFG模型成功解决了现有方法无法生成连续情感变化的问题，为创建更真实、情感丰富的说话人脸视频提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在图像描述和视觉问答等任务上表现优异，但往往难以准确解释视觉数据中的深度信息，这限制了三维场景理解能力。

Method: 1) 构建新颖的深度图像-文本对数据集和深度指令数据集；2) 使用GLPN模型从视觉图像生成深度图，GPT-4生成对应深度指令；3) 修改CLIP中的ViT编码器以融入局部物体信息，更好地捕捉深度连续变化；4) 基于现有深度图像数据集开发深度问答基准测试。

Result: 实验结果表明，DeepSight显著增强了深度感知能力和下游任务性能，在多模态三维理解方面取得了实质性进展。

Conclusion: DeepSight作为首个专门用于深度感知的MLLM，通过创新的数据集构建和模型架构改进，有效解决了现有模型在深度信息理解方面的不足，推动了三维场景理解技术的发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有联邦域泛化行人重识别方法依赖全局特征和简单平均聚合，难以捕捉细粒度局部特征，且忽略客户端在鲁棒特征提取能力上的差异

Method: 提出FedARKS框架，包含RK（鲁棒知识）和KS（知识选择）两个机制，通过鲁棒知识选择和集成来提升模型泛化能力

Result: 未在摘要中明确说明具体实验结果

Conclusion: FedARKS框架能够有效解决联邦域泛化行人重识别中的特征表示和聚合问题，提升模型在未见域上的泛化能力
...
### [63] [Cross-Resolution Distribution Matching for Diffusion Distillation](https://arxiv.org/abs/2603.06136)
*Feiyang Chen,Hongpeng Pan,Haonan Xu,Xinyu Duan,Yang Yang,Zhefeng Wang*

Main category: cs.CV

TL;DR: RMD是一种新的扩散蒸馏框架，通过跨分辨率分布匹配解决低分辨率生成的质量退化问题，实现高保真、少步骤的多分辨率级联推理加速。


<details>
  <summary>Details</summary>
Motivation: 现有扩散蒸馏方法受限于去噪过程，步骤减少已基本饱和；部分时间步低分辨率生成虽能加速但存在跨分辨率分布差异导致质量下降的问题。

Method: 1) 使用对数信噪比曲线为每个分辨率划分时间步区间；2) 引入基于logSNR的映射补偿分辨率引起的偏移；3) 沿分辨率轨迹进行分布匹配以减少低分辨率生成器分布与教师高分辨率分布之间的差距；4) 在上采样过程中加入预测噪声重注入机制稳定训练并提高合成质量。

Result: RMD在保持高保真生成的同时显著加速推理，在SDXL上实现33.4倍加速，在Wan2.1-14B上实现25.6倍加速，同时保持高视觉保真度。

Conclusion: RMD通过跨分辨率分布匹配有效解决了低分辨率生成的质量退化问题，为扩散模型的高效推理提供了一种新颖的蒸馏框架，在加速和保真度之间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频编辑技术主要关注视觉保真度而非物理因果关系，导致插入的对象与环境物理不一致。需要开发能够理解物理场景并进行合理交互推理的系统。

Method: 采用"思考-放置"范式：1) MLLM进行物理场景理解和交互推理，生成环境感知的思维链令牌和有效插入区域；2) 引入MLLM引导的空间直接偏好优化，通过反馈评分优化视觉自然度；3) 推理时MLLM迭代触发细化循环，形成闭环优化；4) 提供两种用户可选模式：灵活模式（允许环境修改）和标准模式（保持场景完整性）。

Result: 实验表明，Place-it-R1相比最先进解决方案和商业模型，能够实现物理一致的视频对象插入，在物理连贯性方面表现优异。

Conclusion: 该框架成功地将MLLM的认知推理能力与视频扩散生成能力相结合，通过闭环优化实现了物理一致且视觉自然的视频对象插入，为用户提供了在物理合理性和视觉保真度之间的明确控制。
...
### [65] [Spatial Colour Mixing Illusions as a Perception Stress Test for Vision-Language Models](https://arxiv.org/abs/2603.06141)
*Nicoleta-Nina Basoc,Adrian Cosma,Emilian Radoi*

Main category: cs.CV

TL;DR: 论文研究了视觉语言模型在结构化色彩失真下的系统性感知缺陷，发现即使场景对人类仍可识别，模型也会产生自信但荒谬的预测，而人类表现显著优于模型。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在基准测试中表现良好，但它们存在系统性的感知弱点：像素值的结构化大幅变化会导致模型产生自信但荒谬的预测，即使底层场景对人类来说仍然容易识别。研究者希望通过研究这种差距来理解模型的感知局限性。

Method: 使用"空间色彩混合"方法，这是一种程序化的色彩失真家族，在自然图像上叠加结构化模式（包括RGB和Ostwald色彩系统）。研究者引入了8种空间色彩混合变体，评估了9个来自3个模型家族的视觉语言模型在4个数据集上的表现。还进行了包含61名参与者的人类研究，比较人类和模型在相同失真条件下的表现。

Result: 随着失真程度的增加，所有模型和数据集上的准确率都急剧下降，扩大语言模型规模并不能可靠地缓解这种失败。人类研究显示，在相同的失真条件下，人类表现显著优于视觉语言模型。简单的受人类启发的预处理步骤能够为几种失真类型恢复相当一部分性能。

Conclusion: 视觉语言模型存在系统性的感知弱点，特别是在结构化色彩失真下。人类表现显著优于模型，表明模型与人类感知存在差距。受人类启发的预处理方法能够提高模型鲁棒性，这为开发感知感知的预处理工具提供了实用策略。
...
TL;DR: VLM-RobustBench评估视觉语言模型在49种图像失真下的鲁棒性，发现模型对空间扰动（如重采样、几何变换）比视觉上更严重的光度失真更敏感，揭示了当前VLM在语义理解强但空间鲁棒性弱的特点。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在标准高质量数据集上表现良好，但对其在真实世界图像失真下的性能理解不足。研究者希望系统评估VLM在各种图像扰动下的鲁棒性，以揭示其弱点并指导未来改进方向。
...
Result: 视觉严重程度是性能下降的弱预测指标：低严重度的空间扰动（如glass_blur）比视觉上更严重的光度损坏对性能影响更大。重采样和几何失真（如upsample、elastic_transform）导致最大性能下降（高达34个百分点）。模型在语义理解上强但在空间鲁棒性上脆弱。

Conclusion: 当前视觉语言模型具有语义理解优势但空间鲁棒性不足，需要定义新的鲁棒性评估协议和训练机制，特别强调重采样和几何不变性，以提升模型在真实世界失真图像上的性能。
...
TL;DR: 本文提出了RF-Sampling，一种专门为流匹配模型设计的免训练推理增强框架，通过理论推导证明其隐式执行文本-图像对齐分数的梯度上升，显著提升生成质量和提示对齐。


<details>
  <summary>Details</summary>
Motivation: 当前推理时增强策略主要适用于传统扩散模型，在流模型（如FLUX）上表现不佳。需要为流模型特别是CFG蒸馏变体设计专门的推理增强方法。

Method: 提出RF-Sampling框架，基于理论推导证明其隐式执行文本-图像对齐分数的梯度上升。通过文本表示的线性组合并结合流反转技术，使模型能够探索与输入提示更一致的噪声空间。

Result: 在多个基准测试上的广泛实验表明，RF-Sampling能一致提升生成质量和提示对齐。这是首个在FLUX上展现一定程度测试时缩放能力的推理增强方法。

Conclusion: RF-Sampling为流模型提供了有效的推理增强框架，填补了传统增强方法在流模型上的应用空白，具有理论依据和实际效果。
...
TL;DR: FreeOcc是一个无需训练的三维场景理解框架，利用预训练基础模型从多视角图像恢复语义和几何信息，实现道路场景的语义和全景占据预测。


<details>
  <summary>Details</summary>
Motivation: 当前基于相机的三维场景理解方法通常需要昂贵的密集三维监督或需要在目标域数据上训练模型，这限制了在未见环境中的部署。需要一种无需训练的方法来解决这些问题。

Method: FreeOcc使用预训练基础模型：1) 用可提示的基础分割模型提取每视角的全景先验；2) 用重建基础模型恢复度量三维点；3) 通过深度和置信度感知过滤将可靠标签提升到三维；4) 随时间融合并通过确定性细化堆栈进行体素化；5) 通过拟合和合并鲁棒的当前视角三维框候选来恢复实例。

Result: 在Occ3D-nuScenes数据集上，FreeOcc无需训练达到16.9 mIoU和16.5 RayIoU，与最先进的弱监督方法相当。作为伪标签生成管道训练下游模型时，达到21.1 RayIoU，超过之前的弱监督基准。在全景占据预测方面，分别达到3.1 RayPQ（无需训练）和3.9 RayPQ（弱监督）。

Conclusion: FreeOcc展示了基于基础模型的感知是实现无需训练三维场景理解的实用途径，为语义和全景占据预测设定了新的基准。
...
TL;DR: 提出了一种用于乳腺超声图像分割的半监督框架，通过免训练的伪标签生成和标签精炼，在仅有2.5%标注数据下达到全监督模型性能


<details>
  <summary>Details</summary>
Motivation: 传统半监督学习在极有限标注下存在伪标签不稳定问题，而现有视觉语言模型在医学图像上因领域特定提示难以迁移而效果有限

Method: 1) 利用简单外观描述实现跨领域结构迁移，生成结构一致的伪标签；2) 用伪标签预热静态教师模型捕获全局结构先验；3) 结合指数移动平均教师，引入不确定性熵加权融合和自适应不确定性引导的反向对比学习改善边界判别

Result: 在四个乳腺超声数据集上，仅用2.5%标注数据即达到全监督模型性能，显著优于现有半监督方法

Conclusion: 该范式易于扩展，仅需全局外观描述即可获得可靠伪监督，为有限标注下的可扩展半监督医学图像分割提供了新途径
...
TL;DR: JOPP-3D是一个开放词汇语义分割框架，通过联合利用全景图像和点云数据实现语言驱动的场景理解，在2D和3D开放与封闭词汇语义分割任务上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前3D点云和全景图像的语义分割面临两大挑战：标注数据稀缺和固定标签模型适应性有限。需要一种能够利用自然语言查询进行跨模态场景理解的方法。

Method: 将RGB-D全景图像转换为切向透视图像和3D点云，然后利用这两种模态提取和对齐基础视觉语言特征，使自然语言查询能够在两种输入模态上生成语义掩码。

Result: 在Stanford-2D-3D-s和ToF-360数据集上的实验表明，JOPP-3D能够生成跨全景和3D域的一致且语义有意义的分割结果，在开放和封闭词汇2D和3D语义分割任务上相比SOTA有显著提升。
...
TL;DR: 该论文提出了一种结合强化学习与多尺度反馈的3D医学图像生成方法，通过RL优化扩散模型，在保持局部纹理细节的同时提升全局结构一致性，显著改善了生成质量和下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在3D医学图像生成中表现出色，但传统训练目标与临床实际需求之间存在差距。需要一种方法能够同时优化局部细节和全局结构，生成更符合临床需求的医学图像。

Method: 首先在MRI体积数据上预训练3D扩散模型建立生成先验，然后使用近端策略优化（PPO）进行微调。创新性地设计了结合2D切片评估和3D体积分析的多尺度奖励系统，使模型能同时优化局部纹理细节和全局结构一致性。

Result: 在BraTS 2019和OASIS-1数据集上的验证表明，RL反馈有效引导生成过程向更高质量分布发展。定量分析显示Fréchet Inception Distance（FID）显著改善，更重要的是，合成数据在肿瘤和疾病分类等下游任务中表现出比未优化基线更强的实用性。

Conclusion: 通过强化学习与多尺度反馈的结合，成功提升了3D医学图像扩散模型的生成质量，使生成的图像不仅视觉上更逼真，而且在临床相关任务中更具实用价值，为医学图像生成提供了新的优化方向。
...
TL;DR: 本文提出两种技术（自动聚合和逐像素重缩放）来解决训练免费扩散分割器中存在的两个关键问题，使分割性能能够更好地利用生成模型的强大能力。


<details>
  <summary>Details</summary>
Motivation: 现有的训练免费扩散分割器通常依赖交叉注意力图来捕获像素与文本标记之间的语义关系，理论上更强的生成模型应该带来更好的分割效果。但实际观察发现现有方法未能有效扩展，存在两个关键问题：跨多头和多层的注意力图与统一全局表示之间存在差异；即使有全局图，由于不同文本标记的得分不平衡，也无法直接转化为准确的分割语义相关性。

Method: 提出两种技术：1) 自动聚合：解决多头多层注意力图与全局表示之间的差异问题；2) 逐像素重缩放：解决不同文本标记得分不平衡问题，使全局图能更准确地转化为分割所需的语义相关性。

Result: 在标准语义分割基准上评估了该方法，并进一步将其集成到生成技术中，展示了性能提升和广泛的适用性。

Conclusion: 通过提出的自动聚合和逐像素重缩放技术，训练免费扩散分割器能够更好地利用生成模型的强大能力，实现更准确的分割效果，并在多种应用中展现出良好的适用性。
...
### [73] [Contrastive-to-Self-Supervised: A Two-Stage Framework for Script Similarity Learning](https://arxiv.org/abs/2603.06180)
*Claire Roman,Philippe Meyer*

Main category: cs.CV

TL;DR: 提出两阶段框架解决字形相似度度量问题：第一阶段在标注人造字母表上训练对比学习编码器作为教师模型；第二阶段通过师生蒸馏扩展到历史文字，学生模型在教师指导下学习无监督表示，发现跨文字潜在相似性。


<details>
  <summary>Details</summary>
Motivation: 字形和文字系统相似度度量面临根本挑战：人造字母表中的单个字素可以可靠标注，但不同文字之间的历史关系不确定且有争议。需要一种方法既能学习清晰区分不同文字系统，又能发现反映潜在历史影响的软相似性。

Method: 两阶段框架：1）在有标注的人造字母表上使用对比损失训练编码器，建立具有鲁棒判别特征的教师模型；2）通过师生蒸馏扩展到历史文字，学生模型学习无监督表示，在教师知识指导下但自由发现跨文字潜在相似性。非对称设置使学生能学习变形不变嵌入，同时继承干净示例的判别结构。

Result: 在多种文字系统上的实验表明，该方法能有效进行少样本字形识别，并在不需要真实进化关系的情况下实现有意义的文字聚类。

Conclusion: 该框架桥接了监督对比学习和无监督发现，既能建立不同系统间的硬边界，又能反映潜在历史影响的软相似性，为解决文字相似度度量中的认识论约束提供了有效方案。
...
### [74] [Point-Supervised Skeleton-Based Human Action Segmentation](https://arxiv.org/abs/2603.06201)
...
<details>
  <summary>Details</summary>
Motivation: 全监督方法需要昂贵的逐帧标注且对模糊动作边界敏感，点监督方法可以显著减少标注成本并解决边界模糊问题。
...
Conclusion: 提出的点监督框架为骨架动作分割提供了一种高效且有效的解决方案，在保持性能的同时大幅降低了标注成本，具有实际应用价值。
...
TL;DR: VG3S是一个利用视觉基础模型的几何感知能力来增强3D高斯泼溅占用预测的新框架，通过层次化几何特征适配器将VFM的几何先验注入到占用预测中，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于3D高斯泼溅的占用预测方法虽然计算效率高，但严重依赖准确的几何线索，而纯视觉范式往往几何信息不足。视觉基础模型具有强大的几何感知能力，但如何将其有效注入到占用预测中是一个挑战。

Method: 提出了VG3S框架，包含一个即插即用的层次化几何特征适配器。该适配器通过特征聚合、任务特定对齐和多尺度重构三个步骤，将冻结的视觉基础模型的通用token转换为适合占用预测任务的几何特征。

Result: 在nuScenes占用基准测试中，VG3S相比基线在IoU上提升了12.6%，在mIoU上提升了7.5%。该框架能够无缝适配不同的视觉基础模型，一致性地提升占用预测精度。

Conclusion: VG3S证明了将预训练的几何感知视觉基础模型的先验知识整合到占用预测中的巨大价值，为全面场景理解提供了有效的解决方案。
...
### [76] [Cut to the Chase: Training-free Multimodal Summarization via Chain-of-Events](https://arxiv.org/abs/2603.06213)
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态摘要方法存在三个主要问题：依赖领域特定监督、隐式融合导致跨模态对齐弱、平面时间建模缺乏事件转换。需要一种能够进行显式结构化推理的解决方案。

Method: 提出CoE框架，基于层次化事件图（HEG）进行事件链推理。HEG将文本语义编码为显式的事件层次结构，指导跨模态对齐和时间推理。框架定位关键视觉线索、建模事件演化和因果转换，并通过轻量级风格适应进行领域对齐。

Result: 在八个多样化数据集上的实验表明，CoE一致优于最先进的视频CoT基线方法，平均提升ROUGE +3.04、CIDEr +9.51、BERTScore +1.88，展现了鲁棒性、可解释性和跨领域泛化能力。

Conclusion: CoE通过事件链引导的显式结构化推理有效解决了多模态摘要中的关键挑战，实现了无需训练的高性能跨模态摘要生成，具有很好的泛化能力和可解释性。
...
### [77] [EntON: Eigenentropy-Optimized Neighborhood Densification in 3D Gaussian Splatting](https://arxiv.org/abs/2603.06216)
...
<details>
  <summary>Details</summary>
Motivation: 标准3DGS方法产生的高斯中心与底层物体几何对齐不佳，而专注于表面的重建方法常牺牲光度精度。需要一种既能保持几何精度又能保证渲染质量的方法。

Method: 提出特征值熵优化的邻域致密化策略EntON，通过计算每个高斯中心k近邻协方差矩阵特征值的特征值熵来量化局部结构有序度。采用交替优化框架：标准梯度致密化与特征值熵感知致密化交替进行，在低特征值熵（有序、平坦）区域优先致密化，在高特征值熵（无序、球形）区域进行修剪。

Result: 在DTU和TUM2TWIN数据集上的实验表明，该方法将几何精度提升达33%，渲染质量提升达7%，同时将高斯数量减少达50%，训练时间减少达23%。
...
### [78] [Word-Anchored Temporal Forgery Localization](https://arxiv.org/abs/2603.06220)
*Tianyi Wang,Xi Shao,Harry Cheng,Yinglong Wang,Mohan Kankanhalli*

Main category: cs.CV

TL;DR: WAFL提出了一种基于词级二分类的时域伪造定位新范式，通过将任务从连续帧级检测转换为离散词级分类，解决了特征粒度不对齐和计算成本高的问题。
...
Method: 1) 识别最小有意义的伪造单元——词令牌，并按自然语言边界对齐数据预处理；2) 引入法证特征重对齐模块，将预训练语义空间映射到可区分的法证流形；3) 设计基于伪影的非对称损失函数，动态抑制真实样本梯度并优先处理细微法证伪影。

Result: WAFL在数据集内和跨数据集设置下，定位性能显著优于现有最先进方法，同时需要更少的可学习参数并具有高计算效率。

Conclusion: 将时域伪造定位任务从连续帧级检测范式转变为离散词级二分类范式，通过特征重对齐和非对称损失函数设计，实现了更高效、更精确的伪造定位。
...
### [79] [TaPD: Temporal-adaptive Progressive Distillation for Observation-Adaptive Trajectory Forecasting in Autonomous Driving](https://arxiv.org/abs/2603.06231)
...
<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测器通常假设固定长度的历史观测，但在真实世界中由于遮挡或传感器限制，观测长度往往是可变的或极短的，这会导致性能显著下降。需要一种能够适应不同观测长度的统一预测框架。

Method: 提出TaPD框架，包含两个协作模块：观测自适应预测器（OAF）和时序回填模块（TBM）。OAF基于渐进知识蒸馏（PKD），通过分层特征回归将长时程"教师"的运动模式知识转移到短时程"学生"，使短观测能恢复更丰富的运动上下文。采用余弦退火蒸馏权重方案平衡预测监督和特征对齐。对于极短历史，TBM基于场景演化回填缺失的历史片段，生成上下文丰富的轨迹来增强PKD。

Result: 在Argoverse 1和Argoverse 2数据集上的大量实验表明，TaPD在所有观测长度下都持续优于强基线，在极短输入下获得特别大的性能提升，并能以即插即用方式改进其他预测器（如HiVT）。

Conclusion: TaPD提供了一个统一的即插即用框架，能够有效处理可变历史长度的轨迹预测问题，特别在极短观测场景下表现出色，通过渐进知识蒸馏和时序回填的结合实现了观测自适应预测。
...
### [80] [Hierarchical Collaborative Fusion for 3D Instance-aware Referring Expression Segmentation](https://arxiv.org/abs/2603.06250)
*Keshen Zhou,Runnan Chen,Mingming Gong,Tongliang Liu*

Main category: cs.CV

TL;DR: HCF-RES提出了一种多模态框架，通过分层视觉语义分解和渐进多级融合，解决3D指代表达式分割中视觉语义不足的问题，在多个基准测试中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有3D指代表达式分割方法仅依赖稀疏点云，缺乏丰富的视觉语义信息，难以处理细粒度描述。当描述匹配多个或零个目标时，现有方法表现不佳。
...
Result: 在ScanRefer和Multi3DRefer两个基准测试上都达到了最先进的性能。

Conclusion: HCF-RES通过结合2D视觉语义和3D几何信息，有效解决了3D指代表达式分割中的视觉语义不足问题，显著提升了性能。
...
### [81] [NOVA: Next-step Open-Vocabulary Autoregression for 3D Multi-Object Tracking in Autonomous Driving](https://arxiv.org/abs/2603.06254)
*Kai Luo,Xu Wang,Rui Fan,Kailun Yang*

Main category: cs.CV

TL;DR: NOVA提出了一种创新的开放词汇自回归3D多目标跟踪范式，通过将轨迹建模为结构化时空语义序列，利用大语言模型的自回归能力实现跨未知目标的泛化跟踪。
...
Method: 将3D轨迹重新定义为结构化时空语义序列，利用大语言模型的自回归能力，将跟踪任务转化为基于原则的下一个步骤序列补全过程，通过语言空间层次结构解决语义模糊性。

Result: 在nuScenes、V2X-Seq-SPD和KITTI数据集上表现出优越性能，在nuScenes数据集上，NOVA在Novel类别上达到22.41%的AMOTA，相比基线有20.21%的绝对提升，仅使用0.5B参数的自回归模型。

Conclusion: NOVA通过生成式时空语义建模范式，成功将3D跟踪从传统的基于距离的碎片化匹配转向语义感知的连续跟踪，实现了对未知目标的开放词汇泛化能力。
...
TL;DR: HiPP-Prune：一个分层偏好条件结构化剪枝框架，用于视觉语言模型的高效部署，通过全局剪枝蓝图实现可控的鲁棒性-效用权衡


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型剪枝面临挑战，因为压缩不仅影响任务效用，还会影响视觉基础能力，甚至在相同稀疏度下也会加剧物体幻觉问题

Method: 提出分层偏好条件结构化剪枝框架，将剪枝视为多目标下的条件资源分配。通过单一策略调用输出全局剪枝蓝图，将决策分解为总体稀疏度预算和分层分配。策略状态整合视觉敏感性信号，防止过度剪枝视觉关键层。使用计划级组相对策略优化在多目标回报下优化剪枝计划

Result: 在LLaVA模型上的POPE和ScienceQA实验表明，HiPP-Prune能够发现多样化的非支配剪枝计划，并在匹配的稀疏度预算下提供可控的鲁棒性-效用权衡

Conclusion: HiPP-Prune框架通过分层偏好条件剪枝，有效解决了视觉语言模型剪枝中的多目标优化问题，实现了在保持任务效用的同时减少幻觉，为高效部署提供了可控的权衡方案
...
### [83] [Spectral and Trajectory Regularization for Diffusion Transformer Super-Resolution](https://arxiv.org/abs/2603.06275)
*Jingkai Wang,Yixin Tang,Jue Gong,Jiatong Li,Shu Li,Libo Liu,Jianliang Lan,Yutong Liu,Yulun Zhang*

Main category: cs.CV

TL;DR: StrSR提出了一种用于扩散变换器（DiT）的单步对抗蒸馏框架，通过谱和轨迹正则化解决Real-ISR中的轨迹失配和周期性伪影问题。


<details>
  <summary>Details</summary>
Motivation: 扩散变换器在真实图像超分辨率中表现出潜力，但其迭代采样计算成本高，需要单步蒸馏。现有单步蒸馏方法在DiT上存在轨迹失配问题，并产生严重的网格状周期性伪影。
...
TL;DR: 本文提出了OccNL基准和DPR-Occ框架，用于解决3D语义占据预测中标签噪声问题，在极端噪声条件下仍能保持性能。
...
Method: 提出了DPR-Occ框架，通过双源部分标签推理构建可靠监督。该方法结合时间模型记忆和表示层结构相似性，动态扩展和修剪候选标签集，以保留真实语义同时抑制噪声传播。

Result: 在SemanticKITTI上的实验表明，DPR-Occ在极端噪声下能防止几何和语义崩溃。即使在90%标签噪声下，相比现有标签噪声学习方法，该方法仍能获得显著性能提升（最高2.57% mIoU和13.91% IoU）。
...
### [85] [Attribute Distribution Modeling and Semantic-Visual Alignment for Generative Zero-shot Learning](https://arxiv.org/abs/2603.06281)
...
<details>
  <summary>Details</summary>
Motivation: 生成式零样本学习虽然能合成未见类特征，但面临两个内在挑战：1）类级属性无法捕捉实例特定的视觉外观，导致类-实例差距；2）语义与视觉特征分布不匹配，表现为类间相关性，导致语义-视觉域差距。
...
### [86] [FlowMotion: Training-Free Flow Guidance for Video Motion Transfer](https://arxiv.org/abs/2603.06289)
...
<details>
  <summary>Details</summary>
Motivation: 现有无训练方法依赖预训练T2V模型的中间输出构建运动指导，导致计算开销大且灵活性有限。作者发现早期潜在预测天然编码丰富的时序信息，这为直接利用流基T2V模型的预测输出实现高效运动迁移提供了可能。

Method: 提出FlowMotion框架：1）流指导：基于潜在预测提取运动表示，对齐源视频与生成视频的运动模式；2）速度正则化策略：稳定优化过程，确保平滑的运动演化。整个框架完全基于模型预测操作，无需额外训练。

Result: 与最先进方法相比，FlowMotion在时间和资源效率方面表现优越，同时保持有竞争力的性能。通过直接在模型预测层面操作，显著减少了计算开销并提高了灵活性。

Conclusion: FlowMotion通过直接利用流基T2V模型的预测输出，实现了高效灵活的视频运动迁移。该方法的核心洞察是早期潜在预测编码的丰富时序信息，为无训练运动迁移提供了新的有效途径。
...
Result: 该方法能有效生成高质量、伪影减少的3D CBCT图像，显示出在改善临床成像方面的潜力。

Conclusion: 提出的基于垂直分数扩散模型的3D牙科种植体修复方法是一种有前景的解决方案，能够提高CBCT图像质量，改善临床诊断准确性。
...
TL;DR: DEX-AR是一种为自回归视觉语言模型设计的动态可解释性方法，通过生成逐token和序列级别的2D热图来突出显示影响模型文本响应的关键图像区域。


<details>
  <summary>Details</summary>
Motivation: 随着视觉语言模型变得越来越复杂和广泛应用，理解其决策过程变得至关重要。传统的可解释性方法是为分类任务设计的，难以处理现代自回归VLM的复杂token-by-token生成过程以及视觉和文本模态之间的复杂交互。

Method: DEX-AR通过计算token-by-token生成过程中注意力图的逐层梯度来解读自回归VLM。该方法包含两个关键创新：1）动态头过滤机制，识别专注于视觉信息的注意力头；2）序列级过滤方法，聚合逐token解释同时区分视觉基础和纯语言token。
...
### [89] [WMoE-CLIP: Wavelet-Enhanced Mixture-of-Experts Prompt Learning for Zero-Shot Anomaly Detection](https://arxiv.org/abs/2603.06313)
*Peng Chen,Chao Huang*

Main category: cs.CV

TL;DR: 提出了一种小波增强的专家混合提示学习方法，用于零样本异常检测，通过变分自编码器建模全局语义表示，小波分解提取多频特征，语义感知专家混合模块聚合上下文信息。
...
Method: 1) 使用变分自编码器建模全局语义表示并整合到提示中；2) 小波分解提取多频图像特征，通过跨模态交互动态优化文本嵌入；3) 引入语义感知专家混合模块聚合上下文信息。

Result: 在14个工业和医学数据集上的广泛实验证明了该方法的有效性。

Conclusion: 提出的小波增强专家混合提示学习方法能够更好地捕捉复杂语义和多频特征，显著提升了零样本异常检测的性能。
...
### [90] [P-SLCR: Unsupervised Point Cloud Semantic Segmentation via Prototypes Structure Learning and Consistent Reasoning](https://arxiv.org/abs/2603.06321)
*Lixin Zhan,Jie Jiang,Tianjian Zhou,Yukun Du,Yan Zheng,Xuehu Duan*

Main category: cs.CV

TL;DR: 提出P-SLCR方法，通过原型库驱动的无监督点云语义分割策略，结合结构学习和一致性推理，在多个数据集上取得优于无监督方法的效果，甚至在某些情况下超越经典全监督方法。
...
Conclusion: 提出的P-SLCR方法通过原型库驱动的无监督点云语义分割策略，结合结构学习和一致性推理，有效解决了无监督点云语义分割的挑战，在多个数据集上取得了显著性能提升。
...
### [91] [WorldCache: Accelerating World Models for Free via Heterogeneous Token Caching](https://arxiv.org/abs/2603.06331)
...
TL;DR: WorldCache提出了一种针对扩散世界模型的缓存框架，通过曲率引导的异构令牌预测和混沌优先自适应跳过技术，实现了3.7倍加速同时保持98%的推演质量。


<details>
  <summary>Details</summary>
Motivation: 基于扩散的世界模型在统一世界模拟方面表现出强大潜力，但迭代去噪过程计算成本过高，难以支持交互式使用和长时域推演。现有特征缓存方法虽然可以加速推理，但由于令牌异构性和非均匀时间动态性这两个世界模型特有的障碍，导致策略迁移效果不佳。

Method: 提出了WorldCache框架，包含两个核心技术：1) 曲率引导的异构令牌预测：使用基于物理的曲率分数估计令牌可预测性，对混沌令牌应用Hermite引导的阻尼预测器；2) 混沌优先自适应跳过：累积曲率归一化的无量纲漂移信号，仅在瓶颈令牌开始漂移时重新计算。

Result: 在扩散世界模型上的实验表明，WorldCache实现了最高3.7倍的端到端加速，同时保持了98%的推演质量，在资源受限场景下展现出显著优势和实用性。

Conclusion: WorldCache通过针对扩散世界模型特定挑战设计的缓存机制，有效解决了令牌异构性和非均匀时间动态性问题，为资源受限环境下的高效世界模拟提供了实用解决方案。
...
### [92] [K-MaT: Knowledge-Anchored Manifold Transport for Cross-Modal Prompt Learning in Medical Imaging](https://arxiv.org/abs/2603.06340)
*Jiajun Zeng,Shadi Albarqouni*

Main category: cs.CV

TL;DR: K-MaT是一种无需低端模态训练图像的提示学习框架，通过知识锚定流形传输将决策结构从高端医学影像模态（如CT）迁移到低端模态（如X光），解决了跨模态部署中的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 当前大规模生物医学视觉语言模型在高端影像（如CT）上适配后，迁移到前线低端模态（如放射摄影）时常常失败，陷入模态特定的捷径学习。需要一种无需低端训练图像就能实现跨模态知识迁移的方法。

Method: 提出K-MaT框架：1）分解提示并将其锚定到临床文本描述；2）使用融合Gromov-Wasserstein最优传输将低端提示流形与视觉基础的高端空间对齐；3）实现零样本跨模态部署。

Result: 在四个跨模态基准测试（皮肤镜、乳腺X光到超声、CT到胸部X光）中取得最先进结果：平均准确率调和均值从42.0%提升到44.1%，宏观F1分数达到36.2%。在具有挑战性的乳腺成像任务中，避免了标准方法（如CoOp）的灾难性遗忘（准确率降至27.0%），保持了跨模态的稳健性能。

Conclusion: 通过最优传输对齐提示流形为医学视觉语言模型的零样本跨模态部署提供了高效途径，解决了模态间知识迁移中的灾难性遗忘问题。
...
### [93] [LATO: 3D Mesh Flow Matching with Structured TOpology Preserving LAtents](https://arxiv.org/abs/2603.06357)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于等值面/三角形的扩散模型和自回归生成方法在生成复杂几何形状和良好拓扑结构方面存在效率问题，需要一种更高效、能保持拓扑结构的3D网格生成方法。

Method: 1. 使用顶点位移场表示网格，通过稀疏体素VAE压缩为结构化、拓扑感知的体素潜在表示；2. VAE解码器通过逐步细分和修剪体素来实例化精确顶点位置；3. 专用连接头查询体素潜在表示直接预测顶点对之间的边连接性；4. 采用两阶段流匹配过程，先合成结构体素，再细化体素级拓扑特征。

Result: LATO能够生成具有复杂几何形状和良好拓扑结构的网格，在推理效率上优于先前的等值面/三角形扩散模型和自回归生成方法。

Conclusion: LATO提供了一种新颖的拓扑保持潜在表示方法，实现了高效、可扩展的显式3D网格生成，避免了传统等值面提取和启发式网格化的限制。
...
### [94] [OralGPT-Plus: Learning to Use Visual Tools via Reinforcement Learning for Panoramic X-ray Analysis](https://arxiv.org/abs/2603.06366)
*Yuxuan Fan,Jing Hao,Hong Chen,Jiahao Bao,Yihua Shao,Yuci Liang,Kuo Feng Hung,Hao Tang*

Main category: cs.CV

TL;DR: OralGPT-Plus是一个用于全景牙科X光片分析的智能视觉语言模型，通过迭代和对称感知的诊断推理提升临床可靠性，并构建了DentalProbe数据集和MMOral-X基准测试。
...
Method: 1. 构建DentalProbe数据集（5000张图像），包含专家策划的诊断轨迹，为局部检查和对比侧比较提供结构化监督；2. 开发重新检查驱动的强化学习框架，通过基于评分标准的奖励和条件诊断驱动奖励鼓励临床有意义的重新检查并稳定长时程推理；3. 提出OralGPT-Plus模型，实现迭代和对称感知的诊断推理。

Result: OralGPT-Plus在MMOral-X基准测试和现有全景牙科基准测试上相比强基线模型表现出持续可靠的改进，表明交互式和对称感知推理的有效性。

Conclusion: 该研究突出了智能建模在牙科影像中的价值，为未来临床对齐的全景X光片分析研究奠定了基础，证明了迭代和对称感知推理在提升诊断可靠性方面的重要性。
...
### [95] [Rewis3d: Reconstruction Improves Weakly-Supervised Semantic Segmentation](https://arxiv.org/abs/2603.06374)
...
Method: 提出基于3D场景重建的弱监督分割框架，利用从2D视频恢复的3D几何结构传播稀疏标注，采用师生双架构强制2D图像与重建3D点云间的语义一致性，使用最先进的前馈重建生成可靠的几何监督

Result: Rewis3d在稀疏监督设置下达到最先进性能，比现有方法提升2-7%，且不需要额外标注或增加推理开销
...
### [96] [CHMv2: Improvements in Global Canopy Height Mapping using DINOv3](https://arxiv.org/abs/2603.06382)
...
<details>
  <summary>Details</summary>
Motivation: 准确的冠层高度信息对于量化森林碳储量、监测恢复与退化、评估栖息地结构至关重要，但全球范围内高精度的机载激光扫描数据分布不均，需要更广泛可用的替代方案。

Method: 使用基于DINOv3的深度估计模型，从高分辨率光学卫星影像生成冠层高度图；通过大规模地理多样化训练数据扩展、自动化数据整理与配准，以及针对冠层高度分布定制的损失函数和数据采样策略来实现改进。

Result: CHMv2相比现有产品显著提高了精度，减少了高森林的偏差，更好地保留了冠层边缘和间隙等精细结构；在独立ALS测试集以及数千万个GEDI和ICESat-2观测数据上验证，在主要森林生物群落中表现一致。

Conclusion: CHMv2提供了一个全球、高精度的冠层高度图，通过先进的光学影像深度估计方法克服了ALS数据分布不均的限制，为全球森林监测和研究提供了重要工具。
...
### [97] [Prompt Group-Aware Training for Robust Text-Guided Nuclei Segmentation](https://arxiv.org/abs/2603.06384)
*Yonghuang Wu,Zhenyang Liang,Wenwen Zeng,Xuan Xie,Jinhua Yu*

Main category: cs.CV

TL;DR: 提出一种针对SAM3等基础模型在医学图像分割中提示敏感性的解决方案，通过提示组感知训练框架提高文本引导细胞核分割的鲁棒性。
...
Method: 将提示敏感性重新定义为组一致性问题，将语义相关的提示组织成共享相同真实掩码的提示组，提出提示组感知训练框架：1) 质量引导的组正则化，利用分割损失作为隐式排序信号；2) 采用停止梯度策略的对数级一致性约束，对齐每组内的预测。该方法无需架构修改，推理过程保持不变。

Result: 在多数据集细胞核基准测试中，该方法在文本提示下获得一致提升，显著降低了不同提示质量水平下的性能方差。在六个零样本跨数据集任务中，平均Dice系数提高了2.16个百分点。

Conclusion: 该方法提高了计算病理学中视觉语言分割的鲁棒性和泛化能力，为缓解基础模型在医学图像分割中的提示敏感性提供了有效解决方案。
...
### [98] [REACT++: Efficient Cross-Attention for Real-Time Scene Graph Generation](https://arxiv.org/abs/2603.06386)
...
<details>
  <summary>Details</summary>
Motivation: 当前场景图生成方法通常只专注于关系预测精度、物体检测精度或降低延迟中的单一目标，缺乏同时平衡这三个目标的解决方案。为了支持实时应用（如具身智能体推理），需要解决性能与推理速度之间的权衡问题。

Method: 基于REACT架构，采用高效特征提取和原型空间中的主体-客体交叉注意力机制，在保持模型表示能力的同时优化计算效率。

Result: REACT++在现有SGG模型中达到最高推理速度，相比前代REACT版本速度快20%，关系预测精度平均提升10%，且不牺牲物体检测性能。
...
### [99] [Solving Jigsaw Puzzles in the Wild: Human-Guided Reconstruction of Cultural Heritage Fragments](https://arxiv.org/abs/2603.06389)
*Omidreza Safaei,Sinem Aslan,Sebastiano Vascon,Luca Palmieri,Marina Khoroshiltseva,Marcello Pelillo*

Main category: cs.CV

TL;DR: 提出人机协同的考古碎片重组框架，结合自动求解器与人工交互，解决真实文物碎片大规模、不规则、缺失区域的复杂重组问题。


<details>
  <summary>Details</summary>
Motivation: 真实考古文物碎片重组面临侵蚀、缺失、不规则形状和大规模模糊性等挑战，传统拼图求解器在清洁合成场景中设计，难以应对RePAIR基准中数千碎片的复杂情况。

Method: 提出人机协同拼图求解框架，整合自动松弛标记求解器与交互式人工指导，用户可迭代锁定已验证位置、纠正错误，引导系统实现语义和几何一致的组装。引入两种互补交互策略：迭代锚定和连续交互优化。

Result: 在多个RePAIR数据集上的实验表明，该混合方法在准确性和效率上显著优于完全自动化和手动基线，为大规模专家参与文物重组提供了实用解决方案。

Conclusion: 人机协同框架有效解决了真实世界文化遗产重建的复杂性和规模问题，通过结合自动求解与人工指导，实现了大规模文物碎片的准确高效重组。
...
### [100] [Locating and Editing Figure-Ground Organization in Vision Transformers](https://arxiv.org/abs/2603.06407)
*Stefan Arnold,René Gröbner*

Main category: cs.CV

TL;DR: BEiT视觉Transformer在感知冲突中偏好凸性完成，研究发现这种偏好由特定注意力头控制，特别是L0H9头作为早期种子引入凸性偏置，下调该头可使模型转向凹性完成。


<details>
  <summary>Details</summary>
Motivation: 研究旨在定位BEiT模型中实现经典格式塔凸性先验的内部组件，通过合成飞镖形状的感知冲突来探究模型在局部几何证据与全局组织先验之间的选择机制。

Method: 使用基于合成飞镖形状的受控感知冲突，系统性地掩码既支持凹性完成也支持凸性完成的区域；通过logit归因将内部激活投影到离散视觉码本空间；分解注意力头的直接影响来识别功能单元。

Result: BEiT在感知冲突中可靠地偏好凸性完成；图-底组织在早期和中间层保持模糊，在后期层突然解决；识别出头L0H9作为早期种子引入弱凸性偏置；下调该单个注意力头可改变感知冲突的分布质量，使凹性证据引导完成。

Conclusion: BEiT中的格式塔凸性先验由特定可识别的功能单元实现，特别是L0H9注意力头作为早期偏置种子，表明Transformer子结构中存在专门处理感知组织的模块化组件。
...
TL;DR: PSIVG框架通过将物理模拟器集成到视频扩散过程中，解决AI生成视频中物体运动违反物理定律的问题，生成物理一致的运动轨迹。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散的视频生成方法虽然视觉质量显著，但经常违反重力、惯性和碰撞等基本物理定律，导致物体运动不一致、动力学不合理，限制了AI生成视频的真实性和可靠性。

Method: 提出PSIVG框架：1) 从预训练扩散模型生成的模板视频中重建4D场景和前景物体网格；2) 在物理模拟器中初始化这些网格并生成物理一致的运动轨迹；3) 用模拟轨迹指导视频生成器产生时空物理一致的运动；4) 提出TTCO技术，基于模拟器的像素对应关系调整文本和特征嵌入，改善物体运动时的纹理一致性。

Result: 综合实验表明，PSIVG生成的视频能更好地遵守真实世界的物理规律，同时保持视觉质量和多样性。

Conclusion: PSIVG通过集成物理模拟器到视频扩散过程，有效解决了AI生成视频中的物理不一致问题，提高了视频的真实性和可靠性。
...
TL;DR: CLoPA是一种持续适应策略，通过在标注缓存上微调nnInteractive模型的一小部分参数，快速提升医学图像分割性能至专家水平。


<details>
  <summary>Details</summary>
Motivation: 现有零样本模型如nnInteractive在不同医学成像任务中无法稳定达到专家级性能，而标注活动产生持续增长的任务特定标注数据，需要在线适应分割模型作为零样本推理的补充。

Method: 提出CLoPA持续适应策略，通过轻量级事件调度触发，在标注缓存上微调nnInteractive的一小部分参数，无需新参数或改变推理流程，完全在现有标注工作流中运行。

Result: 在八个Medical Segmentation Decathlon任务中，CLoPA快速将性能提升至专家水平，即使对于nnInteractive先前失败的任务，多数增益在单个训练事件后实现。不同参数组调优效果取决于任务特性和数据状况。

Conclusion: CLoPA能有效提升交互式分割性能，但对于复杂几何目标（如肝血管），实例归一化和低级特征调优会饱和，表明在最具挑战性场景中需要更深层的特征表示对齐。
...
### [103] [What if? Emulative Simulation with World Models for Situated Reasoning](https://arxiv.org/abs/2603.06445)
...
TL;DR: WanderDream是首个用于心理探索模拟的大规模数据集，包含15.8K全景视频和158K问答对，让AI模型能够在无需实际探索的情况下进行空间推理。
...
Conclusion: WanderDream为无需主动探索的心理模拟提供了首个大规模数据集，证明了心理探索对空间推理的重要性，并展示了其在真实世界应用中的潜力。
...
### [104] [CaTok: Taming Mean Flows for One-Dimensional Causal Image Tokenization](https://arxiv.org/abs/2603.06449)
*Yitong Chen,Zuxuan Wu,Xipeng Qiu,Yu-Gang Jiang*

Main category: cs.CV

TL;DR: CaTok是一种1D因果图像分词器，通过MeanFlow解码器学习因果视觉表示，支持快速一步生成和高保真多步采样，在ImageNet重建上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉分词器将2D图像块展平为非因果序列，或采用启发式排序，与"下一个token预测"模式不匹配。扩散自编码器也存在缺乏因果性或引入不平衡的问题，需要开发真正的因果视觉分词器。

Method: 提出CaTok：1D因果图像分词器，通过选择时间间隔内的token并将其绑定到MeanFlow目标，学习因果1D表示。还提出REPA-A正则化方法，将编码器特征与视觉基础模型对齐以稳定和加速训练。

Result: 在ImageNet重建上达到最先进结果：FID 0.75，PSNR 22.53，SSIM 0.674，且训练轮数更少。自回归模型性能与领先方法相当。

Conclusion: CaTok成功将因果分词范式扩展到视觉领域，学习到的因果1D表示支持快速生成和高保真采样，同时自然地捕捉了跨token间隔的多样化视觉概念。
...
### [105] [Pinterest Canvas: Large-Scale Image Generation at Pinterest](https://arxiv.org/abs/2603.06453)
...
<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型虽然灵活，但难以通过提示或简单推理适应来控制，不适合有严格产品要求的用例。需要构建一个既能保持广泛能力又能针对特定任务优化的系统。

Method: 首先在多样化多模态数据集上训练基础扩散模型，然后针对不同下游任务在任务特定数据集上快速微调，生成专门化模型。系统包括数据集管理、训练和推理等关键组件。

Result: 在线A/B实验显示，背景增强和宽高比外绘任务分别获得18.0%和12.5%的参与度提升。人工评估表明模型在这些任务上优于第三方模型。系统还可扩展到多图像场景合成和图像到视频生成等任务。

Conclusion: Pinterest Canvas通过基础模型加任务特定微调的方法，成功构建了既能保持广泛图像编辑能力又能满足严格产品需求的图像生成系统，可推广到多种下游任务。
...
### [106] [Training Flow Matching: The Role of Weighting and Parameterization](https://arxiv.org/abs/2603.06454)
*Anne Gagneux,Ségolène Martin,Rémi Gribonval,Mathurin Massias*

Main category: cs.CV

TL;DR: 该研究系统分析了去噪生成模型的训练目标，重点关注损失权重和输出参数化（噪声、干净图像、速度三种形式），通过实验探究这些选择与数据流形内在维度、模型架构、数据集大小的相互作用。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解去噪生成模型中各种训练选择（损失权重、输出参数化）如何影响模型性能，而不是提出新方法。旨在理清训练流匹配模型时各种因素的重要性，为实际设计选择提供指导。

Method: 采用系统性数值研究方法，在合成数据集（具有可控几何结构）和真实图像数据上进行实验。比较不同训练目标，使用定量指标评估去噪精度（不同噪声水平的PSNR）和生成质量（FID）。

Result: 通过实验分析了训练选择与数据流形内在维度、模型架构、数据集大小的相互作用关系，为不同训练目标提供了量化评估结果。

Conclusion: 研究为训练流匹配模型的设计选择提供了实用见解，理清了各种因素的重要性，帮助研究人员更好地理解去噪生成模型的训练机制。
...
### [107] [GreenRFM: Toward a resource-efficient radiology foundation model](https://arxiv.org/abs/2603.06467)
...
<details>
  <summary>Details</summary>
Motivation: 现有放射学基础模型依赖暴力扩展方法，直接移植自然图像处理方法，导致模型在临床实践中脆弱且昂贵。需要资源高效的方法来克服这些问题。

Method: 提出GreenRFM框架，采用MUST（More distilled, Ubiquitous, Semantic-enforcing, and Task-aligning）监督设计原则，最大化利用监督信号而非简单堆叠训练数据量。

Result: 在两个配置下均取得优异性能：高性能模型在单张24GB GPU上24小时内达到SOTA；轻量级模型仅需6GB显存在4小时内匹配现有基准。在超过20万张图像、四个机构、两种模态的实验中均表现优越。

Conclusion: GreenRFM挑战了"规模决定一切"的教条，通过资源高效的方法实现了SOTA性能，使临床医生即使在笔记本电脑上也能公平地开发先进的放射学基础模型。
...
### [108] [Match4Annotate: Propagating Sparse Video Annotations via Implicit Neural Feature Matching](https://arxiv.org/abs/2603.06471)
*Zhuorui Zhang,Roger Pallarès-López,Praneeth Namburi,Brian W. Anthony*

Main category: cs.CV

TL;DR: Match4Annotate：基于SIREN隐式神经表示和DINOv3特征的轻量级视频标注传播框架，支持帧内和帧间点与掩码标注传播


<details>
  <summary>Details</summary>
Motivation: 在医疗影像等专业领域，逐帧视频标注成本高昂且耗时。现有标注传播方法存在局限性：视频跟踪和分割模型需要逐视频初始化且无法跨视频泛化；传统对应点方法在低纹理场景表现不佳；密集特征匹配和一次性分割方法缺乏时空平滑性且不支持点与掩码的统一处理。

Method: 提出Match4Annotate框架：1）在测试时使用SIREN隐式神经表示拟合DINOv3特征，生成连续高分辨率时空特征场；2）学习帧对之间的平滑隐式变形场来指导对应点匹配；3）支持帧内和帧间的点与掩码标注传播。

Result: 在三个具有挑战性的临床超声数据集上评估：1）在帧间传播方面达到最先进水平，优于特征匹配和一次性分割基线；2）在帧内传播方面与专用跟踪器保持竞争力；3）展示了轻量级、测试时优化的特征匹配流程在可扩展标注工作流中的潜力。

Conclusion: 轻量级、测试时优化的特征匹配流程为可扩展的标注工作流提供了高效且易于访问的解决方案，特别是在需要专家标注的专业领域如医疗影像中。
...
### [109] [Self-Supervised Flow Matching for Scalable Multi-Modal Synthesis](https://arxiv.org/abs/2603.06507)
...
TL;DR: AI系统通过45,139张超声图像训练，诊断胎儿口面裂的敏感性和特异性分别超过93%和95%，性能与资深放射科医生相当，并能提升初级医生的诊断能力。
...
Method: 开发了一个人工智能系统，使用来自22家医院9,215名胎儿的45,139张超声图像进行训练，用于诊断胎儿口面裂。该系统还作为医疗副驾驶辅助初级放射科医生，并通过涉及24名放射科医生和学员的试点研究评估其对专业知识发展的影响。

Result: AI系统诊断胎儿口面裂的敏感性和特异性分别超过93%和95%，与资深放射科医生表现相当，并显著优于初级放射科医生。作为医疗副驾驶使用时，能将初级放射科医生的敏感性提高超过6%。试点研究表明，该模型能够改善罕见疾病的专业知识发展。
...
### [111] [AV-Unified: A Unified Framework for Audio-visual Scene Understanding](https://arxiv.org/abs/2603.06530)
*Guangyao Li,Xin Wang,Wenwu Zhu*

Main category: cs.CV

TL;DR: AV-Unified是一个统一的音频-视觉场景理解框架，通过将不同任务的输入输出转换为离散token序列，实现跨任务联合学习，包含多尺度时空感知模块和跨模态引导机制。
...
Method: 1) 标准化所有任务的输入输出格式，转换为离散token序列；2) 设计多尺度时间感知模块捕捉不同粒度的事件；3) 设计跨模态引导的空间感知模块建模空间关联；4) 使用任务特定文本提示增强适应性和任务意识。

Result: 在多个基准数据集（AVE、LLP、MUSIC-AVQA、VGG-SS、AVS）上的实验表明，AV-Unified在时间、空间和时空任务上均表现出有效性。

Conclusion: AV-Unified通过统一框架实现了跨音频-视觉任务的联合学习，能够更全面地理解复杂场景，为音频-视觉场景理解提供了新的研究方向。
...
Method: 使用扫描的反光贴片结合背景减法，恢复每个像素在共位RGB图像平面中的响应图，建立LiDAR到RGB的显式对应关系

Result: 在ams OSRAM TMF8828上演示了该方法，能够为跨模态对齐和融合提供明确的LiDAR-RGB对应关系

Conclusion: 该方法解决了漫反射LiDAR与RGB相机校准的特殊挑战，为多模态感知系统的精确对齐提供了实用解决方案
...
Method: 将否定重新解释为扩散动态中语义引导的结构化可行性约束。将无分类器引导定义为语义更新方向，并通过将更新投影到从语言结构导出的凸约束集上来强制执行否定。该方法无需训练，与预训练的扩散主干兼容。

Result: 实验表明该方法实现了稳健的否定遵从性，同时保持了视觉保真度和结构连贯性。该方法能够处理多种否定现象，包括对象缺失、分级非反转语义、多否定组合和范围敏感消歧。

Conclusion: 该工作建立了基于扩散的生成模型中语言否定的第一个统一公式，超越了表示级评估，为处理复杂语言约束提供了训练免费框架。
...
TL;DR: 研究分析了自动驾驶数据集中冗余性作为数据质量因素的影响，通过多源摄像头和多模态图像-LiDAR数据的冗余建模与测量，发现选择性去除冗余标签能提升目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统依赖多源多模态数据，但数据质量因环境和传感器限制而异。现有研究主要关注算法设计，而忽略了数据质量分析，特别是冗余性这一基础但未被充分探索的数据质量问题。

Method: 使用nuScenes和Argoverse 2数据集，对多源摄像头数据和多模态图像-LiDAR数据的冗余性进行建模和测量。通过选择性去除冗余标签，评估对YOLOv8目标检测任务的影响。

Result: 实验表明：1）选择性去除共享视野摄像头间的冗余图像目标标签能提升检测性能（nuScenes中mAP50从0.66提升到0.70等）；2）AV2中移除4.1-8.6%标签后mAP50仍接近基线0.64；3）多模态分析显示图像和LiDAR数据间存在显著冗余。

Conclusion: 冗余性是自动驾驶数据集中可测量且可操作的数据质量因素，直接影响系统性能。这项工作强调了冗余性在自动驾驶感知中的作用，并推动了以数据为中心的视角来评估和改进自动驾驶数据集。
...
Method: 提出EgoReasoner两阶段框架：1）任务自适应思维模板指导结构化思维链生成，通过监督微调实现跨任务自适应推理；2）任务感知奖励函数验证实体接地、时间对齐和逻辑一致性，通过GRPO强化微调选择性强化推理路径

Result: 仅用16K样本训练的3B参数模型在HD-EPIC基准测试中达到37.5%平均准确率，显著超越Qwen2.5-VL-7B（25.7%）超过10个百分点

Conclusion: 通过将推理框架和奖励信号与任务认知结构对齐，EgoReasoner能够有效解决自我中心4D推理任务，在少量数据下实现优于更大模型的性能
...
TL;DR: Penguin-VL提出了一种紧凑型视觉语言模型，通过使用基于纯文本LLM初始化的视觉编码器替代传统的对比预训练视觉编码器，在保持轻量级架构的同时实现了与大型VLMs相当的性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型主要依赖模型规模扩展，这限制了在移动和边缘设备上的部署。同时，主流VLMs依赖对比预训练（如CLIP/SigLIP）初始化的视觉编码器存在目标不匹配问题：对比学习优化的判别性任务会抑制细粒度视觉线索，而这些线索对于密集描述和复杂推理至关重要。

Method: 提出Penguin-VL模型，其视觉编码器从纯文本LLM初始化，而不是传统的对比预训练视觉编码器。这种方法解决了目标不匹配问题，保留了细粒度的空间和时间线索，提高了视觉保真度和数据效率。
...
Conclusion: Penguin-Encoder作为传统对比预训练编码器的优越替代方案，为计算高效的VLMs提供了强大的即插即用选项，能够在资源受限的环境中实现高性能。该方法证明了改进视觉表示比单纯扩大模型规模更为关键。
...
TL;DR: SUREON是一个大规模外科手术视频问答数据集，通过从外科教学视频中提取专家讲解来训练AI理解手术推理过程，包括安全评估、决策理由和预测等12类问题。


<details>
  <summary>Details</summary>
Motivation: 当前外科AI无法理解手术推理过程（如为什么选择特定器械、存在什么风险、下一步是什么），因为大规模标注这类推理数据非常困难。但外科教学视频中已经包含了专家讲解的意图、理由和预期，这些正是AI所缺乏的推理信息。

Method: 从外科学术视频中系统性地提取训练信号，定义了12个问题类别（覆盖安全评估、决策理由和预测），使用多智能体流程大规模提取和结构化监督数据。构建了包含134.7K个视频片段、170种手术类型、206.8k个问答对的数据集，并创建了354个专家验证的基准测试。

Result: 开发了两个模型：SureonVLM（通过监督微调适应的视觉语言模型）和SureonVLM-R1（使用Group Relative Policy Optimization训练的推理模型）。两个模型都能回答复杂的外科问题，在SUREON基准测试中准确率超过84%，并且在标准外科感知任务上优于更大的通用领域模型。

Conclusion: 通过从外科教学视频中提取专家讲解，可以大规模训练AI理解手术推理过程。SUREON数据集和模型展示了将非结构化教学叙述转化为结构化监督的有效方法，使AI能够进行外科推理，如从视觉上下文中推断手术意图。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将多视角多帧图像独立输入LLMs，导致计算冗余和空间一致性不足；而传统BEV表示缺乏语义丰富性，需要桥接空间结构与语义理解

Method: 提出BEVLM框架，将空间一致且语义蒸馏的BEV表示与LLMs连接，使用BEV特征作为统一输入，同时将LLMs的语义知识蒸馏到BEV表示中

Result: BEVLM使LLMs在跨视角驾驶场景中的推理准确率提升46%，通过语义蒸馏显著改善端到端驾驶性能，在安全关键场景中提升29%

Conclusion: BEVLM成功桥接了空间一致的BEV表示与LLMs的语义能力，显著提升了自动驾驶中的3D空间推理和决策性能
...
TL;DR: 研究发现多模态大语言模型（MLLM）的分类性能评估存在严重偏差，主要源于评估协议缺陷和标注质量问题，修正这些问题后MLLM与监督模型的差距显著缩小。


<details>
  <summary>Details</summary>
Motivation: 当前关于多模态大语言模型（MLLM）与监督模型、视觉语言模型在分类任务上的比较研究存在矛盾结论，这些矛盾源于评估协议和标注质量的缺陷，需要系统性地识别和解决这些问题。

Method: 1. 识别并修正常见评估协议中的关键问题：模型输出超出类别列表被丢弃、弱干扰项导致结果虚高、输出映射不佳导致开放世界设置表现差；2. 量化常被忽视的设计选择影响：批次大小、图像顺序、文本编码器选择；3. 使用ReGT（ImageNet-1k的625类多标签重新标注）进行评估；4. 进行受控案例研究评估MLLM辅助人工标注的效果。

Result: 1. MLLM从修正标注中获益最大（提升高达+10.8%），显著缩小了与监督模型的感知差距；2. 常被忽视的设计选择（批次大小等）对准确率有实质性影响；3. 较少依赖监督训练信号的模型对标注质量最敏感；4. 在困难案例中，标注者在大约50%的情况下确认或整合了MLLM的预测，显示其在数据集整理中的潜力。

Conclusion: MLLM在分类任务上表现不佳很大程度上是噪声标注和有缺陷评估协议的产物，而非模型本身的缺陷。通过修正评估协议和标注质量，MLLM表现显著改善，并显示出辅助大规模数据集整理的潜力。
